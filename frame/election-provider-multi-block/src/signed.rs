...
impl<T: Config> Pallet<T> {
	...
	/// Try and finalize the signed phase.
	///
	/// If an ongoing solution is being verified, it continues. If none is being verified, one is
	/// started.
	///
	/// Returns
	/// - Ok(n) where n is the number of remaining verification blocks needed. `Ok(0)` means
	///   everything is done.
	pub fn try_finalize_signed_phase() -> Result<PageIndex, crate::Error<T>> {
		todo!();
	}
