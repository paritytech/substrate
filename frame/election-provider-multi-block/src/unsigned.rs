...
use crate::{
	helpers, Call, Config, ElectionCompute, Error, FeasibilityError, PagedRawSolution, Pallet,
...
impl<T: Config> Pallet<T> {
	...
	pub fn full_checks(
		...
		Self::unsigned_pre_dispatch_checks(paged_solution).map_err(|err| {
		...
		let _ = paged_solution
			...
			.map(|(page_index, page_solution)| {
				Self::feasibility_check_page(page_solution.clone(), page_index as PageIndex)
			})
	...
	pub fn unsigned_pre_dispatch_checks(
		...
		ensure!(Self::round() == paged_solution.round, Error::<T>::OcwCallWrongEra);
