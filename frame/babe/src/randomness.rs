...
impl<T: Config> RandomnessT<T::Hash, frame_system::BlockNumberOf<T>> for RandomnessFromTwoEpochsAgo<T> {
	fn random(subject: &[u8]) -> (T::Hash, frame_system::BlockNumberOf<T>) {
...
impl<T: Config> RandomnessT<T::Hash, frame_system::BlockNumberOf<T>> for RandomnessFromOneEpochAgo<T> {
	fn random(subject: &[u8]) -> (T::Hash, frame_system::BlockNumberOf<T>) {
		let mut subject = subject.to_vec();
		subject.reserve(RANDOMNESS_LENGTH);
...
impl<T: Config> RandomnessT<Option<T::Hash>, frame_system::BlockNumberOf<T>> for ParentBlockRandomness<T> {
	fn random(subject: &[u8]) -> (Option<T::Hash>, frame_system::BlockNumberOf<T>) {
...
#[allow(deprecated)]
impl<T: Config> RandomnessT<Option<T::Hash>, frame_system::BlockNumberOf<T>> for CurrentBlockRandomness<T> {
	fn random(subject: &[u8]) -> (Option<T::Hash>, frame_system::BlockNumberOf<T>) {
