...
fn first_block_epoch_zero_start() {
	...
	ext.execute_with(|| {
		...
		assert_eq!(Babe::genesis_slot(), Slot(0));
		System::initialize(
			&1,
			&Default::default(),
			&pre_digest,
...
fn can_predict_next_epoch_change() {
	new_test_ext(1).execute_with(|| {
		assert_eq!(<Test as Config>::EpochDuration::get().0, 3);
		// this sets the genesis slot to 6;
		go_to_block(1, 6);
		assert_eq!(Babe::genesis_slot().0, 6);
		assert_eq!(Babe::current_slot().0, 6);
		assert_eq!(Babe::epoch_index(), 0);

		progress_to_block(5);

		assert_eq!(Babe::epoch_index(), 5 / 3);
		assert_eq!(Babe::current_slot().0, 10);

		// next epoch change will be at
		assert_eq!(Babe::current_epoch_start().0, 9); // next change will be 12, 2 slots from now
...
fn report_equivocation_invalid_equivocation_proof() {
	...
	ext.execute_with(|| {
		...
		equivocation_proof.slot = Slot(0);
