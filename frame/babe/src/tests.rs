...
fn check_module() {
    new_test_ext(vec![0, 1, 2, 3]).execute_with(|| {
        assert!(
            !Babe::should_end_session(0),
            "Genesis does not change sessions"
        );
        assert!(
...
fn first_block_epoch_zero_start() {
    new_test_ext(vec![0, 1, 2, 3]).execute_with(|| {
        ...
        let pre_digest =
            make_pre_digest(0, genesis_slot, first_vrf.clone(), RawVRFProof([0xff; 64]));
...
fn can_predict_next_epoch_change() {
    new_test_ext(vec![]).execute_with(|| {
        ...
        assert_eq!(
            Babe::next_expected_epoch_change(System::block_number()),
            Some(5 + 2)
        );
    })
}
