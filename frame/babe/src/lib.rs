...
pub trait EpochChangeTrigger {
	...
	fn trigger<T: Config>(now: frame_system::BlockNumberOf<T>);
}
...
impl EpochChangeTrigger for ExternalTrigger {
	fn trigger<T: Config>(_: frame_system::BlockNumberOf<T>) {} // nothing - trigger is external.
}
...
impl<T: Config> pallet_session::ShouldEndSession<frame_system::BlockNumberOf<T>> for Pallet<T> {
	fn should_end_session(now: frame_system::BlockNumberOf<T>) -> bool {
...
impl<T: Config> Pallet<T> {
	...
	pub fn should_epoch_change(now: frame_system::BlockNumberOf<T>) -> bool {
	...
	pub fn next_expected_epoch_change(now: frame_system::BlockNumberOf<T>) -> Option<frame_system::BlockNumberOf<T>> {
		...
		next_slot.checked_sub(*CurrentSlot::<T>::get()).map(|slots_remaining| {
			...
			let blocks_remaining: frame_system::BlockNumberOf<T> = slots_remaining.saturated_into();
	...
	fn initialize(now: frame_system::BlockNumberOf<T>) {
...
impl<T: Config> frame_support::traits::Lateness<frame_system::BlockNumberOf<T>> for Pallet<T> {
	fn lateness(&self) -> frame_system::BlockNumberOf<T> {
		Self::lateness()
	}
}
