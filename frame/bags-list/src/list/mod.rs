...
mod bags_list {
	...
	impl<T: Config<I>, I: 'static> List<T, I> {
		...
		pub fn remove_many<'a>(ids: impl IntoIterator<Item = &'a T::AccountId>) -> u32 {
		...
		pub fn insert_at_unchecked(mut at: Node<T, I>, mut node: Node<T, I>) {
	...
	impl<T: Config<I>, I: 'static> Node<T, I> {
		...
		#[cfg(any(test, feature = "try-runtime", feature = "fuzz"))]
		pub fn do_try_state(&self) -> Result<(), &'static str> {
	...
	impl<T: Config<I>, I: 'static> Bag<T, I> {
		...
		pub fn put(self) {
			if self.is_empty() {
				ListBags::<T, I>::remove(self.bag_upper);
			} else {
				ListBags::<T, I>::insert(self.bag_upper, self);
			}
		}
		...
		pub fn remove_node_unchecked(&mut self, node: &Node<T, I>) {
