...
mod list {
	...
	fn put_in_front_of_exits_early_if_nodes_not_found() {
		...
		ExtBuilder::default().build_and_execute(|| {
			...
			// then
			assert_storage_noop!(assert_eq!(
				List::<Runtime>::put_in_front_of(&4, &5, weight_fn).unwrap_err(),
		...
		ExtBuilder::default().build_and_execute(|| {
			...
			// then
			assert_storage_noop!(assert_eq!(
				List::<Runtime>::put_in_front_of(&5, &4, weight_fn).unwrap_err(),
				crate::pallet::Error::<Runtime>::IdNotFound
			));
		});
	}
	...
	fn put_in_front_of_exits_early_if_nodes_not_in_same_bag() {
		ExtBuilder::default().build_and_execute(|| {
			...
			// then
			assert_storage_noop!(assert_eq!(
				List::<Runtime>::put_in_front_of(&1, &4, weight_fn).unwrap_err(),
	...
	#[test]
	#[should_panic(expected = "bag that should exist cannot be found")]
