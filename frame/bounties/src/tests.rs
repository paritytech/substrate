...
use frame_support::{
    assert_noop, assert_ok, parameter_types, traits::OnInitialize, weights::Weight,
};
...
fn last_event() -> RawEvent<u64, u128> {
    System::events()
        ...
        .map(|r| r.event)
        .filter_map(|e| {
            if let Event::pallet_bounties(inner) = e {
                Some(inner)
            } else {
                None
            }
...
fn propose_bounty_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_eq!(
            Bounties::bounty_descriptions(0).unwrap(),
            b"1234567890".to_vec()
        );
...
fn approve_bounty_works() {
    new_test_ext().execute_with(|| {
        ...
        Balances::make_free_balance_be(&Treasury::account_id(), 101);
        assert_noop!(
            Bounties::approve_bounty(Origin::root(), 0),
            Error::<Test>::InvalidIndex
        );
...
fn assign_curator_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Bounties::accept_curator(Origin::signed(1), 0),
...
fn award_and_claim_bounty_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Bounties::claim_bounty(Origin::signed(1), 0),
...
fn expire_and_unassign() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Bounties::unassign_curator(Origin::signed(0), 0),
            Error::<Test>::Premature
        );
