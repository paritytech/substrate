...
use frame_support::{
	dispatch::RawOrigin,
	traits::{fungible, tokens::{Expendability::Expendable, Privilege::Regular}, Currency, Get},
};
...
benchmarks_instance_pallet! {
	...
	unlock {
		...
		let orig_usable = <T::Currency as fungible::Inspect<T::AccountId>>::reducible_balance(&caller, Expendable, Regular);
		let polls = &all_polls[&class];
		...
		let now_usable = <T::Currency as fungible::Inspect<T::AccountId>>::reducible_balance(&caller, Expendable, Regular);
	...
	verify {
		assert_eq!(orig_usable, <T::Currency as fungible::Inspect<T::AccountId>>::reducible_balance(&caller, Expendable, Regular));
	}
