...
use frame_support::{
	dispatch::RawOrigin,
	traits::{fungible, tokens::{KeepAlive::CanKill, Privilege::Regular}, Currency, Get},
};
...
benchmarks_instance_pallet! {
	...
	unlock {
		...
		let orig_usable = <T::Currency as fungible::Inspect<T::AccountId>>::reducible_balance(&caller, CanKill, Regular);
		let polls = &all_polls[&class];
		...
		let now_usable = <T::Currency as fungible::Inspect<T::AccountId>>::reducible_balance(&caller, CanKill, Regular);
	...
	verify {
		assert_eq!(orig_usable, <T::Currency as fungible::Inspect<T::AccountId>>::reducible_balance(&caller, CanKill, Regular));
	}
