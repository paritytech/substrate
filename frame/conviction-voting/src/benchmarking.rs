...
use frame_support::{
	dispatch::RawOrigin,
	traits::{fungible, Currency, Get, tokens::KeepAlive::CanKill},
};
...
benchmarks_instance_pallet! {
	...
	unlock {
		...
		let orig_usable = <T::Currency as fungible::Inspect<T::AccountId>>::reducible_balance(&caller, CanKill, false);
		let polls = &all_polls[&class];
		...
		let now_usable = <T::Currency as fungible::Inspect<T::AccountId>>::reducible_balance(&caller, CanKill, false);
	...
	verify {
		assert_eq!(orig_usable, <T::Currency as fungible::Inspect<T::AccountId>>::reducible_balance(&caller, CanKill, false));
	}
