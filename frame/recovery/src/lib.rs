...
pub mod pallet {
	...
	pub type Recoverable<T: Config> = StorageMap<
		...
		Twox64Concat,
		T::AccountId,
		RecoveryConfig<frame_system::BlockNumberOf<T>, BalanceOf<T>, FriendsOf<T>>,
	>;
	...
	pub type ActiveRecoveries<T: Config> = StorageDoubleMap<
		...
		Twox64Concat,
		T::AccountId,
		ActiveRecovery<frame_system::BlockNumberOf<T>, BalanceOf<T>, FriendsOf<T>>,
	>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_recovery(
			...
			threshold: u16,
			delay_period: frame_system::BlockNumberOf<T>,
		) -> DispatchResult {
			let who = ensure_signed(origin)?;
