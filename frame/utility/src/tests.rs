...
fn batch_handles_weight_refund() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			...
			Weight::from_computation(<Test as Config>::WeightInfo::batch(2)) + end_weight * 2,
		);
	});
}
	...
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			...
			DispatchErrorWithPostInfo {
				post_info: PostDispatchInfo {
					actual_weight: Some(
						Weight::from_computation(<Test as Config>::WeightInfo::batch_all(2)) +
							info.weight * 2
					),
					pays_fee: Pays::Yes
				},
...
fn batch_all_handles_weight_refund() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			extract_actual_weight(&result, &info),
			// Real weight is 2 calls at end_weight
			Weight::from_computation(<Test as Config>::WeightInfo::batch_all(2)) + end_weight * 2,
		);
	});
}
	...
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			...
			DispatchErrorWithPostInfo {
				post_info: PostDispatchInfo {
					actual_weight: Some(
						Weight::from_computation(<Test as Config>::WeightInfo::batch_all(1)) +
							info.weight
					),
					pays_fee: Pays::Yes
				},
