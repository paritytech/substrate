...
fn cancel_multisig_returns_deposit() {
    new_test_ext().execute_with(|| {
        ...
        assert_ok!(Utility::approve_as_multi(
            Origin::signed(1),
            3,
            vec![2, 3],
            None,
        ...
        assert_ok!(Utility::cancel_as_multi(
            Origin::signed(1),
            3,
            vec![2, 3],
            now(),
...
fn multisig_2_of_3_cannot_reissue_same_call() {
    new_test_ext().execute_with(|| {
        ...
        expect_event(RawEvent::MultisigExecuted(
            3,
            now(),
            multi,
...
fn batch_with_signed_filters() {
    new_test_ext().execute_with(|| {
        assert_ok!(Utility::batch(
            Origin::signed(1),
            vec![Call::System(frame_system::Call::remark(vec![]))]
        ),);
