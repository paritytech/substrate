...
fn deposit_event_should_work() {
	new_test_ext().execute_with(|| {
		...
		System::deposit_event(SysEvent::NewAccount { new_account: 32 });
		System::note_finished_initialize();
		System::deposit_event(SysEvent::KilledAccount { reaped_account: 42 });
		System::note_applied_extrinsic(&Ok(().into()), Default::default());
		System::note_applied_extrinsic(&Err(DispatchError::BadOrigin.into()), Default::default());
		System::note_finished_extrinsics();
		System::deposit_event(SysEvent::NewAccount { new_account: 3 });
		System::finalize();
		assert_eq!(
			System::events(),
			vec![
				EventRecord {
					...
					event: SysEvent::NewAccount { new_account: 32 }.into(),
					topics: vec![],
				},
				EventRecord {
					phase: Phase::ApplyExtrinsic(0),
					event: SysEvent::KilledAccount { reaped_account: 42 }.into(),
					topics: vec![]
				},
				EventRecord {
					phase: Phase::ApplyExtrinsic(0),
					event: SysEvent::ExtrinsicSuccess { extrinsic_info: Default::default() }.into(),
					topics: vec![]
				},
				EventRecord {
					phase: Phase::ApplyExtrinsic(1),
					event: SysEvent::ExtrinsicFailed {
						extrinsic_error: DispatchError::BadOrigin.into(),
						extrinsic_info: Default::default()
					}
					.into(),
					topics: vec![]
				},
				EventRecord {
					phase: Phase::Finalization,
					event: SysEvent::NewAccount { new_account: 3 }.into(),
					topics: vec![]
				},
			]
		);
		...
		assert_eq!(
			...
			vec![
				...
				EventRecord {
					...
					event: SysEvent::ExtrinsicFailed {
						extrinsic_error: DispatchError::BadOrigin.into(),
						extrinsic_info: DispatchInfo { weight: 999, ..Default::default() },
					}
					.into(),
					topics: vec![]
				},
	...
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			...
			vec![
				EventRecord {
					phase: Phase::Finalization,
					event: SysEvent::NewAccount { new_account: 1 }.into(),
					topics: topics[0..3].to_vec(),
				},
				EventRecord {
					phase: Phase::Finalization,
					event: SysEvent::NewAccount { new_account: 2 }.into(),
