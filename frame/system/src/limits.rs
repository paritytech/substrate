...
impl BlockWeights {
	...
	pub fn validate(self) -> ValidationResult {
		...
		for class in DispatchClass::all() {
			...
			error_assert!(
				(
					max_for_class.is_strictly_greater_than(&self.base_block) &&
					max_for_class.is_strictly_greater_than(&base_for_class)
				)
				|| max_for_class == Zero::zero(),
				&mut error,
			...
			error_assert!(
				weights
					.max_extrinsic
					.unwrap_or(Zero::zero())
					.is_strictly_less_than_or_equal(&max_for_class.saturating_sub(base_for_class)),
				&mut error,
			...
			error_assert!(
				reserved.is_strictly_greater_than(&base_for_class) || reserved == Zero::zero(),
				&mut error,
			...
			error_assert!(
				self.max_block
					.is_strictly_greater_than_or_equal(&weights.max_total.unwrap_or(Zero::zero())),
				&mut error,
			...
			error_assert!(
				self.max_block.is_strictly_greater_than(&(base_for_class + self.base_block)),
				&mut error,
	...
	pub fn build(self) -> ValidationResult {
		...
		for class in DispatchClass::all() {
			weights.max_block = match weights.per_class.get(*class).max_total {
				Some(max) if weights.max_block.is_strictly_less_than(&max) => max,
				_ => weights.max_block,
			};
		}
