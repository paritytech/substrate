...
use frame_support::{
    ...
    storage,
    traits::{
        Contains, EnsureOrigin, Filter, Get, HandleLifetime, OnKilledAccount, OnNewAccount,
        OriginTrait, PalletInfo, StoredMap,
    },
    weights::{
...
impl<T: Config> HandleLifetime<T::AccountId> for Consumer<T> {
    fn created(t: &T::AccountId) -> Result<(), StoredMapError> {
        Module::<T>::inc_consumers(t).map_err(|e| match e {
