...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn remark_with_event(
			...
			Self::deposit_event(Event::Remarked { origin: who, remarked_hash: hash });
			Ok(().into())
		}
	}
...
impl<T: Config> Pallet<T> {
	...
	pub fn note_applied_extrinsic(r: &DispatchResultWithPostInfo, mut info: DispatchInfo) {
		...
		Self::deposit_event(match r {
			Ok(_) => Event::ExtrinsicSuccess { extrinsic_info: info },
			Err(err) => {
				log::trace!(
					target: "runtime::system",
				...
				Event::ExtrinsicFailed { extrinsic_error: err.error, extrinsic_info: info }
			},
		});
	...
	pub fn on_created_account(who: T::AccountId, _a: &mut AccountInfo<T::Index, T::AccountData>) {
		...
		Self::deposit_event(Event::NewAccount { new_account: who });
	}

	/// Do anything that needs to be done after an account has been killed.
	fn on_killed_account(who: T::AccountId) {
		T::OnKilledAccount::on_killed_account(&who);
		Self::deposit_event(Event::KilledAccount { reaped_account: who });
	}
