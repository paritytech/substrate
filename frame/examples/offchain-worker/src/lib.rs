...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn offchain_worker(block_number: frame_system::BlockNumberOf<T>) {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn submit_price_unsigned(
			origin: OriginFor<T>,
			_block_number: frame_system::BlockNumberOf<T>,
			price: u32,
		...
		pub fn submit_price_unsigned_with_signed_payload(
			origin: OriginFor<T>,
			price_payload: PricePayload<T::Public, frame_system::BlockNumberOf<T>>,
...
impl<T: SigningTypes> SignedPayload<T> for PricePayload<T::Public, frame_system::BlockNumberOf<T>> {
...
impl<T: Config> Pallet<T> {
	...
	fn choose_transaction_type(block_number: frame_system::BlockNumberOf<T>) -> TransactionType {
		...
		match res {
			...
			Ok(block_number) => {
				...
				} else if transaction_type == BlockNumberOf::<T>::from(1u32) {
					TransactionType::UnsignedForAny
				} else if transaction_type == BlockNumberOf::<T>::from(2u32) {
					TransactionType::UnsignedForAll
				} else {
					TransactionType::Raw
				}
