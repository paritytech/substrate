...
pub fn create_heartbeat<T: Trait>(k: u32, e: u32) ->
	Result<(crate::Heartbeat<T::BlockNumber>, <T::AuthorityId as RuntimeAppPublic>::Signature, u32), &'static str>
{
	...
	Ok((input_heartbeat, signature, keys.len() as u32))
}
...
benchmarks! {
	...
	heartbeat {
		...
		let (input_heartbeat, signature, keys_len) = create_heartbeat::<T>(k, e)?;
	}: _(RawOrigin::None, input_heartbeat, signature, keys_len)

	validate_unsigned {
		let k in 1 .. MAX_KEYS;
		let e in 1 .. MAX_EXTERNAL_ADDRESSES;
		let (input_heartbeat, signature, keys_len) = create_heartbeat::<T>(k, e)?;
		let call = Call::heartbeat(input_heartbeat, signature, keys_len);
	}: {
