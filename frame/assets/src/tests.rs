...
fn transfer_approved_all_funds() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Assets::transfer_approved(Origin::signed(2), 0, Box::new(1), Box::new(3), 50));
		assert_eq!(Asset::<Test>::get(0).unwrap().approvals, 0);
...
fn approval_deposits_work() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(
			Assets::transfer_approved(Origin::signed(2), 0, Box::new(1), Box::new(3), 50)
		);
		assert_eq!(Balances::reserved_balance(&1), 0);
...
fn cannot_transfer_more_than_approved() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Assets::transfer_approved(Origin::signed(2), 0, Box::new(1), Box::new(3), 51),
			e,
		);
	});
}
...
fn cannot_transfer_more_than_exists() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Assets::transfer_approved(Origin::signed(2), 0, Box::new(1), Box::new(3), 101),
			e,
		);
	});
}
...
fn force_cancel_approval_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Assets::force_cancel_approval(Origin::signed(2), 0, Box::new(1), Box::new(2)),
			e,
		);
		assert_noop!(
			Assets::force_cancel_approval(Origin::signed(1), 1, Box::new(1), Box::new(2)),
			Error::<Test>::Unknown,
		);
		assert_noop!(
			Assets::force_cancel_approval(Origin::signed(1), 0, Box::new(2), Box::new(2)),
			Error::<Test>::Unknown,
		);
		assert_noop!(
			Assets::force_cancel_approval(Origin::signed(1), 0, Box::new(1), Box::new(3)),
			Error::<Test>::Unknown,
		);
		assert_eq!(Asset::<Test>::get(0).unwrap().approvals, 1);
		assert_ok!(Assets::force_cancel_approval(Origin::signed(1), 0, Box::new(1), Box::new(2)));
		assert_eq!(Asset::<Test>::get(0).unwrap().approvals, 0);
		assert_noop!(
			Assets::force_cancel_approval(Origin::signed(1), 0, Box::new(1), Box::new(2)),
			Error::<Test>::Unknown,
		);
	});
}
...
fn non_providing_should_work() {
	new_test_ext().execute_with(|| {
		...
		// ...or force-transfer
		assert_noop!(
			Assets::force_transfer(Origin::signed(1), 0, Box::new(0), Box::new(1), 50),
		...
		assert_ok!(Assets::force_transfer(Origin::signed(1), 0, Box::new(0), Box::new(2), 25));
	});
}
...
fn min_balance_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Assets::force_transfer(Origin::signed(1), 0, Box::new(1), Box::new(2), 9),
		...
		assert_ok!(Assets::force_transfer(Origin::signed(1), 0, Box::new(2), Box::new(1), 91));
...
fn origin_guards_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Assets::set_team(Origin::signed(2), 0, Box::new(2), Box::new(2), Box::new(2)),
		...
		assert_noop!(
			Assets::force_transfer(Origin::signed(2), 0, Box::new(1), Box::new(2), 100),
			Error::<Test>::NoPermission,
		);
		let w = Asset::<Test>::get(0).unwrap().destroy_witness();
...
fn set_team_should_work() {
	new_test_ext().execute_with(|| {
		assert_ok!(Assets::force_create(Origin::root(), 0, 1, true, 1));
		assert_ok!(
			Assets::set_team(Origin::signed(1), 0, Box::new(2), Box::new(3), Box::new(4))
		);
		...
		assert_ok!(Assets::force_transfer(Origin::signed(3), 0, Box::new(2), Box::new(3), 100));
...
fn freezer_should_work() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			Assets::transfer_approved(Origin::signed(2), 0, Box::new(1), Box::new(2), 21),
			e,
		);
		// a force transfer won't work also.
		let e = Error::<Test>::BalanceLow;
		assert_noop!(Assets::force_transfer(Origin::signed(1), 0, Box::new(1), Box::new(2), 21), e);

		// reduce it to only 49 frozen...
		set_frozen_balance(0, 1, 49);
		// ...and it's all good:
		assert_ok!(Assets::force_transfer(Origin::signed(1), 0, Box::new(1), Box::new(2), 21));
