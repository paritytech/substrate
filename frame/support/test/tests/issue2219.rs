...
mod module {
	...
	frame_support::decl_storage! {
		trait Store for Module<T: Trait> as Actors {
			...
			}): map hasher(blake2_64_concat) Role => Option<RoleParameters<T>>;
			...
			pub AccountIdsByRole get(fn account_ids_by_role):
				map hasher(blake2_64_concat) Role => Vec<T::AccountId>;

			/// tokens locked until given block number
			pub Bondage get(fn bondage):
				map hasher(blake2_64_concat) T::AccountId => T::BlockNumber;
