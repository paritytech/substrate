...
fn pallet_metadata_expands() {
	...
	let mut infos = PalletInstances::infos();
...
fn pallet_hooks_expand() {
	TestExternalities::default().execute_with(|| {
		...
		assert_eq!(PalletInstancesRev::on_initialize(1), 21);
		PalletInstancesRev::on_finalize(1);

		assert_eq!(PalletInstancesRev::on_runtime_upgrade(), 61);
