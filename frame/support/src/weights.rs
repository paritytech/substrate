...
mod tests {
    ...
    fn weights_are_correct() {
        ...
        assert_eq!(
            Call::<TraitImpl>::f11(10, 20).get_dispatch_info().weight,
            120
        );
        assert_eq!(
        ...
        assert_eq!(
            Call::<TraitImpl>::f2().get_dispatch_info().class,
            DispatchClass::Normal
        );
    }
    ...
    fn polynomial_does_not_overflow() {
        assert_eq!(
            Poly::calc(&Weight::max_value()),
            Balance::max_value() - 10_000
        );
    }
    ...
    fn identity_fee_works() {
        ...
        assert_eq!(
            IdentityFee::<Balance>::calc(&Weight::max_value()),
            Balance::max_value()
        );
    }
}
