...
pub struct BytesCodecWrapper<T>(pub T);

impl<T: codec::Encode> codec::Encode for BytesCodecWrapper<T> {
	fn size_hint(&self) -> usize {
		// Compact<u32> usually takes less than 4 bytes
...
impl<T: codec::Decode> codec::Decode for BytesCodecWrapper<T> {
...
impl<T> From<T> for BytesCodecWrapper<T> {
	fn from(t: T) -> Self {
		Self(t)
	}
}
