...
impl OnRuntimeUpgrade for Tuple {
	...
	#[cfg(feature = "try-runtime")]
	fn pre_upgrade() -> Result<Vec<u8>, &'static str> {
		Ok((for_tuples!( #( Tuple::pre_upgrade()? ),* )).encode())
	}

	#[cfg(feature = "try-runtime")]
	fn post_upgrade(state: Vec<u8>) -> Result<(), &'static str> {
		// a type alias to workaround the `for_tuples` macro parsing
		type Vecu8 = Vec<u8>;
		let state: (for_tuples!( #( Vecu8 ),* )) = Decode::decode(&mut state.as_slice())
			.expect("the state parameter should be something that generated by pre_upgrade");
		for_tuples!( #( Tuple::post_upgrade(state.Tuple)?; )* );
		Ok(())
	}
}
...
pub trait Hooks<BlockNumber, PreUpgradeState: Default + Encode + Decode = ()> {
	/// The block is being finalized. Implement to have something happen.
	fn on_finalize(_n: BlockNumber) {}
	...
	/// This hook is never meant to be executed on-chain but is meant to be used by testing tools.
	#[cfg(feature = "try-runtime")]
	fn post_upgrade(_state: PreUpgradeState) -> Result<(), &'static str> {
		Ok(())
	}
