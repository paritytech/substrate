...
#[cfg_attr(all(feature = "tuples-96", not(feature = "tuples-128")), impl_for_tuples(96))]
#[cfg_attr(feature = "tuples-128", impl_for_tuples(128))]
impl<BlockNumber: Clone> Poll<BlockNumber> for Tuple {
	fn poll(n: BlockNumber) -> Weight {
		let mut weight = Weight::zero();
		for_tuples!( #( weight = weight.saturating_add(Tuple::poll(n.clone())); )* );
		weight
	}
}
...
pub trait Hooks<BlockNumber> {
	...
	// TODO: compile time check for having on-finalize without an on-initialize.
	fn on_finalize(_n: BlockNumber) {}
	...
	/// catastrophic.
	///
	/// TODO: add an example, perhaps using docify.
	///
