...
pub fn get<T: Decode + Sized>(storage_key: &[u8], child_info: ChildInfo, key: &[u8]) -> Option<T> {
    let (data, child_type) = child_info.info();
    sp_io::storage::child_get(storage_key, data, child_type, key).and_then(|v| {
...
pub fn put<T: Encode>(storage_key: &[u8], child_info: ChildInfo, key: &[u8], value: &T) {
    let (data, child_type) = child_info.info();
    value.using_encoded(|slice| {
        sp_io::storage::child_set(storage_key, data, child_type, key, slice)
    });
}

/// Remove `key` from storage, returning its value if it had an explicit entry or `None` otherwise.
pub fn take<T: Decode + Sized>(storage_key: &[u8], child_info: ChildInfo, key: &[u8]) -> Option<T> {
...
pub fn exists(storage_key: &[u8], child_info: ChildInfo, key: &[u8]) -> bool {
    let (data, child_type) = child_info.info();
    sp_io::storage::child_read(storage_key, data, child_type, key, &mut [0; 0][..], 0).is_some()
}

/// Remove all `storage_key` key/values
pub fn kill_storage(storage_key: &[u8], child_info: ChildInfo) {
    let (data, child_type) = child_info.info();
    sp_io::storage::child_storage_kill(storage_key, data, child_type)
}

/// Ensure `key` has no explicit entry in storage.
pub fn kill(storage_key: &[u8], child_info: ChildInfo, key: &[u8]) {
    let (data, child_type) = child_info.info();
    sp_io::storage::child_clear(storage_key, data, child_type, key);
}

/// Get a Vec of bytes from storage.
pub fn get_raw(storage_key: &[u8], child_info: ChildInfo, key: &[u8]) -> Option<Vec<u8>> {
    let (data, child_type) = child_info.info();
    sp_io::storage::child_get(storage_key, data, child_type, key)
}

/// Put a raw byte slice into storage.
pub fn put_raw(storage_key: &[u8], child_info: ChildInfo, key: &[u8], value: &[u8]) {
    let (data, child_type) = child_info.info();
    sp_io::storage::child_set(storage_key, data, child_type, key, value)
}

/// Calculate current child root value.
pub fn child_root(storage_key: &[u8]) -> Vec<u8> {
    sp_io::storage::child_root(storage_key)
}
