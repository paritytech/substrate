	...
	MaxValues: Get<Option<u32>>,
{
	...
	/// Enumerate all elements in the map starting from a specified `starting_key` in no
	/// particular order.
	///
	/// If you alter the map while doing this, you'll get undefined results.
	pub fn iter_from(starting_key: Vec<u8>) -> crate::storage::PrefixIterator<(Key, Value)> {
		<Self as crate::storage::IterableStorageMap<Key, Value>>::iter_from(starting_key)
	}
	...
	///
	/// If you alter the map while doing this, you'll get undefined results.
	pub fn iter_keys_from(starting_key: Vec<u8>) -> crate::storage::KeyPrefixIterator<Key> {
		<Self as crate::storage::IterableStorageMap<Key, Value>>::iter_keys_from(starting_key)
	}
