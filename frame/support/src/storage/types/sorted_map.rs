...
//! Storage sorted map type. Implements StorageMap, StorageIterableMap, StoragePrefixedMap traits
//! and their methods directly.
...
/// Implementors of `ComputeSortKey` can compute their sort key from something which encodes like
/// a `SortedStorageMap`'s `Key`.
pub trait ComputeSortKey {
	/// Compute the sort key from the slice provided.
	///
	/// This is a workaround for the issue that we don't actually have access to a `Key` type when
	/// we want to compute the sorted key. Instead, we only have access to a `KeyArg:
	/// EncodeLike<Key>`. Therefore, the expected usage pattern is for the sort key fn to decode
	/// the encoded key into the actual key type, and compute the sort key from there.
	fn compute_sort_key(encoded_key: &[u8]) -> Vec<u8>;
}

impl<Getter> ComputeSortKey for Getter
where
	Getter: Get<Box<dyn Fn(&[u8]) -> Vec<u8>>>,
{
	fn compute_sort_key(encoded_key: &[u8]) -> Vec<u8> {
		let sort_key_fn = Getter::get();
		sort_key_fn(encoded_key)
	}
}

/// A type that allow to store value for given key, allowing to insert/remove/iterate on values.
///
/// This type also incorporates a type-level function, `SortKeyFn`, which must implement
/// `Fn(Key) -> SortKey where SortKey: Encode`. The key generated by this function is
/// incorporated into the storage in such a way that iterating on the items of this storage
/// always produces results in sorted order.
///
/// Note that, just like a `BTreeMap`, sort order depends only on the key.
///
...
/// # Warning
///
/// It is the pallet author's responsibility to ensure that `SortKeyFn` produces reasonable output.
/// Very long output and collisions are discouraged.
///
/// # Warning
///
/// If the keys are not trusted (e.g. can be set by a user), a cryptographic `hasher` such as
/// `blake2_128_concat` must be used.  Otherwise, other values in storage can be compromised.
pub struct SortedStorageMap<
	Prefix,
	Hasher,
	Key,
	Value,
...
>(
	core::marker::PhantomData<Prefix>,
	core::marker::PhantomData<Hasher>,
	core::marker::PhantomData<Key>,
	core::marker::PhantomData<Value>,
	core::marker::PhantomData<SortKeyFn>,
	core::marker::PhantomData<QueryKind>,
	core::marker::PhantomData<OnEmpty>,
	core::marker::PhantomData<MaxValues>,
);

impl<Prefix, Hasher, Key, Value, SortKeyFn, QueryKind, OnEmpty, MaxValues>
	crate::storage::generator::StorageMap<Key, Value>
	for SortedStorageMap<Prefix, Hasher, Key, Value, SortKeyFn, QueryKind, OnEmpty, MaxValues>
where
	Prefix: StorageInstance,
	Hasher: crate::hash::StorageHasher,
	Key: FullCodec,
	Value: FullCodec,
	SortKeyFn: ComputeSortKey,
	...
	MaxValues: Get<Option<u32>>,
{
	...
	{
		use crate::{Twox128, hash::StorageHasher};

		let module_prefix_hashed = Twox128::hash(<Self as crate::storage::generator::StorageMap<
			Key,
			Value,
		>>::module_prefix());
		let storage_prefix_hashed = Twox128::hash(
			<Self as crate::storage::generator::StorageMap<Key, Value>>::storage_prefix(),
		);
		let encoded_key = key.encode();
		let sort_key = SortKeyFn::compute_sort_key(&encoded_key);
		// avoid creating a temporary which is freed at the end of the statement
		let key_hashed_output = Self::Hasher::hash(&encoded_key);
		let key_hashed = key_hashed_output.as_ref();

		let mut final_key = Vec::with_capacity(
			module_prefix_hashed.len()
				+ storage_prefix_hashed.len()
				+ sort_key.len() + key_hashed.len(),
		);

		final_key.extend_from_slice(&module_prefix_hashed);
		final_key.extend_from_slice(&storage_prefix_hashed);
		final_key.extend_from_slice(&sort_key);
...
impl<Prefix, Hasher, Key, Value, SortKeyFn, QueryKind, OnEmpty, MaxValues> StoragePrefixedMap<Value>
	for SortedStorageMap<Prefix, Hasher, Key, Value, SortKeyFn, QueryKind, OnEmpty, MaxValues>
where
	Prefix: StorageInstance,
	Hasher: crate::hash::StorageHasher,
...
impl<Prefix, Hasher, Key, Value, SortKeyFn, QueryKind, OnEmpty, MaxValues>
	SortedStorageMap<Prefix, Hasher, Key, Value, SortKeyFn, QueryKind, OnEmpty, MaxValues>
where
	Prefix: StorageInstance,
	Hasher: crate::hash::StorageHasher,
	Key: FullCodec,
...
where
	...
	MaxValues: Get<Option<u32>>,
{
	/// Iterate all elements in the map in order.
	///
	...
	/// Remove all elements from the map and iterate through them in order.
	///
	...
	/// Translate the values of all elements by a function `f`, in the map in order.
	///
	/// By returning `None` from `f` for an element, you'll remove it from the map.
	///
	/// NOTE: If a value fail to decode because storage is corrupted then it is skipped.
...
impl<Prefix, Hasher, Key, Value, SortKeyFn, QueryKind, OnEmpty, MaxValues> StorageMapMetadata
	for SortedStorageMap<Prefix, Hasher, Key, Value, SortKeyFn, QueryKind, OnEmpty, MaxValues>
where
	Prefix: StorageInstance,
	Hasher: crate::hash::StorageHasher,
	...
	MaxValues: Get<Option<u32>>,
{
	fn storage_info() -> Vec<StorageInfo> {
		vec![StorageInfo {
			...
			max_size: Some(
				Hasher::max_len::<Key>().saturating_add(Value::max_encoded_len()).saturated_into(),
			),
		}]
	}
}
...
mod test {
	...
	#[test]
	fn test() {
		type A = SortedStorageMap<Prefix, Blake2_128Concat, u16, u32, OptionQuery>;
		type AValueQueryWithAnOnEmpty =
			SortedStorageMap<Prefix, Blake2_128Concat, u16, u32, ValueQuery, ADefault>;
		type B = SortedStorageMap<Prefix, Blake2_256, u16, u32, ValueQuery>;
		type C = SortedStorageMap<Prefix, Blake2_128Concat, u16, u8, ValueQuery>;
		type WithLen = SortedStorageMap<Prefix, Blake2_128Concat, u16, Vec<u32>>;
