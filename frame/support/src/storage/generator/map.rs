		...
		F: FnMut(K, O) -> Option<V>,
	{
			...
			sp_io::storage::next_key(&previous_key).filter(|n| max > 0 && n.starts_with(&prefix))
		{
			...
			let key = match K::decode(&mut key_material) {
				Ok(key) => key,
				Err(_) => {
					log::error!("Invalid partial translate: fail to decode key");
					continue
				},
			};
...
mod test_partial_translate {
	use crate::storage::{generator::StorageMap, unhashed, IterableStorageMap};
	...
	fn partial_translate_works() {
		sp_io::TestExternalities::default().execute_with(|| {
			use crate::hash::Identity;
			#[crate::storage_alias]
			type MyMap = StorageMap<MyModule, Identity, u64, f64>;
			...
			for i in 0..max_entries {
				let final_key = MyMap::storage_map_final_key(i as u64);
...

			{
				...
				for i in 0..quarter {
					assert_eq!(MyMap::get(i as u64), Some((i * 10) as f64 + 1.0));
				}
				...
				let cursor = MyMap::partial_translate(quarter, Some(&cursor), |k, v: u64| {
					(k % 2 != 0).then(|| v as f64 + 1.0)
				})
				.unwrap();
				let mut rest_cursor = cursor.clone();
				...
				for i in quarter..(2 * quarter) {
					assert_eq!(MyMap::get(i as u64), (i % 2 != 0).then(|| (i * 10) as f64 + 1.0));
				}
