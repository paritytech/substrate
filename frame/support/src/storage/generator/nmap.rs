...
mod test_partial_translate {
	use crate::storage::{generator::StorageNMap, unhashed, IterableStorageNMap, Key as NMapKey};
	...
	fn partial_translate_works() {
		sp_io::TestExternalities::default().execute_with(|| {
			...
			for i in 0..max_entries {
				let final_key =
					MyMap::storage_n_map_final_key::<Key, _>((i as u64, i as u64, i as u64));
...

			{
				...
				let cursor =
					MyMap::partial_translate(0, None, |_, v: u64| Some(v as f64 + 1.0)).unwrap();
...

			{
				...
				let cursor =
					MyMap::partial_translate(quarter, None, |_, v: u64| Some(v as f64 + 1.0))
						.unwrap();
				...
				for i in 0..quarter {
					assert_eq!(
						MyMap::get((i as u64, i as u64, i as u64)),
						Some((i * 10) as f64 + 1.0)
					);
				}
				...
				let cursor =
					MyMap::partial_translate(quarter, Some(&cursor), |(k1, k2, k3), v: u64| {
						((k1 + k2 + k3) % 2 != 0).then(|| v as f64 + 1.0)
					})
					.unwrap();
				let mut rest_cursor = cursor.clone();
				...
				for i in quarter..(2 * quarter) {
					assert_eq!(
						MyMap::get((i as u64, i as u64, i as u64)),
						((i + i + i) % 2 != 0).then(|| (i * 10) as f64 + 1.0)
					);
				}
				...
				let cursor = MyMap::partial_translate(max_entries, Some(&cursor), |_, v: u64| {
					Some(v as f64 + 1.0)
				});
