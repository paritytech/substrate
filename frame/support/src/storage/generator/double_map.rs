...
use crate::hash::{ReversibleStorageHasher, StorageHasher, Twox128};
use crate::{
    storage::{self, unhashed},
    traits::Len,
};
use codec::{Decode, Encode, EncodeAppend, EncodeLike, FullCodec, FullEncode, Ref};
    ...
    G: StorageDoubleMap<K1, K2, V>,
{
    ...
    >(
        ...
        unhashed::take(old_key.as_ref()).map(|value| {
            unhashed::put(
                Self::storage_double_map_final_key(key1, key2).as_ref(),
                &value,
            );
            value
        })
