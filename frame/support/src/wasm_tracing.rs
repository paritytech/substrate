...
impl Drop for TracingSpanGuard {
	fn drop(&mut self) {
		if let Some(id) = self.0.take() {
			crate::sp_tracing::interface::wasm_tracing::exit(id);
		}
	}
}
...
macro_rules! enter_span {
	( $name:expr, $values:expr ) => {
		...
		let __span_id__ = if $crate::wasm_tracing::wasm_tracing_enabled() {
				...
				let metadata = $crate::sp_tracing::types::WasmMetadata {
					name: $name.bytes().collect::<Vec<_>>(),
					target: module_path!().bytes().collect::<Vec<_>>(),
					level: $crate::sp_tracing::types::WasmLevel::INFO,
					file: file!().bytes().collect::<Vec<_>>(),
					line: line!(),
					module_path: module_path!().bytes().collect::<Vec<_>>(),
					is_span: true,
					fields: Vec::new(),
				};
