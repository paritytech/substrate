...
/// implicit.
#[derive(Debug)]
pub enum BundleDeclaration {
	Implicit(ImplicitBundleDeclaration),
	Explicit(ExplicitBundleDeclaration),
	ExplicitExpanded(ExplicitBundleDeclaration),
}

/// Declaration of a runtime with some pallet with implicit declaration of parts.
#[derive(Debug)]
pub struct ImplicitBundleDeclaration {
	pub name: Ident,
...
#[derive(Debug)]
pub struct ExplicitBundleDeclaration {
	pub name: Ident,
	pub pallets: Vec<Pallet>,
	pub pallets_token: token::Brace,
}

impl Parse for BundleDeclaration {
	fn parse(input: ParseStream) -> Result<Self> {
		...
		// let _gen: Ident = input.parse()?;
		// let _rt = input.parse::<Token![>]>()?;

		// let _lt = input.parse::<ext::Parens<Token![_]>>()?;
		...
		match convert_pallets(pallets.content.inner.into_iter().collect())? {
			PalletsConversion::Implicit(pallets) =>
				Ok(BundleDeclaration::Implicit(ImplicitBundleDeclaration {
					name,
					pallets,
				})),
			PalletsConversion::Explicit(pallets) =>
				Ok(BundleDeclaration::Explicit(ExplicitBundleDeclaration {
					name,
					pallets,
					pallets_token,
				})),
			PalletsConversion::ExplicitExpanded(pallets) =>
				Ok(BundleDeclaration::ExplicitExpanded(ExplicitBundleDeclaration {
					name,
					pallets,
					pallets_token,
				})),
