...
use parse::{
	ExplicitBundleDeclaration, ImplicitBundleDeclaration, Pallet, BundleDeclaration,
};
...
pub fn construct_bundle(input: TokenStream) -> TokenStream {
	...
	let definition = syn::parse_macro_input!(input as BundleDeclaration);

	let res = match definition {
		BundleDeclaration::Implicit(implicit_def) =>
			...
			BundleDeclaration::Explicit(explicit_decl) => check_pallet_number(
		...
		BundleDeclaration::ExplicitExpanded(explicit_decl) =>
	...
	let res = expander::Expander::new("construct_bundle")
...
fn construct_runtime_implicit_to_explicit(
	input: TokenStream2,
	definition: ImplicitBundleDeclaration,
) -> Result<TokenStream2> {
	let frame_support = generate_crate_access_2018("frame-support")?;
	let mut expansion = quote::quote!(
		#frame_support::construct_bundle! { #input }
	);
...
fn construct_runtime_explicit_to_explicit_expanded(
	input: TokenStream2,
	definition: ExplicitBundleDeclaration,
) -> Result<TokenStream2> {
	let frame_support = generate_crate_access_2018("frame-support")?;
	let mut expansion = quote::quote!(
		#frame_support::construct_bundle! { #input }
...
fn construct_runtime_final_expansion(
	...
	let hidden_crate_name = "construct_bundle";
...
fn decl_all_pallets<'a>(
	...
	for pallet_declaration in pallet_declarations {
		...
		let type_decl = quote!(
			// #(#attrs)*
			pub type #type_name<T> = #pallet::Pallet <#(#generics),*>;
		);
