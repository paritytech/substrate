...
pub fn expand_outer_dispatch(
	...
	let type_impl_gen = quote!(T: Config + pallet_template::Config + pallet_template_2::Config);
	...
	for pallet_declaration in pallets_with_call {
		...
		variant_defs.extend(quote! {
			#attr
			#[codec(index = #index)]
			#name( #scrate::dispatch::CallableCallFor<#name<#type_use_gen>, #runtime<#type_use_gen>> ),
		});
		variant_patterns.push(quote!(Call::<#type_use_gen>::#name(call)));
	...
	quote! {
		...
		impl<#type_impl_gen> #scrate::dispatch::Callable<T> for Pallet<#type_use_gen>
		{
			type RuntimeCall = Call<#type_use_gen>;
		}

		// impl<#type_impl_gen> #scrate::dispatch::Callable<T> for Bundle<#type_use_gen>
		// {
		// 	type RuntimeCall = Call<#type_use_gen>;
		// }

		impl<#type_impl_gen> #scrate::dispatch::GetDispatchInfo for Call<#type_use_gen> {
		...
		#[allow(deprecated)]
		impl<#type_impl_gen> #scrate::weights::GetDispatchInfo for Call<#type_use_gen> {}

		impl<#type_impl_gen> #scrate::dispatch::GetCallName for Call<#type_use_gen>
		{
		...
		impl<#type_impl_gen> #scrate::dispatch::GetCallIndex for Call<#type_use_gen>
		{
		...
		// impl<#type_impl_gen> #scrate::dispatch::GetCallMetadata for Call<#type_use_gen> {
		...
		// 	}
		// }
		// impl<#type_impl_gen> #scrate::dispatch::Dispatchable for Call<#type_use_gen> {
		// 	type RuntimeOrigin = frame_system::pallet_prelude::OriginFor<T>;
		// 	type Config = Call<#type_use_gen>;
		...
		// 	}
		// }
		impl<#type_impl_gen> #scrate::traits::UnfilteredDispatchable for Call<#type_use_gen> {
			type RuntimeOrigin = frame_system::pallet_prelude::OriginFor<T>;
