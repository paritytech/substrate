...
impl syn::parse::Parse for PalletEventAttr {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        if lookahead.peek(keyword::metadata) {
            ...
            Ok(PalletEventAttr::DepositEvent {
                fn_vis,
                span,
                fn_span,
            })
    ...
    fn from_attrs(attrs: Vec<PalletEventAttr>) -> syn::Result<Self> {
        ...
        for attr in attrs {
            match attr {
                ...
                PalletEventAttr::DepositEvent {
                    fn_vis, fn_span, ..
                } if deposit_event.is_none() => deposit_event = Some((fn_vis, fn_span)),
                attr => {
    ...
    pub fn try_from(
        ...
        let item = if let syn::Item::Enum(item) = item {
            ...
            return Err(syn::Error::new(
                item.span(),
                "Invalid pallet::event, expected item enum",
            ));
        };
