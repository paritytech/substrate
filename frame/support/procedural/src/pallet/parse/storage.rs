...
impl syn::parse::Parse for PalletStorageAttr {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        Ok(Self {
            getter: generate_content.parse::<syn::Ident>()?,
        })
    }
}
...
fn retrieve_arg(segment: &syn::PathSegment, arg_pos: usize) -> syn::Result<syn::GenericArgument> {
    if let syn::PathArguments::AngleBracketed(args) = &segment.arguments {
        if arg_pos < args.args.len() {
            Ok(args.args[arg_pos].clone())
        } else {
            let msg = format!(
                "pallet::storage unexpected number of generic argument, expected at \
				least {} args, found {}",
                arg_pos + 1,
                args.args.len()
            );
    ...
    pub fn try_from(
        ...
        let item = if let syn::Item::Type(item) = item {
            ...
            return Err(syn::Error::new(
                item.span(),
                "Invalid pallet::storage, expected item type",
            ));
        };
        ...
        instances.push(helper::check_type_def_gen(
            &item.generics,
            item.ident.span(),
        )?);
