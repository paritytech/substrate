...
fn construct_runtime_parsed(definition: RuntimeDefinition) -> Result<TokenStream2> {
    ...
    let all_but_system_modules = modules
        .iter()
...
fn decl_all_modules<'a>(
    ...
    let all_modules = names.iter().filter(|n| **n != SYSTEM_MODULE_NAME).fold(
        TokenStream2::default(),
        |combined, name| quote!((#name, #combined)),
    );

    let all_modules_with_system = names.iter().fold(
        TokenStream2::default(),
        |combined, name| quote!((#name, #combined)),
    );
...
fn decl_pallet_runtime_setup(
    ...
    let indices = module_declarations
        .iter()
