...
use crate::construct_runtime_v2::parse::pallets::PalletDef;
...
pub fn expand_outer_event(
	...
	for pallet_decl in pallet_decls {
		// if let Some(pallet_entry) = pallet_decl.find_part("Event") {
			...
			let index = pallet_decl.index.unwrap(); //Todo
			let instance = None; //pallet_decl.instance.as_ref();
			...
			let part_is_generic = false; // !generics.params.is_empty();
...
fn expand_event_conversion(
	...
	quote! {
		// #attr
		impl From<#pallet_event> for RuntimeEvent2 {
			fn from(x: #pallet_event) -> Self {
				RuntimeEvent2::#variant_name(x)
			}
		}
		// #attr
		impl TryInto<#pallet_event> for RuntimeEvent2 {
			type Error = ();
