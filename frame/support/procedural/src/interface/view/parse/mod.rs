...
impl Def {
	pub fn try_from(mut item: syn::ItemEnum) -> syn::Result<Self> {
		...
		// TODO: Check if other high-level attributes of #[view_entry::*] are present and error out
		...
		let runtime = match item.generics.params.len().cmp(&1) {
			Ordering::Equal => {
				...
				match param {
					...
					syn::GenericParam::Const(_) | syn::GenericParam::Lifetime(_) => {
						let msg = format!(
							"Invalid view_entry enum. Expected type generic. \
			...
			Ordering::Greater => {
				let msg = format!(
					"Invalid view_entry enum. Expected one generic. Got more. \
					  Try using the following: pub {}<Runtime>",
					item.ident.clone()
				);
				return Err(syn::Error::new(item_span, msg))
			...
			Ordering::Less => {
				let msg = format!(
					"Invalid view_entry enum. Expected one generic. Got none.. \
					  Try using the following: pub {}<Runtime>",
					item.ident.clone()
				);
				return Err(syn::Error::new(item_span, msg))
		...
		for variant in &mut item.variants {
			let call_idx_attr: Option<ViewEntryAttr> =
                crate::interface::helper::take_item_interface_attrs(&mut variant.attrs)?
                    .into_iter()
                    .try_fold(None, |mut call_idx_attr, attr| {
                        match attr {
                            ViewEntryAttr::Index(span, index) => {
                                if variant.discriminant.is_some() {
                                    let msg =
                                        "Invalid view_entry variant. Explicit discriminant is given. \
								Remove and use `#[view_entry::index($expr)]` instead.";
                                    return Err(syn::Error::new(variant.span(), msg))
                                }

                                if call_idx_attr.is_some() {
                                    let msg =
                                        "Invalid view_entry variant, multiple `#[view_entry::index($expr)]` \
...
impl syn::parse::Parse for ViewEntryAttr {
	fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
		input.parse::<syn::Token![#]>()?;
		let content;
		syn::bracketed!(content in input);
		content.parse::<keyword::view_entry>()?;
