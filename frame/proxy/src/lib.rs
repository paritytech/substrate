...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn kill_pure(
			...
			proxy_type: T::ProxyType,
			index: u16,
			#[pallet::compact] height: frame_system::BlockNumberOf<T>,
...
impl<T: Config> Pallet<T> {
	...
	pub fn pure_account(
		...
		proxy_type: &T::ProxyType,
		index: u16,
		maybe_when: Option<(frame_system::BlockNumberOf<T>, u32)>,
	) -> T::AccountId {
	...
	fn edit_announcements<
		F: FnMut(&Announcement<T::AccountId, CallHashOf<T>, frame_system::BlockNumberOf<T>>) -> bool,
	>(
	...
	fn do_proxy(
		def: ProxyDefinition<T::AccountId, T::ProxyType, frame_system::BlockNumberOf<T>>,
		real: T::AccountId,
