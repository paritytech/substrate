...
pub mod pallet {
	...
	pub type Proxies<T: Config> = StorageMap<
		...
		T::AccountId,
		(
			BoundedVec<ProxyDefinition<T::AccountId, T::ProxyType, frame_system::pallet_prelude::BlockNumberFor::<T>>, T::MaxProxies>,
			BalanceOf<T>,
		),
		ValueQuery,
	>;
	...
	pub type Announcements<T: Config> = StorageMap<
		...
		T::AccountId,
		(
			BoundedVec<Announcement<T::AccountId, CallHashOf<T>, frame_system::pallet_prelude::BlockNumberFor::<T>>, T::MaxPending>,
			BalanceOf<T>,
		),
		ValueQuery,
	>;
...
impl<T: Config> Pallet<T> {
	...
	fn edit_announcements<
		F: FnMut(&Announcement<T::AccountId, CallHashOf<T>, frame_system::pallet_prelude::BlockNumberFor::<T>>) -> bool,
	>(
	...
	pub fn find_proxy(
		...
		delegate: &T::AccountId,
		force_proxy_type: Option<T::ProxyType>,
	) -> Result<ProxyDefinition<T::AccountId, T::ProxyType, frame_system::pallet_prelude::BlockNumberFor::<T>>, DispatchError> {
		let f = |x: &ProxyDefinition<T::AccountId, T::ProxyType, frame_system::pallet_prelude::BlockNumberFor::<T>>| -> bool {
			&x.delegate == delegate &&
	...
	fn do_proxy(
		def: ProxyDefinition<T::AccountId, T::ProxyType, frame_system::pallet_prelude::BlockNumberFor::<T>>,
		real: T::AccountId,
