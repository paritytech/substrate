...
fn remove_announcement_works() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            Proxy::remove_announcement(Origin::signed(3), 1, [0; 32].into()),
            e
        );
...
fn reject_announcement_works() {
    new_test_ext().execute_with(|| {
        ...
        let e = Error::<Test>::NotFound;
        assert_noop!(
            Proxy::reject_announcement(Origin::signed(1), 3, [0; 32].into()),
            e
        );
        let e = Error::<Test>::NotFound;
        assert_noop!(
            Proxy::reject_announcement(Origin::signed(4), 3, [1; 32].into()),
            e
        );
...
fn delayed_requires_pre_announcement() {
    new_test_ext().execute_with(|| {
        ...
        let e = Error::<Test>::Unannounced;
        assert_noop!(
            Proxy::proxy_announced(Origin::signed(0), 2, 1, None, call.clone()),
            e
        );
...
fn proxy_announced_removes_announcement_and_returns_deposit() {
    new_test_ext().execute_with(|| {
        ...
        let e = Error::<Test>::Unannounced;
        assert_noop!(
            Proxy::proxy_announced(Origin::signed(0), 3, 1, None, call.clone()),
            e
        );
...
fn anonymous_works() {
    new_test_ext().execute_with(|| {
        ...
        expect_event(RawEvent::AnonymousCreated(
            anon.clone(),
            1,
            ProxyType::Any,
            0,
        ...
        let call = Box::new(Call::Proxy(ProxyCall::kill_anonymous(
            1,
            ProxyType::Any,
            0,
            1,
