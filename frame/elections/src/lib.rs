...
decl_storage! {
	trait Store for Module<T: Trait> as Elections {
		...
		pub ApprovalsOf get(fn approvals_of):
			map hasher(twox_32_concat) (T::AccountId, SetIndex) => Vec<ApprovalFlag>;
		...
		pub RegisterInfoOf get(fn candidate_reg_info):
			map hasher(twox_32_concat) T::AccountId => Option<(VoteIndex, u32)>;
		/// Basic information about a voter.
		pub VoterInfoOf get(fn voter_info):
			map hasher(twox_32_concat) T::AccountId => Option<VoterInfo<BalanceOf<T>>>;
		/// The present voter list (chunked and capped at [`VOTER_SET_SIZE`]).
		pub Voters get(fn voters): map hasher(twox_32_concat) SetIndex => Vec<Option<T::AccountId>>;
		...
		pub Proxy get(fn proxy): map hasher(blake2_64_concat) T::AccountId => Option<T::AccountId>;
	}
}
