...
//! Consensus extension module for Sassafras consensus. Collects on-chain randomness
...
pub trait Trait: pallet_timestamp::Trait {
	...
	/// Sassafras requires some logic to be triggered on every block to query for whether an epoch
...
/// A type signifying to Sassafras that it should perform epoch changes
...
decl_storage! {
	trait Store for Module<T: Trait> as Sassafras {
		...
		pub Authorities get(fn authorities): Vec<(AuthorityId, SassafrasAuthorityWeight)>;
...
/// A Sassafras public key
pub type SassafrasKey = [u8; PUBLIC_KEY_LENGTH];
...
impl<T: Trait> Module<T> {
	...
	pub fn enact_epoch_change(
		authorities: Vec<(AuthorityId, SassafrasAuthorityWeight)>,
		next_authorities: Vec<(AuthorityId, SassafrasAuthorityWeight)>,
	) {
	...
	fn deposit_consensus<U: Encode>(new: U) {
		let log: DigestItem<T::Hash> = DigestItem::Consensus(SASSAFRAS_ENGINE_ID, new.encode());
	...
	fn initialize_authorities(authorities: &[(AuthorityId, SassafrasAuthorityWeight)]) {
