...
fn rewards_distribution_is_fair_basic_with_fractions() {
	// basically checks the case where the amount of rewards is less than the pool shares. for this,
	// we have to rely on fixed point arithmetic.
	ExtBuilder::default().build_and_execute(|| {
		let ed = Balances::minimum_balance();
		...
		// uneven upcoming rewards are shared equally.
...
fn rewards_distribution_is_fair_bond_extra() {
	todo!("10 20 30 all join, reward comes in, equal, one bonds more, rewards come in, more share")
}

#[test]
fn rewards_distribution_is_fair_unbond() {
	todo!("10 20 30 all join, reward comes in, equal, one unbonds, rewards come in, less share")
}
...
// 				123,
// 				RewardPool::<Runtime> { balance: Zero::zero(), total_earnings: Zero::zero() },
// 			);
...
// 	fn del(points: Balance, last_recorded_reward_counter: FixedU128) -> PoolMember<Runtime> {
// 		PoolMember {
// 			pool_id: 1,
// 			points,
...
// 	fn rew(
// 		last_recorded_reward_counter: FixedU128,
