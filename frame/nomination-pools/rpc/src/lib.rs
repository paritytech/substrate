...
pub trait NominationPoolsRpc<BlockHash, AccountId, ResponseType> {
	#[rpc(name = "nompools_status")]
	fn status(&self, member: AccountId, at: Option<BlockHash>) -> Result<ResponseType, ()>;
}
	...
	Balance: Codec,
{
	...
	) -> Result<MemberStatus<Balance>, ()> {
		let api = self.client.runtime_api();
		let at = BlockId::hash(at.unwrap_or_else(|| self.client.info().best_hash));
		api.status(&at, member).map_err(|_| ())
	}
}
