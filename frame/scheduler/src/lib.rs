...
pub mod pallet {
	...
	#[pallet::storage]
	pub type IncompleteSince<T: Config> = StorageValue<_, frame_system::pallet_prelude::BlockNumberFor::<T>>;
	...
	pub(crate) type Lookup<T: Config> =
		StorageMap<_, Twox64Concat, TaskName, TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>>;
	...
	pub enum Event<T: Config> {
		/// Scheduled some task.
		Scheduled { when: frame_system::pallet_prelude::BlockNumberFor::<T>, index: u32 },
		/// Canceled some task.
		Canceled { when: frame_system::pallet_prelude::BlockNumberFor::<T>, index: u32 },
		/// Dispatched some task.
		Dispatched {
			task: TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>,
			id: Option<TaskName>,
			result: DispatchResult,
		},
		/// The call for the provided hash was not found so the task has been aborted.
		CallUnavailable { task: TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>, id: Option<TaskName> },
		/// The given task was unable to be renewed since the agenda is full at that block.
		PeriodicFailed { task: TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>, id: Option<TaskName> },
		/// The given task can never be executed since it is overweight.
		PermanentlyOverweight { task: TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>, id: Option<TaskName> },
	}
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn on_initialize(now: frame_system::pallet_prelude::BlockNumberFor::<T>) -> Weight {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn schedule(
			...
			maybe_periodic: Option<schedule::Period<frame_system::pallet_prelude::BlockNumberFor::<T>>>,
			priority: schedule::Priority,
		...
		pub fn cancel(origin: OriginFor<T>, when: frame_system::pallet_prelude::BlockNumberFor::<T>, index: u32) -> DispatchResult {
			T::ScheduleOrigin::ensure_origin(origin.clone())?;
			let origin = <T as Config>::RuntimeOrigin::from(origin);
		...
		pub fn schedule_after(
			origin: OriginFor<T>,
			after: frame_system::pallet_prelude::BlockNumberFor::<T>,
			maybe_periodic: Option<schedule::Period<frame_system::pallet_prelude::BlockNumberFor::<T>>>,
			priority: schedule::Priority,
			call: Box<<T as Config>::RuntimeCall>,
		) -> DispatchResult {
...
impl<T: Config<Hash = PreimageHash>> Pallet<T> {
	...
	pub fn migrate_v1_to_v4() -> Weight {
		...
		Agenda::<T>::translate::<
			Vec<Option<ScheduledV1<<T as Config>::RuntimeCall, frame_system::pallet_prelude::BlockNumberFor::<T>>>>,
			_,
		>(|_, agenda| {
...
impl<T: Config> Pallet<T> {
	...
	fn resolve_time(when: DispatchTime<frame_system::pallet_prelude::BlockNumberFor::<T>>) -> Result<frame_system::pallet_prelude::BlockNumberFor::<T>, DispatchError> {
	...
	fn place_task(
		when: frame_system::pallet_prelude::BlockNumberFor::<T>,
		what: ScheduledOf<T>,
	) -> Result<TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>, (DispatchError, ScheduledOf<T>)> {
	...
	fn cleanup_agenda(when: frame_system::pallet_prelude::BlockNumberFor::<T>) {
		let mut agenda = Agenda::<T>::get(when);
	...
	fn do_schedule(
		when: DispatchTime<frame_system::pallet_prelude::BlockNumberFor::<T>>,
		maybe_periodic: Option<schedule::Period<frame_system::pallet_prelude::BlockNumberFor::<T>>>,
		priority: schedule::Priority,
		origin: T::PalletsOrigin,
		call: Bounded<<T as Config>::RuntimeCall>,
	) -> Result<TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>, DispatchError> {
	...
	fn do_cancel(
		...
		(when, index): TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>,
	...
	fn do_reschedule(
		(when, index): TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>,
		new_time: DispatchTime<frame_system::pallet_prelude::BlockNumberFor::<T>>,
	) -> Result<TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>, DispatchError> {
...
impl<T: Config> Pallet<T> {
	...
	fn service_agendas(weight: &mut WeightMeter, now: frame_system::pallet_prelude::BlockNumberFor::<T>, max: u32) {
...
impl<T: Config<Hash = PreimageHash>>
	schedule::v2::Anon<frame_system::pallet_prelude::BlockNumberFor::<T>, <T as Config>::RuntimeCall, T::PalletsOrigin> for Pallet<T>
{
	type Address = TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>;
	type Hash = T::Hash;
	...
	fn next_dispatch_time((when, index): Self::Address) -> Result<frame_system::pallet_prelude::BlockNumberFor::<T>, ()> {
		Agenda::<T>::get(when).get(index as usize).ok_or(()).map(|_| when)
	}
}

impl<T: Config<Hash = PreimageHash>>
	schedule::v2::Named<frame_system::pallet_prelude::BlockNumberFor::<T>, <T as Config>::RuntimeCall, T::PalletsOrigin> for Pallet<T>
{
	type Address = TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>;
	type Hash = T::Hash;
	...
	fn next_dispatch_time(id: Vec<u8>) -> Result<frame_system::pallet_prelude::BlockNumberFor::<T>, ()> {
		let name = blake2_256(&id[..]);
		Lookup::<T>::get(name)
...
impl<T: Config> schedule::v3::Anon<frame_system::pallet_prelude::BlockNumberFor::<T>, <T as Config>::RuntimeCall, T::PalletsOrigin>
	for Pallet<T>
{
	type Address = TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>;
	...
	fn next_dispatch_time((when, index): Self::Address) -> Result<frame_system::pallet_prelude::BlockNumberFor::<T>, DispatchError> {
		Agenda::<T>::get(when)
			.get(index as usize)
...
impl<T: Config> schedule::v3::Named<frame_system::pallet_prelude::BlockNumberFor::<T>, <T as Config>::RuntimeCall, T::PalletsOrigin>
	for Pallet<T>
{
	type Address = TaskAddress<frame_system::pallet_prelude::BlockNumberFor::<T>>;
	...
	fn next_dispatch_time(id: TaskName) -> Result<frame_system::pallet_prelude::BlockNumberFor::<T>, DispatchError> {
		Lookup::<T>::get(id)
			.and_then(|(when, index)| Agenda::<T>::get(when).get(index as usize).map(|_| when))
			.ok_or(DispatchError::Unavailable)
	}
