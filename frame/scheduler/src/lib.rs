...
pub mod pallet {
	...
	#[pallet::storage]
	pub type IncompleteSince<T: Config> = StorageValue<_, frame_system::BlockNumberOf<T>>;
	...
	pub(crate) type Lookup<T: Config> =
		StorageMap<_, Twox64Concat, TaskName, TaskAddress<frame_system::BlockNumberOf<T>>>;
	...
	pub enum Event<T: Config> {
		/// Scheduled some task.
		Scheduled { when: frame_system::BlockNumberOf<T>, index: u32 },
		/// Canceled some task.
		Canceled { when: frame_system::BlockNumberOf<T>, index: u32 },
		...
		CallUnavailable { task: TaskAddress<frame_system::BlockNumberOf<T>>, id: Option<TaskName> },
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn schedule(
			...
			maybe_periodic: Option<schedule::Period<frame_system::BlockNumberOf<T>>>,
			priority: schedule::Priority,
		...
		pub fn schedule_after(
			origin: OriginFor<T>,
			after: frame_system::BlockNumberOf<T>,
			maybe_periodic: Option<schedule::Period<frame_system::BlockNumberOf<T>>>,
			priority: schedule::Priority,
			call: Box<<T as Config>::RuntimeCall>,
		) -> DispatchResult {
...
impl<T: Config<Hash = PreimageHash>> Pallet<T> {
	...
	pub fn migrate_v1_to_v4() -> Weight {
		...
		Agenda::<T>::translate::<
			Vec<Option<ScheduledV1<<T as Config>::RuntimeCall, frame_system::BlockNumberOf<T>>>>,
			_,
		>(|_, agenda| {
...
impl<T: Config> Pallet<T> {
	...
	fn place_task(
		when: frame_system::BlockNumberOf<T>,
		what: ScheduledOf<T>,
	) -> Result<TaskAddress<frame_system::BlockNumberOf<T>>, (DispatchError, ScheduledOf<T>)> {
	...
	fn cleanup_agenda(when: frame_system::BlockNumberOf<T>) {
		let mut agenda = Agenda::<T>::get(when);
	...
	) -> Result<TaskAddress<frame_system::BlockNumberOf<T>>, DispatchError> {
	...
	fn do_cancel(
		...
		(when, index): TaskAddress<frame_system::BlockNumberOf<T>>,
	...
	fn do_reschedule(
		(when, index): TaskAddress<frame_system::BlockNumberOf<T>>,
		new_time: DispatchTime<frame_system::BlockNumberOf<T>>,
	) -> Result<TaskAddress<frame_system::BlockNumberOf<T>>, DispatchError> {
...
impl<T: Config> Pallet<T> {
	...
	fn service_agendas(weight: &mut WeightMeter, now: frame_system::BlockNumberOf<T>, max: u32) {
	...
	schedule::v2::Named<frame_system::BlockNumberOf<T>, <T as Config>::RuntimeCall, T::PalletsOrigin> for Pallet<T>
{
	...
	fn next_dispatch_time(id: Vec<u8>) -> Result<frame_system::BlockNumberOf<T>, ()> {
		let name = blake2_256(&id[..]);
		Lookup::<T>::get(name)
	...
	for Pallet<T>
{
	...
	fn next_dispatch_time(id: TaskName) -> Result<frame_system::BlockNumberOf<T>, DispatchError> {
		Lookup::<T>::get(id)
			.and_then(|(when, index)| Agenda::<T>::get(when).get(index as usize).map(|_| when))
			.ok_or(DispatchError::Unavailable)
	}
