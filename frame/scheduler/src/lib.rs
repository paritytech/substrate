...
impl<T: Trait> Module<T> {
    ...
    fn do_cancel((when, index): TaskAddress<T::BlockNumber>) -> Result<(), DispatchError> {
        if let Some(s) = Agenda::<T>::mutate(when, |agenda| {
            agenda.get_mut(index as usize).and_then(Option::take)
        }) {
...
mod tests {
    ...
    use frame_support::{
        ...
        weights::{constants::RocksDbWeight, DispatchClass, FunctionOf, Pays},
    };
    use sp_core::H256;
    ...
    fn on_initialize_weight_is_correct() {
        new_test_ext().execute_with(|| {
            let base_weight: Weight =
                <Test as frame_system::Trait>::DbWeight::get().reads_writes(1, 2) + 10_000_000;
