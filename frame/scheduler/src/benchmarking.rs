...
fn fill_schedule<T: Config>(when: frame_system::BlockNumberOf<T>, n: u32) -> Result<(), &'static str> {
...
benchmarks! {
	...
	service_agendas_base {
		let now = BlockNumberOf::<T>::from(BLOCK_NUMBER);
	...
	schedule {
		...
		let periodic = Some((BlockNumberOf::<T>::one(), 100));
		let priority = 0;
