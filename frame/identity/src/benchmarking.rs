...
benchmarks! {
	...
	add_registrar {
		let r in ...;
	}: (RawOrigin::Root, account::<T>("registrar", r + 1))
	...
	request_judgement {
		...
		let r in ...;
		let x in ...;
	}: (RawOrigin::Signed(caller), r - 1, 10.into())
	...
	}: (RawOrigin::Signed(caller), r, 10.into())
	...
	set_account_id {
		...
		Identity::<T>::add_registrar(RawOrigin::Root.into(), caller.clone())?;
	}: (RawOrigin::Signed(caller), r, account::<T>("new", 0))
	...
	}: (RawOrigin::Signed(caller), r, fields)
	...
	}: (RawOrigin::Signed(caller), r, user_lookup, Judgement::Reasonable)
