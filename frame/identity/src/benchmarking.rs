...
benchmarks! {
	...
	verify {
		assert_last_event::<T>(PalletEvent::<T>::IdentitySet { who: caller }.into());
	}
	...
	verify {
		assert_last_event::<T>(PalletEvent::<T>::JudgementRequested { who: caller, registrar_index: r-1 }.into());
	}
	...
	verify {
		assert_last_event::<T>(PalletEvent::<T>::JudgementUnrequested { who: caller, registrar_index: r-1 }.into());
	}
	...
	verify {
		assert_last_event::<T>(PalletEvent::<T>::JudgementGiven { target: user, registrar_index: r }.into())
	}
