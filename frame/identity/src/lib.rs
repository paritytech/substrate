...
decl_storage! {
	trait Store for Module<T: Trait> as Identity {
		...
		pub IdentityOf get(fn identity):
			map hasher(twox_32_concat) T::AccountId => Option<Registration<BalanceOf<T>>>;

		/// The super-identity of an alternative "sub" identity together with its name, within that
		/// context. If the account is not some other account's sub-identity, then just `None`.
		pub SuperOf get(fn super_of):
			map hasher(blake2_64_concat) T::AccountId => Option<(T::AccountId, Data)>;

		/// Alternative "sub" identities of this account.
		///
		/// The first item is the deposit, the second is a vector of the accounts.
		pub SubsOf get(fn subs_of):
			map hasher(twox_32_concat) T::AccountId => (BalanceOf<T>, Vec<T::AccountId>);
