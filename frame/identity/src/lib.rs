...
pub mod benchmarking {
	...
	mod on_initialize {
		fn components() -> Vec<(&'static str, u32, u32)> { ... }
		fn instance(components: &[(&'static str, u32)]) { ... }
	}
}
...
impl<T: Trait> Benchmarking for Module<T> {
	...
	fn run_benchmarks() -> Vec<BenchmarkResults> {
		...
		let components = set_identity::components();
		for (i, (name, low, high)) in components.iter().enumerate() {

			for x in 0..100 {
				let step = x * (high - low) / 100 + low;

				let c = components.iter().enumerate().map(|(j, (n, low, high))| (n, if x == y { step } else { (high - low) / 2 + low }}) ).collect();
				for x in 0..REPEATS {
