...
use frame_support::{decl_module, decl_event, decl_storage, ensure, decl_error, traits::{Currency, ReservableCurrency, OnUnbalanced, Get}, weights::SimpleDispatchInfo, Callable};
...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		#[benchmark = benchmarks::set_identity]
...
pub mod benchmarking {
	fn set_identity_components() -> Vec<(&'static str, u32, u32)> {
		vec![("R", 1, 16), ("X", 1, 20)]
	}
	/// Assumes externalities are set up with a mutable State.
	///
	/// panics is component_name isn't from set_identity_components or component_value is out of
	/// the range of set_identity_components.
	///
	/// sets up state randomly and returns a randomly generated set_identity with sensible (fixed)
	/// values for all complexity components except that mentioned in the identity.
	fn set_identity<T: Trait>(component_name: &'static str, component_value: u32) -> Call {
		<
		Call::set_identity(...)
	}


	fn on_initialize_components() -> ...
	fn on_initialize<T: Trait>()

	fn set_identity_weight(c: Call) -> Weight {
		...
	}
}

impl<T: Trait> Benchmarking for Module<T> {
	// autogenerated from the decl_dispatch macro:
	fn run_benchmarks() -> Vec<BenchmarkResults> {
		// Go through every dispatchable;
		benchmarks::set_identity(component, value)
		for x in [1, 2, 5, 10, 20, 50, 100].iter() {
			let elapsed = new_ext().execute_with() || {
				let c = create_call_and_initial_random_state(x, 50);
				let t = start_timer();
