...
use sp_runtime::{
	traits::{AppendZerosInput, Saturating, StaticLookup, Zero},
	RuntimeDebug,
};
...
pub mod pallet {
	...
	pub(super) type SubsOf<T: Config> =
		StorageMap<_, Twox64Concat, T::AccountId, (BalanceOf<T>, Vec<T::AccountId>), ValueQuery>;
	...
	pub(super) type Registrars<T: Config> =
		StorageValue<_, Vec<Option<RegistrarInfo<BalanceOf<T>, T::AccountId>>>, ValueQuery>;
