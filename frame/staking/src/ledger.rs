...
pub struct StakingLedger<T: Config> {
	...
	pub(crate) stash: T::AccountId,
	/// The total amount of the stash's balance that we are currently accounting for.
	/// It's just `active` plus all the `unlocking` balances.
	#[codec(compact)]
	pub(crate) total: BalanceOf<T>,
	/// The total amount of the stash's balance that will be at stake in any forthcoming
	/// rounds.
	#[codec(compact)]
	pub(crate) active: BalanceOf<T>,
	/// Any balance that is becoming free, which may eventually be transferred out of the stash
	/// (assuming it doesn't get slashed first). It is assumed that this will be treated as a first
	/// in, first out queue where the new (higher value) eras get pushed on the back.
	pub(crate) unlocking: BoundedVec<UnlockChunk<BalanceOf<T>>, T::MaxUnlockingChunks>,
	/// List of eras for which the stakers behind a validator have claimed rewards. Only updated
	/// for validators.
	pub(crate) claimed_rewards: BoundedVec<EraIndex, T::HistoryDepth>,
	/// The controller that is associated with this ledger's stash.
	///
	/// Note that this is not stored onchain, and is only bundled when the ledger is read from
	/// state.
	///
	/// This is always set to `Some`, use [`controller`] function to get it.
	#[codec(skip)]
...
impl<T: Config> StakingLedger<T> {
	...
	pub fn new(stash: T::AccountId, controller: T::AccountId, stake: BalanceOf<T>) -> Self {
	...
	pub fn controller(&self) -> T::AccountId {
		self.controller
			.clone()
			.expect("controller is always set when ledger is read from state; qed")
	}

	pub fn get_storage(controller: &T::AccountId) -> Option<Self> {
