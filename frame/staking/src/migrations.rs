	...
	impl<T: Config> OnRuntimeUpgrade for MigrateToV13<T> {
		...
		fn pre_upgrade() -> Result<Vec<u8>, &'static str> {
			frame_support::ensure!(
				StorageVersion::<T>::get() == obsolete::Releases::V12_0_0,
...
pub mod v12 {
	...
	impl<T: Config> OnRuntimeUpgrade for MigrateToV12<T> {
		...
		fn pre_upgrade() -> Result<Vec<u8>, &'static str> {
			frame_support::ensure!(
				StorageVersion::<T>::get() == obsolete::Releases::V11_0_0,
...
pub mod v9 {
	...
	#[cfg(feature = "try-runtime")]
	use sp_std::vec::Vec;
	use frame_election_provider_support::{SortedListProvider, ReadOnlySortedListProvider};
	...
	impl<T: MigrationConfig> OnRuntimeUpgrade for InjectValidatorsIntoVoterList<T> {
			...
			if obsolete::StorageVersion::<T::Config>::get() == obsolete::Releases::V8_0_0 {
				...
				for (v, _) in Validators::<T::Config>::iter() {
					...
					let _ = T::VoterList::on_insert(v.clone(), weight).map_err(|err| {
						frame_support::log::warn!("failed to insert {:?} into VoterList: {:?}", v, err)
					});
				}
		...
		fn pre_upgrade() -> Result<Vec<u8>, &'static str> {
			frame_support::ensure!(
				obsolete::StorageVersion::<T>::get() == obsolete::Releases::V8_0_0,
				"must upgrade linearly"
			);
		...
		fn post_upgrade(prev_count: Vec<u8>) -> Result<(), &'static str> {
			...
			frame_support::ensure!(
				obsolete::StorageVersion::<T>::get() == obsolete::Releases::V9_0_0,
				"must upgrade "
			);
			Ok(())
		}
