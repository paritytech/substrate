...
decl_module! {
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		fn on_initialize(_now: T::BlockNumber) -> Weight {
			let task_weight = if <SlashTaskExecutor<T>>::decode_len().unwrap_or_default() > 0 {
			...
			let finalize_weight = T::DbWeight::get().reads(1);
			task_weight.saturating_add(finalize_weight)
		}
