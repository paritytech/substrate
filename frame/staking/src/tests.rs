...
mod election_data_provider {
	...
	fn voters_exclude_slashed() {
		ExtBuilder::default().build_and_execute(|| {
			assert_eq!(Staking::nominators(101).unwrap().targets, vec![11, 21]);
			assert_eq!(
				<Staking as ElectionDataProvider>::electing_voters(None, 0)
					.unwrap()
					.iter()
					.find(|x| x.0 == 101)
					.unwrap()
	...
	fn can_paginate() {
		ExtBuilder::default()
			...
			.build_and_execute(|| {
				...
				assert_eq!(
					Staking::electing_voters(Some(4), 1).unwrap(),
					vec![
				...
				assert_eq!(
					Staking::electing_voters(Some(4), 0).unwrap(),
					vec![
	...
	fn respects_snapshot_len_limits() {
		ExtBuilder::default()
			...
			.build_and_execute(|| {
				...
				// if limits is less..
				assert_eq!(Staking::electing_voters(Some(1), 0).unwrap().len(), 1);

				// if limit is equal..
				assert_eq!(Staking::electing_voters(Some(5), 0).unwrap().len(), 5);

				// if limit is more.
				assert_eq!(Staking::electing_voters(Some(55), 0).unwrap().len(), 5);

				// if target limit is more..
				assert_eq!(Staking::electable_targets(Some(6), 0).unwrap().len(), 4);
				assert_eq!(Staking::electable_targets(Some(4), 0).unwrap().len(), 4);

				// if target limit is less, then we return an error.
				assert_eq!(
					Staking::electable_targets(Some(1), 0).unwrap_err(),
	...
	fn only_iterates_max_2_times_nominators_quota() {
		ExtBuilder::default()
			...
			.build_and_execute(|| {
				...
				assert_eq!(
					Staking::electing_voters(Some(3), 0)
						.unwrap()
						.iter()
						.map(|(stash, _, _)| stash)
						.copied()
	...
	fn get_max_len_voters_even_if_some_nominators_are_slashed() {
		ExtBuilder::default()
			...
			.build_and_execute(|| {
				...
				// we take 5 voters
				assert_eq!(
					Staking::electing_voters(Some(5), 0)
						.unwrap()
						.iter()
						.map(|(stash, _, _)| stash)
						.copied()
				...
				// we take 4 voters
				assert_eq!(
					Staking::electing_voters(Some(4), 0)
						.unwrap()
						.iter()
						.map(|(stash, _, _)| stash)
						.copied()
...
fn change_of_max_nominations() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Staking::electing_voters(None, 0).unwrap().len(), 3 + 3);
			...
			assert_eq!(Staking::electing_voters(None, 0).unwrap().len(), 3 + 2);
			...
			assert_eq!(Staking::electing_voters(None, 0).unwrap().len(), 3 + 1);
