...
fn payout_stakers_handles_weight_refund() {
	...
	ExtBuilder::default().has_stakers(false).build_and_execute(|| {
		...
		let call = TestRuntimeCall::Staking(StakingCall::payout_stakers { validator_stash: 11, era: 1 });
		...
		let call = TestRuntimeCall::Staking(StakingCall::payout_stakers { validator_stash: 11, era: 2 });
		let info = call.get_dispatch_info();
		let result = call.dispatch(Origin::signed(20));
		assert_ok!(result);
		...
		let call = TestRuntimeCall::Staking(StakingCall::payout_stakers { validator_stash: 11, era: 3 });
		let info = call.get_dispatch_info();
		let result = call.dispatch(Origin::signed(20));
		assert_ok!(result);
		...
		let call = TestRuntimeCall::Staking(StakingCall::payout_stakers { validator_stash: 11, era: 5 });
		let info = call.get_dispatch_info();
		let result = call.dispatch(Origin::signed(20));
		assert_ok!(result);
