...
fn basic_setup_works() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		if cfg!(feature = "equalize") {
			...
			// initial slot_stake
			assert_eq!(Staking::slot_stake_for_era(Staking::current_era()), 1250);
		} else {
			assert_eq!(
				Staking::validator_for_era(Staking::current_era()),
				vec![
			...
			// initial slot_stake
			assert_eq!(Staking::slot_stake_for_era(Staking::current_era()), 1125);
		}
...
fn rewards_should_work() {
	...
	ExtBuilder::default().nominate(true).build().execute_with(|| {
		...
		assert_eq!(Staking::current_era_reward(), EraPoints {
			total: 50*3,
			individual: vec![100, 50],
		});
		...
		assert_eq!(Session::validators(), vec![11, 21]);
		<Module<Test>>::reward_by_indices(vec![(0, 1)]);
...
fn less_than_needed_candidates_works() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert!(
				Staking::validator_for_era(Staking::current_era()).iter()
					.all(|info| info.exposure.others.is_empty())
			);
			check_exposure_all(Staking::current_era());
			check_nominator_all(Staking::current_era());
		});
	...
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		on_offence_now(
			&[OffenceDetails {
				offender: (
					11,
					mock::validator_current_info(11).exposure,
				),
				reporters: vec![],
			}],
			&[Perbill::from_percent(5)],
		);
		let expo = mock::validator_current_info(11).exposure;
		let slash_value = 50;
...
fn forcing_new_era_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		start_session(8);
		assert_eq!(Staking::current_era(), 1); // There is one session delay
		start_session(9);
		assert_eq!(Staking::current_era(), 2);
		...
		start_session(10);
		assert_eq!(Staking::current_era(), 2); // There is one session delay
		start_session(11);
		assert_eq!(Staking::current_era(), 3);
		start_session(12);
...
fn cannot_transfer_staked_balance() {
	...
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		assert_eq!(mock::validator_current_info(11).exposure.total, 1000);
...
fn cannot_transfer_staked_balance_2() {
	...
	ExtBuilder::default().nominate(false).fair(true).build().execute_with(|| {
		...
		assert_eq!(mock::validator_current_info(21).exposure.total, 1000);
...
fn cannot_reserve_staked_balance() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Balances::free_balance(&11), 1000);
		// Confirm account 11 (via controller 10) is totally staked
		assert_eq!(mock::validator_current_info(11).exposure.own, 1000);
		// Confirm account 11 cannot transfer as a result
...
fn bond_extra_and_withdraw_unbonded_works() {
	...
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		assert_eq!(mock::validator_current_info(11).exposure, Exposure { total: 1000, own: 1000, others: vec![] });
		...
		assert_ne!(mock::validator_current_info(11).exposure, Exposure { total: 1000 + 100, own: 1000 + 100, others: vec![] });
		...
		assert_eq!(mock::validator_current_info(11).exposure, Exposure { total: 1000 + 100, own: 1000 + 100, others: vec![] });
...
fn slot_stake_is_least_staked_validator_and_exposure_defines_maximum_punishment() {
	...
	ExtBuilder::default().nominate(false).fair(false).build().execute_with(|| {
		...
		assert_eq!(mock::validator_current_info(11).exposure.total, 1000);
		assert_eq!(mock::validator_current_info(21).exposure.total, 2000);
		...
		assert_eq!(Staking::slot_stake_for_era(Staking::current_era()), mock::validator_current_info(11).exposure.total);

		// Now lets lower account 20 stake
		mock::bypass_logic_change_current_exposure(21, Exposure { total: 69, own: 69, others: vec![] });
		assert_eq!(mock::validator_current_info(21).exposure.total, 69);
		...
		assert_eq!(mock::validator_current_info(11).exposure.total, 1000);
		assert_eq!(mock::validator_current_info(21).exposure.total, 69);
		assert_eq!(Staking::slot_stake_for_era(Staking::current_era()), 69);
		...
		// -- new infos
		assert_eq!(mock::validator_current_info(11).exposure.total, 1000 + total_payout_0 / 2);
		assert_eq!(mock::validator_current_info(21).exposure.total, 69 + total_payout_0 / 2);
		// -- slot stake should also be updated.
		assert_eq!(Staking::slot_stake_for_era(Staking::current_era()), 69 + total_payout_0 / 2);
...
fn bond_with_little_staked_value_bounded_by_slot_stake() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(Staking::slot_stake_for_era(Staking::current_era()), 1);
...
fn phragmen_linear_worse_case_equalize() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq_error_rate!(mock::validator_current_info(11).exposure.total, 3000, 2);
			assert_eq_error_rate!(mock::validator_current_info(21).exposure.total, 2255, 2);
			assert_eq_error_rate!(mock::validator_current_info(31).exposure.total, 2255, 2);
			assert_eq_error_rate!(mock::validator_current_info(41).exposure.total, 1925, 2);
			assert_eq_error_rate!(mock::validator_current_info(51).exposure.total, 1870, 2);
			assert_eq_error_rate!(mock::validator_current_info(61).exposure.total, 1890, 2);
			assert_eq_error_rate!(mock::validator_current_info(71).exposure.total, 1800, 2);
...
fn phragmen_should_not_overflow_validators() {
	ExtBuilder::default().nominate(false).build().execute_with(|| {
		...
		// check_exposure_all();
		assert_eq!(mock::validator_current_info(3).exposure.total, u64::max_value());
		assert_eq!(mock::validator_current_info(5).exposure.total, u64::max_value());
	})
}
...
fn reward_validator_slashing_validator_doesnt_overflow() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let exposure = Exposure { total: stake, own: stake, others: vec![] };
		let prefs = ValidatorPrefs { commission: Perbill::zero() };

		// Check reward
		let _ = Staking::reward_validator(&11, &prefs, &exposure, reward_slash);
		...
		mock::bypass_logic_change_current_exposure(11, Exposure {
			total: stake,
			own: 1,
		...
		on_offence_now(
			&[
				OffenceDetails {
					offender: (11, mock::validator_current_info(11).exposure),
					reporters: vec![],
				},
			],
...
fn reward_from_authorship_event_handler_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(CurrentEraPointsEarned::get().individual, vec![20 + 2 * 3 + 1, 1]);
...
fn add_reward_points_fns_works() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(CurrentEraPointsEarned::get().individual, vec![4, 2]);
...
fn era_is_always_same_length() {
	...
	ExtBuilder::default().build().execute_with(|| {
		start_era(1);
		assert_eq!(Staking::era_start_session_index()[0], SessionsPerEra::get());

		start_era(2);
		assert_eq!(Staking::era_start_session_index()[0], SessionsPerEra::get() * 2);
		...
		advance_session();
		assert_eq!(Staking::current_era(), 3);
		assert_eq!(Staking::era_start_session_index()[0], session + 2);

		start_era(4);
		assert_eq!(Staking::era_start_session_index()[0], session + 2 + SessionsPerEra::get());
	});
}
...
fn reporters_receive_their_slice() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(mock::validator_current_info(11).exposure.total, initial_balance);
...
fn invulnerables_are_not_slashed() {
	...
	ExtBuilder::default().invulnerables(vec![11]).build().execute_with(|| {
		...
		let exposure = mock::validator_current_info(21).exposure;
		...
		on_offence_now(
			&[
				...
				OffenceDetails {
					offender: (21, mock::validator_current_info(21).exposure),
					reporters: vec![],
				},
			],
...
fn garbage_collection_on_window_pruning() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let exposure = mock::validator_current_info(11).exposure;
...
fn slashing_nominators_by_span_max() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let exposure_11 = mock::validator_current_info(11).exposure;
		let exposure_21 = mock::validator_current_info(21).exposure;
		assert_eq!(Balances::free_balance(&101), 2000);
...
#[test]
fn unplanned_future_era() {
	ExtBuilder::default().build().execute_with(|| {
		assert_eq!(Session::current_index(), 0);
		Staking::on_session_ending(0, 10);
		assert_eq!(Staking::era_start_session_index(), vec![0, 10]);
		Staking::on_session_ending(1, 20);
		assert_eq!(Staking::era_start_session_index(), vec![0, 10, 20]);
		assert_ok!(Staking::force_new_era(Origin::ROOT));
		Staking::on_session_ending(2, 5);
		assert_eq!(Staking::era_start_session_index(), vec![0, 5]);
	});
}
