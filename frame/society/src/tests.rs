...
fn founding_works() {
    EnvBuilder::new()
        ...
        .execute(|| {
            ...
            assert_noop!(
                Society::found(Origin::signed(5), 20, 100, vec![]),
                BadOrigin
            );
            // Account 1 can start a society, where 10 is the founding member
            assert_ok!(Society::found(
                Origin::signed(1),
                10,
                100,
                b"be cool".to_vec()
...
fn unfounding_works() {
    EnvBuilder::new()
        ...
        .execute(|| {
            ...
            assert_noop!(
                Society::unfound(Origin::signed(2)),
                Error::<Test, _>::NotFounder
            );
            ...
            assert_noop!(
                Society::unfound(Origin::signed(20)),
                Error::<Test, _>::NotHead
            );
        });
}
...
fn basic_new_member_works() {
    EnvBuilder::new().execute(|| {
        ...
        assert_eq!(
            Society::candidates(),
            vec![create_bid(0, 20, BidKind::Deposit(25))]
        );
...
fn bidding_works() {
    EnvBuilder::new().execute(|| {
        ...
        assert_eq!(
            Society::candidates(),
            vec![create_bid(500, 50, BidKind::Deposit(25))]
        );
        ...
        assert_eq!(
            Society::candidates(),
            vec![create_bid(1900, 60, BidKind::Deposit(25))]
        );
...
fn unbidding_works() {
    EnvBuilder::new().execute(|| {
        ...
        assert_noop!(
            Society::unbid(Origin::signed(30), 1),
            Error::<Test, _>::BadPosition
        );
        ...
        assert_eq!(
            Society::candidates(),
            vec![create_bid(1000, 20, BidKind::Deposit(25))]
        );
    });
}
...
fn payout_works() {
    EnvBuilder::new().execute(|| {
        ...
        // payout not ready
        assert_noop!(
            Society::payout(Origin::signed(20)),
            Error::<Test, _>::NoPayout
        );
        run_to_block(9);
...
fn suspended_member_life_cycle_works() {
    EnvBuilder::new().execute(|| {
        ...
        assert_ok!(Society::judge_suspended_member(
            Origin::signed(2),
            20,
            false
        ));
...
fn unvouch_works() {
    EnvBuilder::new().execute(|| {
        ...
        assert_eq!(
            <Bids<Test>>::get(),
            vec![create_bid(100, 20, BidKind::Vouch(10, 0))]
        );
        // 10 is vouched
        assert_eq!(<Vouching<Test>>::get(10), Some(VouchingStatus::Vouching));
        // To unvouch, you must know the right bid position
        assert_noop!(
            Society::unvouch(Origin::signed(10), 2),
            Error::<Test, _>::BadPosition
        );
        ...
        assert_noop!(
            Society::unvouch(Origin::signed(10), 0),
            Error::<Test, _>::BadPosition
        );
