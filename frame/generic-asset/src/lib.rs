...
decl_storage! {
	trait Store for Module<T: Trait> as GenericAsset {
		...
		}): map hasher(twox_32_concat) T::AssetId => T::Balance;

		/// The free balance of a given asset under an account.
		pub FreeBalance:
			double_map hasher(twox_32_concat) T::AssetId, hasher(blake2_64_concat) T::AccountId => T::Balance;
		...
		pub Permissions get(fn get_permission):
			map hasher(twox_32_concat) T::AssetId => PermissionVersions<T::AccountId>;

		/// Any liquidity locks on some account balances.
		pub Locks get(fn locks):
			map hasher(blake2_64_concat) T::AccountId => Vec<BalanceLock<T::Balance>>;
