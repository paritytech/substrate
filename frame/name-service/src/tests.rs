...
fn renew_handles_errors() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			NameService::renew(RuntimeOrigin::signed(1), name_hash, new_expiry_less_than_current),
			Error::<Test>::ExpiryInvalid,
		);
...
fn deregister_subnode_handles_errors() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			NameService::deregister_subnode(RuntimeOrigin::signed(owner), parent_hash, label_hash),
			Error::<Test>::RegistrationNotFound
		);
