...
pub mod pallet {
				...
				if let Some(config) = &mut lottery {
					...
					if payout_block <= n {
						...
						if config.repeat {
							...
							config.start = n;
							return Weight::from_computation(T::WeightInfo::on_initialize_repeat())
						} else {
							// Else, kill the lottery storage.
							*lottery = None;
							return Weight::from_computation(T::WeightInfo::on_initialize_end())
						}
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::weight({
			let benchmarked_weight = Weight::from_computation(T::WeightInfo::buy_ticket());
			benchmarked_weight
