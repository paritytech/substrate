...
///
/// It create a handle which will trigger the pallet benchmark depending on the given params.
/// This is useful to implement the runtime api `Benchmark::dispatch_benchmark`.
///
/// The macro takes 4 parameters and 1 optional parameter:
///
/// ```ignore
/// add_benchmark!(params, batches, pallet_path, pallet_type, optional_pallet_name);
/// ```
///
/// # Arguments:
///
/// ## params:
///
/// This parameter is the name of the variable which holds the benchmark configuration and
/// the specified whitelist, it must of type `(&TrackedStorageKey, &Vec<TrackedStorageKey>)`.
///
/// Create an object that holds in the input parameters for the benchmark:
///
/// ```ignore
/// let params: (&BenchmarkConfig, &Vec<TrackedStorageKey>) = (&config, &whitelist);
/// ```
///
/// The `config` part is a variable of type [`BenchmarkConfig`] usually directly the one required as
/// parameter of `dispatch_benchmark` function.
///
/// The `whitelist` part is a parameter you pass to control the DB read/write tracking.
...
///
/// ```
/// let whitelist: Vec<frame_benchmarking::TrackedStorageKey> = vec![
/// 	// Block Number
...
/// ## batches:
///
/// This parameter is the name of a mutable variable which holds the benchmark results:
/// `Vec<BenchmarkBatch>`
///
/// Simply define an empty vec as such:
///
/// ```
...
/// ## `pallet_path`:
///
/// This parameter is the path of the pallet. It is typically the path which contains the weight
/// info trait: `WeightInfo`. E.g. `pallet_balances`, `frame_system`.
///
/// ## pallet_type:
///
/// This parameter is the type which contains the benchmarks, it must implement [`Benchmarking`].
/// This is typically the name generated by `construct_runtime` like `Balances`, `Treasury`.
/// But it can also be some specific types like `frame_system_benchmarking::Pallet::<Runtime>`
///
...
/// add_benchmark!(params, batches, pallet_session, frame_system_benchmarking::pallet::<runtime>);
/// ```
///
/// ## optional_pallet_name:
///
/// This parameter is optional and allow to give a unique name to the pallet. This name must be
/// unique as it allow to trigger the specific benchmark. By default `pallet_path` is used.
///
/// E.g.:
/// ```ignore
/// add_benchmark!(params, batches, pallet_collective, Council, pallet_collective_council);
/// add_benchmark!(params, batches, pallet_collective, Committee, pallet_collective_committee);
/// ```
///
/// # Usage
///
/// Typical usage is:
/// * create the variables `params` and `batches`
/// * generate the benchmark handle by calling `add_benchmark!(...)`
/// * handle the result by returning error if batches is emtpy or return the batches.
///
/// # Example
///
/// ```
/// # mod pallet_collective {
/// # 	use frame_benchmarking::*;
/// # 	pub struct Pallet<I>(core::marker::PhantomData<I>);
/// # 	impl<I> Benchmarking for Pallet<I> {
/// # 		fn benchmarks(_extra: bool) -> Vec<BenchmarkMetadata> {
/// # 			unimplemented!();
/// # 		}
/// # 		fn run_benchmark(
/// # 			_name: &[u8],
/// # 			_selected_components: &[(BenchmarkParameter, u32)],
/// # 			_whitelist: &[TrackedStorageKey],
/// # 			_verify: bool,
/// # 			_internal_repeats: u32
/// # 		) -> Result<Vec<BenchmarkResult>, BenchmarkError> {
/// # 			unimplemented!();
/// # 		}
/// # 	}
/// # }
/// # type Council = pallet_collective::Pallet<u8>;
/// # type Committ = pallet_collective::Pallet<u16>;
...
/// 	let mut batches = Vec::<frame_benchmarking::BenchmarkBatch>::new();
///
/// 	frame_benchmarking::add_benchmark!(params, batches, pallet_collective, Council, council);
/// 	frame_benchmarking::add_benchmark!(params, batches, pallet_collective, Committ, committ);
///
...
macro_rules! add_benchmark {
	( $params:ident, $batches:ident, $pallet_path:path, $( $rest:tt )* ) => {
		$crate::add_benchmark!(@parse
			{ $params }
			{ $batches }
			{ $pallet_path }
			{}
	...
	// If last tokens are comma and optional name then go to expansion
	(@parse
		{ $params:ident }
		{ $batches:ident }
		{ $pallet_path:path }
	...
	// If there is nothing or a comma then use pallet_path and go to expansion
	(@parse
		{ $params:ident }
		{ $batches:ident }
		{ $pallet_path:path }
	...
	(@expand
		...
		let pallet_name = stringify!($pallet_name).as_bytes();
		let pallet_path = stringify!($pallet_path).as_bytes();
		...
		if &pallet[..] == &pallet_name[..] {
			...
			$batches.push($crate::BenchmarkBatch {
				pallet: pallet_path.to_vec(),
				pallet_instance_name: pallet_name.to_vec(),
...
/// ```
///
/// Then pass this `list` to the macro, along with the `extra` boolean, the pallet path, and
/// pallet struct and the optional pallet name (similar to [`add_benchmark`]):
///
/// ```ignore
/// list_benchmark!(list, extra, pallet_balances, Balances);
/// list_benchmark!(list, extra, pallet_session, SessionBench::<Runtime>);
/// list_benchmark!(list, extra, frame_system, SystemBench::<Runtime>);
/// list_benchmark!(list, extra, pallet_collective, Council, council);
/// list_benchmark!(list, extra, pallet_collective, TechnicalCommittee, technical_committee);
/// ```
///
...
macro_rules! list_benchmark {
	( $list:ident, $extra:ident, $pallet_path:path, $( $rest:tt )* ) => {
		$crate::list_benchmark!(@parse
			{ $list }
			{ $extra }
			{ $pallet_path }
			{}
