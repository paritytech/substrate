...
decl_storage! {
	trait Store for Module<T: Trait> as Benchmark {
		...
		MyMemberMap: map hasher(blake2_64_concat) T::AccountId => bool;
		MyValue: u32;
		MyMap: map hasher(twox_32_concat) u32 => u32;
		MyDoubleMap: double_map hasher(twox_32_concat) u32, hasher(identity) u32 => u32;
	}
}
