...
mod weight_for {
    ...
    pub(crate) fn execute<T: Trait<I>, I: Instance>(
        ...
        T::DbWeight::get()
            .reads(1) // read members for `is_member`
    ...
    pub(crate) fn propose_execute<T: Trait<I>, I: Instance>(
        ...
        T::DbWeight::get()
            .reads(2) // `is_member` + `contains_key`
    ...
    pub(crate) fn vote<T: Trait<I>, I: Instance>(members: Weight) -> Weight {
        T::DbWeight::get()
            .reads(1) // read `Members`
    ...
    pub(crate) fn close_without_finalize<T: Trait<I>, I: Instance>(
        ...
        T::DbWeight::get()
            .reads(3) // `Members`, `Voting`, `ProposalOf`
...
impl<T: Trait<I>, I: Instance> Module<T, I> {
    ...
    fn remove_proposal(proposal_hash: T::Hash) -> Weight {
        ...
        T::DbWeight::get()
            .reads_writes(1, 3) // `Voting`, `Proposals`, `ProposalOf`
...
mod tests {
    ...
    use frame_support::{assert_noop, assert_ok, parameter_types, weights::Weight, Hashable};
    ...
    fn close_works() {
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                System::events(),
                vec![
                    record(Event::collective_Instance1(RawEvent::Proposed(
                        1,
                        0,
                        hash.clone(),
                        3
                    ))),
                    record(Event::collective_Instance1(RawEvent::Voted(
                        2,
                        hash.clone(),
                        true,
                        2,
                    ...
                    record(Event::collective_Instance1(RawEvent::Closed(
                        hash.clone(),
                        2,
                        1
                    ))),
                    record(Event::collective_Instance1(RawEvent::Disapproved(
                        hash.clone()
                    )))
                ]
            );
    ...
    fn proposal_weight_limit_works_on_approve() {
        new_test_ext().execute_with(|| {
            let proposal =
                Call::Collective(crate::Call::set_members(vec![1, 2, 3], None, MAX_MEMBERS));
            let proposal_len: u32 = proposal.using_encoded(|p| p.len() as u32);
            let proposal_weight = proposal.get_dispatch_info().weight;
            let hash = BlakeTwo256::hash_of(&proposal);
        ...
        new_test_ext().execute_with(|| {
            ...
            assert_eq!(
                ...
                vec![
                    ...
                    record(Event::collective_Instance1(RawEvent::Approved(
                        hash.clone()
                    ))),
                    record(Event::collective_Instance1(RawEvent::Executed(
                        hash.clone(),
