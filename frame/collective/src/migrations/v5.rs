...
pub fn migrate<T: Config<I>, I: 'static>() -> Weight {
	...
	if storage_version <= 4 {
		...
		crate::ProposalOf::<T, I>::translate::<<T as Config<I>>::Proposal, _>(|_, proposal| {
			count.saturating_inc();
			Some(
				proposal.encode().try_into().expect(
					"runtime::collective migration failed, proposal encoded length overflow",
				),
			)
		});
