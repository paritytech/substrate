...
benchmarks_instance_pallet! {
	...
	verify {
		...
		assert_last_event::<T, I>(
			PalletEvent::MemberExecuted { proposal_hash, result: Err(DispatchError::BadOrigin) }.into()
		);
	}
	...
	verify {
		...
		// Note that execution fails due to mis-matched origin
		assert_last_event::<T, I>(
			PalletEvent::Executed { proposal_hash, result: Err(DispatchError::BadOrigin) }.into()
		);
	}
	...
	verify {
		...
		let proposal_hash = T::Hashing::hash_of(&proposal);
		assert_last_event::<T, I>(PalletEvent::Proposed { account: caller, proposal_index: p - 1, proposal_hash, threshold }.into());
	}
	...
	verify {
		...
		assert_eq!(Collective::<T, I>::proposals().len(), (p - 1) as usize);
		assert_last_event::<T, I>(PalletEvent::Disapproved { proposal_hash: last_hash }.into());
	}
	...
	verify {
		// The last proposal is removed.
		assert_eq!(Collective::<T, I>::proposals().len(), (p - 1) as usize);
		assert_last_event::<T, I>(PalletEvent::Executed { proposal_hash: last_hash, result: Err(DispatchError::BadOrigin) }.into());
	}
