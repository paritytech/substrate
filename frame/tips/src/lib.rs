...
pub mod pallet {
	...
	impl<T: Config<I>, I: 'static> Pallet<T, I> {
		...
		pub fn report_awesome(
			...
			Self::deposit_event(Event::<T, I>::NewTip { tip_hash: hash });
			Ok(())
		}
		...
		pub fn retract_tip(origin: OriginFor<T>, hash: T::Hash) -> DispatchResult {
			...
			Self::deposit_event(Event::<T, I>::TipRetracted { tip_hash: hash });
			Ok(())
		}
		...
		pub fn tip(
			...
			if Self::insert_tip_and_check_closing(&mut tip, tipper, tip_value) {
				Self::deposit_event(Event::<T, I>::TipClosing { tip_hash: hash });
			}
			Tips::<T, I>::insert(&hash, tip);
			Ok(())
		}
		...
		pub fn slash_tip(origin: OriginFor<T>, hash: T::Hash) -> DispatchResult {
			...
			Reasons::<T, I>::remove(&tip.reason);
			Self::deposit_event(Event::<T, I>::TipSlashed {
				tip_hash: hash,
				finder: tip.finder,
				deposit: tip.deposit,
			});
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	fn payout_tip(
		...
		debug_assert!(res.is_ok());
		Self::deposit_event(Event::<T, I>::TipClosed { tip_hash: hash, who: tip.who, payout });
	}
