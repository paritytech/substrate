...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn slash_tip(origin: OriginFor<T>, hash: T::Hash) -> DispatchResult {
			...
			Reasons::<T>::remove(&tip.reason);
			Self::deposit_event(Event::TipSlashed{ tip_hash: hash, finder: tip.finder, deposit: tip.deposit });
			Ok(())
		}
	}
}
...
impl<T: Config> Pallet<T> {
	...
	fn payout_tip(
		...
		debug_assert!(res.is_ok());
		Self::deposit_event(Event::TipClosed{ tip_hash: hash, who: tip.who, payout: payout });
	}
