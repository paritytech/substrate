...
mod benchmarks {
	...
	#[benchmark(extra)]
	fn transfer_best_case() -> BenchmarkResult<()> {
		let caller = whitelisted_caller();
		let recipient: T::AccountId = account("recipient", 0, SEED);
		let recipient_lookup = T::Lookup::unlookup(recipient.clone());
	...
	#[benchmark]
	fn transfer_keep_alive() -> BenchmarkResult<()> {
		let caller = whitelisted_caller();
		let recipient: T::AccountId = account("recipient", 0, SEED);
		let recipient_lookup = T::Lookup::unlookup(recipient.clone());
	...
	#[benchmark]
	fn set_balance_creating() -> BenchmarkResult<()> {
	...
	#[benchmark]
	fn set_balance_killing() -> BenchmarkResult<()> {
		let user: T::AccountId = account("user", 0, SEED);
		let user_lookup = T::Lookup::unlookup(user.clone());
	...
	// * Transfer will create the recipient account.
	#[benchmark]
	fn force_transfer() -> BenchmarkResult<()> {
		let existential_deposit = T::ExistentialDeposit::get();
	...
	// trie and PoV size.
	#[benchmark(extra)]
	fn transfer_increasing_users(u: Linear<0, 1_000>) -> BenchmarkResult<()> {
	...
	#[benchmark]
	fn transfer_all() -> BenchmarkResult<()> {
		let caller = whitelisted_caller();
		let recipient: T::AccountId = account("recipient", 0, SEED);
		let recipient_lookup = T::Lookup::unlookup(recipient.clone());
