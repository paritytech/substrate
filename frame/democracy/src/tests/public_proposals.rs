...
#[test]
fn public_to_external_works() {
	new_test_ext().execute_with(|| {
		System::set_block_number(0);
		assert_ok!(propose_set_balance_and_note(1, 2, 4));
		assert_ok!(Democracy::promote(Origin::signed(2), set_balance_proposal_hash(2)));
		assert_ok!(Democracy::public_to_external(Origin::signed(2)));
	})
}
