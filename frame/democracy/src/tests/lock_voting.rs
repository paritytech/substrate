...
fn lock_voting_should_work() {
	new_test_ext().execute_with(|| {
		...
		for i in 1..=5 {
			assert_eq!(Balances::locks(i).inner(), vec![the_lock(i * 10)]);
		}
		...
		assert_eq!(Balances::locks(1).inner(), vec![]);
		assert_eq!(Balances::locks(2).inner(), vec![the_lock(20)]);
		assert_eq!(Balances::locks(3).inner(), vec![the_lock(30)]);
		assert_eq!(Balances::locks(4).inner(), vec![the_lock(40)]);
		...
		assert_ok!(Democracy::unlock(Origin::signed(1), 4));
		assert_eq!(Balances::locks(4).inner(), vec![]);
		...
		assert_ok!(Democracy::unlock(Origin::signed(1), 3));
		assert_eq!(Balances::locks(3).inner(), vec![]);
		...
		fast_forward_to(18);
		assert_ok!(Democracy::unlock(Origin::signed(1), 2));
		assert_eq!(Balances::locks(2).inner(), vec![]);
	});
}
...
fn prior_lockvotes_should_be_enforced() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Balances::locks(5).inner(), vec![the_lock(50)]);
		fast_forward_to(6);
		assert_ok!(Democracy::remove_other_vote(Origin::signed(1), 5, r.2));
		assert_ok!(Democracy::unlock(Origin::signed(5), 5));
		assert_eq!(Balances::locks(5).inner(), vec![the_lock(20)]);
		fast_forward_to(9);
		...
		assert_ok!(Democracy::unlock(Origin::signed(5), 5));
		assert_eq!(Balances::locks(5).inner(), vec![the_lock(10)]);
		fast_forward_to(17);
...
fn multi_consolidation_of_lockvotes_should_be_conservative() {
	new_test_ext().execute_with(|| {
		...
		fast_forward_to(6);
		assert_ok!(Democracy::unlock(Origin::signed(5), 5));
		assert!(Balances::locks(5).inner()[0].amount >= 20);

		fast_forward_to(10);
		assert_ok!(Democracy::unlock(Origin::signed(5), 5));
		assert!(Balances::locks(5).inner()[0].amount >= 10);
...
fn locks_should_persist_from_delegation_to_voting() {
	new_test_ext().execute_with(|| {
		...
		fast_forward_to(18);
		assert_ok!(Democracy::unlock(Origin::signed(5), 5));
		assert!(Balances::locks(5).inner()[0].amount >= 5);
