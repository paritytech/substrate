...
fn add_proposal<T: Config>(n: u32) -> Result<T::Hash, &'static str> {
	...
	Democracy::<T, _>::propose(RawOrigin::Signed(other).into(), proposal_hash, value)?;
...
fn add_referendum<T: Config<()>>(n: u32) -> Result<ReferendumIndex, &'static str> {
	...
	Democracy::<T, _>::inject_referendum(
		T::LaunchPeriod::get(),
		proposal_hash,
		vote_threshold,
		0u32.into(),
	);
	let referendum_index: ReferendumIndex = ReferendumCount::<T, _>::get() - 1;
	T::Scheduler::schedule_named(
		(T::DemocracyId::get(), referendum_index).encode(),
...
fn account_vote<T: pallet_democracy::Config<I>, I>(
	b: BalanceOf<T, I>,
...
benchmarks! {
	...
	verify {
		assert_eq!(Democracy::<T, _>::public_props().len(), p as usize, "Proposals not created.");
	}
	...
	second {
		...
		for i in 0 .. s {
			...
			Democracy::<T, _>::second(RawOrigin::Signed(seconder).into(), 0, u32::MAX)?;
		}

		let deposits = Democracy::<T, _>::deposit_of(0).ok_or("Proposal not created")?;
	...
	vote_new {
		...
		for i in 0 .. r {
			...
			Democracy::<T, _>::vote(RawOrigin::Signed(caller.clone()).into(), ref_idx, account_vote)?;
		}
	...
	vote_existing {
		...
		let referendum_index = Democracy::<T, _>::referendum_count() - 1;
	...
	verify {
		...
		let referendum_info = Democracy::<T, _>::referendum_info(referendum_index)
	...
	emergency_cancel {
		...
		assert_ok!(Democracy::<T, _>::referendum_status(referendum_index));
	}: { Democracy::<T, _>::emergency_cancel(origin, referendum_index)? }
	verify {
		// Referendum has been canceled
		assert_noop!(
			Democracy::<T, _>::referendum_status(referendum_index),
			Error::<T, _>::ReferendumInvalid,
		);
	}
	...
	blacklist {
		...
		assert_ok!(
			Democracy::<T, _>::external_propose(T::ExternalOrigin::successful_origin(), hash)
		);
		let origin = T::BlacklistOrigin::successful_origin();
		// Add a referendum of our proposal.
		let referendum_index = add_referendum::<T>(0)?;
		assert_ok!(Democracy::<T, _>::referendum_status(referendum_index));
	}: { Democracy::<T, _>::blacklist(origin, hash, Some(referendum_index))? }
	verify {
		// Referendum has been canceled
		assert_noop!(
			Democracy::<T, _>::referendum_status(referendum_index),
	...
	external_propose {
		...
		let addresses = (0..v)
			...
			.collect::<Vec<_>>();
		Blacklist::<T, _>::insert(
			proposal_hash,
			(T::BlockNumber::zero(), addresses),
		);
	}: { Democracy::<T, _>::external_propose(origin, proposal_hash)? }
	verify {
	...
	external_propose_majority {
		...
		let proposal_hash = T::Hashing::hash_of(&0);
	}: { Democracy::<T, _>::external_propose_majority(origin, proposal_hash)? }
	verify {
		// External proposal created
		ensure!(<NextExternal<T>>::exists(), "External proposal didn't work");
	}
	...
	external_propose_default {
		...
		let proposal_hash = T::Hashing::hash_of(&0);
	}: { Democracy::<T, _>::external_propose_default(origin, proposal_hash)? }
	verify {
		// External proposal created
		ensure!(<NextExternal<T>>::exists(), "External proposal didn't work");
	}
	...
	fast_track {
		...
		let proposal_hash: T::Hash = T::Hashing::hash_of(&0);
		Democracy::<T, _>::external_propose_default(origin_propose, proposal_hash)?;
		...
		let delay = 0u32;
	}: { Democracy::<T, _>::fast_track(origin_fast_track, proposal_hash, voting_period, delay.into())? }
	verify {
		assert_eq!(Democracy::<T, _>::referendum_count(), 1, "referendum not created")
	}
	...
	veto_external {
		...
		vetoers.sort();
		Blacklist::<T, _>::insert(proposal_hash, (T::BlockNumber::zero(), vetoers));

		let origin  = T::VetoOrigin::successful_origin();
		ensure!(NextExternal::<T, _>::get().is_some(), "no external proposal");
	}: { Democracy::<T, _>::veto_external(origin, proposal_hash)? }
	verify {
		assert!(NextExternal::<T, _>::get().is_none());
	...
	on_initialize_external {
		...
		assert_eq!(Democracy::<T, _>::referendum_count(), r, "referenda not created");

		// Launch external
		LastTabledWasExternal::<T, _>::put(false);

		let origin = T::ExternalMajorityOrigin::successful_origin();
		let proposal_hash = T::Hashing::hash_of(&r);
		let call = Call::<T, _>::external_propose_majority { proposal_hash };
	...
	}: { Democracy::<T, _>::on_initialize(block_number) }
	verify {
		// One extra because of next external
		assert_eq!(Democracy::<T, _>::referendum_count(), r + 1, "referenda not created");
		ensure!(!<NextExternal<T>>::exists(), "External wasn't taken");

		// All but the new next external should be finished
		for i in 0 .. r {
			if let Some(value) = ReferendumInfoOf::<T, _>::get(i) {
				match value {
	...
	on_initialize_public {
		...
		LastTabledWasExternal::<T, _>::put(true);
	...
	verify {
		...
		assert_eq!(Democracy::<T, _>::referendum_count(), r + 1, "proposal not accepted");
	...
	on_initialize_base {
		...
		for (key, mut info) in ReferendumInfoOf::<T, _>::iter() {
			if let ReferendumInfo::Ongoing(ref mut status) = info {
				status.end += 100u32.into();
			}
			ReferendumInfoOf::<T, _>::insert(key, info);
		}

		assert_eq!(Democracy::<T, _>::referendum_count(), r, "referenda not created");
		assert_eq!(Democracy::<T, _>::lowest_unbaked(), 0, "invalid referenda init");

	}: { Democracy::<T, _>::on_initialize(1u32.into()) }
	verify {
	...
	delegate {
		...
		let initial_balance: BalanceOf<T, _> = 100u32.into();
		let delegated_balance: BalanceOf<T, _> = 1000u32.into();
		...
		for i in 0..r {
			let ref_idx = add_referendum::<T>(i)?;
			Democracy::<T, _>::vote(RawOrigin::Signed(new_delegate.clone()).into(), ref_idx, account_vote)?;
		}
		let votes = match VotingOf::<T, _>::get(&new_delegate) {
			Voting::Direct { votes, .. } => votes,
			_ => return Err("Votes are not direct".into()),
		};
		assert_eq!(votes.len(), r as usize, "Votes were not recorded.");
	...
	verify {
		...
		assert_eq!(balance, delegated_balance, "delegation balance didn't work");
		let delegations = match VotingOf::<T, _>::get(&new_delegate) {
	...
	undelegate {
		...
		let votes = match VotingOf::<T, _>::get(&the_delegate) {
			Voting::Direct { votes, .. } => votes,
			_ => return Err("Votes are not direct".into()),
		};
		assert_eq!(votes.len(), r as usize, "Votes were not recorded.");
	...
	verify {
		...
		match Preimages::<T, _>::get(proposal_hash) {
	...
	note_imminent_preimage {
		...
		Preimages::<T, _>::insert(&proposal_hash, PreimageStatus::Missing(block_number));
	...
	reap_preimage {
		...
		Democracy::<T, _>::note_preimage(RawOrigin::Signed(submitter).into(), encoded_proposal.clone())?;
		...
		assert!(Preimages::<T, _>::contains_key(proposal_hash));
	...
	unlock_remove {
		...
		for i in 0 .. r {
			let ref_idx = add_referendum::<T>(i)?;
			Democracy::<T, _>::vote(RawOrigin::Signed(locker.clone()).into(), ref_idx, small_vote)?;
			Democracy::<T, _>::remove_vote(RawOrigin::Signed(locker.clone()).into(), ref_idx)?;
		}
	...
	verify {
		...
		assert_eq!(voting.locked_balance(), BalanceOf::<T, _>::zero());
	}
	...
	unlock_set {
		...
		let big_vote = account_vote::<T, _>(base_balance * 10u32.into());
		let referendum_index = add_referendum::<T>(r)?;
		Democracy::<T, _>::vote(RawOrigin::Signed(locker.clone()).into(), referendum_index, big_vote)?;
		...
		Democracy::<T, _>::remove_vote(RawOrigin::Signed(locker.clone()).into(), referendum_index)?;
	...
	enact_proposal_execute {
		...
		let proposal_hash = T::Hashing::hash(&encoded_proposal[..]);
		Democracy::<T, _>::note_preimage(RawOrigin::Signed(proposer).into(), encoded_proposal)?;
	...
	verify {
		...
		assert_last_event::<T>(Event::<T, _>::Executed { ref_index: 0, result: Err(BadOrigin.into()) }.into());
	}
	...
	enact_proposal_slash {
		...
		let call = Call::<T, _>::enact_proposal { proposal_hash, index: 0 }.encode();
	}: {
		assert_eq!(
			<Call<T> as Decode>::decode(&mut &*call)
				.expect("call is encoded above, encoding must be correct")
				.dispatch_bypass_filter(origin),
			Err(Error::<T, _>::PreimageInvalid.into())
		);
	}
