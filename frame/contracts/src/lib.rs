...
use crate::{
	exec::{AccountIdOf, ErrorOrigin, ExecError, Executable, VarSizedKey, Stack as ExecStack},
	gas::GasMeter,
...
impl<T: Config> Pallet<T> {
	...
	pub fn get_storage(address: T::AccountId, key: Vec<u8>) -> GetStorageResult {
		...
		let maybe_value = contract_info.read(
			&VarSizedKey::<T>::try_from(key).map_err(|_| ContractAccessError::KeyDecodingFailed)?.into(),
		);
		Ok(maybe_value)
	}
