...
use parity_wasm::elements::{BlockType, FuncBody, Instruction, Instructions, ValueType};
    ...
    T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>,
{
    fn from(def: ModuleDefinition) -> Self {
        ...
        if def.inject_stack_metering {
            code = inject_limiter(code, Contracts::<T>::current_schedule().limits.stack_height)
                .unwrap();
        }
