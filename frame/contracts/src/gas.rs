	...
	T::AccountId: UncheckedFrom<<T as frame_system::Config>::Hash> + AsRef<[u8]>,
{
	...
	pub fn nested(&mut self, amount: Weight) -> Result<Self, DispatchError> {
		let amount = if amount.is_zero() { self.gas_left } else { amount };

		// NOTE that it is ok to allocate all available gas since it still ensured
		// by `charge` that it doesn't reach zero.
		if self.gas_left.is_any_less_than(&amount) {
...
mod tests {
	...
	impl Token<Test> for SimpleToken {
		fn weight(&self) -> Weight {
			Weight::todo_from_v1(self.0)
		}
	}

	#[test]
	fn it_works() {
		let gas_meter = GasMeter::<Test>::new(Weight::todo_from_v1(50000));
		assert_eq!(gas_meter.gas_left(), Weight::todo_from_v1(50000));
	}

	#[test]
	fn tracing() {
		let mut gas_meter = GasMeter::<Test>::new(Weight::todo_from_v1(50000));
	...
	fn overcharge_is_unrecoverable() {
		let mut gas_meter = GasMeter::<Test>::new(Weight::todo_from_v1(200));
	...
	fn charge_exact_amount() {
		let mut gas_meter = GasMeter::<Test>::new(Weight::todo_from_v1(25));
