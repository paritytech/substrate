...
fn instantiate_and_call_and_deposit_event() {
    let (wasm, code_hash) = compile_module::<Test>(&load_wasm("return_from_start_fn.wat")).unwrap();
...
fn dispatch_call() {
    ...
    let (wasm, code_hash) = compile_module::<Test>(&load_wasm("dispatch_call.wat")).unwrap();
...
fn dispatch_call_not_dispatched_after_top_level_transaction_failure() {
    ...
    let (wasm, code_hash) =
        compile_module::<Test>(&load_wasm("dispatch_call_then_trap.wat")).unwrap();
...
#[test]
fn run_out_of_gas() {
    let (wasm, code_hash) = compile_module::<Test>(&load_wasm("run_out_of_gas.wat")).unwrap();
...
fn default_rent_allowance_on_instantiate() {
    let (wasm, code_hash) =
        compile_module::<Test>(&load_wasm("check_default_rent_allowance.wat")).unwrap();
...
fn storage_max_value_limit() {
    let (wasm, code_hash) = compile_module::<Test>(&load_wasm("storage_size.wat")).unwrap();
...
fn deploy_works_without_gas_price() {
    let (wasm, code_hash) = compile_module::<Test>(&load_wasm("get_runtime_storage.wat")).unwrap();
    ExtBuilder::default()
        .existential_deposit(50)
        .gas_price(0)
        .build()
...
fn cannot_self_destruct_through_draning() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_err!(
                Contracts::call(Origin::signed(ALICE), BOB, 0, 100_000, vec![],),
                "contract trapped during execution"
            );
        });
}
...
fn cannot_self_destruct_while_live() {
    let (wasm, code_hash) = compile_module::<Test>(&load_wasm("self_destruct.wat")).unwrap();
    ExtBuilder::default()
        .existential_deposit(50)
        .build()
        .execute_with(|| {
