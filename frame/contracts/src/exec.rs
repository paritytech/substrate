...
use crate::{
    ...
    BalanceOf, CodeHash, Config, ContractInfo, ContractInfoOf, Error, Event, Module as Contracts,
    RawEvent, Schedule, TrieId,
};
use frame_support::{
...
mod tests {
    ...
    impl MockLoader {
        fn insert(f: impl Fn(MockCtx) -> ExecResult + 'static) -> CodeHash<Test> {
            LOADER.with(|loader| {
                ...
                loader
                    .map
                    .insert(hash, MockExecutable(Rc::new(f), hash.clone()));
                hash
            })
        }
    }
    ...
    fn changes_are_reverted_on_failing_call() {
        ...
        let return_ch = MockLoader::insert(|_| {
            Ok(ExecReturnValue {
    ...
    fn instantiation_work_with_success_output() {
        let dummy_ch = MockLoader::insert(|_| {
            Ok(ExecReturnValue {
                flags: ReturnFlags::empty(),
    ...
    fn instantiation_traps() {
        let dummy_ch = MockLoader::insert(|_| Err("It's a trap!".into()));
        let instantiator_ch = MockLoader::insert({
            let dummy_ch = dummy_ch.clone();
            move |ctx| {
                // Instantiate a contract and save it's address in `instantiated_contract_address`.
