...
// // How new API will look like
// #[define(seal0)]
// fn gas(ctx: Runtime<E>, amount: u32) -> Result<sp_sandbox::ReturnValue, sp_sandbox::HostError> {
...
// here: create a module and add host funcs in it, like
...
// 	// Returns the size of the pre-existing value at the specified key if any. Otherwise
// 	// `SENTINEL` is returned as a sentinel value.
// 	[__unstable__] seal_set_storage(ctx, key_ptr: u32, key_len: u32, value_ptr: u32, value_len: u32)
// -> u32 => { 		ctx.set_storage(KeyType::Variable(key_len), key_ptr, value_ptr, value_len)
// 	},
...
// 	//
// 	// `ReturnCode::KeyNotFound`
// 	[__unstable__] seal_get_storage(ctx, key_ptr: u32, key_len: u32, out_ptr: u32, out_len_ptr: u32)
// -> ReturnCode => { 		ctx.get_storage(KeyType::Variable(key_len), key_ptr, out_ptr, out_len_ptr)
// 	},
...
// 	//
// 	// `ReturnCode::KeyNotFound`
// 	[__unstable__] seal_take_storage(ctx, key_ptr: u32, key_len: u32, out_ptr: u32, out_len_ptr: u32)
// -> ReturnCode => { 		let charged =
// ctx.charge_gas(RuntimeCosts::TakeStorage(ctx.ext.max_value_size()))?; 		let key =
// ctx.read_sandbox_memory(key_ptr, key_len)?; 		if let crate::storage::WriteOutcome::Taken(value) =
// ctx.ext.set_storage_transparent(&VarSizedKey::<E::T>::try_from(key).map_err(|_|
// Error::<E::T>::DecodingFailed)?, None, true)? { 			ctx.adjust_gas(charged,
// RuntimeCosts::TakeStorage(value.len() as u32)); 			ctx.write_sandbox_output(out_ptr, out_len_ptr,
// &value, false, already_charged)?; 			Ok(ReturnCode::Success)
// 		} else {
...
// 	[seal0] seal_ecdsa_recover(ctx, signature_ptr: u32, message_hash_ptr: u32, output_ptr: u32) ->
// ReturnCode => { 		ctx.charge_gas(RuntimeCosts::EcdsaRecovery)?;
