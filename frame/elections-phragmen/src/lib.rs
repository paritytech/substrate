...
use frame_support::{
    ...
    traits::{
        BalanceStatus, ChangeMembers, Contains, Currency, Get, InitializeMembers, LockIdentifier,
...
mod migration {
    use super::*;
    use frame_support::{
        migration::{take_storage_item, StorageKeyIterator},
        Twox64Concat,
    };
    pub fn migrate<T: Trait>() {
        for (who, votes) in
            StorageKeyIterator::<T::AccountId, Vec<T::AccountId>, Twox64Concat>::new(
                b"PhragmenElection",
                b"VotesOf",
            )
            .drain()
        {
            if let Some(stake) = take_storage_item::<_, BalanceOf<T>, Twox64Concat>(
                b"PhragmenElection",
                b"StakeOf",
                &who,
            ) {
