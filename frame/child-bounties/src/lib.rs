...
pub mod pallet {
	...
	pub type ChildBounties<T: Config> = StorageDoubleMap<
		...
		Twox64Concat,
		BountyIndex,
		ChildBounty<T::AccountId, BalanceOf<T>, frame_system::pallet_prelude::BlockNumberFor::<T>>,
	>;
...
impl<T: Config> Pallet<T> {
	...
	) -> Result<(T::AccountId, frame_system::pallet_prelude::BlockNumberFor::<T>), DispatchError> {
