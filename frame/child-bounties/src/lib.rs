			...
			ChildBounties::<T>::try_mutate_exists(
				...
				|maybe_child_bounty| -> DispatchResult {
					...
					if let ChildBountyStatus::CuratorProposed { ref curator } = child_bounty.status
					{
						...
						// fee is set as zero.
						let deposit = Self::calculate_child_curator_deposit(
							&parent_curator,
							curator,
							&child_bounty.value,
							&child_bounty.fee,
...
impl<T: Config> Pallet<T> {
	...
	fn calculate_child_curator_deposit(
		parent_curator: &T::AccountId,
		child_curator: &T::AccountId,
		child_bounty_value: &BalanceOf<T>,
		child_bounty_fee: &BalanceOf<T>,
	) -> BalanceOf<T> {
		let deposit: BalanceOf<T> = if child_bounty_fee.is_zero() {
			if parent_curator == child_curator {
				return Zero::zero()
			}
			T::ChildCuratorDepositMultiplierWithNoFee::get() * *child_bounty_value
		} else {
			T::ChildCuratorDepositMultiplierWithFee::get() * *child_bounty_fee
		};
