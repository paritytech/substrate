...
fn should_report_an_authority_and_trigger_on_offence() {
	new_test_ext().execute_with(|| {
		...
		// when
		assert!(Offences::report_offence(vec![], offence).is_ok());
...
fn should_not_report_the_same_authority_twice_in_the_same_slot() {
	new_test_ext().execute_with(|| {
		...
		assert!(Offences::report_offence(vec![], offence.clone()).is_ok());
		with_on_offence_fractions(|f| {
			assert_eq!(f.clone(), vec![Perbill::from_percent(25)]);
			f.clear();
		});
...
fn should_properly_count_offences() {
	...
	new_test_ext().execute_with(|| {
		...
		assert!(Offences::report_offence(vec![], offence1).is_ok());
		with_on_offence_fractions(|f| {
			assert_eq!(f.clone(), vec![Perbill::from_percent(25)]);
			f.clear();
		});
		...
		// when
		// report for the second time
		assert!(Offences::report_offence(vec![], offence2).is_ok());
