...
fn should_not_report_the_same_authority_twice_in_the_same_slot() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Offences::report_offence(offence), Err(OffenceError::DuplicateReport));
...
fn reports_if_an_offence_is_dup() {
	new_test_ext().execute_with(|| {
		...
		// offence
		assert!(!<Offences as ReportOffence<Offence>>::is_known_offence(
...
fn should_properly_sort_offences() {
	new_test_ext().execute_with(|| {
		...
		// report for the second time
		Offences::report_offence(offence2).unwrap();
		Offences::report_offence(offence3).unwrap();
		Offences::report_offence(offence4).unwrap();
