...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn store(origin: OriginFor<T>, data: Vec<u8>) -> DispatchResult {
			...
			Self::deposit_event(Event::Stored { stored_index: index });
			Ok(())
		}
		...
		pub fn renew(
			...
			Self::deposit_event(Event::Renewed { renewed_index: index });
			Ok(().into())
		}
	...
	pub enum Event<T: Config> {
		...
		Stored { stored_index: u32 },
		/// Renewed data under specified index.
		Renewed { renewed_index: u32 },
