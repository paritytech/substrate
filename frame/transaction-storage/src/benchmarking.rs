...
pub fn run_to_block<T: Config>(n: frame_system::BlockNumberOf<T>) {
...
benchmarks! {
	...
	}: _(RawOrigin::Signed(caller.clone()), BlockNumberOf::<T>::zero(), 0)
	verify {
	...
	check_proof_max {
		...
		run_to_block::<T>(StoragePeriod::<T>::get() + BlockNumberOf::<T>::one());
