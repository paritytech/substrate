...
fn should_submit_unsigned_transaction_on_chain_for_any_account() {
	...
	t.execute_with(|| {
		...
		if let Call::Example(crate::Call::submit_price_unsigned_with_signed_payload { price_payload: body, _signature: signature }) = tx.call {
...
fn should_submit_raw_unsigned_transaction_on_chain() {
	...
	t.execute_with(|| {
		...
		let tx = Extrinsic::decode(&mut &*tx).unwrap();
		assert_eq!(tx.signature, None);
		assert_eq!(tx.call, Call::Example(crate::Call::submit_price_unsigned { _block_number: 1, price: 15523 }));
	});
}
