...
use sp_runtime::traits::{
    AtLeast32Bit, BlakeTwo256, Hash, LookupError, Member, One, Saturating, StaticLookup, Zero,
};
use sp_std::prelude::*;
...
impl<T: Trait> Module<T> {
    ...
    /// Do any migrations.
    fn migrations() {
        if let Some(set_count) =
            take_storage_value::<T::AccountIndex>(b"Indices", b"NextEnumSet", b"")
        {
            ...
            while set_index < set_count {
                let maybe_accounts = take_storage_value::<Vec<T::AccountId>>(
                    b"Indices",
                    b"EnumSet",
                    BlakeTwo256::hash_of(&set_index).as_ref(),
                );
                if let Some(accounts) = maybe_accounts {
                    for (item_index, target) in accounts.into_iter().enumerate() {
                        ...
                        {
                            let index =
                                set_index * set_size + T::AccountIndex::from(item_index as u32);
