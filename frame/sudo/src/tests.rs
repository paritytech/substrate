...
fn sudo_emits_events_correctly() {
	new_test_ext(1).execute_with(|| {
		...
		let call =
			Box::new(Call::Logger(LoggerCall::privileged_i32_log { i: 42, weight: Weight::from_ref_time(1) }));
		assert_ok!(Sudo::sudo(Origin::signed(1), call));
...
fn sudo_as_basics() {
	new_test_ext(1).execute_with(|| {
		...
		let call =
			Box::new(Call::Logger(LoggerCall::non_privileged_log { i: 42, weight: Weight::from_ref_time(1) }));
