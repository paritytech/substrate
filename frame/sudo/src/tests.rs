...
fn sudo_basics() {
	...
	new_test_ext(1).execute_with(|| {
		...
		let call = Box::new(Call::Logger(LoggerCall::privileged_i32_log { i: 42, weight: Weight::todo_from_v1(1_000) }));
...
fn sudo_emits_events_correctly() {
	new_test_ext(1).execute_with(|| {
		...
		let call = Box::new(Call::Logger(LoggerCall::privileged_i32_log { i: 42, weight: Weight::todo_from_v1(1) }));
		assert_ok!(Sudo::sudo(Origin::signed(1), call));
...
fn sudo_unchecked_weight_basics() {
	new_test_ext(1).execute_with(|| {
		// A privileged function should work when `sudo` is passed the root `key` as origin.
		let call = Box::new(Call::Logger(LoggerCall::privileged_i32_log { i: 42, weight: Weight::todo_from_v1(1_000) }));
		assert_ok!(Sudo::sudo_unchecked_weight(Origin::signed(1), call, Weight::todo_from_v1(1_000)));
		assert_eq!(Logger::i32_log(), vec![42i32]);
		...
		let call = Box::new(Call::Logger(LoggerCall::privileged_i32_log { i: 42, weight: Weight::todo_from_v1(1) }));
		let sudo_unchecked_weight_call = SudoCall::sudo_unchecked_weight { call, weight: Weight::todo_from_v1(1_000) };
...
fn sudo_as_basics() {
	new_test_ext(1).execute_with(|| {
		...
		let call = Box::new(Call::Logger(LoggerCall::non_privileged_log { i: 42, weight: Weight::todo_from_v1(1) }));
