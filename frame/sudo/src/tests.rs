...
fn sudo_basics() {
	...
	new_test_ext(1).execute_with(|| {
		...
		let call = Box::new(Call::Logger(LoggerCall::privileged_i32_log {
			i: 42,
			weight: Weight::from_computation(1_000),
		}));
...
fn sudo_emits_events_correctly() {
	new_test_ext(1).execute_with(|| {
		...
		let call = Box::new(Call::Logger(LoggerCall::privileged_i32_log {
			i: 42,
			weight: Weight::from_computation(1),
		}));
		assert_ok!(Sudo::sudo(Origin::signed(1), call));
...
fn sudo_unchecked_weight_basics() {
	new_test_ext(1).execute_with(|| {
		...
		assert_noop!(
			Sudo::sudo_unchecked_weight(Origin::signed(2), call, Weight::from_computation(1_000)),
		...
		let sudo_unchecked_weight_call =
			SudoCall::sudo_unchecked_weight { call, weight: Weight::from_computation(1_000) };
		let info = sudo_unchecked_weight_call.get_dispatch_info();
		assert_eq!(info.weight, Weight::from_computation(1_000));
	});
}
