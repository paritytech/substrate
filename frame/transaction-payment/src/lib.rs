	...
	BalanceOf<T>: Send + Sync + FixedPointOperand,
{
	...
	/// final priority is `fee * min(2, 4) = fee * 2`. If it consumed `1/4th` of the block length and the entire block weight `(1/1)`, its priority is `fee * min(1, 4) = fee * 1`. This means that the transaction which consumes more resources (either length or weight) with the same `fee` ends up having lower priority.
