    ...
    T::Call: Dispatchable<Info = DispatchInfo, PostInfo = PostDispatchInfo>,
{
    ...
    pub fn compute_fee(
        ...
        if info.pays_fee {
            ...
            let adjusted_fee = targeted_fee_adjustment
                .saturated_multiply_accumulate(adjustable_fee.saturated_into());

            let base_fee = T::TransactionBaseFee::get();
            base_fee
                .saturating_add(adjusted_fee.saturated_into())
                .saturating_add(tip)
        } else {
            tip
        }
    ...
    pub fn weight_to_fee_with_adjustment(weight: Weight) -> BalanceOf<T>
    where
        BalanceOf<T>: From<u64>,
    {
        NextFeeMultiplier::get().saturated_multiply_accumulate(Self::weight_to_fee(weight))
    }
...
mod tests {
    ...
    use frame_support::{
        ...
        weights::{DispatchClass, DispatchInfo, GetDispatchInfo, PostDispatchInfo, Weight},
    };
    ...
    fn upgrade_to_fixed128_works() {
        ...
        sp_io::TestExternalities::new(s).execute_with(|| {
            ...
            assert_eq!(
                new_value,
                Fixed128::from_rational(1, NonZeroI128::new(2).unwrap())
            );
        });
    }
}
