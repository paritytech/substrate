...
//! This module contains helper functions to perform the buy and sell functionalities of the NFTs pallet.
...
impl<T: Config<I>, I: 'static> Pallet<T, I> {
	...
	/// are specified as a `BoundedVec` of `ItemTipOf` with a maximum length of `T::MaxTips`. For each
	/// tip, the function transfers the `amount` to the `receiver` account. The sender is responsible
	/// for ensuring the validity of the provided tips.
	///
	...
	///
	/// This function is used to set the price and whitelist information for an item in the specified
	/// `collection`. The `sender` account must be the owner of the item. The item's price and
	/// whitelist information can be set to allow trading the item. If `price` is `None`, the item will
	/// be marked as not for sale.
	///
	/// - `collection`: The identifier of the collection containing the item.
	/// - `item`: The identifier of the item for which the price and whitelist information will be set.
	/// - `sender`: The account that sets the price and whitelist information for the item.
	/// - `price`: The optional price for the item.
	/// - `whitelisted_buyer`: The optional account that is whitelisted to buy the item at the set price.
	...
	///
	/// This function is used to buy an item from the specified `collection`. The `buyer` account will
	/// attempt to buy the item with the provided `bid_price`. The item's current owner will receive
	/// the bid price if it is equal to or higher than the item's set price. If `whitelisted_buyer` is
	/// specified in the item's price information, only that account is allowed to buy the item. If the
	/// item is not for sale, or the bid price is too low, the function will return an error.
	///
