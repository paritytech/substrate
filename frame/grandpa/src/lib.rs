...
pub mod pallet {
	...
	pub trait Config: frame_system::Config {
		...
		type EquivocationReportSystem: OffenceReportSystem<
			...
			(EquivocationProof<Self::Hash, frame_system::pallet_prelude::BlockNumberFor<Self>>, Self::KeyOwnerProof),
		>;
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn report_equivocation(
			origin: OriginFor<T>,
			equivocation_proof: Box<EquivocationProof<T::Hash, frame_system::pallet_prelude::BlockNumberFor::<T>>>,
	...
	#[pallet::type_value]
	pub(super) fn DefaultForState<T: Config>() -> StoredState<frame_system::pallet_prelude::BlockNumberFor::<T>> {
		StoredState::Live
	}
	...
	pub(super) type State<T: Config> =
		StorageValue<_, StoredState<frame_system::pallet_prelude::BlockNumberFor::<T>>, ValueQuery, DefaultForState<T>>;

	/// Pending change: (signaled at, scheduled change).
	#[pallet::storage]
	#[pallet::getter(fn pending_change)]
	pub(super) type PendingChange<T: Config> =
		StorageValue<_, StoredPendingChange<frame_system::pallet_prelude::BlockNumberFor::<T>, T::MaxAuthorities>>;

	/// next block number where we can force a change.
	#[pallet::storage]
	#[pallet::getter(fn next_forced)]
	pub(super) type NextForced<T: Config> = StorageValue<_, frame_system::pallet_prelude::BlockNumberFor::<T>>;

	/// `true` if we are currently stalled.
	#[pallet::storage]
	#[pallet::getter(fn stalled)]
	pub(super) type Stalled<T: Config> = StorageValue<_, (frame_system::pallet_prelude::BlockNumberFor::<T>, frame_system::pallet_prelude::BlockNumberFor::<T>)>;
...
impl<T: Config> Pallet<T> {
	...
	pub fn schedule_pause(in_blocks: frame_system::pallet_prelude::BlockNumberFor::<T>) -> DispatchResult {
	...
	pub fn schedule_resume(in_blocks: frame_system::pallet_prelude::BlockNumberFor::<T>) -> DispatchResult {
	...
	pub fn submit_unsigned_equivocation_report(
		equivocation_proof: EquivocationProof<T::Hash, frame_system::pallet_prelude::BlockNumberFor::<T>>,
		key_owner_proof: T::KeyOwnerProof,
	) -> Option<()> {
		T::EquivocationReportSystem::publish_evidence((equivocation_proof, key_owner_proof)).ok()
	}

	fn on_stalled(further_wait: frame_system::pallet_prelude::BlockNumberFor::<T>, median: frame_system::pallet_prelude::BlockNumberFor::<T>) {
