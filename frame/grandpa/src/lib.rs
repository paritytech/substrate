...
pub mod pallet {
	...
	#[pallet::type_value]
	pub(super) fn DefaultForState<T: Config>() -> StoredState<frame_system::BlockNumberOf<T>> {
		StoredState::Live
	}
	...
	pub(super) type PendingChange<T: Config> =
		StorageValue<_, StoredPendingChange<frame_system::BlockNumberOf<T>, T::MaxAuthorities>>;

	/// next block number where we can force a change.
	#[pallet::storage]
	#[pallet::getter(fn next_forced)]
	pub(super) type NextForced<T: Config> = StorageValue<_, frame_system::BlockNumberOf<T>>;
...
impl<T: Config> Pallet<T> {
	...
	pub fn schedule_pause(in_blocks: frame_system::BlockNumberOf<T>) -> DispatchResult {
	...
	pub fn schedule_resume(in_blocks: frame_system::BlockNumberOf<T>) -> DispatchResult {
	...
	pub fn schedule_change(
		...
		in_blocks: frame_system::BlockNumberOf<T>,
		forced: Option<frame_system::BlockNumberOf<T>>,
	) -> DispatchResult {
	...
	fn deposit_log(log: ConsensusLog<frame_system::BlockNumberOf<T>>) {
	...
	pub fn submit_unsigned_equivocation_report(
		equivocation_proof: EquivocationProof<T::Hash, frame_system::BlockNumberOf<T>>,
		key_owner_proof: T::KeyOwnerProof,
	) -> Option<()> {
