...
impl<T: Config> Pallet<T> {
	...
	pub fn ensure_offchain_repeat_frequency(now: frame_system::pallet_prelude::BlockNumberFor::<T>) -> Result<(), MinerError> {
		let threshold = T::OffchainRepeat::get();
		let last_block = StorageValueRef::persistent(OFFCHAIN_LAST_BLOCK);

		let mutate_stat = last_block.mutate::<_, &'static str, _>(
			|maybe_head: Result<Option<frame_system::pallet_prelude::BlockNumberFor::<T>>, _>| {
				match maybe_head {
