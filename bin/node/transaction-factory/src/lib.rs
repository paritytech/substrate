...
use sp_api::{ApiExt, CallApiAt, ProvideRuntimeApi, TransactionFor};
use sp_block_builder::BlockBuilder;
use sp_blockchain::HeaderBackend;
use sp_consensus::block_import::BlockImport;
use sp_consensus::{BlockImportParams, BlockOrigin, ForkChoiceStrategy, InherentData, SelectChain};
use sp_runtime::generic::BlockId;
use sp_runtime::traits::{AtLeast32Bit, Block as BlockT, Header as HeaderT, One, Zero};
...
where
    ...
    Client: BlockBuilderProvider<Backend, Block, Client>
        ...
        + HeaderBackend<Block>,
    Client::Api: BlockBuilder<Block, Error = sp_blockchain::Error>
...
{
    let best_header: Result<<Block as BlockT>::Header, sc_cli::Error> = select_chain
        .best_chain()
    ...
    while factory_state.block_number() < factory_state.blocks() {
        ...
        let inherents = client
            .runtime_api()
            .inherent_extrinsics(&best_block_id, inherents)
        ...
        for tx_num in 0..tx_per_block {
            ...
            info!(
                "Pushing transfer {}/{} to {} into block.",
                tx_num + 1,
                tx_per_block,
                to
            );
