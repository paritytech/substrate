...
use node_runtime::{Call, Executive, Indices, Runtime, TransactionSubmitterOf, UncheckedExtrinsic};
...
fn should_submit_unsigned_transaction() {
    ...
    t.execute_with(|| {
        ...
        <SubmitTransaction as SubmitUnsignedTransaction<Runtime, Call>>::submit_unsigned(call)
            .unwrap();
...
fn should_submit_signed_transaction() {
    ...
    t.execute_with(|| {
        let keys =
            <SubmitTransaction as SubmitSignedTransaction<Runtime, Call>>::find_all_local_keys();
        assert_eq!(keys.len(), 3, "Missing keys: {:?}", keys);

        let can_sign = <SubmitTransaction as SubmitSignedTransaction<Runtime, Call>>::can_sign();
        assert!(
            can_sign,
            "Since there are keys, `can_sign` should return true"
        );
