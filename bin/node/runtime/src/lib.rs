...
impl_runtime_apis! {
	...
	impl frame_benchmarking::Benchmark<Block> for Runtime {
		fn dispatch_benchmark(
			...
			let mut final_whitelist: Vec<Vec<u8>> = vec![
				// Block Number
			...
			final_whitelist.extend(whitelist);

			let mut batches = Vec::<BenchmarkBatch>::new();
			let params = (&pallet, &benchmark, &lowest_range_values, &highest_range_values, &steps, repeat, &final_whitelist);
