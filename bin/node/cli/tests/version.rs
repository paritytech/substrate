...
fn expected_regex() -> Regex {
	Regex::new(r"^cere (\d+\.\d+\.\d+(?:-.+?)?)-([a-f\d]+|unknown)-(.+?)-(.+?)(?:-(.+))?$")
		.unwrap()
}
...
fn test_regex_matches_properly() {
	...
	let captures = expected.captures("cere 2.0.0-da487d19d-x86_64-linux-gnu").unwrap();
	...
	let captures = expected.captures("cere 2.0.0-alpha.5-da487d19d-x86_64-linux-gnu").unwrap();
	...
	let captures = expected.captures("cere 2.0.0-alpha.5-da487d19d-x86_64-linux").unwrap();
	assert_eq!(&captures[1], "2.0.0-alpha.5");
	assert_eq!(&captures[2], "da487d19d");
	assert_eq!(&captures[3], "x86_64");
	assert_eq!(&captures[4], "linux");
