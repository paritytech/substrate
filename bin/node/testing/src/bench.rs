...
use sp_core::{blake2_256, traits::CloneableSpawn, ExecutionContext};
...
impl Clone for BenchDb {
    fn clone(&self) -> Self {
        ...
        let seed_db_files = std::fs::read_dir(seed_dir)
            ...
            .map(|f_result| {
                f_result
                    .expect("failed to read file in seed db")
                    .path()
                    .clone()
            })
            .collect();
...
impl BenchDb {
    ...
    pub fn generate_block(&mut self, block_type: BlockType) -> Block {
        let (client, _backend) =
            Self::bench_client(self.directory_guard.path(), Profile::Wasm, &self.keyring);
        ...
        for _ in 0..block_type.transactions() {
            ...
            let receiver =
                get_account_id_from_seed::<sr25519::Public>(&format!("random-user//{}", iteration));
            ...
            let opaque =
                OpaqueExtrinsic::decode(&mut &encoded[..]).expect("Failed  to decode opaque");
