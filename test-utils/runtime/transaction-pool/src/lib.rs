...
impl TestApi {
    ...
    pub fn push_block(&self, block_number: BlockNumber, xts: Vec<Extrinsic>) -> Header {
        ...
        let header = Header {
            ...
            parent_hash: block_number
                .checked_sub(1)
                .and_then(|num| chain.header_by_number.get(&num).cloned().map(|h| h.hash()))
                .unwrap_or_default(),
    ...
    pub fn push_fork_block_with_parent(&self, parent: Hash, xts: Vec<Extrinsic>) -> Header {
        ...
        let blocknum = chain
            ...
            .keys()
            .max()
            .expect("block_by_number shouldn't be empty");
        let header = Header {
            number: *blocknum,
            digest: Default::default(),
            extrinsics_root: Default::default(),
...
impl sc_transaction_graph::ChainApi for TestApi {
    ...
    fn validate_transaction(
        ...
        {
            return futures::future::ready(Ok(Err(TransactionValidityError::Invalid(
