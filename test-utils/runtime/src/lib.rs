...
pub mod storage_key_generator {
	...
	/// Generate the hashed keys from the raw literals
	pub fn generate_expected_keys() -> Vec<String> {
	...
	/// Provides the hashed keys in human readable form.
	pub fn get_expected_keys() -> Vec<String> {
	...
	fn expected_keys_vec_are_matching() {
		assert_eq!(
			...
			storage_key_generator::generate_expected_keys(),
		);
	}
}
