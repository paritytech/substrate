...
. You must also link your Substrate PR in the _description_ of your Polkadot PR (link in both directions).
. Now you should see in your Polkadot PR that your Substrate branch will be used in the CI job.
. On the Substrate PR you should see that there is a step `check-polkadot-companion-status` that will ensure that the Polkadot PR is green and has been approved (and is mergeable).
. If the Polkadot PR gets approved later on, you can re-run the `check-polkadot-companion-status` step only and it will simply re-run that check specifically.
