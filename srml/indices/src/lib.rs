...
use primitives::traits::{As, Member, One, SimpleArithmetic, StaticLookup};
...
impl<AccountId: Encode, AccountIndex: As<usize>> ResolveHint<AccountId, AccountIndex>
    for SimpleResolveHint<AccountId, AccountIndex>
{
    fn resolve_hint(who: &AccountId) -> Option<AccountIndex> {
        Some(AccountIndex::sa(
            who.using_encoded(|e| e[0] as usize + e[1] as usize * 256),
        ))
    }
}
