...
use primitives::traits::{As, Bounded, Zero};
use rstd::prelude::*;
use rstd::result;
use srml_support::dispatch::Result;
use srml_support::traits::{
    Currency, LockIdentifier, LockableCurrency, OnFreeBalanceZero, ReservableCurrency,
    WithdrawReason,
};
use srml_support::{decl_event, decl_module, decl_storage, ensure};
use srml_support::{
    Dispatchable, EnumerableStorageMap, IsSubType, Parameter, StorageMap, StorageValue,
};
...
impl<T: Trait> Module<T> {
    ...
    pub fn tally(ref_index: ReferendumIndex) -> (BalanceOf<T>, BalanceOf<T>, BalanceOf<T>) {
        let (approve, against, capital): (BalanceOf<T>, BalanceOf<T>, BalanceOf<T>) =
            Self::voters_for(ref_index)
                .iter()
                .map(|voter| {
                    (
                ...
                .map(|(bal, vote)| {
                    if vote.is_aye() {
                        (
                            bal * BalanceOf::<T>::sa(vote.multiplier() as u64),
                            Zero::zero(),
                            bal,
                        )
                    } else {
    ...
    fn delegated_votes(
        ...
        <Delegations<T>>::enumerate()
            ...
            .fold(
                (Zero::zero(), Zero::zero()),
                |(votes_acc, balance_acc), (delegator, (_delegate, periods))| {
                    let lock_periods = if min_lock_periods <= periods {
                        min_lock_periods
                    } else {
                        periods
                    };
                    let balance = T::Currency::total_balance(&delegator);
                    let votes = T::Currency::total_balance(&delegator)
                        * BalanceOf::<T>::sa(lock_periods as u64);
                    ...
                    (
                        votes_acc + votes + del_votes,
                        balance_acc + balance + del_balance,
                    )
                },
            )
    }
    ...
    fn do_vote(who: T::AccountId, ref_index: ReferendumIndex, vote: Vote) -> Result {
            ...
            vote.multiplier() <= Self::max_lock_periods(),
            "vote has too great a strength"
        );
        ensure!(
            Self::is_active_referendum(ref_index),
    ...
    fn inject_referendum(
        ...
        if ref_index > 0
            && Self::referendum_info(ref_index - 1)
                .map(|i| i.end > end)
                .unwrap_or(false)
        {
    ...
    fn bake_referendum(
        ...
        let approved = info
            .threshold
            .approved(approve, against, capital, total_issuance);
        let lock_period = Self::public_delay();
        ...
        for (a, vote) in Self::voters_for(index)
            .into_iter()
        ...
        if approved {
            ...
            if info.delay.is_zero() {
                ...
                <DispatchQueue<T>>::mutate(now + info.delay, |q| {
                    q.push(Some((info.proposal, index)))
                });
            }
        } else {
...
mod tests {
    ...
    use srml_support::{assert_noop, assert_ok, impl_outer_dispatch, impl_outer_origin};
    ...
    fn new_test_ext_with_public_delay(
        public_delay: u64,
    ...
    fn proxy_should_work() {
        with_externalities(&mut new_test_ext(), || {
            ...
            assert_noop!(
                Democracy::set_proxy(Origin::signed(2), 10),
                "already a proxy"
            );
            ...
            assert_noop!(
                Democracy::remove_proxy(Origin::signed(2), 10),
                "wrong proxy"
            );
    ...
    fn poor_seconder_should_not_work() {
        with_externalities(&mut new_test_ext(), || {
            ...
            assert_noop!(
                Democracy::second(Origin::signed(1), 0),
