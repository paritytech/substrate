...
use primitives::traits::{
    self, As, BlockNumberToHash, Bounded, CheckEqual, CurrentHeight, Digest as DigestT,
    EnsureOrigin, Hash, Lookup, MaybeDisplay, MaybeSerializeDebug,
    MaybeSerializeDebugButNotDeserialize, Member, One, SimpleArithmetic, SimpleBitOps,
    StaticLookup, Zero,
};
...
use srml_support::{
    decl_event, decl_module, decl_storage, storage, Parameter, StorageMap, StorageValue,
};
...
pub fn extrinsics_root<H: Hash, E: parity_codec::Encode>(extrinsics: &[E]) -> H::Output {
    extrinsics_data_root::<H>(
        extrinsics
            .iter()
            .map(parity_codec::Encode::encode)
            .collect(),
    )
}
