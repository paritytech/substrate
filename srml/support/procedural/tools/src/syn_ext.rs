...
use srml_support_procedural_tools_derive::{Parse, ToTokens};
use std::iter::once;
...
impl<T: CustomKeyword> CustomKeyword for CustomToken<T> {
    fn ident() -> &'static str {
        <T as CustomKeyword>::ident()
    }
    fn display() -> &'static str {
        <T as CustomKeyword>::display()
    }
}
...
pub fn is_parametric_type_def(typ: &syn::Type, default: bool) -> bool {
    match *typ {
        syn::Type::Path(ref path) => path.path.segments.iter().any(|v| {
        ...
        syn::Type::Tuple(ref inner) => inner
            .elems
            .iter()
            .any(|t| is_parametric_type_def(t, default)),
...
pub fn has_parametric_type_def(typ: &syn::Type, ident: &Ident, default: bool) -> bool {
    match *typ {
        ...
        syn::Type::Tuple(ref inner) => inner
            .elems
            .iter()
            .any(|t| has_parametric_type_def(t, ident, default)),
        syn::Type::TraitObject(ref to) => to.bounds.iter().any(|bound| {
        ...
        syn::Type::ImplTrait(ref it) => it.bounds.iter().any(|bound| {
            if let syn::TypeParamBound::Trait(ref t) = bound {
                has_parametric_type_def_in_path(&t.path, ident, default)
            } else {
                false
