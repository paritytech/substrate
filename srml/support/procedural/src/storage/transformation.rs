...
use srml_support_procedural_tools::{
    clean_type_string, generate_crate_access, generate_hidden_includes,
};
...
pub fn decl_storage_impl(input: TokenStream) -> TokenStream {
    ...
    let instance_opts =
        match get_instance_opts(mod_instance, mod_instantiable, mod_default_instance) {
            Ok(opts) => opts,
    ...
    let impl_store_items =
        impl_store_items(&traitinstance, &instance_opts.instance, &storage_lines);
    ...
    for sline in storage_lines.inner.iter() {
        ...
        if let Some(ref config) = config {
            ...
            if type_infos.kind.is_simple()
                && ext::has_parametric_type(type_infos.value_type, traitinstance)
            {
...
fn decl_storage_items(
    ...
    if instance.is_some() {
        ...
        for sline in storage_lines.inner.iter() {
            ...
            let const_name = syn::Ident::new(
                &format!("{}{}", impls::PREFIX_FOR, name.to_string()),
                proc_macro2::Span::call_site(),
            );
            let partial_const_value = prefix.clone();
            const_names.push((const_name, partial_const_value));

            if let DeclStorageTypeInfosKind::Map {
                is_linked: true, ..
            } = type_infos.kind
            {
                let const_name = syn::Ident::new(
                    &format!("{}{}", impls::HEAD_KEY_FOR, name.to_string()),
                    proc_macro2::Span::call_site(),
                );
        ...
        let instances = (0..NUMBER_OF_INSTANCE)
            ...
            .chain(default_instance.clone().map(|ident| {
                (
                    String::new(),
                    ident,
                    quote! {#[doc=r"Default module instance"]},
                )
            }));
    ...
    for sline in storage_lines.inner.iter() {
        ...
        let implementation = match kind {
            DeclStorageTypeInfosKind::Simple => i.simple_value(),
            DeclStorageTypeInfosKind::Map {
                key_type,
                is_linked: false,
            } => i.map(key_type),
            DeclStorageTypeInfosKind::Map {
            ...
            DeclStorageTypeInfosKind::DoubleMap {
                key1_type,
                key2_type,
                key2_hasher,
...
fn store_functions_to_metadata(
    ...
    for sline in storage_lines.inner.iter() {
        ...
        let struct_name =
            proc_macro2::Ident::new(&("__GetByteStruct".to_string() + &str_name), name.span());
        let cache_name = proc_macro2::Ident::new(
...
fn get_type_infos(storage_type: &DeclStorageType) -> DeclStorageTypeInfos {
    let (value_type, kind) = match storage_type {
        ...
        DeclStorageType::LinkedMap(ref map) => (
            &map.value,
            DeclStorageTypeInfosKind::Map {
                key_type: &map.key,
                is_linked: true,
        ...
        DeclStorageType::DoubleMap(ref map) => (
            &map.value,
            DeclStorageTypeInfosKind::DoubleMap {
                key1_type: &map.key1,
                key2_type: &map.key2.content,
                key2_hasher: {
                    let h = &map.key2_hasher;
                    quote! { #h }
                },
            },
        ),
...
fn get_instance_opts(
    ...
    match (instance, instantiable, default_instance) {
        (Some(instance), Some(instantiable), default_instance_def) => {
            let (equal_default_instance, default_instance) =
                if let Some(default_instance) = default_instance_def {
