...
mod tests {
	...
	fn weight_should_attach_to_call_enum() {
		...
		assert_eq!(
			...
			DispatchInfo { weight: 5 * 1000, class: DispatchClass::Operational },
		);
		// default weight.
		assert_eq!(
			Call::<TraitImpl>::aux_0().get_dispatch_info(),
			DispatchInfo { weight: 10 * 1000, class: DispatchClass::Normal },
		);
		// custom basic
		assert_eq!(
			Call::<TraitImpl>::aux_3().get_dispatch_info(),
			DispatchInfo { weight: 3 * 1000, class: DispatchClass::Normal },
		);
	}
}
