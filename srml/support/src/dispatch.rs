...
/// ```
/// 
/// Declaration is set with this header where:
/// * `Module`: the struct generated by the macro, with type `Trait`.
/// * `Call`: the enum generated for every module, which implements `Callable`.
/// * `origin`: alias of `T::Origin`, declared by the `impl_outer_origin!` macro.
/// * `Result`: the expected return type from module functions.
///
/// Function declaration variations include:**
/// * [Optional] Public functions: `pub` declares dispatched functions that are not recommended for use within modules, e.g. `foo()`.
/// * [Optional] Omit returning `Result`: the macro auto-expands functions to return a `Result` type, e.g. `bar()`.
/// * [Optional] Priviledged calls: if the `origin` param is omitted, the macro assumes the `origin` to be `Root`, e.g. `baz()`.
///
/// ## Special Functions
///
...
/// * `on_initialise`: Executes at the beginning of the block.
/// * `on_finalise`: Executes at the end of the block.
/// * `offchain_worker`: Starts offchain worker tasks at the end of the block and produces `inherents`.
/// 
/// ## Module with instances
/// 
/// `decl_module!` macro supports modules with instances with the following syntax: (DefaultInstance type is optional)
/// ```nocompile
