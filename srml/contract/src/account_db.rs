...
use super::{AccountInfo, AccountInfoOf, BalanceOf, CodeHash, CodeHashOf, Trait, TrieId};
...
use srml_support::{
    storage::child,
    traits::{Currency, Imbalance, SignedImbalance, UpdateBalanceOutcome},
    StorageMap,
};
use system;
...
impl<'a, T: Trait> OverlayAccountDb<'a, T> {
    ...
    pub fn reg_cache_new_rc(
        &self,
    ) -> Rc<RefCell<AccountTrieIdMapping<<T as system::Trait>::AccountId>>> {
        ...
        if self.trie_account_cache {
            v.as_ref().map(|v| {
                self.trie_account
                    .as_ref()
                    .borrow_mut()
                    .insert(account.clone(), v.trie_id.clone())
            });
        }
        v
    }
