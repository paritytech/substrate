...
use crate::{
    AccountInfo, AccountInfoOf, ComputeDispatchFee, ContractAddressFor, GenesisConfig, Module,
    RawEvent, Trait, TrieId, TrieIdGenerator,
};
    ...
    with_externalities(
        ...
        || {
            // Set up two accounts with free balance above the existential threshold.
            {
                ...
                assert_eq!(
                    child::get(&unique_id1[..], &b"foo".to_vec()),
                    Some(b"1".to_vec())
                );
...
"#;
const HASH_RETURN_FROM_START_FN: [u8; 32] =
    hex!("e6411d12daa2a19e4e9c7d8306c31c7d53a352cb8ed84385c8a1d48fc232e708");
...
"#;
const HASH_DISPATCH_CALL: [u8; 32] =
    hex!("49dfdcaf9c1553be10634467e95b8e71a3bc15a4f8bf5563c0312b0902e0afb9");
...
fn dispatch_call() {
    ...
    let encoded =
        parity_codec::Encode::encode(&Call::Balances(balances::Call::transfer(CHARLIE, 50)));
