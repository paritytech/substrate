...
use super::{BalanceOf, CodeHash, Config, ContractAddressFor, Event, RawEvent, Trait, TrieId};
use crate::account_db::{AccountDb, AccountTrieIdMapping, DirectAccountDb, OverlayAccountDb};
...
use srml_support::traits::{Currency, WithdrawReason};
use timestamp;
    ...
    V: Vm<T, Executable = E>,
{
    ...
    pub fn top_level(origin: T::AccountId, cfg: &'a Config<T>, vm: &'a V, loader: &'a L) -> Self {
        let overlay = OverlayAccountDb::<T>::new(
            &DirectAccountDb,
            Rc::new(RefCell::new(AccountTrieIdMapping::new())),
            true,
        );
    ...
    fn instantiation_from_contract() {
        ...
        let creator_ch = loader.insert({
            ...
            move |ctx| {
                ...
                *created_contract_address.borrow_mut() = ctx
                    .ext
                    .instantiate(&dummy_ch, 15u64, ctx.gas_meter, &[])
                    .unwrap()
                    .address
                    .into();
        ...
        with_externalities(
            ...
            || {
                ...
                let created_contract_address =
                    created_contract_address.borrow().as_ref().unwrap().clone();
    ...
    fn instantiation_fails() {
        ...
        with_externalities(
            ...
            || {
                ...
                // event here.
                assert_eq!(&ctx.events, &[RawEvent::Transfer(ALICE, BOB, 20),]);
            },
        );
    }
}
