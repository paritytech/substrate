...
fn authorities_change_logged_after_delay() {
	with_externalities(&mut new_test_ext(vec![(1, 1), (2, 1), (3, 1)]), || {
		...
		System::initialize(&2, &header.hash(), &Default::default(), None);
		System::note_finished_extrinsics();
...
fn cannot_schedule_change_when_one_pending() {
	with_externalities(&mut new_test_ext(vec![(1, 1), (2, 1), (3, 1)]), || {
		...
		System::initialize(&3, &header.hash(), &Default::default(), None);
...
fn dispatch_forced_change() {
	with_externalities(&mut new_test_ext(vec![(1, 1), (2, 1), (3, 1)]), || {
		...
		for i in 2..7 {
			System::initialize(&i, &header.hash(), &Default::default(), None);
		...
		// add a normal change.
		{
			System::initialize(&7, &header.hash(), &Default::default(), None);
			assert!(Grandpa::pending_change().is_none());
		...
		{
			System::initialize(&8, &header.hash(), &Default::default(), None);
			assert!(Grandpa::pending_change().is_some());
			assert_eq!(Grandpa::grandpa_authorities(), vec![(4, 1), (5, 1), (6, 1)]);
			assert!(Grandpa::schedule_change(vec![(5, 1)], 1, None).is_err());
		...
		{
			System::initialize(&11, &header.hash(), &Default::default(), None);
			assert!(Grandpa::pending_change().is_none());
