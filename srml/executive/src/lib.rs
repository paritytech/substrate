...
#[cfg(test)]
mod tests_weights {
	use codec::{Encode, Decode, EncodeLike, Codec};
	use sr_primitives::traits::{BlakeTwo256, IdentityLookup, Verify, StaticLookup};
	use sr_primitives::{Perbill, transaction_validity::TransactionValidityError, generic, testing};
	use system::ChainContext;
	use super::*;
	...
	mod custom {
		...
		support::decl_module! {
			pub struct Module<T: Trait> for enum Call where origin: T::Origin {
				...
				// module hooks.
				// one with block number arg and one without
				#[weight = SimpleDispatchInfo::FixedNormal(50)]
				fn on_initialize(n: T::BlockNumber) {
					println!("on_initialize({})", n);
				}
				#[weight = SimpleDispatchInfo::FixedNormal(150)]
	...
	// impl<Context> Checkable<Context> for UncheckedExtrinsic {
	// 	type Checked = Self;
	// 	fn check(self, _: &Context) -> Result<Self::Checked, TransactionValidityError> { Ok(self) }
	// }

	type HashT = BlakeTwo256;
	type Address = <Indices as StaticLookup>::Source;
	type Signature = primitives::sr25519::Signature;
	type AccountId = <Signature as Verify>::Signer;
	// type AccountId = u64;
	type UncheckedExtrinsic = generic::UncheckedExtrinsic<Address, Call, Signature, ()>;
	// type Header = testing::Header;
	// type Block = testing::Block<UncheckedExtrinsic>;
	type Header = generic::Header<u64, BlakeTwo256>;
	type Block = generic::Block<Header, UncheckedExtrinsic>;
	type Executive = super::Executive<Runtime, Block, ChainContext<Runtime>, Runtime, AllModules>;
	...
	support::construct_runtime!(
			...
			UncheckedExtrinsic = UncheckedExtrinsic
		{
			...
			Indices: indices::{Module, Call},
			Custom: custom::{Module, Call},
		}
	);
	...
	fn block_hooks_weight_is_stored() {
		new_test_ext().execute_with(|| {
			// let a = UncheckedExtrinsic::new_unsigned(Call::System(system::Call::remark(vec![0])));
			// <UncheckedExtrinsic as Checkable<ChainContext<Runtime>>>::check(a, &Default::default());
