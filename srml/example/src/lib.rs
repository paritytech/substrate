...
use sr_primitives::call_attributes::WeightedTransaction;
...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		//
		// Each transaction can optionally indicate a weight.
		//
		// A basic weight is a tuple of `(base_weight, byte_weight)`. Upon including each
		// transaction in a block, the final weight is calculated as
		// `base_weight + byte_weight * tx_size`. If this value, added to the weight of all included
		// transactions, exceeds `MAX_TRANSACTION_WEIGHT`, the transaction is not included. If no
		// weight attribute is provided, the `::default()` implementation of `WeightedTransaction`
		// is used.
		//
		// The example below showcases a transaction which is relatively costly, but less dependent
		// on the input, hence `byte_weight` is configured smaller.
		#[meta = WeightedTransaction::Basic(100_000, 10)]
