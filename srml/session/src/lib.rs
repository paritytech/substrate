...
use rstd::prelude::*;
use srml_support::{
    decl_event, decl_module, decl_storage, for_each_tuple, StorageMap, StorageValue,
};
...
mod tests {
    ...
    fn new_test_ext() -> runtime_io::TestExternalities<Blake2Hasher> {
        ...
        t.extend(
            consensus::GenesisConfig::<Test> {
                code: vec![],
                authorities: NEXT_VALIDATORS
                    .with(|l| l.borrow().iter().cloned().map(UintAuthorityId).collect()),
            }
            .build_storage()
            .unwrap()
            .0,
    ...
    fn authorities_should_track_validators() {
        with_externalities(&mut new_test_ext(), || {
            ...
            assert_eq!(
                Consensus::authorities(),
                vec![UintAuthorityId(1), UintAuthorityId(2)]
            );
