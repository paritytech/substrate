...
use super::*;
use mock::{Balances, ExtBuilder, Origin, Session, Staking, System, Test, Timestamp};
use phragmen;
...
fn offline_grace_should_delay_slashing() {
    ...
    with_externalities(&mut ExtBuilder::default().build(), || {
        ...
        assert_eq!(
            ...
            ValidatorPrefs {
                unstake_threshold: default_unstake_threshold,
                validator_payment: 0
            }
        );
        ...
        Staking::on_offline_validator(
            10,
            default_unstake_threshold as usize + offline_slash_grace as usize,
        );
        // Confirm slash count
        assert_eq!(Staking::slash_count(&11), 4);
...
fn max_unstake_threshold_works() {
    ...
    with_externalities(&mut ExtBuilder::default().build(), || {
        ...
        assert_eq!(
            Balances::free_balance(&11),
            u64::max_value() - 2_u64.pow(MAX_UNSTAKE_THRESHOLD) * 100
        );
        assert_eq!(
            Balances::free_balance(&21),
            u64::max_value() - 2_u64.pow(MAX_UNSTAKE_THRESHOLD) * 200
        );
    });
}
    ...
    with_externalities(
        ...
        || {
            ...
            assert_eq!(
                ...
                1 + (3 * session_reward - delay) / 2
            );
            assert_eq!(
                Balances::total_balance(&2),
                500 + (3 * session_reward - delay) / 2
            );
        },
    );
}
...
fn multi_era_reward_should_work() {
    ...
    with_externalities(
        ...
        || {
            ...
            assert_eq!(
                Balances::total_balance(&10),
                3 * new_session_reward + recorded_balance
            );
        },
    );
}
...
fn nominating_and_rewards_should_work() {
    ...
    with_externalities(
        ...
        || {
            ...
            assert_eq!(
                Balances::total_balance(&2),
                initial_balance + (2 * new_session_reward / 9 + 3 * new_session_reward / 11)
            );
...
fn nominators_also_get_slashed() {
    ...
    with_externalities(&mut ExtBuilder::default().nominate(false).build(), || {
        ...
        assert_eq!(
            Balances::total_balance(&10),
            initial_balance + 10 - validator_slash
        );
        assert_eq!(
            Balances::total_balance(&2),
...
fn double_staking_should_fail() {
    ...
    with_externalities(
        &mut ExtBuilder::default().sessions_per_era(2).build(),
        || {
...
fn cannot_transfer_staked_balance_2() {
    ...
    with_externalities(
        &mut ExtBuilder::default().nominate(false).fare(true).build(),
        || {
...
fn reward_destination_works() {
    ...
    with_externalities(&mut ExtBuilder::default().nominate(false).build(), || {
        ...
        assert_eq!(
            Balances::free_balance(&11),
            1000 + session_reward0 + session_reward1
        );
        // Record this value
...
fn slot_stake_is_least_staked_validator_and_exposure_defines_maximum_punishment() {
    ...
    with_externalities(
        &mut ExtBuilder::default().nominate(false).fare(false).build(),
        || {
...
fn on_free_balance_zero_stash_removes_validator() {
    ...
    with_externalities(
        &mut ExtBuilder::default().existential_deposit(10).build(),
        || {
            // Check the balance of the validator account
...
fn phragmen_election_works_with_post_processing() {
    ...
    with_externalities(&mut ExtBuilder::default().nominate(false).build(), || {
        ...
        for i in &[1, 3] {
            let _ = Balances::deposit_creating(i, 2000);
        }
        ...
        assert_eq!(
            winner_10.score,
            PerU128::from_max_value(165991398498018762665060784113057664)
        );
        ...
        assert_eq!(
            winner_30.score,
            PerU128::from_max_value(253136882709478613064217695772412937)
        );
    ...
    with_externalities(
        ...
        || {
            ...
            assert_eq!(
                ...
                initial_balance_2 + reward.max(1)
            );
            // same for 10
            assert_eq!(
                Balances::free_balance(&10),
                initial_balance_10 + 10 + reward.max(1)
            );
        },
    );
}
...
fn phragmen_should_not_overflow_validators() {
    with_externalities(&mut ExtBuilder::default().nominate(false).build(), || {
        ...
        let check_exposure = |a| {
            ...
            assert_eq!(
                expo.total,
                expo.own + expo.others.iter().map(|e| e.value).sum::<u64>()
            );
        };
