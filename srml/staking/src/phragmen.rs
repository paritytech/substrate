...
use crate::{BalanceOf, Exposure, IndividualExposure, Trait, ValidatorPrefs};
...
) -> Option<Vec<Candidate<T::AccountId, BalanceOf<T>>>>
where
    FR: Fn() -> usize,
    FV: Fn() -> Box<dyn Iterator<Item = (T::AccountId, ValidatorPrefs<BalanceOf<T>>)>>,
    FN: Fn() -> Box<dyn Iterator<Item = (T::AccountId, Vec<T::AccountId>)>>,
    for<'r> FS: Fn(&'r T::AccountId) -> BalanceOf<T>,
{
    let expand = |b: BalanceOf<T>| {
        <T::CurrencyToVote as Convert<BalanceOf<T>, u64>>::convert(b) as ExtendedBalance
    };
    let shrink = |b: ExtendedBalance| {
    ...
    let mut candidates = get_validators()
        .map(|(who, _)| {
            ...
            for e in &mut n.edges {
                ...
                if let Some(c) = elected_candidates.iter_mut().find(|c| c.who == e.who) {
                    ...
                    if c.who != n.who {
                        ...
                        c.exposure.others.push(IndividualExposure {
                            who: n.who.clone(),
                            value: shrink(e.backing_stake),
                        });
                    }
                }
            }
...
pub fn equalize<T: Trait + 'static>(
    ...
    let mut elected_edges = nominator
        .edges
        .iter_mut()
        .filter(|e| e.elected)
    ...
    elected_edges.iter_mut().take(split_ways).for_each(|e| {
        ...
        c.exposure.total = c.exposure.total.saturating_add(shrink(e.backing_stake));
        c.backing_stake = c.backing_stake.saturating_add(e.backing_stake);
        if let Some(i_expo) = c
            .exposure
            .others
            .iter_mut()
            .find(|i| i.who == nominator_address)
        {
