...
use super::*;
use mock::{Balances, ExtBuilder, Runtime, System, TestCall};
...
fn signed_extension_take_fees_work() {
	with_externalities(
		...
		|| {
			let len = 10;
			assert!(TakeFees::<Runtime>::from(0).pre_dispatch(&1, &TestCall::Five, len).is_ok());
			assert_eq!(Balances::free_balance(&1), 100 - 20 - 25);
			assert!(TakeFees::<Runtime>::from(5 /* tipped */).pre_dispatch(&1, &TestCall::Two, len).is_ok());
			assert_eq!(Balances::free_balance(&1), 100 - 20 - 25 - 20 - 5 - 10);
		}
	);
}
...
fn signed_extension_take_fees_is_bounded() {
	with_externalities(
		...
		|| {
			...
			assert!(TakeFees::<Runtime>::from(0).pre_dispatch(&1, &TestCall::Max, 10).is_ok());
