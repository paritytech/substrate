...
use crate::mock::{new_test_ext, Consensus, System};
use inherents::{InherentData, ProvideInherent};
use primitives::{
    generic,
    testing::{self, UintAuthorityId},
    traits::OnFinalize,
};
...
fn authorities_change_logged() {
    with_externalities(&mut new_test_ext(vec![1, 2, 3]), || {
        ...
        assert_eq!(
            header.digest,
            testing::Digest {
                logs: vec![generic::DigestItem::AuthoritiesChange(vec![
...
fn offline_report_can_be_excluded() {
    with_externalities(&mut new_test_ext(vec![1, 2, 3]), || {
        ...
        data.put_data(super::INHERENT_IDENTIFIER, &offline_report)
            .unwrap();
...
fn set_and_kill_storage_work() {
    ...
    with_externalities(&mut new_test_ext(vec![1, 2, 3]), || {
        ...
        assert_eq!(storage::unhashed::get_raw(&item.0), Some(item.1),);

        Consensus::kill_storage(vec![item.0.clone()]).unwrap();

        assert_eq!(storage::unhashed::get_raw(&item.0), None,);
    });
}
