...
use srml_support::{decl_module, decl_storage, storage, Parameter};
...
impl<T: Trait> ProvideInherent for Module<T> {
    ...
    fn create_inherent(data: &InherentData) -> Option<Self::Call> {
        if let Ok(Some(data)) = data
            .get_data::<<T::InherentOfflineReport as InherentOfflineReport>::Inherent>(
                &INHERENT_IDENTIFIER,
            )
        {
    ...
    fn check_inherent(call: &Self::Call, data: &InherentData) -> Result<(), Self::Error> {
        ...
        let expected = data
            ...
            .ok_or(RuntimeString::from(
                "No `offline_report` found in the inherent data!",
            ))?;

        <T::InherentOfflineReport as InherentOfflineReport>::check_inherent(&offline, &expected)
