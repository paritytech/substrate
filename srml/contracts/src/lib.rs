...
/// This is a static seed content for the `KeySpaceGenerator`.
/// Ideally it should be generated by a macro over the Module name
/// or from a Module function pointing to a unique module identifier.
const MODULE_KEYSPACE_SEED: &'static [u8; 13] = b"contract_seed";
	...
	T::AccountId: AsRef<[u8]>
{
	fn generate_keyspace(&mut self) -> KeySpace {
		...
		let mut buf = Vec::new();
		buf.extend_from_slice(MODULE_KEYSPACE_SEED);
