...
use democracy;
use primitives::traits::{As, One, StaticLookup, Zero};
use rstd::prelude::*;
...
type BalanceOf<T> =
    <<T as democracy::Trait>::Currency as Currency<<T as system::Trait>::AccountId>>::Balance;
type NegativeImbalanceOf<T> = <<T as democracy::Trait>::Currency as Currency<
...
impl<T: Trait> Module<T> {
    ...
    fn finalize_tally() -> Result {
        ...
        let incoming: Vec<T::AccountId> = leaderboard
            .iter()
            .rev()
...
mod tests {
    ...
    fn candidate_submission_not_using_free_slot_should_not_work() {
        with_externalities(&mut new_test_ext_with_candidate_holes(), || {
            System::set_block_number(1);
            assert_noop!(
                Council::submit_candidacy(Origin::signed(4), 3),
                "invalid candidate slot"
            );
        });
    }
    ...
    fn bad_candidate_slot_submission_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_eq!(Council::candidates(), Vec::<u64>::new());
            assert_noop!(
                Council::submit_candidacy(Origin::signed(1), 1),
                "invalid candidate slot"
            );
        });
    }
    ...
    fn non_free_candidate_slot_submission_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_eq!(Council::candidates(), vec![1]);
            assert_noop!(
                Council::submit_candidacy(Origin::signed(2), 0),
                "invalid candidate slot"
            );
        });
    }
    ...
    fn poor_candidate_submission_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_eq!(Council::candidates(), Vec::<u64>::new());
            assert_noop!(
                Council::submit_candidacy(Origin::signed(7), 0),
    ...
    fn proxy_voting_should_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_ok!(Council::proxy_set_approvals(
                Origin::signed(11),
                vec![true],
                0
            ));
    ...
    fn setting_an_approval_vote_count_more_than_candidate_count_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_noop!(
                Council::set_approvals(Origin::signed(4), vec![true, true], 0),
                "amount of candidate approval votes cannot exceed amount of candidates"
            );
        });
    }
    ...
    fn invalid_retraction_index_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_noop!(
                Council::retract_voter(Origin::signed(1), 1),
    ...
    fn non_voter_retraction_should_not_work() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_ok!(Council::set_approvals(Origin::signed(1), vec![true], 0));
            assert_noop!(
                Council::retract_voter(Origin::signed(2), 0),
    ...
    fn double_presentations_should_be_punished() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            assert_eq!(
                Council::present_winner(Origin::signed(4), 5, 50, 0),
    ...
    fn invalid_present_tally_should_slash() {
        with_externalities(&mut new_test_ext(false), || {
            ...
            System::set_block_number(6);
            assert_err!(
                Council::present_winner(Origin::signed(4), 2, 80, 0),
                "incorrect total"
            );
