...
use super::{Module as Council, Trait as CouncilTrait};
use primitives::traits::{EnsureOrigin, Hash};
use rstd::prelude::*;
use rstd::result;
...
pub fn ensure_council_members<OuterOrigin>(
    o: OuterOrigin,
    n: u32,
...
mod tests {
    ...
    use srml_support::{assert_noop, assert_ok, Hashable};
            ...
            assert_eq!(
                ...
                vec![EventRecord {
                    ...
                    event: OuterEvent::motions(RawEvent::Proposed(
                        1,
                        0,
                        hex!["cd0b662a49f004093b80600415cf4126399af0d27ed6c185abeb1469c17eb5bf"]
                            .into(),
                        3
                    ))
                }]
    ...
    fn motions_ignoring_non_council_votes_works() {
        with_externalities(&mut new_test_ext(true), || {
            ...
            assert_noop!(
                CouncilMotions::vote(Origin::signed(42), hash.clone(), 0, true),
                "voter not on council"
            );
        });
    }
    ...
    fn motions_ignoring_bad_index_council_vote_works() {
        with_externalities(&mut new_test_ext(true), || {
            ...
            assert_noop!(
                CouncilMotions::vote(Origin::signed(2), hash.clone(), 1, true),
                "mismatched index"
            );
        });
    }
    ...
    fn motions_revoting_works() {
        with_externalities(&mut new_test_ext(true), || {
            ...
            assert_eq!(
                CouncilMotions::voting(&hash),
                Some((0, 2, vec![1], Vec::<u64>::new()))
            );
            assert_noop!(
                CouncilMotions::vote(Origin::signed(1), hash.clone(), 0, true),
                "duplicate vote ignored"
            );
            assert_ok!(CouncilMotions::vote(
                Origin::signed(1),
                hash.clone(),
                0,
                false
            ...
            assert_eq!(
                CouncilMotions::voting(&hash),
                Some((0, 2, Vec::<u64>::new(), vec![1]))
            );
            assert_noop!(
                CouncilMotions::vote(Origin::signed(1), hash.clone(), 0, false),
                "duplicate vote ignored"
            );
                ...
                vec![
                    EventRecord {
                        ...
                        event: OuterEvent::motions(RawEvent::Proposed(
                            ...
                            0,
                            hex![
                                "cd0b662a49f004093b80600415cf4126399af0d27ed6c185abeb1469c17eb5bf"
                            ]
                            .into(),
                            2
                        ))
