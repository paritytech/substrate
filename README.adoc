...
For most users that want to run a validator node, the `author_rotateKeys` RPC call is sufficient.
The RPC call will generate `N` session keys for you and returns their public keys. `N` is the number
