...
7. After, you need to add OpenSSL to your System Variables:
    ...
    * If the procedure above does not work, go manual. First, start typing "env" in your Windows start menu search bar to summon the Environment Variables option. Click on it, then click on "Environment Variables" in the popup which appears. Following that, use the "New" button to add two new values into either the User section (upper) or the System section (lower). Follow the images below for a step by step process.

    * image:https://imgur.com/4HCbJLY.png[image]	
    * image:https://imgur.com/CDkg5Rp.png[image]
    * image:https://imgur.com/HHFgFF3.png[image]
    * image:https://imgur.com/sTpWqKW.png[image]
