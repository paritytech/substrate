...
mod tests {
	...
	fn sandbox_should_work() {
		...
		assert_eq!(
			WasmExecutor.call(&mut ext, &test_code[..], "test_sandbox", &code, true).0.unwrap(),
			vec![1],
		);
	}
	...
	fn invoke_args() {
		...
		assert_eq!(
			WasmExecutor.call(&mut ext, &test_code[..], "test_sandbox_args", &code, true).0.unwrap(),
			vec![1],
		);
	}
	...
	fn return_val() {
		...
		assert_eq!(
			WasmExecutor.call(&mut ext, &test_code[..], "test_sandbox_return_val", &code, true).0.unwrap(),
			vec![1],
		);
	}
}
