...
mod tests {
	...
	fn sandbox_should_work() {
		...
		assert_eq!(
			WasmExecutor::new(8).call(&mut ext, 8, &test_code[..], "test_sandbox", &code).unwrap(),
			vec![1],
		);
	}
	...
	fn invoke_args() {
		...
		assert_eq!(
			WasmExecutor::new(8).call(&mut ext, 8, &test_code[..], "test_sandbox_args", &code).unwrap(),
			vec![1],
		);
	}
	...
	fn return_val() {
		...
		assert_eq!(
			WasmExecutor::new(8).call(&mut ext, 8, &test_code[..], "test_sandbox_return_val", &code).unwrap(),
			vec![1],
		);
	}
}
