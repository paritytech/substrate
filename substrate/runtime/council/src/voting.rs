...
use super::{Module as Council, Trait};
use primitives::traits::{Executable, Hashing, RefInto};
use rstd::borrow::Borrow;
use rstd::prelude::*;
use runtime_io::print;
use substrate_runtime_support::dispatch::Result;
use substrate_runtime_support::{IsSubType, StorageMap, StorageValue};
...
impl<T: Trait> Module<T> {
	...
	fn propose(aux: &T::PublicAux, proposal: Box<T::Proposal>) -> Result {
		let expiry = <system::Module<T>>::block_number() + Self::voting_period();
			...
			Self::will_still_be_councillor_at(aux.ref_into(), expiry),
		...
		let proposal_hash = T::Hashing::hash_of(&proposal);
			...
			!<ProposalOf<T>>::exists(proposal_hash),
	...
	fn veto(aux: &T::PublicAux, proposal_hash: T::Hash) -> Result {
			...
			Self::is_councillor(aux.ref_into()),
			"only councillors may veto council proposals"
		);
		ensure!(
			<ProposalVoters<T>>::exists(&proposal_hash),
			"proposal must exist to be vetoed"
		);
		...
		let insert_position = existing_vetoers
			.binary_search(aux.ref_into())
			.err()
			.ok_or("a councillor may not veto a proposal twice")?;
	...
	fn end_block(now: T::BlockNumber) -> Result {
		while let Some((proposal, proposal_hash)) = Self::take_proposal_if_expiring_at(now) {
			...
			if let Some(&democracy::PrivCall::cancel_referendum(ref_index)) =
				IsSubType::<democracy::Module<T>>::is_sub_type(&proposal)
			{
				if let (_, 0, 0) = tally {
				...
				if tally.0 > tally.1 + tally.2 {
					...
					match tally {
						(_, 0, 0) => <democracy::Module<T>>::internal_start_referendum(
							proposal,
							democracy::VoteThreshold::SuperMajorityAgainst,
						).map(|_| ())?,
						_ => <democracy::Module<T>>::internal_start_referendum(
							proposal,
							democracy::VoteThreshold::SimpleMajority,
						).map(|_| ())?,
					};
				}
			}
...
mod tests {
	...
	fn referendum_cancellation_should_work_when_unanimous() {
		with_externalities(&mut new_test_ext(true), || {
			...
			assert_eq!(
				...
				vec![(0, 4, proposal, VoteThreshold::SuperMajorityApprove)]
			);
	...
	fn unanimous_proposal_should_expire_with_biased_referendum() {
		with_externalities(&mut new_test_ext(true), || {
			...
			assert_eq!(
				Democracy::active_referendums(),
				vec![(0, 5, proposal, VoteThreshold::SuperMajorityAgainst)]
			);
		});
	}
	...
	fn majority_proposal_should_expire_with_unbiased_referendum() {
		with_externalities(&mut new_test_ext(true), || {
			...
			assert_eq!(
				Democracy::active_referendums(),
				vec![(0, 5, proposal, VoteThreshold::SimpleMajority)]
			);
		});
	}
