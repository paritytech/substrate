...
fn to_journal_key(block: u64, index: u64) -> Hash {
...
impl UnfinalizedOverlay {
	...
	pub fn insert(&mut self, hash: &Hash, number: u64, parent_hash: &Hash, changeset: Changeset) -> Changeset {
	...
	fn discard(
		...
		parents: &mut HashMap<Hash, Hash>,
		discarded_journals: &mut Vec<Hash>,
		number: u64,
		hash: &Hash,
	) {
	...
	pub fn finalize(&mut self, hash: &Hash) -> Changeset {
	...
	pub fn get(&self, key: &Hash) -> Option<DBValue> {
...
mod tests {
	...
	fn make_db(inserted: &[u64]) -> HashMap<Hash, DBValue> {
		inserted.iter().map(|v| (to_key(b"test", v), to_key(b"value", v).to_vec())).collect()
	}

	fn apply_changeset(db: &mut HashMap<Hash, DBValue>, changeset: &Changeset) {
	...
	fn insert_ahead_panics() {
		...
		let h2 = Hash::random();
		let mut overlay = UnfinalizedOverlay::new(&db).unwrap();
		overlay.insert(&h1, 2, &Hash::default(), Changeset::default());
	...
	fn insert_behind_panics() {
		...
		let db = HashMap::new();
		let mut overlay = UnfinalizedOverlay::new(&db).unwrap();
		overlay.insert(&h1, 1, &Hash::default(), Changeset::default());
	...
	fn insert_unknown_parent_panics() {
		...
		let mut overlay = UnfinalizedOverlay::new(&db).unwrap();
		overlay.insert(&h1, 1, &Hash::default(), Changeset::default());
		overlay.insert(&h2, 2, &Hash::default(), Changeset::default());
	}
	...
	fn insert_finalize_one() {
		...
		let insertion = overlay.insert(&h1, 1, &Hash::default(), changeset.clone());
	...
	fn insert_finalize_two() {
		...
		apply_changeset(&mut db, &overlay.insert(&h1, 1, &Hash::default(), changeset1));
	...
	fn complex_tree() {
		...
		let (h_1, c_1) = (Hash::random(), make_changeset(&[1], &[]));
		let (h_2, c_2) = (Hash::random(), make_changeset(&[2], &[]));

		let (h_1_1, c_1_1) = (Hash::random(), make_changeset(&[11], &[]));
		let (h_1_2, c_1_2) = (Hash::random(), make_changeset(&[12], &[]));
		let (h_2_1, c_2_1) = (Hash::random(), make_changeset(&[21], &[]));
		let (h_2_2, c_2_2) = (Hash::random(), make_changeset(&[22], &[]));

		let (h_1_1_1, c_1_1_1) = (Hash::random(), make_changeset(&[111], &[]));
		let (h_1_2_1, c_1_2_1) = (Hash::random(), make_changeset(&[121], &[]));
		let (h_1_2_2, c_1_2_2) = (Hash::random(), make_changeset(&[122], &[]));
		let (h_1_2_3, c_1_2_3) = (Hash::random(), make_changeset(&[123], &[]));
		let (h_2_1_1, c_2_1_1) = (Hash::random(), make_changeset(&[211], &[]));

		let mut overlay = UnfinalizedOverlay::new(&db).unwrap();
		apply_changeset(&mut db, &overlay.insert(&h_1, 1, &Hash::default(), c_1));

		apply_changeset(&mut db, &overlay.insert(&h_1_1, 2, &h_1, c_1_1));
		apply_changeset(&mut db, &overlay.insert(&h_1_2, 2, &h_1, c_1_2));

		apply_changeset(&mut db, &overlay.insert(&h_2, 1, &Hash::default(), c_2));
