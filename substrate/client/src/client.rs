	...
	error::Error: From<<<B as backend::Backend<Block>>::State as StateBackend>::Error>,
{
	...
	pub fn header_proof(&self, _id: &BlockId<Block>) -> error::Result<(Block::Header, Vec<Vec<u8>>)> {
/*		let proof_error = || error::ErrorKind::Backend(format!("Failed to generate header proof for {:?}", id));
		...
		let cht_start = cht::start_number(cht_num).as_();
		let proof = cht::build_proof::<Block::Header, _>(cht_num, block_num, (cht_start..)
				.map(|num| self.block_hash(As::sa(num)).unwrap_or_default()))
