...
mod tests {
	...
	fn construct_block(backend: &InMemory<BlakeHasher, BlakeRlpCodec>, number: BlockNumber, parent_hash: Hash, state_root: Hash, txs: Vec<Transfer>) -> (Vec<u8>, Hash) {
	...
	fn block1(genesis_hash: Hash, backend: &InMemory<BlakeHasher, BlakeRlpCodec>) -> (Vec<u8>, Hash) {
		construct_block(
			backend,
			1,
			genesis_hash,
