...
// /// Convenience type for a Blake2b/Rlp flavoured NodeCodec
// pub type RlpCodecBlake = RlpNodeCodec<BlakeHasher>;

/// Convenience type for a Keccak/Rlp flavoured NodeCodec
pub type RlpCodecKeccak = RlpNodeCodec<KeccakHasher>;

/// Convenience type for a Blake2b/Rlp flavoured NodeCodec
pub type RlpCodec = RlpCodecKeccak;

impl NodeCodec<BlakeHasher> for RlpNodeCodec<BlakeHasher> {
	...
	fn try_decode_hash(data: &[u8]) -> Option<<BlakeHasher as Hasher>::Out> {
		let r = Rlp::new(data);
		if r.is_data() && r.size() == BlakeHasher::LENGTH {
	...
	fn ext_node(partial: &[u8], child_ref: ChildReference<<BlakeHasher as Hasher>::Out>) -> ElasticArray1024<u8> {
		let mut stream = RlpStream::new_list(2);
		stream.append(&partial);
		match child_ref {
