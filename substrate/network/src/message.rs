...
pub use self::generic::{BlockAnnounce, RemoteCallRequest, RemoteReadRequest,
	RemoteHeaderProofRequest, ConsensusVote, SignedConsensusVote, FromBlock, Body};
...
pub struct RemoteHeaderProofResponse<Header> {
	/// Id of a request this response was made for.
	pub id: RequestId,
	/// Header.
	pub header: Header,
...
pub mod generic {
	...
	use super::{Role, BlockAttribute, RemoteCallResponse, RemoteReadResponse,
		RemoteHeaderProofResponse, RequestId, Transactions, Direction};
	...
	pub enum Message<Block, Header, Hash, Number, Extrinsic> {
		...
		RemoteHeaderProofRequest(RemoteHeaderProofRequest<Number>),
		/// Remote header proof response.
		RemoteHeaderProofResponse(RemoteHeaderProofResponse<Header>),
	}
	...
	#[derive(Debug, PartialEq, Eq, Clone, Serialize, Deserialize)]
	/// Remote header proof request.
	pub struct RemoteHeaderProofRequest<N> {
		/// Unique request id.
		pub id: RequestId,
