...
impl<B: BlockT> ConsensusGossip<B> where B::Header: HeaderT<Number=u64> {
	...
	pub fn new_peer(&mut self, protocol: &mut Context<B>, who: NodeIndex, roles: Roles) {
		if roles.intersects(Roles::AUTHORITY | Roles::FULL) {
			...
			propagate_history_to(protocol, &self.messages, who, &mut data, true);
	...
	pub fn propagate_history(&mut self, protocol: &mut Context<B>) {
		for (who, data) in self.peers.iter_mut().filter(|&(_, ref data)| !data.knows_history) {
			propagate_history_to(protocol, &self.messages, *who, data, false);
		}
	}
...
fn propagate_history_to<B: BlockT>(
	...
	const INCREMENTAL_DURATION: Duration = Duration::from_secs(5);

	let now = Instant::now();

	if data.knows_history || (!fresh && data.last_incremental + INCREMENTAL_DURATION >= now) {
		return
	}
	...
	for entry in messages {
		if sent_count == INCREMENTAL_COUNT {
			debug!(target: "gossip", "Sent peer {} {} old gossip messages of {}.",
				who, INCREMENTAL_COUNT, messages.len());
			return
		}
