...
use rustc_hex::ToHex;
use serde_json;
use runtime_primitives::traits::{Block as BlockT, Header as HeaderT, Hashing, HashingFor, Zero};
	...
	B::Header: HeaderT<Number=u64>
{
	...
	pub fn handle_packet(&self, io: &mut SyncIo, peer_id: PeerId, data: &[u8]) {
		...
		match message {
			...
			GenericMessage::RemoteHeaderProofRequest(request) => self.on_remote_header_proof_request(io, peer_id, request),
			GenericMessage::RemoteHeaderProofResponse(response) => self.on_remote_header_proof_response(io, peer_id, response),
		}
	}
	...
	fn on_remote_header_proof_request(&self, io: &mut SyncIo, peer_id: PeerId, request: message::RemoteHeaderProofRequest<<<B as BlockT>::Header as HeaderT>::Number>) {
		...
		let (header, proof) = match self.chain.header_proof(request.block) {
			...
			Err(error) => {
				...
				(
					B::Header::new(
						<<B as BlockT>::Header as HeaderT>::Number::zero(),
						Default::default(),
						Default::default(),
						Default::default(),
						Default::default(),
		...
		self.send_message(io, peer_id, GenericMessage::RemoteHeaderProofResponse(message::RemoteHeaderProofResponse {
			id: request.id, header, proof,
		}));
	}

	fn on_remote_header_proof_response(&self, io: &mut SyncIo, peer_id: PeerId, response: message::RemoteHeaderProofResponse<B::Header>) {
