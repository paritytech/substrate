...
use client::{BlockStatus, ClientInfo, ImportResult};
use io::SyncIo;
	...
	B::Header: HeaderT<Number = u64>,
{
	...
	pub fn status(&self) -> Status<B> {
		...
		let state = match &best_seen {
			&Some(n) if n > self.best_queued_number && n - self.best_queued_number > 5 =>
	...
	pub fn new_peer(&mut self, io: &mut SyncIo, protocol: &Protocol<B>, peer_id: PeerId) {
		if let Some(info) = protocol.peer_info(peer_id) {
			match (
				protocol
					.chain()
					.block_status(&BlockId::Hash(info.best_hash)),
				info.best_number,
			) {
				(Err(e), _) => {
	...
	fn download_new(&mut self, io: &mut SyncIo, protocol: &Protocol<B>, peer_id: PeerId) {
		if let Some(ref mut peer) = self.peers.get_mut(&peer_id) {
			...
			match peer.state {
				PeerSyncState::Available => if let Some(range) = self.blocks.needed_blocks(
					peer_id,
