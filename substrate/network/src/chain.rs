...
pub trait Client<Block: BlockT>: Send + Sync {
	...
	fn header_proof(&self, block: <<Block as BlockT>::Header as HeaderT>::Number) -> Result<(Block::Header, Vec<Vec<u8>>), Error>;
...
impl<B, E, Block> Client<Block> for PolkadotClient<B, E, Block> where
	...
	fn header_proof(&self, block: <<Block as BlockT>::Header as HeaderT>::Number) -> Result<(Block::Header, Vec<Vec<u8>>), Error> {
		(self as &PolkadotClient<B, E, Block>).header_proof(&BlockId::Number(block))
	}
	fn execution_proof(&self, block: &Block::Hash, method: &str, data: &[u8]) -> Result<(Vec<u8>, Vec<Vec<u8>>), Error> {
