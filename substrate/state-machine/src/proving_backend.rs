...
mod tests {
	...
	fn test_proving() -> ProvingBackend<BlakeHasher, RlpCodec> {
	...
	fn proof_is_invalid_when_does_not_contains_root() {
		assert!(create_proof_check_backend::<BlakeHasher, RlpCodec>(1.into(), vec![]).is_err());
	}
	...
	fn proof_recorded_and_checked() {
		...
		let in_memory = InMemory::<BlakeHasher, RlpCodec>::default();
		...
		let proof_check = create_proof_check_backend::<BlakeHasher, RlpCodec>(in_memory_root.into(), proof).unwrap();
