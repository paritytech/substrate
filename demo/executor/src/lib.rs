...
mod tests {
	...
	fn panic_execution_with_foreign_code_gives_error() {
		let mut t: TestExternalities<KeccakHasher> = map![
			...
			twox_128(<balances::TotalStake<Concrete>>::key()).to_vec() => vec![69u8, 0, 0, 0, 0, 0, 0, 0],
	...
	fn successful_execution_with_native_equivalent_code_gives_ok() {
		let mut t: TestExternalities<KeccakHasher> = map![
			...
			twox_128(<balances::TotalStake<Concrete>>::key()).to_vec() => vec![111u8, 0, 0, 0, 0, 0, 0, 0],
		...
		runtime_io::with_externalities(&mut t, || {
			assert_eq!(Balances::voting_balance(&alice()), 42);
			assert_eq!(Balances::voting_balance(&bob()), 69);
		});
	}
	...
	fn block1() -> (Vec<u8>, Hash) {
		construct_block(
			...
			// Keccak
			hex!("d80a4a89e0a02864b5624ea9abb00f75285b8148c169c859d330b573914f1784").into(),
			vec![BareExtrinsic {
				signed: alice(),
				index: 0,
	...
	fn block2() -> (Vec<u8>, Hash) {
		construct_block(
			...
			// Keccak
			hex!("83e23a6dfbcf783ceb8eb4704344b8ec0478159fc670fa7a9f0c368a28eb8b74").into(),
			vec![
				BareExtrinsic {
					signed: bob(),
					index: 0,
	...
	fn block1big() -> (Vec<u8>, Hash) {
		construct_block(
			...
			// Keccak
			hex!("bd24909630b9431ca0e07492f880a5622c5f3c1471a0b0a2653b8c8e341df40b").into(),
			vec![BareExtrinsic {
				signed: alice(),
				index: 0,
	...
	fn full_native_block_import_works() {
		...
		runtime_io::with_externalities(&mut t, || {
			assert_eq!(Balances::voting_balance(&alice()), 41);
			assert_eq!(Balances::voting_balance(&bob()), 69);
		...
		runtime_io::with_externalities(&mut t, || {
			assert_eq!(Balances::voting_balance(&alice()), 30);
			assert_eq!(Balances::voting_balance(&bob()), 78);
			assert_eq!(System::events(), vec![
				EventRecord {
