...
use sc_client::leaves::{FinalizationDisplaced, LeafSet};
use sc_client_api::{
    backend::{NewBlockState, PrunableStateChangesTrieStorage},
    execution_extensions::ExecutionExtensions,
    BadBlocks, CloneableSpawn, ForkBlocks, IoInfo, MemoryInfo, MemorySize, UsageInfo,
};
...
use sp_core::{traits::CodeExecutor, ChangesTrieConfiguration};
use sp_database::Transaction;
use sp_runtime::traits::{
    Block as BlockT, HashFor, Header as HeaderT, NumberFor, One, SaturatedConversion, Zero,
};
use sp_runtime::{generic::BlockId, BuildStorage, Justification, Storage};
...
impl<B: BlockT> StateBackend<HashFor<B>> for RefTrackingState<B> {
    ...
    {
        self.state
            .child_storage_root(storage_key, child_info, delta)
    }
...
impl<Block: BlockT> Backend<Block> {
    ...
    fn force_delayed_canonicalize(
        ...
        if number_u64 > self.canonicalization_delay {
            ...
            let hash = if new_canonical == number_u64 {
                ...
                ::sc_client::blockchain::HeaderBackend::hash(
                    &self.blockchain,
    ...
    fn try_commit_operation(&self, mut operation: BlockImportOperation<Block>) -> ClientResult<()> {
        ...
        let cache_update = if let Some(set_head) = operation.set_head {
            if let Some(header) =
                sc_client::blockchain::HeaderBackend::header(&self.blockchain, set_head)?
            {
                let number = header.number();
    ...
    fn have_state_at(&self, hash: &Block::Hash, number: NumberFor<Block>) -> bool {
        if self.is_archive {
            match self.blockchain.header(BlockId::Hash(hash.clone())) {
                Ok(Some(header)) => sp_state_machine::Storage::get(
                    self.storage.as_ref(),
                    &header.state_root(),
                    (&[], None),
                )
