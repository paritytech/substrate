...
use sp_state_machine::{backend::Backend as StateBackend, DBValue};
...
impl<B: BlockT> BenchmarkingState<B> {
    ...
    pub fn new(genesis: Storage, _cache_size_mb: Option<usize>) -> Result<Self, String> {
        ...
        let (root, transaction): (B::Hash, _) = state
            .state
            .borrow_mut()
            .as_mut()
            .unwrap()
...
impl<B: BlockT> StateBackend<HashFor<B>> for BenchmarkingState<B> {
    ...
    fn child_storage(
        ...
        self.state
            ...
            .as_ref()
            .ok_or_else(state_err)?
            .child_storage(storage_key, child_info, key)
    }
    ...
    fn exists_child_storage(
        ...
        self.state
            ...
            .as_ref()
            .ok_or_else(state_err)?
            .exists_child_storage(storage_key, child_info, key)
    }
        ...
        I: IntoIterator<Item = (Vec<u8>, Option<Vec<u8>>)>,
    {
        self.state
            .borrow()
            .as_ref()
            .map_or(Default::default(), |s| {
                s.child_storage_root(storage_key, child_info, delta)
            })
    }
    ...
    fn pairs(&self) -> Vec<(Vec<u8>, Vec<u8>)> {
        self.state
            .borrow()
            .as_ref()
            .map_or(Default::default(), |s| s.pairs())
    }
    ...
    fn keys(&self, prefix: &[u8]) -> Vec<Vec<u8>> {
        self.state
            .borrow()
            .as_ref()
            .map_or(Default::default(), |s| s.keys(prefix))
    }

    fn child_keys(&self, storage_key: &[u8], child_info: ChildInfo, prefix: &[u8]) -> Vec<Vec<u8>> {
        self.state
            .borrow()
            .as_ref()
            .map_or(Default::default(), |s| {
                s.child_keys(storage_key, child_info, prefix)
            })
    }
    ...
    fn register_overlay_stats(&mut self, stats: &sp_state_machine::StateMachineStats) {
        self.state
            .borrow_mut()
            .as_mut()
            .map(|s| s.register_overlay_stats(stats));
    }
