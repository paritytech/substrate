...
impl<Block: BlockT> DbChangesTrieStorage<Block> {
    ...
    pub fn finalize(
        ...
        let parent_block_num = block_num
            .checked_sub(&One::one())
...
fn read_tries_meta<Block: BlockT>(
    ...
    match db.get(meta_column, meta_keys::CHANGES_TRIES_META) {
        Some(h) => match Decode::decode(&mut &h[..]) {
            Ok(h) => Ok(h),
            Err(err) => Err(ClientError::Backend(format!(
                "Error decoding changes tries metadata: {}",
                err
            ))),
        },
...
mod tests {
    ...
    fn insert_header_with_configuration_change(
        ...
        digest.push(DigestItem::ChangesTrieSignal(
            ChangesTrieSignal::NewConfiguration(new_configuration),
        ));
    ...
    fn changes_tries_are_pruned_on_finalization() {
        ...
        let is_pruned = |number| {
            let trie_root = backend
                ...
                .cloned();
            match trie_root {
                Some(trie_root) => backend
                    .changes_tries_storage
                    .get(&trie_root, EMPTY_PREFIX)
                    .unwrap()
                    .is_none(),
                None => true,
            }
    ...
    fn changes_tries_configuration_is_updated_on_block_insert() {
        ...
        assert_eq!(
            storage
                .configuration_at(&BlockId::Hash(block1))
                .unwrap()
                .config,
            config_at_1.clone(),
        );
        assert_eq!(
            storage
                .configuration_at(&BlockId::Hash(block2))
                .unwrap()
                .config,
            config_at_1.clone(),
        );
        assert_eq!(
            storage
                .configuration_at(&BlockId::Hash(block3))
                .unwrap()
                .config,
            config_at_3.clone(),
        );
        assert_eq!(
            storage
                .configuration_at(&BlockId::Hash(block4))
                .unwrap()
                .config,
            config_at_3.clone(),
        );
        assert_eq!(
            storage
                .configuration_at(&BlockId::Hash(block5))
                .unwrap()
                .config,
            config_at_5.clone(),
        );
        assert_eq!(
            storage
                .configuration_at(&BlockId::Hash(block6))
                .unwrap()
                .config,
            config_at_5.clone(),
        );
        assert_eq!(
            storage
                .configuration_at(&BlockId::Hash(block7))
                .unwrap()
                .config,
            config_at_7.clone(),
        );
    ...
    fn test_finalize_several_configuration_change_blocks_in_single_operation() {
        ...
        let configs = (0..=7)
            .map(|i| Some(ChangesTrieConfiguration::new(2, i)))
            .collect::<Vec<_>>();
        ...
        let header3 = Header {
            ...
            digest: Digest {
                logs: vec![DigestItem::ChangesTrieSignal(
                    ChangesTrieSignal::NewConfiguration(configs[3].clone()),
                )],
            },
            extrinsics_root: Default::default(),
        };
        ...
        op.set_block_data(header3, None, None, NewBlockState::Final)
            .unwrap();
        backend.commit_operation(op).unwrap();
        ...
        let header7 = Header {
            ...
            digest: Digest {
                logs: vec![DigestItem::ChangesTrieSignal(
                    ChangesTrieSignal::NewConfiguration(configs[7].clone()),
                )],
            },
            extrinsics_root: Default::default(),
        };
        let mut op = backend.begin_operation().unwrap();
        backend
            .begin_state_operation(&mut op, BlockId::Hash(block6))
            .unwrap();
        ...
        op.set_block_data(header7, None, None, NewBlockState::Final)
            .unwrap();
        backend.commit_operation(op).unwrap();
    }
    ...
    fn changes_tries_configuration_is_reverted() {
        ...
        backend
            .finalize_block(BlockId::Number(1), Some(vec![42]))
            .unwrap();
