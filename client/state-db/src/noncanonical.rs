            ...
            for (i, overlay) in level.into_iter().enumerate() {
                ...
                if self.pinned.contains_key(&overlay.hash) {
                    self.pinned_insertions
                        .insert(overlay.hash.clone(), overlay.inserted);
                } else {
...
mod tests {
    ...
    fn keeps_pinned_ref_count() {
        ...
        db.commit(
            &overlay
                .insert::<io::Error>(&h_3, 1, &H256::default(), c_3)
                .unwrap(),
        );
        overlay.apply_pending();
        ...
        let mut commit = CommitSet::default();
        overlay
            .canonicalize::<io::Error>(&h_3, &mut commit)
            .unwrap();
        db.commit(&commit);
    ...
    fn pin_keeps_parent() {
        ...
        db.commit(
            &overlay
                .insert::<io::Error>(&h_11, 1, &H256::default(), c_11)
                .unwrap(),
        );
        db.commit(
            &overlay
                .insert::<io::Error>(&h_12, 1, &H256::default(), c_12)
                .unwrap(),
        );
        ...
        let mut commit = CommitSet::default();
        overlay
            .canonicalize::<io::Error>(&h_12, &mut commit)
            .unwrap();
        db.commit(&commit);
