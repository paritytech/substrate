...
/// Statement store api Prometheus metrics.
	...
	pub fn register(registry: &Registry) -> Result<Self, PrometheusError> {
		Ok(Self {
			validations_scheduled: register(
				Counter::new(
					"substrate_sub_statement_store_validations_scheduled",
					"Total number of statements scheduled for validation",
				)?,
				registry,
			)?,
			validations_finished: register(
				Counter::new(
					"substrate_sub_statement_store_validations_finished",
					"Total number of statements that finished validation",
				)?,
				registry,
			)?,
		})
