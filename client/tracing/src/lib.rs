...
impl Visit for Values {
    ...
    fn record_str(&mut self, field: &Field, value: &str) {
        self.string_values
            .insert(field.name().to_string(), value.to_owned());
    }
...
mod tests {
    ...
    fn test_span_values() {
        ...
        assert_eq!(
            values.string_values.get("test_str").unwrap(),
            &test_str.to_owned()
        );
    }
    ...
    fn test_parent_id_with_threads() {
        ...
        if std::env::var("RUN_TEST_PARENT_ID_WITH_THREADS").is_err() {
            ...
            assert_eq!(
                event1.values.string_values.get("message").unwrap(),
                "test_event1"
            );
            assert_eq!(
                event2.values.string_values.get("message").unwrap(),
                "test_event2"
            );
