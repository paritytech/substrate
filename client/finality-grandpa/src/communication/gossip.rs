...
impl<N: Ord> Peers<N> {
    ...
    fn reshuffle(&mut self) {
        let mut lucky_peers: Vec<_> = self
            ...
            .iter()
            .filter_map(|(id, info)| {
                if matches!(info.roles, ObservedRole::Full | ObservedRole::Light) {
                    Some(id.clone())
                } else {
                    None
                }
...
impl<Block: BlockT> GossipValidator<Block> {
        ...
        F: FnOnce(Vec<PeerId>, NeighborPacket<NumberFor<Block>>),
    {
        let maybe_msg = self
            .inner
            .write()
            .note_commit_finalized(round, set_id, finalized);
        ...
        Box::new(move |who, intent, topic, mut data| {
            ...
            match GossipMessage::<Block>::decode(&mut data) {
                ...
                Ok(GossipMessage::Commit(full)) => {
                    ...
                    peer.view
                        .consider_global(set_id, full.message.target_number)
                        == Consider::Accept
...
mod tests {
    ...
    fn unanswerable_catch_up_requests_discarded() {
        ...
        let (val, _) = GossipValidator::<Block>::new(config(), set_state.clone(), None);
    ...
    fn detects_honest_out_of_scope_catch_requests() {
        ...
        val.inner
            .write()
            .peers
            .new_peer(peer.clone(), ObservedRole::Authority);
    ...
    fn doesnt_send_catch_up_requests_to_non_authorities_when_observer_enabled() {
        ...
        val.inner
            .write()
            .peers
            .new_peer(peer_full.clone(), ObservedRole::Full);
    ...
    fn only_restricts_gossip_to_authorities_after_a_minimum_threshold() {
        ...
        for _ in 0..5 {
            ...
            val.inner
                .write()
                .peers
                .new_peer(peer_id.clone(), ObservedRole::Authority);
