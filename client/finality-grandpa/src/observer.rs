    ...
    Client: crate::ClientForGrandpa<Block, BE>,
{
    ...
    let observer = commits.try_fold(
        last_finalized_number,
        move |last_finalized_number, global| {
...
mod tests {
    ...
    fn observer_work_polls_underlying_network_bridge() {
        ...
        let persistent_data =
            aux_schema::load_persistent(&*backend, client.info().genesis_hash, 0, || Ok(vec![]))
                .unwrap();
        ...
        executor::block_on(async move {
            ...
            assert_matches!(
                tester.events.next().now_or_never(),
