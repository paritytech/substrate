...
fn finalize_3_voters_no_observers() {
	...
	for i in 0..3 {
		assert_eq!(net.peer(i).client().chain_info().best_number, 20,
...
fn sync_justifications_on_change_blocks() {
	...
	for i in 0..4 {
		assert_eq!(net.peer(i).client().chain_info().best_number, 25,
			"Peer #{} failed to sync", i);
	}
...
fn finalizes_multiple_pending_changes_in_order() {
	...
	for i in 0..6 {
		assert_eq!(net.peer(i).client().chain_info().best_number, 30,
			"Peer #{} failed to sync", i);
	}
...
fn force_change_to_new_set() {
	...
	for (i, peer) in net.lock().peers().iter().enumerate() {
		assert_eq!(peer.client().chain_info().best_number, 26,
				"Peer #{} failed to sync", i);
...
fn voter_persists_its_votes() {
	...
	assert_eq!(net.peer(0).client().chain_info().best_number, 20,
		...
		runtime.spawn(round_rx.for_each(move |signed| {
			if state.compare_and_swap(0, 1, Ordering::SeqCst) == 0 {
				...
				future::Either::A(interval
					.take_while(move |_| {
						Ok(net2.lock().peer(1).client().chain_info().best_number != 40)
					})
					.for_each(|_| Ok(()))
					.and_then(move |_| {
						let block_30_hash =
...
fn finality_proof_is_fetched_by_light_client_when_consensus_data_changes() {
	...
	runtime.block_on(futures::future::poll_fn(move || -> std::result::Result<_, ()> {
		if net.lock().peer(1).client().chain_info().finalized_number == 1 {
			Ok(Async::Ready(()))
		} else {
			net.lock().poll();
			Ok(Async::NotReady)
...
fn empty_finality_proof_is_returned_to_light_client_when_authority_set_is_different() {
	...
	assert_eq!(
		net.lock().peer(3).client().chain_info().finalized_number,
...
fn grandpa_environment_respects_voting_rules() {
	...
	assert_eq!(
		unrestricted_env.best_chain_containing(
			peer.client().chain_info().finalized_hash
		).unwrap().1,
		20,
	);
