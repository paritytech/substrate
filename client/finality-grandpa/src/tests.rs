...
#[test]
fn grandpa_environment_passes_actual_best_block_to_voting_rules() {
	...
	select_chain.set_best_chain(client.header(&BlockId::Number(21)).unwrap().unwrap());
	select_chain.set_finality_target(client.header(&BlockId::Number(21)).unwrap().unwrap().hash());

	assert_eq!(
		block_on(env.best_chain_containing(peer.client().info().finalized_hash))
			.unwrap()
			.unwrap()
			.1,
		16,
	...
	select_chain.set_best_chain(client.header(&BlockId::Number(21)).unwrap().unwrap());
	select_chain.set_finality_target(client.header(&BlockId::Number(10)).unwrap().unwrap().hash());
	...
	select_chain.set_best_chain(client.header(&BlockId::Number(21)).unwrap().unwrap());
	select_chain.set_finality_target(client.header(&BlockId::Number(42)).unwrap().unwrap().hash());
