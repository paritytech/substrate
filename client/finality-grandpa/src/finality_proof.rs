...
pub fn prove_warp_sync<Block: BlockT, B: BlockchainBackend<Block>>(
    ...
    let end_number = blockchain
        .block_number_from_id(&end)?
    ...
    if begin_number > end_number {
        return Err(ClientError::Backend(
            "Unfinalized start for authority proof".to_string(),
        ));
    }
    ...
    while index > Zero::zero() {
        index = index - One::one();
        if let Some((fragment, apply_block)) =
            get_warp_sync_proof_fragment(blockchain, index, &mut cache)?
        {
            if last_apply
                .map(|next| &next > header.number())
                .unwrap_or(false)
            {
    ...
    while index <= end_number {
        if max_fragment_limit
            .map(|limit| result.len() >= limit)
            .unwrap_or(false)
        {
            break;
        }

        if let Some((fragement, apply_block)) =
            get_warp_sync_proof_fragment(blockchain, index, &mut cache)?
        {
    ...
    let at_limit = max_fragment_limit
        .map(|limit| result.len() >= limit)
        .unwrap_or(false);
    ...
    if !at_limit && result.last().as_ref().map(|head| head.header.number()) != Some(&end_number) {
        ...
        if let Some(justification) =
            blockchain.justification(BlockId::Number(end_number.clone()))?
        {
...
fn get_warp_sync_proof_fragment<Block: BlockT, B: BlockchainBackend<Block>>(
    ...
    )) = crate::import::find_forced_change::<Block>(&header)
    {
        ...
        if let Some(justification) = blockchain.justification(BlockId::Number(index.clone()))? {
            ...
            return Err(ClientError::Backend(
                "Unjustified block with authority set change".to_string(),
            ));
        }
    }
    ...
    cache
        .as_mut()
        .map(|cache| cache.new_item(index, result.clone()));
    Ok(result)
}
    ...
    J: Decode + ProvableJustification<Block::Header> + BlockJustification<Block::Header>,
{
    ...
    for (ix, fragment) in proof.into_iter().enumerate() {
        ...
        result = check_warp_sync_proof_fragment::<Block, J>(
            result.0, &result.1, &result.2, is_last, &fragment,
        )?;
    ...
    J: Decode + ProvableJustification<Block::Header> + BlockJustification<Block::Header>,
{
    ...
    {
        return Err(ClientError::Backend(
            "Invalid authority warp proof, justification do not match header".to_string(),
        ));
    }

    if authorities_proof.header.number() <= previous_checked_block {
        return Err(ClientError::Backend(
            "Invalid authority warp proof".to_string(),
        ));
    }
...
pub(crate) mod tests {
    ...
    fn finality_proof_check_works() {
        ...
        let proof =
            check_finality_proof::<_, TestJustification>(1, auth.clone(), finality_proof.encode())
                .unwrap();
    ...
    fn warp_sync_proof_encoding_decoding() {
        fn test_blockchain(
            ...
            for i in 0..nb_blocks {
                ...
                set_change.first().map(|j| {
                    if i == j.0 {
                        set_change = &set_change[1..];
                        let next_authorities: Vec<_> =
                            j.1.iter()
                                .map(|i| (AuthorityId::from_slice(&[*i; 32]), 1u64))
                                .collect();
                        set_id_next += 1;
                        header
                            .digest_mut()
                            .logs
                            .push(sp_runtime::generic::DigestItem::Consensus(
                ...
                let justification = justifications.first().and_then(|j| {
                    if i == j.0 {
                        justifications = &justifications[1..];

                        let authority =
                            j.1.iter()
                                .map(|j| (AuthorityId::from_slice(&[*j; 32]), 1u64))
                                .collect();
        ...
        let proof_no_cache =
            prove_warp_sync(&blockchain, hashes[6], None, Some(&mut cache)).unwrap();
        let proof = prove_warp_sync(&blockchain, hashes[6], None, Some(&mut cache)).unwrap();
        assert_eq!(proof_no_cache, proof);

        let initial_authorities: Vec<_> = [1u8, 2, 3]
            .iter()
            .map(|i| (AuthorityId::from_slice(&[*i; 32]), 1u64))
            .collect();

        let authorities_next: Vec<_> = [9u8]
            .iter()
            .map(|i| (AuthorityId::from_slice(&[*i; 32]), 1u64))
            .collect();
        ...
        .is_err());
        let (_header, current_set_id, current_set) = check_warp_sync_proof::<
            Block,
            TestBlockJustification,
        >(
            1, authorities_next.clone(), proof.clone()
        )
        .unwrap();
        ...
        let (_header, current_set_id, current_set) =
