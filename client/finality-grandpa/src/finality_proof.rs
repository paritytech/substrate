	...
	J: ProvableJustification<Block::Header>,
{
	...
	let justification = if let Some(justification) = blockchain.justification(last_block_of_the_set_id)? {
		justification
	} else {
		trace!(
			target: "afg",
	...
	let last_block_of_the_set_hash = blockchain.expect_block_hash_from_id(&last_block_of_the_set_id)?;
	...
	let unknown_headers = {
		...
		loop {
			if current_number >= last_block_of_the_set {
				break;
			}
...
pub(crate) mod tests {
	...
	fn finality_proof_works_for_basic_setup() {
		...
		blockchain.insert(
			header(4).hash(), header(4), Some(just4), None, NewBlockState::Final
		).unwrap();
		blockchain.insert(
			header(5).hash(), header(5), Some(just5.clone()), None, NewBlockState::Final
		).unwrap();
		...
		assert_eq!(proof_of_5, FinalityProof {
			block: header(5).hash(),
			justification: just5,
