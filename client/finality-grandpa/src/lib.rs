...
fn local_authority_id(
	...
	match keystore {
		Some(keystore) => {
			let keys: Vec<_> = voters.iter().map(|v| (v.0.to_raw_vec(), AuthorityId::ID)).collect();
			SyncCryptoStore::has_keys(&**keystore, &keys).into_found().next().and_then(|i| {
				Some(voters.nth(i).unwrap().0.clone())
			})
        },
		None => None,
	}
