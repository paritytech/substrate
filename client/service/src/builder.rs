...
use crate::config::{Configuration, KeystoreConfig, OffchainWorkerConfig, PrometheusConfig};
use crate::metrics::MetricsService;
use crate::status_sinks;
use crate::{
    build_network_future, start_rpc_servers, SpawnTaskHandle, TaskManager, TransactionPoolAdapter,
};
use crate::{
    error::Error, MallocSizeOfWasm, NetworkState, NetworkStatus, Service, DEFAULT_PROTOCOL_ID,
};
...
use sc_client_api::{
    self, backend::RemoteBackend, execution_extensions::ExtensionsFactory, light::RemoteBlockchain,
    BadBlocks, BlockchainEvents, CallExecutor, CloneableSpawn, ExecutorProvider, ForkBlocks,
    UsageProvider,
};
use sc_client_db::{Backend, DatabaseSettings};
use sc_executor::{NativeExecutionDispatch, NativeExecutor, RuntimeInfo};
use sc_keystore::Store as Keystore;
use sc_network::config::{BoxFinalityProofRequestBuilder, FinalityProofProvider, OnDemand, Role};
...
use sp_runtime::traits::{Block as BlockT, HashFor, NumberFor, SaturatedConversion};
...
pub type TLightBackend<TBl> =
    crate::client::light::backend::Backend<sc_client_db::light::LightStorage<TBl>, HashFor<TBl>>;

/// Light call executor type.
pub type TLightCallExecutor<TBl, TExecDisp> =
    crate::client::light::call_executor::GenesisCallExecutor<
    ...
    > {
        ...
        let keystore = match &config.keystore {
            KeystoreConfig::Path { path, password } => {
                Keystore::open(path.clone(), password.clone())?
            }
            KeystoreConfig::InMemory => Keystore::new_in_memory(),
        };
        ...
        let fetch_checker = Arc::new(crate::client::light::new_fetch_checker::<_, TBl, _>(
    ...
    ServiceBuilder<TBl, TRtApi, TCl, TFchr, TSc, TImpQu, TFprb, TFpp, TExPool, TRpc, Backend>
{
    ...
    pub fn with_opt_select_chain<USc>(
        self,
        select_chain_builder: impl FnOnce(&Configuration, &Arc<Backend>) -> Result<Option<USc>, Error>,
    ) -> Result<
        ServiceBuilder<TBl, TRtApi, TCl, TFchr, USc, TImpQu, TFprb, TFpp, TExPool, TRpc, Backend>,
        Error,
    > {
    ...
    pub fn with_import_queue<UImpQu>(
        ...
        ServiceBuilder<TBl, TRtApi, TCl, TFchr, TSc, UImpQu, TFprb, TFpp, TExPool, TRpc, Backend>,
        Error,
    >
    where
        TSc: Clone,
    ...
    pub fn with_import_queue_and_opt_fprb<UImpQu, UFprb>(
        ...
        ServiceBuilder<TBl, TRtApi, TCl, TFchr, TSc, UImpQu, UFprb, TFpp, TExPool, TRpc, Backend>,
        Error,
    >
    where
        TSc: Clone,
        ...
        TFchr: Clone,
    {
        self.with_import_queue_and_opt_fprb(|cfg, cl, b, f, sc, tx, tb, pr| {
            builder(cfg, cl, b, f, sc, tx, tb, pr).map(|(q, f)| (q, Some(f)))
        })
    }
    ...
    pub fn with_transaction_pool<UExPool>(
        ...
        ServiceBuilder<TBl, TRtApi, TCl, TFchr, TSc, TImpQu, TFprb, TFpp, UExPool, TRpc, Backend>,
        Error,
    >
    where
        TSc: Clone,
        TFchr: Clone,
    {
        ...
        if let Some(background_task) = background_task {
            self.task_manager
                .spawn_handle()
                .spawn("txpool-background", background_task);
        }
    ...
    pub fn with_block_announce_validator(
        self,
        block_announce_validator_builder: impl FnOnce(Arc<TCl>) -> Box<dyn BlockAnnounceValidator<TBl> + Send>
            + Send
            + 'static,
    ) -> Result<
        ServiceBuilder<TBl, TRtApi, TCl, TFchr, TSc, TImpQu, TFprb, TFpp, TExPool, TRpc, Backend>,
    ...
    TRpc: sc_rpc::RpcExtension<sc_rpc::Metadata>,
{
    ...
    pub fn with_execution_extensions_factory(
        ...
        self.client
            .execution_extensions()
            .set_extensions_factory(execution_extensions_factory);
        Ok(self)
    }
        ...
        TExec: CallExecutor<TBl, Backend = TBackend>,
    {
        ...
        let offchain_workers = match (config.offchain_worker.clone(), offchain_storage.clone()) {
            (OffchainWorkerConfig { enabled: true, .. }, Some(db)) => Some(Arc::new(
                sc_offchain::OffchainWorkers::new(client.clone(), db),
            )),
...

        {
            ...
            let (import_stream, finality_stream) = (
                ...
                client
                    ...
                    .map(|n| ChainEvent::Finalized { hash: n.hash }),
            );
            let events =
                futures::stream::select(import_stream, finality_stream).for_each(move |event| {
                    ...
                    if let Some(txpool) = txpool.as_ref() {
                        notifications_spawn_handle.spawn("txpool-maintain", txpool.maintain(event));
                    }

                    ready(())
                });

            spawn_handle.spawn("txpool-and-offchain-notif", events);
        }

        {
            ...
            spawn_handle.spawn("on-transaction-imported", events);
        }
        ...
        let (state_tx, state_rx) =
            tracing_unbounded::<(NetworkStatus<_>, NetworkState)>("mpsc_netstat1");
        network_status_sinks
            .lock()
            .push(std::time::Duration::from_millis(5000), state_tx);
        let tel_task = state_rx.for_each(move |(net_status, _)| {
            let info = client_.usage_info();
            metrics_service.tick(&info, &transaction_pool_.status(), &net_status);
            ready(())
        });

        spawn_handle.spawn("telemetry-periodic-send", tel_task);

        // Periodically send the network state to the telemetry.
        let (netstat_tx, netstat_rx) =
            tracing_unbounded::<(NetworkStatus<_>, NetworkState)>("mpsc_netstat2");
        network_status_sinks
            .lock()
        ...
        spawn_handle.spawn("telemetry-periodic-network-state", tel_task_2);
        ...
        let gen_handler = |deny_unsafe: sc_rpc::DenyUnsafe| {
                ...
                (remote_backend.as_ref(), on_demand.as_ref())
            {
                ...
                let (state, child_state) =
                    sc_rpc::state::new_full(client.clone(), subscriptions.clone());
                (chain, state, child_state)
            };
        ...
        let telemetry_connection_sinks: Arc<Mutex<Vec<TracingUnboundedSender<()>>>> =
            Default::default();
        ...
        let telemetry = config.telemetry_endpoints.clone().map(|endpoints| {
            ...
            let startup_time = SystemTime::UNIX_EPOCH
                ...
                .unwrap_or(0);
            let future = telemetry.clone().for_each(move |event| {
            ...
            spawn_handle.spawn("telemetry-worker", future);
