...
use crate::{build_network_future, start_rpc_servers, TaskManagerBuilder, TransactionPoolAdapter};
use crate::{
...
use sc_client_api::{
    ...
    BlockchainEvents, CallExecutor, ExecutorProvider,
};
...
pub type TFullCallExecutor<TBl, TExecDisp> =
    sc_client::LocalCallExecutor<sc_client_db::Backend<TBl>, NativeExecutor<TExecDisp>>;
...
pub type TLightBackend<TBl> =
    sc_client::light::backend::Backend<sc_client_db::light::LightStorage<TBl>, HashFor<TBl>>;
...
impl ServiceBuilder<(), (), (), (), (), (), (), (), (), (), ()> {
    ...
    > {
        ...
        let fetch_checker = Arc::new(sc_client::light::new_fetch_checker::<_, TBl, _>(
        ...
        TFchr: Clone,
    {
        ...
        if let Some(background_task) = background_task {
            self.background_tasks
                .push(("txpool-background", background_task));
        }
    ...
    TRpc: sc_rpc::RpcExtension<sc_rpc::Metadata> + Clone,
{
        ...
        TExec: CallExecutor<TBl, Backend = TBackend>,
    {
        ...
        let offchain_workers = match (config.offchain_worker, offchain_storage.clone()) {
            (true, Some(db)) => Some(Arc::new(sc_offchain::OffchainWorkers::new(
                client.clone(),
                db,
            ))),
            (true, None) => {
...

        {
            ...
            spawn_handle.spawn("telemetry-on-block", events);
        }
