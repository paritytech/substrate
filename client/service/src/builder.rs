...
use sc_network::config::{OnDemand, Role};
...
use sp_runtime::traits::{Block as BlockT, BlockIdTo, HashFor, Zero};
    ...
    <TCl as ProvideRuntimeApi<TBl>>::Api: sc_offchain::OffchainWorkerApi<TBl>,
{
    let offchain_workers = match backend.offchain_storage() {
        Some(db) => Some(Arc::new(sc_offchain::OffchainWorkers::new(
            client.clone(),
            db,
        ))),
        None => {
    ...
    TRpc: sc_rpc::RpcExtension<sc_rpc::Metadata>,
{
    ...
    let telemetry_connection_notifier =
        init_telemetry(&mut config, network.clone(), client.clone());
...
fn init_telemetry<TBl: BlockT, TCl: BlockBackend<TBl>>(
    ...
    config
        .telemetry_handle
        .as_mut()
        .map(|handle| handle.start_telemetry(telemetry_span, endpoints, connection_message))
}
