...
impl PrometheusMetrics {
    fn setup(
        ...
        Ok(Self {
            ...
            #[cfg(all(any(unix, windows), not(target_os = "android")))]
            load_avg: register(
                GaugeVec::new(Opts::new("load_avg", "System load average"), &["over"])?,
                registry,
            )?,

            // process
            memory_usage_bytes: register(
                Gauge::new(
                    "memory_usage_bytes",
                    "Node memory (resident set size) usage",
                )?,
                registry,
            )?,

            cpu_usage_percentage: register(
                Gauge::new("cpu_usage_percentage", "Node CPU usage")?,
                registry,
            )?,
            ...
            threads: register(
                Gauge::new("threads", "Number of threads used by the process")?,
                registry,
            )?,
            ...
            open_files: register(
                GaugeVec::new(
                    Opts::new(
                        "open_file_handles",
                        "Open file handlers held by the process",
                    ),
                    &["fd_type"],
                )?,
                registry,
            ...
            // I/ O
            network_per_sec_bytes: register(
                GaugeVec::new(
...
impl MetricsService {
    fn inner_new(metrics: Option<PrometheusMetrics>) -> Self {
        let process = procfs::process::Process::myself().expect("Procfs doesn't fail on unix. qed");
...
impl MetricsService {
    ...
    fn process_info(&mut self) -> ProcessInfo {
        self.pid
            .map(|pid| self.process_info_for(&pid))
            .unwrap_or_default()
    }
}
...
impl MetricsService {
    pub fn with_prometheus(
        ...
        PrometheusMetrics::setup(registry, name, version, roles).map(|p| Self::inner_new(Some(p)))
    }
        ...
        self.pid.as_ref().and_then(|pid| {
            ...
            iterate_sockets_info(af_flags, proto_flags)
                ...
                .map(|iter| {
                    ...
                    .fold(
                        ConnectionsCount::default(),
                        |mut counter, socket_state| {
                            match socket_state {
