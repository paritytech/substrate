...
fn ensure_tasks_are_awaited_on_shutdown() {
	...
	let task_manager = TaskManager::new(task_executor, None, None).unwrap();
...
fn ensure_keep_alive_during_shutdown() {
	...
	let mut task_manager = TaskManager::new(task_executor, None, None).unwrap();
	let spawn_handle = task_manager.spawn_handle();
	let drop_tester = DropTester::new();
...
fn ensure_children_tasks_ends_when_task_manager_terminated() {
	...
	let mut task_manager = TaskManager::new(task_executor.clone(), None, None).unwrap();
	let child_1 = TaskManager::new(task_executor.clone(), None, None).unwrap();
	let spawn_handle_child_1 = child_1.spawn_handle();
	let child_2 = TaskManager::new(task_executor.clone(), None, None).unwrap();
