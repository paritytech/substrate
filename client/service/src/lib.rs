...
pub use sc_transaction_pool_primitives::{TransactionPool, InPoolTransaction, error::IntoPoolError};
...
where
	...
	E: IntoPoolError + From<sc_transaction_pool_primitives::error::Error>,
{
	pool.ready()
...
where
	...
	B: BlockT,
	H: std::hash::Hash + Eq + sp_runtime::traits::Member + sp_runtime::traits::MaybeSerialize,
	E: 'static + IntoPoolError + From<sc_transaction_pool_primitives::error::Error>,
{
	...
	fn import(
		...
		let import_future = self.pool.submit_one(&best_block_id, sc_transaction_pool_primitives::TransactionSource::External, uxt);
		Box::pin(async move {
			match import_future.await {
				Ok(_) => TransactionImport::NewGood,
				Err(e) => match e.into_pool_error() {
					Ok(sc_transaction_pool_primitives::error::Error::AlreadyImported(_)) => TransactionImport::KnownGood,
					Ok(e) => {
