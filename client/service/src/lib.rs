...
use sc_network::{network_state::NetworkState, NetworkService, PeerId, ReportHandle};
use sp_runtime::generic::BlockId;
use sp_runtime::traits::{Block as BlockT, NumberFor};
use sp_utils::mpsc::{tracing_unbounded, TracingUnboundedReceiver, TracingUnboundedSender};

pub use self::builder::{
    new_full_client, ServiceBuilder, ServiceBuilderCommand, TFullBackend, TFullCallExecutor,
    TFullClient, TLightBackend, TLightCallExecutor, TLightClient,
};
pub use self::error::Error;
pub use config::{Configuration, DatabaseConfig, PruningMode, Role};
...
#[doc(hidden)]
pub use sc_network::config::{BoxFinalityProofRequestBuilder, FinalityProofProvider, OnDemand};
...
pub use task_manager::{SpawnTaskHandle, TaskManagerBuilder};
...
impl<TBl, TCl, TSc, TNetStatus, TNet, TTxPool, TOc> Unpin
    for Service<TBl, TCl, TSc, TNetStatus, TNet, TTxPool, TOc>
{
}
    ...
    'static + Future<Output = Result<(), Error>> + Spawn + Send + Unpin
{
    ...
    fn client(
        &self,
    ) -> Arc<sc_client::Client<Self::Backend, Self::CallExecutor, Self::Block, Self::RuntimeApi>>;

    /// Get clone of select chain.
    fn select_chain(&self) -> Option<Self::SelectChain>;

    /// Get shared network instance.
    fn network(&self) -> Arc<NetworkService<Self::Block, <Self::Block as BlockT>::Hash>>;
    ...
    #[deprecated(
        note = "Use `spawn_task`/`spawn_essential_task` instead, those functions will attach on_exit signal."
    )]
...
impl<TBl, TBackend, TExec, TRtApi, TSc, TExPool, TOc> AbstractService
    for Service<
        TBl,
        Client<TBackend, TExec, TBl, TRtApi>,
        TSc,
        NetworkStatus<TBl>,
        NetworkService<TBl, TBl::Hash>,
        TExPool,
        TOc,
    >
where
...
impl<TBl, TCl, TSc, TNetStatus, TNet, TTxPool, TOc> Future
    for Service<TBl, TCl, TSc, TNetStatus, TNet, TTxPool, TOc>
{
...
impl<TBl, TCl, TSc, TNetStatus, TNet, TTxPool, TOc> Spawn
    for Service<TBl, TCl, TSc, TNetStatus, TNet, TTxPool, TOc>
{
...
fn build_network_future<B: BlockT, C: sc_client::BlockchainEvents<B>, H: sc_network::ExHashT>(
    role: Role,
    ...
    futures::future::poll_fn(move |cx| {
        ...
        while let Poll::Ready(Some(notification)) =
            Pin::new(&mut imported_blocks_stream).poll_next(cx)
        {
            ...
            if announce_imported_blocks {
                network
                    .service()
                    .announce_block(notification.hash, Vec::new());
            }
        }
...
fn start_rpc_servers<H: FnMut() -> sc_rpc_server::RpcHandler<sc_rpc::Metadata>>(
    ...
    Ok(Box::new((
        maybe_start_server(config.rpc_http, |address| {
            sc_rpc_server::start_http(address, config.rpc_cors.as_ref(), gen_handler())
        })?
    ...
    fn import(
        ...
        match Decode::decode(&mut &encoded[..]) {
            Ok(uxt) => {
                ...
                let import_future = import_future.map(move |import_result| match import_result {
