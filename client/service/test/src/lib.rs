...
use sc_service::{
    ...
    AbstractService, ChainSpecExtension, Configuration, Error, GenericChainSpec, Role,
    RuntimeGenesis, TaskType,
};
    ...
    fn insert_nodes(
        ...
        for (key, authority) in authorities {
            ...
            let addr = addr.with(multiaddr::Protocol::P2p(
                service.get().network().local_peer_id().clone().into(),
            ));
    ...
    G: RuntimeGenesis + 'static,
{
    ...
    network.run_until_all_full(
        |_index, service| service.get().client().info().best_number == (NUM_BLOCKS as u32).into(),
        |_index, service| service.get().client().info().best_number == (NUM_BLOCKS as u32).into(),
    );
    ...
    futures::executor::block_on(first_service.get().transaction_pool().submit_one(
        &best_block,
        source,
        extrinsic,
    ))
