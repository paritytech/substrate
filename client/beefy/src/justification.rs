...
use beefy_primitives::{ValidatorSet, VersionedFinalityProof};
...
>(
	...
	verify_with_validator_set::<Block, AuthId, TSignature, BKS>(
		target_number,
		validator_set,
		&proof,
	)
...
pub(crate) mod tests {
		...
		TSignature: Encode + Decode + Debug + Clone + Sync + Send + std::cmp::PartialEq + 'static,
	{
		...
		let signatures = keys
			.iter()
			.map(|key| Some(<Keyring as GenericKeyring<TKeyPair>>::sign(*key, &message)))
			.collect();
		...
		BKS: BeefyKeystore<AuthId, TSignature, Public = AuthId>,
	{
		...
		let block_num = 42;
		let proof =
			new_finality_proof::<TKeyPair, AuthId, TSignature>(block_num, &validator_set, keys);
		...
		let good_proof = proof.clone().into();
		match verify_with_validator_set::<Block, AuthId, TSignature, BKS>(
			block_num + 1,
			&validator_set,
			&good_proof,
		) {
		...
		BKS: BeefyKeystore<AuthId, TSignature, Public = AuthId>,
	{
		...
		let verified = decode_and_verify_finality_proof::<Block, AuthId, TSignature, BKS>(
			&encoded,
			block_num,
			&validator_set,
		)
	...
	fn should_decode_and_verify_finality_proof_with_ecdsa_keys() {
		should_decode_and_verify_finality_proof::<
			ECDSAKeyPair,
			ECDSAPublic,
			ECDSASignature,
			BeefyECDSAKeystore,
