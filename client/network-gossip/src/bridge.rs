...
impl<B: BlockT> GossipEngine<B> {
    ...
    pub fn broadcast_topic(&mut self, topic: B::Hash, force: bool) {
        self.state_machine
            .broadcast_topic(&mut *self.network, topic, force);
    }
    ...
    pub fn send_topic(&mut self, who: &PeerId, topic: B::Hash, force: bool) {
        self.state_machine
            .send_topic(&mut *self.network, who, topic, force)
    }

    /// Multicast a message to all peers.
    pub fn gossip_message(&mut self, topic: B::Hash, message: Vec<u8>, force: bool) {
        self.state_machine
            .multicast(&mut *self.network, topic, message, force)
    }
    ...
    pub fn send_message(&mut self, who: Vec<sc_network::PeerId>, data: Vec<u8>) {
        for who in &who {
            self.state_machine
                .send_message(&mut *self.network, who, data.clone());
        }
    }
...
impl<B: BlockT> Future for GossipEngine<B> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Self::Output> {
                    ...
                    match this.network_event_stream.poll_next_unpin(cx) {
                        Poll::Ready(Some(event)) => match event {
                            Event::NotificationStreamOpened {
                                ...
                                this.state_machine
                                    .new_peer(&mut *this.network, remote, role);
                            }
        ...
        while let Poll::Ready(()) = this.periodic_maintenance_interval.poll_unpin(cx) {
            this.periodic_maintenance_interval
                .reset(PERIODIC_MAINTENANCE_INTERVAL);
        ...
        fn prop(channels: Vec<ChannelLengthAndTopic>, notifications: Vec<Vec<Message>>) {
            ...
            let expected_msgs_per_topic_all_chan = notifications
                ...
                .map(|(topic, num)| {
                    (
                        topic,
                        num_channels_per_topic.get(&topic).unwrap_or(&0) * num,
                    )
                })
            ...
            for (received_topic, received_num) in expected_msgs_per_topic_all_chan.iter() {
                assert_eq!(
                    expected_msgs_per_topic_all_chan
                        .get(&received_topic)
                        .unwrap_or(&0),
                    received_num,
                );
