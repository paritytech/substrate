    ...
    Number: Ord + One + Zero + Add<Output = Number> + Copy,
{
    ...
    pub fn epoch_descriptor_for_child_of<D: IsDescendentOfBuilder<Hash>>(
        ...
        // we need.
        let predicate = |epoch: &PersistedEpochHeader<E>| match *epoch {
            PersistedEpochHeader::Genesis(ref epoch_0, _) => epoch_0.start_slot <= slot_number,
            PersistedEpochHeader::Regular(ref epoch_n) => epoch_n.start_slot <= slot_number,
        };
...
mod tests {
    ...
    fn epoch_changes_between_blocks() {
        ...
        epoch_changes
            ...
            .unwrap();
        let genesis_epoch = epoch_changes
            .epoch_data(&genesis_epoch, &make_genesis)
            .unwrap();
