...
fn claim_secondary_slot(
    ...
    let expected_author = super::authorship::secondary_slot_author(slot, authorities, *randomness)?;

    for (authority_id, authority_index) in keys {
        if authority_id == expected_author {
            let pre_digest = if author_secondary_vrf {
                let transcript_data =
                    super::authorship::make_transcript_data(randomness, slot, *epoch_index);
...
fn claim_primary_slot(
    ...
    for (authority_id, authority_index) in keys {
        let transcript = super::authorship::make_transcript(randomness, slot, *epoch_index);
        let transcript_data =
            super::authorship::make_transcript_data(randomness, slot, *epoch_index);
...
mod tests {
    ...
    fn claim_secondary_plain_slot_works() {
        ...
        epoch
            .authorities
            .push((valid_public_key.clone().into(), 10));
        assert_eq!(
            claim_slot(10.into(), &epoch, &keystore).unwrap().1,
            valid_public_key.into()
        );
    }
}
