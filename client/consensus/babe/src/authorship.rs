...
use sp_consensus_babe::{
    make_transcript, AuthorityId, AuthorityPair, BabeAuthorityWeight, SlotNumber, BABE_VRF_PREFIX,
};
    ...
    for (pair, authority_index) in key_pairs {
        if pair.public() == *expected_author {
            let pre_digest = if author_secondary_vrf {
                let transcript =
                    super::authorship::make_transcript(randomness, slot_number, *epoch_index);
...
pub fn claim_slot_using_key_pairs(
    ...
    claim_primary_slot(slot_number, epoch, epoch.config.c, &key_pairs).or_else(|| {
        if epoch
            .config
            .allowed_slots
