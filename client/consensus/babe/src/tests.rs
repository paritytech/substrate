...
use sp_consensus_babe::{AllowedSlots, AuthorityPair, SlotNumber};
...
fn propose_and_import_block<Transaction>(
    ...
    let import_result = block_import
        .import_block(import, Default::default())
        .unwrap();
...
fn importing_block_one_sets_genesis_epoch() {
    ...
    let epoch_for_second_block = epoch_changes
        .epoch_data_for_child_of(descendent_query(&*client), &block_hash, 1, 1000, |slot| {
