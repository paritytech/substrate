...
pub use sp_consensus_babe::{
    ...
    BabeConfiguration, ConsensusLog, SlotNumber, BABE_ENGINE_ID, VRF_OUTPUT_LENGTH,
};
use sp_core::Pair;
...
impl Config {
        ...
        C::Api: BabeApi<B, Error = sp_blockchain::Error>,
    {
        ...
        match sc_consensus_slots::SlotDuration::get_or_compute(client, |a, b| a.configuration(b))
            .map(Self)
        {
            Ok(s) => Ok(s),
            Err(s) => {
    ...
    Error: std::error::Error + Send + From<ConsensusError> + From<I::Error> + 'static,
{
    ...
    > {
        Box::new(
            |header, header_hash, body, storage_changes, (_, pair), epoch_descriptor| {
    ...
    Client::Api: BabeApi<Block> + ApiExt<Block>,
{
    ...
    fn import_block(
        ...
        match (first_in_epoch, next_epoch_digest.is_some()) {
            ...
            (false, true) => {
                return Err(ConsensusError::ClientImport(
                    Error::<Block>::UnexpectedEpochChange.into(),
                ));
            }
        }
...
pub mod test_helpers {
        ...
        C::Api: BabeApi<B>,
    {
        ...
        authorship::claim_slot(slot_number, &epoch, &link.config, keystore)
            .map(|(digest, _)| digest)
    }
}
