...
use sc_consensus_slots::{
    check_equivocation, BackoffAuthoringBlocksStrategy, CheckedHeader, SlotCompatible, SlotInfo,
    StorageChanges,
};
...
pub fn start_babe<B, C, SC, E, I, SO, CAW, BS, Error>(
    BabeParams {
        keystore,
        client,
        select_chain,
        ...
        babe_link,
        can_author_with,
    }: BabeParams<B, C, E, I, SO, SC, CAW, BS>,
...
type SlotNotificationSinks<B> = Arc<
    Mutex<
        Vec<
            Sender<(
                Slot,
                ViableEpochDescriptor<<B as BlockT>::Hash, NumberFor<B>, Epoch>,
            )>,
        >,
    >,
>;
...
pub mod test_helpers {
        ...
        C::Api: BabeApi<B>,
    {
        ...
        authorship::claim_slot(slot, &epoch, &keystore).map(|(digest, _)| digest)
    }
}
