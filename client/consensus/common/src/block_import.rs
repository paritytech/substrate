...
impl<Block: BlockT> BlockImportParams<Block> {
	...
	/// Clears the storage changes.
	pub fn clear_storage_changes(&mut self) {
		// Preserve imported state.
		if let StateAction::ApplyChanges(StorageChanges::Changes(_)) = self.state_action {
			self.state_action = StateAction::Skip;
		}
	}
