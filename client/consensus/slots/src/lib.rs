...
pub trait SimpleSlotWorker<B: BlockT> {
	...
	/// Session data necessary for authoring.
	...
	/// Returns the session data necessary for authoring. For time-dependent sessions,
	...
	/// Returns the number of authorities given the session data.
	...
	fn notify_slot(&self, _header: &B::Header, _slot: Slot, _session_data: &Self::SessionData) {}
		...
		Self: Sync,
	{
		...
		let session_data = match self.session_data(&slot_info.chain_head, slot) {
			Ok(session_data) => session_data,
			Err(err) => {
				warn!(
					target: logging_target,
					"Unable to fetch session data at block {:?}: {}",
		...
		self.notify_slot(&slot_info.chain_head, slot, &session_data);

		let authorities_len = self.authorities_len(&session_data);
		...
		let claim = self.claim_slot(&slot_info.chain_head, slot, &session_data).await?;
