    ...
    Error: std::error::Error + Send + From<sp_consensus::Error> + 'static,
{
    ...
    fn pre_digest_data(
        ...
        vec![<DigestItemFor<B> as CompatibleDigestItem<P>>::aura_pre_digest(slot)]
    }
...
#[forbid(deprecated)]
impl<B: BlockT, C, P, CAW> Verifier<B> for AuraVerifier<C, P, CAW>
where
    C: ProvideRuntimeApi<B>
        + Send
        + Sync
    ...
    P::Signature: Encode + Decode,
{
    ...
    fn import_block(
        ...
        let slot = find_pre_digest::<Block, P>(&block.header).expect(
        ...
        if slot <= parent_slot {
            return Err(ConsensusError::ClientImport(
                aura_err(Error::<Block>::SlotMustIncrease(parent_slot, slot)).into(),
            ));
        }
...
mod tests {
    ...
    use sp_consensus::{AlwaysCanAuthor, NoNetwork as DummyOracle, Proposal, RecordProof};
    ...
    fn authoring_blocks() {
        ...
        for (peer_id, key) in peers {
            ...
            aura_futures.push(
                start_aura::<_, _, _, _, _, AuthorityPair, _, _, _, _>(
                    slot_duration,
                    client.clone(),
                    select_chain,
                    client,
