...
pub struct ChainHeadFollow<BE, Block: BlockT, Client> {
	...
	/// Subscription sink to communicate with the client.
		...
		+ 'static,
{
	...
	fn generate_finalized_events(
		...
		for (hash, parent) in finalized_block_hashes.iter().zip(parents) {
			...
			events.extend(self.generate_import_events(*hash, *parent, hash == last_finalized))
		}
	...
	pub async fn generate_events(&mut self, rx_stop: oneshot::Receiver<()>) {
		...
		let (initial_events, pruned_forks) = match self.generate_init_events(&info) {
			...
			Err(err) => {
				...
				let _ = self.sink.send(&FollowEvent::<Block::Hash>::Stop);
				self.subscriptions.remove_subscription(&self.sub_id);
				return
			},
		};
		...
		self.submit_events(&info, stream.boxed(), pruned_forks, rx_stop).await;
	}
}
