...
pub struct OnDemand<B: BlockT> {
	...
	requests_queue: Mutex<Option<TracingUnboundedReceiver<light_client_requests::client::Request<B>>>>,

	/// Sending side of `requests_queue`.
	requests_send: TracingUnboundedSender<light_client_requests::client::Request<B>>,
}
	...
	B::Header: HeaderT,
{
	...
	pub(crate) fn extract_receiver(&self)
		-> Option<TracingUnboundedReceiver<light_client_requests::client::Request<B>>>
	{
	...
	B::Header: HeaderT,
{
	...
	fn remote_header(&self, request: RemoteHeaderRequest<B::Header>) -> Self::RemoteHeaderResult {
		...
		let _ = self
			.requests_send
			.unbounded_send(light_client_requests::client::Request::Header { request, sender });
	...
	fn remote_read(&self, request: RemoteReadRequest<B::Header>) -> Self::RemoteReadResult {
		...
		let _ = self
			.requests_send
			.unbounded_send(light_client_requests::client::Request::Read { request, sender });
		RemoteResponse { receiver }
	}
	...
	fn remote_read_child(
		...
		let _ = self
			.requests_send
			.unbounded_send(light_client_requests::client::Request::ReadChild { request, sender });
		RemoteResponse { receiver }
	}
	...
	fn remote_call(&self, request: RemoteCallRequest<B::Header>) -> Self::RemoteCallResult {
		...
		let _ = self
			.requests_send
			.unbounded_send(light_client_requests::client::Request::Call { request, sender });
		RemoteResponse { receiver }
	}
	...
	fn remote_changes(
		...
		let _ = self
			.requests_send
			.unbounded_send(light_client_requests::client::Request::Changes { request, sender });
		RemoteResponse { receiver }
	}
	...
	fn remote_body(&self, request: RemoteBodyRequest<B::Header>) -> Self::RemoteBodyResult {
		...
		let _ = self
			.requests_send
			.unbounded_send(light_client_requests::client::Request::Body { request, sender });
		RemoteResponse { receiver }
	}
}
