...
use crate::{
    ...
    protocol::{
        self, event::Event, light_client_handler, sync::SyncState, LegacyConnectionKillError,
        PeerInfo, Protocol,
    },
...
impl<B: BlockT + 'static, H: ExHashT> NetworkWorker<B, H> {
    ...
    pub fn on_block_imported(&mut self, header: B::Header, is_best: bool) {
        self.network_service
            .user_protocol_mut()
            .on_block_imported(&header, is_best);
    }
...
impl<B: BlockT + 'static, H: ExHashT> NetworkService<B, H> {
    ...
    pub fn request_justification(&self, hash: &B::Hash, number: NumberFor<B>) {
        let _ = self
            .to_worker
            .unbounded_send(ServiceToWorkerMsg::RequestJustification(
                hash.clone(),
                number,
            ));
    }
