...
use crate::{
    ...
    protocol::{
        self, event::Event, sync::SyncState, NotificationsSink, NotifsHandlerError, PeerInfo,
        Protocol, Ready,
    },
...
impl<B: BlockT + 'static, H: ExHashT> NetworkWorker<B, H> {
    ...
    pub fn new(params: Params<B, H>) -> Result<NetworkWorker<B, H>, Error> {
        ...
        let (mut swarm, bandwidth): (Swarm<B, H>, _) = {
                ...
                let yamux_maximum_buffer_size = {
                    let requests_max =
                        params
                            ...
                            .iter()
                            .map(|cfg| {
                                usize::try_from(cfg.max_request_size).unwrap_or(usize::max_value())
                            });
                    let responses_max = params
                        .network_config
                        .request_response_protocols
                        .iter()
                        .map(|cfg| {
                            usize::try_from(cfg.max_response_size).unwrap_or(usize::max_value())
                        });
                    let notifs_max = params.network_config.extra_sets.iter().map(|cfg| {
                        usize::try_from(cfg.max_notification_size).unwrap_or(usize::max_value())
                    });
            ...
            let behaviour = {
                let bitswap = if params.network_config.ipfs_server {
                    Some(Bitswap::new(client))
                } else {
                    None
                };
...
impl<B: BlockT + 'static, H: ExHashT> NetworkService<B, H> {
    ...
    pub fn trigger_repropagate(&self) {
        let _ = self
            .to_worker
            .unbounded_send(ServiceToWorkerMsg::PropagateTransactions);
    }
    ...
    pub fn propagate_transaction(&self, hash: H) {
        let _ = self
            .to_worker
            .unbounded_send(ServiceToWorkerMsg::PropagateTransaction(hash));
    }
    ...
    pub fn add_peers_to_reserved_set(
        ...
        for (peer_id, addr) in peers.into_iter() {
            ...
            let _ = self
                .to_worker
                .unbounded_send(ServiceToWorkerMsg::AddSetReserved(
                    protocol.clone(),
                    peer_id,
                ));
        }
    ...
    pub fn remove_peers_from_reserved_set(
        ...
        for (peer_id, _) in peers.into_iter() {
            let _ = self
                .to_worker
                .unbounded_send(ServiceToWorkerMsg::RemoveSetReserved(
                    protocol.clone(),
                    peer_id,
                ));
        }
    ...
    pub fn remove_from_peers_set(
        ...
        for (peer_id, _) in peers.into_iter() {
            let _ = self
                .to_worker
                .unbounded_send(ServiceToWorkerMsg::RemoveFromPeersSet(
                    protocol.clone(),
                    peer_id,
                ));
        }
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut std::task::Context) -> Poll<Self::Output> {
        ...
        loop {
            ...
            match msg {
                ...
                ServiceToWorkerMsg::PropagateTransaction(hash) => this
                    .network_service
                    .user_protocol_mut()
                    .propagate_transaction(&hash),
                ServiceToWorkerMsg::PropagateTransactions => this
                    .network_service
                    .user_protocol_mut()
...
impl<'a, B: BlockT, H: ExHashT> Link<B> for NetworkLink<'a, B, H> {
    ...
    fn justification_imported(
        ...
        self.protocol
            .user_protocol_mut()
            .justification_import_result(who, hash.clone(), number, success);
    }
