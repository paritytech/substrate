            ...
            loop {
                ...
                break match (prev, next) {
                    (
                        ...
                        _,
                    ) if downloading < max_parallel => (*start..*start + *len, downloading),
...
mod test {
    ...
    fn insert_blocks() {
        ...
        assert_eq!(
            bc.needed_blocks(peer0.clone(), 40, 150, 0, 1, 200),
            Some(1..41)
        );
        assert_eq!(
            bc.needed_blocks(peer1.clone(), 40, 150, 0, 1, 200),
            Some(41..81)
        );
        assert_eq!(
            bc.needed_blocks(peer2.clone(), 40, 150, 0, 1, 200),
            Some(81..121)
        );
        ...
        assert_eq!(
            bc.needed_blocks(peer2.clone(), 40, 150, 80, 1, 200),
            Some(81..121)
        );
        bc.clear_peer_download(&peer2);
    ...
    fn large_gap() {
        ...
        let peer0 = PeerId::random();
        assert_eq!(
            bc.needed_blocks(peer0.clone(), 128, 10000, 000, 1, 200),
            Some(1..100)
        );
        assert_eq!(
            bc.needed_blocks(peer0.clone(), 128, 10000, 600, 1, 200),
            None
        ); // too far ahead
        assert_eq!(
