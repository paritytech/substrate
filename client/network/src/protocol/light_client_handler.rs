    ...
    B: Block,
{
    ...
    fn on_remote_call_request(
        ...
        let response = {
            let r = api::v1::light::RemoteCallResponse {
                proof: proof.encode(),
            };
            api::v1::light::response::Response::RemoteCallResponse(r)
        };

        Ok(api::v1::light::Response {
    ...
    fn on_remote_read_request(
        ...
        let response = {
            let r = api::v1::light::RemoteReadResponse {
                proof: proof.encode(),
            };
    ...
    fn on_remote_read_child_request(
        ...
        let proof = if let Some(info) =
            ChildInfo::resolve_child_info(request.child_type, &request.child_info[..])
        {
    ...
    fn on_remote_header_request(
        ...
        log::trace!(
            "remote header proof request from {} ({:?})",
            peer,
            request.block
        );
            ...
            Event::Response(id, response) => {
                if let Some(request) = self.outstanding.remove(&id) {
                    ...
                    if request.peer != peer {
                        log::debug!(
                            "Expected response from {} instead of {}.",
                            request.peer,
                            peer
                        );
        ...
        while let Some(mut request) = self.pending_requests.pop_front() {
            ...
            match self.prepare_request(request) {
                ...
                Ok((peer, request)) => {
                    ...
                    self.peers
                        .get_mut(&peer)
                        .map(|p| p.status = PeerStatus::BusyWith(request_id));
    ...
    let request = match request {
        Request::Body { request, .. } => {
            let rq = api::v1::BlockRequest {
                ...
                from_block: Some(api::v1::block_request::FromBlock::Hash(
        ...
        Request::Header { request, .. } => {
            let r = api::v1::light::RemoteHeaderRequest {
    ...
    T: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    fn upgrade_outbound(self, mut s: T, _: Self::Info) -> Self::Future {
        let future = async move {
            ...
            match self.expected {
                ExpectedResponseTy::Light => api::v1::light::Response::decode(&vec[..])
                    .map(|r| Event::Response(self.request_id, Response::Light(r)))
                    .map_err(|e| ReadOneError::Io(io::Error::new(io::ErrorKind::Other, e))),
                ExpectedResponseTy::Block => api::v1::BlockResponse::decode(&vec[..])
