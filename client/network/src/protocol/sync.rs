...
impl<B: BlockT> ChainSync<B> {
    ...
    fn has_slot_for_block_announce_validation(
        ...
        match self
            .block_announce_validation_per_peer_stats
            .entry(peer.clone())
        {
            Entry::Vacant(entry) => {
                entry.insert(1);
    ...
    fn finish_block_announce_validation(
        ...
        let peer = if let Some(peer) = self.peers.get_mut(&who) {
            ...
            return PollBlockAnnounceValidation::Nothing {
                is_best,
                who,
                announce,
            };
        ...
        if !requires_additional_data {
            ...
            return PollBlockAnnounceValidation::ImportHeader {
                is_best,
                announce,
                who,
            };
        ...
        PollBlockAnnounceValidation::Nothing {
            is_best,
            who,
            announce,
        }
...
#[cfg(test)]
mod test {
    use super::message::{BlockData, BlockState, FromBlock};
    use super::*;
    ...
    fn do_not_report_peer_on_block_response_for_block_request() {
        ...
        client
            .import(BlockOrigin::Own, block3_fork.clone())
            .unwrap();
        ...
        let res = sync
            .on_block_data(&peer_id2, Some(request), response)
            .unwrap();
        ...
        let res = sync
            .on_block_data(&peer_id1, Some(request2), response)
            .unwrap();
        assert!(matches!(
            res,
        ...
        let response = create_block_response(vec![block2.clone()]);
        let res = sync
            .on_block_data(&peer_id2, Some(request3), response)
            .unwrap();
        // Nothing to import
        assert!(matches!(res, OnBlockData::Import(_, blocks) if blocks.is_empty()));
    }
    ...
    fn do_ancestor_search_when_common_block_to_best_qeued_gap_is_to_big() {
        ...
        while best_block_num < MAX_DOWNLOAD_AHEAD {
            ...
            let res = sync
                .on_block_data(&peer_id1, Some(request), response)
                .unwrap();
            assert!(matches!(
                res,
        ...
        let res = sync
            .on_block_data(&peer_id2, Some(peer2_req), response)
            .unwrap();
        assert!(matches!(
            res,
    ...
    fn can_sync_huge_fork() {
        ...
        client
            .finalize_block(BlockId::Hash(finalized_block.hash()), Some(Vec::new()))
            .unwrap();
