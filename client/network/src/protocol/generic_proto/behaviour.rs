...
impl GenericProto {
    ...
    fn disconnect_peer_inner(
        ...
        match mem::replace(entry.get_mut(), PeerState::Poisoned) {
            ...
            PeerState::DisabledPendingEnable {
                ...
                self.peerset
                    .dropped(set_id, peer_id.clone(), sc_peerset::DropReason::Unknown);
    ...
    fn peerset_report_accept(&mut self, index: sc_peerset::IncomingIndex) {
        ...
        if !incoming.alive {
            ...
            match self
                .peers
                .get_mut(&(incoming.peer_id.clone(), incoming.set_id))
            {
    ...
    fn inject_event(&mut self, source: PeerId, connection: ConnectionId, event: NotifsHandlerOut) {
        match event {
            NotifsHandlerOut::OpenDesiredByRemote { protocol_index } => {
                ...
                let mut entry = if let Entry::Occupied(entry) =
                    self.peers.entry((source.clone(), set_id))
                {
                    entry
                } else {
            ...
            NotifsHandlerOut::CustomMessage { message } => {
                if self.is_open(&source, sc_peerset::SetId::from(0)) {
                    // TODO: using set 0 here is hacky
