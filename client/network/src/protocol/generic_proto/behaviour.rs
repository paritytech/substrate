...
use crate::protocol::generic_proto::handler::{LegacyProtoHandlerProto, LegacyProtoHandlerOut, LegacyProtoHandlerIn};
...
pub struct GenericProto< TSubstream> {
	...
	events: SmallVec<[NetworkBehaviourAction<LegacyProtoHandlerIn, GenericProtoOut>; 4]>,
...
impl<TSubstream> GenericProto<TSubstream> {
		...
		match mem::replace(entry.get_mut(), PeerState::Poisoned) {
			...
			PeerState::Enabled { open, connected_point } => {
				...
				self.events.push(NetworkBehaviourAction::SendEvent {
					...
					event: LegacyProtoHandlerIn::Disable,
				});
	...
	pub fn send_packet(&mut self, target: &PeerId, message: Vec<u8>) {
		...
		self.events.push(NetworkBehaviourAction::SendEvent {
			...
			event: LegacyProtoHandlerIn::SendCustomMessage {
				message,
			}
		});
	}
		...
		match mem::replace(occ_entry.get_mut(), PeerState::Poisoned) {
			...
			PeerState::Disabled { open, connected_point, banned_until: _ } => {
				...
				self.events.push(NetworkBehaviourAction::SendEvent {
					...
					event: LegacyProtoHandlerIn::Enable,
				});
...
{
	type ProtocolsHandler = LegacyProtoHandlerProto<TSubstream>;
	type OutEvent = GenericProtoOut;

	fn new_handler(&mut self) -> Self::ProtocolsHandler {
		LegacyProtoHandlerProto::new(self.protocol.clone())
	}
	...
	fn inject_node_event(
		...
		match event {
			...
			LegacyProtoHandlerOut::ProtocolError { is_severe, ref error } if !is_severe => {
				debug!(target: "sub-libp2p", "Handler({:?}) => Benign protocol error: {:?}",
					source, error)
			}

			LegacyProtoHandlerOut::ProtocolError { error, .. } => {
