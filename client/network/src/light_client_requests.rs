...
mod tests {
    ...
    use libp2p::PeerId;
    use sc_client_api::light::{self, ChangesProof, RemoteBodyRequest, RemoteReadRequest};
    ...
    fn send_receive(request: sender::Request<Block>, pool: &LocalPool) {
        ...
        pool.spawner()
            .spawn_obj(handler.run().boxed().into())
            .unwrap();
        ...
        let sender::OutEvent::SendRequest {
            pending_response,
            request,
            ..
        ...
        pool.spawner()
            .spawn_obj(
                async move {
                    pending_response
                        .send(Ok(rx.await.unwrap().result.unwrap()))
                        .unwrap();
                }
                .boxed()
                .into(),
