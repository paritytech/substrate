...
pub enum Event {
	...
	InboundRequest {
		...
		/// started sending the response. If `Err`, the error that happened.
...
pub struct RequestResponsesBehaviour {
	...
	pending_responses: stream::FuturesUnordered<
		Pin<Box<dyn Future<Output = (Instant, RequestProcessingOutcome)> + Send>>
	>,
}
...
impl NetworkBehaviour for RequestResponsesBehaviour {
	...
	> {
		'poll_all: loop {
			...
			while let Poll::Ready(Some((start, result))) = self.pending_responses.poll_next_unpin(cx) {
				match result {
					RequestProcessingOutcome::Response {
						peer, protocol: protocol_name, inner_channel, response
					} => {
						...
						RequestResponseEvent::Message {
							...
							self.pending_responses.push(Box::pin(async move {
								...
								if let Ok(response) = rx.await {
									let outcome = RequestProcessingOutcome::Response {
										peer, protocol, inner_channel: channel, response
									};
									(start, outcome)

								} else {
									let outcome = RequestProcessingOutcome::Busy { peer, protocol };
									(start, outcome)
								}
							}));
