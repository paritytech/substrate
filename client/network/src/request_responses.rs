...
pub struct RequestResponsesBehaviour {
	...
	pending_responses_arrival_time: HashMap<(Cow<'static, str>, RequestId), Instant>,
}
		...
		if let Some((protocol, _)) = self.protocols.get_mut(protocol_name) {
			if protocol.is_connected(target) {
				...
				self.pending_requests.insert(
					(protocol_name.to_string().into(), request_id),
					...
					match ev {
						...
						RequestResponseEvent::Message {
							...
							self.pending_responses_arrival_time.insert(
								(protocol.clone(), request_id.clone()),
								Instant::now(),
							);
						...
						RequestResponseEvent::OutboundFailure {
							...
							let started = match self.pending_requests.remove(&(protocol.clone(), request_id)) {
								Some((started, pending_response)) => {
