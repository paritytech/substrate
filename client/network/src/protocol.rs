...
use message::generic::{ConsensusMessage, Message as GenericMessage, Roles};
use message::{BlockAnnounce, Message};
use prometheus_endpoint::{
    register, Gauge, GaugeVec, HistogramVec, Opts, PrometheusError, Registry, U64,
};
...
impl<B: BlockT, H: ExHashT> Protocol<B, H> {
    ...
    pub fn update_chain(&mut self) {
        ...
        self.sync
            .update_chain_info(&info.best_hash, info.best_number);
    }
        ...
        let request = if let Some(ref mut p) = self.context_data.peers.get_mut(&peer) {
            ...
            match p.block_request.take() {
                ...
                None => {
                    ...
                    self.behaviour.disconnect_peer(&peer);
                    self.peerset_handle
                        .report_peer(peer, rep::UNEXPECTED_RESPONSE);
                    return CustomMessageOutcome::None;
                }
            }
        } else {
                ...
                .new_peer(who.clone(), info.best_hash, info.best_number)
            {
                ...
                Ok(Some(mut req)) => {
                    if self.use_new_block_requests_protocol {
                        ...
                        self.pending_messages
                            .push_back(CustomMessageOutcome::BlockRequest {
                                target: who.clone(),
                                request: req,
                            });
                    } else {
    ...
    pub fn write_notification(
        ...
        if let Some(protocol_name) = self.protocol_name_by_engine.get(&engine_id) {
            ...
            .encode();
            self.behaviour
                .write_notification(&target, protocol_name.clone(), message, fallback);
        } else {
            error!(
                target: "sub-libp2p",
    ...
    fn on_extrinsics(&mut self, who: PeerId, extrinsics: message::Transactions<B::Extrinsic>) {
        ...
        if let Some(ref mut peer) = self.context_data.peers.get_mut(&who) {
            for t in extrinsics {
                ...
                self.peerset_handle
                    .report_peer(who.clone(), rep::ANY_EXTRINSIC);
    ...
    fn on_handle_extrinsic_import(&mut self, who: PeerId, import: TransactionImport) {
        match import {
            TransactionImport::KnownGood => self
                .peerset_handle
                .report_peer(who, rep::ANY_EXTRINSIC_REFUND),
    ...
    pub fn propagate_extrinsic(&mut self, hash: &H) {
    ...
    fn on_block_announce(
        ...
        match self
            .sync
            .on_block_announce(&who, hash, &announce, is_their_best)
        {
    ...
    fn on_remote_read_child_request(
        ...
        let proof = match child_info.and_then(|child_info| {
            self.context_data.chain.read_child_proof(
                &BlockId::Hash(request.block),
                &child_info,
                &mut request.keys.iter().map(AsRef::as_ref),
            )
    ...
    fn on_remote_header_request(
        ...
        let (header, proof) = match self
            .context_data
            .chain
            .header_proof(&BlockId::Number(request.block))
        {
