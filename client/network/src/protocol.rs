...
pub use generic_proto::{NotificationsSink, NotifsHandlerError, Ready};
...
impl<B: BlockT, H: ExHashT> Protocol<B, H> {
    ...
    pub fn new_best_block_imported(&mut self, hash: B::Hash, number: NumberFor<B>) {
        ...
        self.behaviour
            .set_legacy_handshake_message(build_status_message::<B>(
                &self.config,
                number,
                hash,
                self.genesis_hash,
    ...
    fn on_custom_message(&mut self, who: PeerId, data: BytesMut) -> CustomMessageOutcome<B> {
        ...
        match message {
            ...
            | GenericMessage::ConsensusBatch(_) => {
                ...
                self.behaviour
                    .disconnect_peer(&who, HARDCODED_PEERSETS_SYNC);
    ...
    fn on_sync_peer_connected(
        ...
        let req = if peer.info.roles.is_full() {
            match self
                .sync
                .new_peer(who.clone(), peer.info.best_hash, peer.info.best_number)
            {
                Ok(req) => req,
                Err(sync::BadPeer(id, repu)) => {
                    self.behaviour.disconnect_peer(&id, HARDCODED_PEERSETS_SYNC);
        ...
        self.pending_messages
            .push_back(CustomMessageOutcome::PeerNewBest(
                who.clone(),
                status.best_number,
            ));
    ...
    fn on_transactions(&mut self, who: PeerId, transactions: message::Transactions<B::Extrinsic>) {
        ...
        if !self.config.roles.is_full() {
            ...
            self.peerset_handle
                .report_peer(who, rep::UNEXPECTED_TRANSACTIONS);
            return;
        }
        ...
        if let Some(ref mut peer) = self.peers.get_mut(&who) {
            for t in transactions {
                ...
                self.peerset_handle
                    .report_peer(who.clone(), rep::ANY_TRANSACTION);
    ...
    fn on_handle_transaction_import(&mut self, who: PeerId, import: TransactionImport) {
        match import {
            TransactionImport::KnownGood => self
                .peerset_handle
                .report_peer(who, rep::ANY_TRANSACTION_REFUND),
    ...
    fn do_propagate_transactions(
        ...
        for (who, peer) in self.peers.iter_mut() {
            ...
            if !to_send.is_empty() {
                ...
                self.behaviour
                    .write_notification(who, HARDCODED_PEERSETS_TX, to_send.encode());
            }
        }
        ...
        if let Some(ref metrics) = self.metrics {
            metrics
                ...
                .inc_by(propagated_transactions as _)
        }
    ...
    fn process_block_announce_validation_result(
        ...
        let (header, is_best, who) = match validation_result {
            ...
            sync::PollBlockAnnounceValidation::Nothing {
                is_best,
                who,
                announce,
            } => {
            ...
            sync::PollBlockAnnounceValidation::ImportHeader {
                announce,
                is_best,
                who,
            } => {
    ...
    pub fn set_reserved_only(&self, reserved_only: bool) {
        self.peerset_handle
            .set_reserved_only(HARDCODED_PEERSETS_SYNC, reserved_only);
        self.peerset_handle
            .set_reserved_only(HARDCODED_PEERSETS_TX, reserved_only);
    }

    /// Removes a `PeerId` from the list of reserved peers for syncing purposes.
    pub fn remove_reserved_peer(&self, peer: PeerId) {
        self.peerset_handle
            .remove_reserved_peer(HARDCODED_PEERSETS_SYNC, peer.clone());
        self.peerset_handle
            .remove_reserved_peer(HARDCODED_PEERSETS_TX, peer);
    }

    /// Adds a `PeerId` to the list of reserved peers for syncing purposes.
    pub fn add_reserved_peer(&self, peer: PeerId) {
        self.peerset_handle
            .add_reserved_peer(HARDCODED_PEERSETS_SYNC, peer.clone());
        self.peerset_handle
            .add_reserved_peer(HARDCODED_PEERSETS_TX, peer);
    }

    /// Sets the list of reserved peers for syncing purposes.
    pub fn set_reserved_peers(&self, peers: HashSet<PeerId>) {
        self.peerset_handle
            .set_reserved_peers(HARDCODED_PEERSETS_SYNC, peers.clone());
        self.peerset_handle
            .set_reserved_peers(HARDCODED_PEERSETS_TX, peers);
    }
...
fn prepare_block_request<B: BlockT, H: ExHashT>(
    ...
    let request = crate::schema::v1::BlockRequest {
        ...
        from_block: match request.from {
            message::FromBlock::Hash(h) => Some(crate::schema::v1::block_request::FromBlock::Hash(
                h.encode(),
            )),
            message::FromBlock::Number(n) => Some(
                crate::schema::v1::block_request::FromBlock::Number(n.encode()),
            ),
        },
...
impl<B: BlockT, H: ExHashT> NetworkBehaviour for Protocol<B, H> {
    ...
    >{
        ...
        for (id, peer) in self.peers.iter_mut() {
            if let Peer {
                block_request: Some((_, pending_response)),
                ..
            } = peer
            {
                match pending_response.poll_unpin(cx) {
                    Poll::Ready(Ok(Ok(resp))) => {
                        ...
                        let protobuf_response = match crate::schema::v1::BlockResponse::decode(
                            &resp[..],
                        ) {
                            Ok(proto) => proto,
                            Err(e) => {
                    ...
                    Poll::Ready(Ok(Err(e))) => {
                        ...
                        match e {
                            ...
                            RequestFailure::Network(OutboundFailure::UnsupportedProtocols) => {
                                self.peerset_handle
                                    .report_peer(id.clone(), rep::BAD_PROTOCOL);
                                self.behaviour.disconnect_peer(id, HARDCODED_PEERSETS_SYNC);
                            }
            ...
            GenericProtoOut::CustomProtocolOpen {
                ...
                if set_id == HARDCODED_PEERSETS_SYNC {
                    ...
                    match <Message<B> as DecodeAll>::decode_all(&mut &received_handshake[..]) {
                        Ok(GenericMessage::Status(handshake)) => {
                                ...
                                .is_ok()
                            {
                                ...
                                self.peerset_handle
                                    .add_reserved_peer(HARDCODED_PEERSETS_TX, peer_id.clone());
                    ...
                    match message::Roles::decode_all(&received_handshake[..]) {
                        Ok(roles) => CustomMessageOutcome::NotificationStreamOpened {
                            remote: peer_id,
            ...
            GenericProtoOut::CustomProtocolClosed { peer_id, set_id } => {
                ...
                if set_id == HARDCODED_PEERSETS_SYNC {
                    if self.on_sync_peer_disconnected(peer_id.clone()).is_ok() {
                        // Set 1 is kept in sync with the connected peers of set 0.
                        self.peerset_handle
                            .remove_reserved_peer(HARDCODED_PEERSETS_TX, peer_id.clone());
