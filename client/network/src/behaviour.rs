...
use crate::{
    ...
    protocol::{
        message::{self, Roles},
        CustomMessageOutcome, Protocol,
    },
    DhtEvent, Event, ExHashT, ObservedRole,
};
    ...
    for Behaviour<B, H>
{
    fn inject_event(&mut self, event: CustomMessageOutcome<B>) {
        match event {
            CustomMessageOutcome::BlockImport(origin, blocks) => self
                .events
                .push_back(BehaviourOut::BlockImport(origin, blocks)),
            CustomMessageOutcome::JustificationImport(origin, hash, nb, justification) => {
                self.events.push_back(BehaviourOut::JustificationImport(
                    origin,
                    hash,
                    nb,
                    justification,
            ...
            CustomMessageOutcome::FinalityProofImport(origin, hash, nb, proof) => self
                .events
                .push_back(BehaviourOut::FinalityProofImport(origin, hash, nb, proof)),
            CustomMessageOutcome::BlockRequest { target, request } => {
                match self.block_requests.send_request(&target, request) {
                    ...
                    block_requests::SendRequestOutcome::Replaced {
                        request_duration, ..
                    } => {
            ...
            CustomMessageOutcome::FinalityProofRequest {
                ...
                self.finality_proof_requests
                    .send_request(&target, block_hash, request);
            }
            ...
            CustomMessageOutcome::PeerNewBest(peer_id, number) => {
                self.light_client_handler
                    .update_best_block(&peer_id, number);
            }
    ...
    for Behaviour<B, H>
{
    fn inject_event(&mut self, event: finality_requests::Event<B>) {
        match event {
            finality_requests::Event::Response {
                peer,
                block_hash,
                proof,
            } => {
