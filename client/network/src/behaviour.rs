...
#[behaviour(out_event = "BehaviourOut<B, O::OutEvent>", poll_method = "poll")]
pub struct Behaviour<B: BlockT, S: NetworkSpecialization<B>, H: ExHashT, O: NetworkBehaviour> {
	...
	/// Other behaviour.
	other_behaviour: event_wrap::WrappedBehaviour<O>,

	/// Queue of events to produce for the outside.
	#[behaviour(ignore)]
	events: Vec<BehaviourOut<B, O::OutEvent>>,
}

/// Event generated by `Behaviour`.
pub enum BehaviourOut<B: BlockT, O> {
...
impl<B: BlockT, S: NetworkSpecialization<B>, H: ExHashT, O: NetworkBehaviour>
Behaviour<B, S, H, O> {
	...
	pub async fn new(
		...
		Behaviour {
			...
			other_behaviour: event_wrap::WrappedBehaviour::new(other_behaviour),
			events: Vec::new(),
		}
	}
...
impl<B: BlockT, S: NetworkSpecialization<B>, H: ExHashT, O: NetworkBehaviour>
NetworkBehaviourEventProcess<void::Void> for Behaviour<B, S, H, O> {
...
impl<B: BlockT, S: NetworkSpecialization<B>, H: ExHashT, O: NetworkBehaviour>
NetworkBehaviourEventProcess<CustomMessageOutcome<B>> for Behaviour<B, S, H, O> {
...
impl<B: BlockT, S: NetworkSpecialization<B>, H: ExHashT, O: NetworkBehaviour>
NetworkBehaviourEventProcess<debug_info::DebugInfoEvent> for Behaviour<B, S, H, O> {
...
impl<B: BlockT, S: NetworkSpecialization<B>, H: ExHashT, O: NetworkBehaviour>
NetworkBehaviourEventProcess<DiscoveryOut> for Behaviour<B, S, H, O> {
...
impl<B: BlockT, S: NetworkSpecialization<B>, H: ExHashT, O: NetworkBehaviour> NetworkBehaviourEventProcess<WrappedEvent<O::OutEvent>>
	for Behaviour<B, S, H, O> {
	fn inject_event(&mut self, out: WrappedEvent<O::OutEvent>) {
		unimplemented!()
	}
}

impl<B: BlockT, S: NetworkSpecialization<B>, H: ExHashT, O: NetworkBehaviour> Behaviour<B, S, H, O> {
	fn poll<TEv>(&mut self, _: &mut Context, _: &mut impl PollParameters) -> Poll<NetworkBehaviourAction<TEv, BehaviourOut<B, O::OutEvent>>> {
