		...
		+ 'static,
{
	...
	/// Protocol name used to send out BEEFY justifications requests via
	/// [`request_responses::RequestResponsesBehaviour`].
	#[behaviour(ignore)]
	beefy_justifications_protocol_name: Option<String>,
}
		...
		+ 'static,
{
	...
	pub fn new(
		...
		beefy_justifications_protocol_config: Option<ProtocolConfig>,
		// All remaining request protocol configs.
		// TODO: could insert BEEFY-sync-proto here in layers above and keep this BEEFY-agnostic.
		...
		let beefy_justifications_protocol_name = match beefy_justifications_protocol_config {
			Some(config) => {
				let name = config.name.to_string();
				request_response_protocols.push(config);
				Some(name)
		...
		Ok(Self {
			...
			beefy_justifications_protocol_name,
		})
	}
		...
		+ 'static,
{
	fn inject_event(&mut self, event: CustomMessageOutcome<B>) {
		match event {
			...
			CustomMessageOutcome::BeefyJustificationRequest {
				target,
				request,
				pending_response,
			} => match &self.beefy_justifications_protocol_name {
				Some(name) => self.request_responses.send_request(
					&target,
					name,
					request.encode(),
					pending_response,
					// TODO: should this be `IfDisconnected::TryConnect`?
					IfDisconnected::ImmediateError,
				),
				None => {
					log::warn!(
							target: BEEFY_LOG_TARGET,
							"Trying to send BEEFY justification request when no protocol is configured {:?}",
							request,
						);
				},
			},
