...
pub struct Behaviour<B: BlockT, H: ExHashT, M: MultihashDigest> {
	...
	pub(crate) bitswap: libp2p_bitswap::Bitswap<M>,
...
impl<B: BlockT, H: ExHashT, M: MultihashDigest> Behaviour<B, H, M> {
	...
	/// Starts providing a key from the DHT. Will later produce a `Providing` or `StartProvidingFailed` event.
	pub fn provide(&mut self, key: &record::Key) {
		self.discovery.provide(key);
	}

	/// Stops providing a key from the DHT.
	pub fn unprovide(&mut self, key: &record::Key) {
		self.discovery.unprovide(key);
	}
...
impl<B: BlockT, H: ExHashT, M: MultihashDigest> NetworkBehaviourEventProcess<void::Void> for
Behaviour<B, H, M> {
...
impl<B: BlockT, H: ExHashT, M: MultihashDigest> NetworkBehaviourEventProcess<CustomMessageOutcome<B>> for
Behaviour<B, H, M> {
...
impl<B: BlockT, H: ExHashT, M: MultihashDigest> NetworkBehaviourEventProcess<peer_info::PeerInfoEvent>
	for Behaviour<B, H, M> {
...
impl<B: BlockT, H: ExHashT, M: MultihashDigest> NetworkBehaviourEventProcess<DiscoveryOut>
	for Behaviour<B, H, M> {
	fn inject_event(&mut self, out: DiscoveryOut) {
		match out {
			...
			DiscoveryOut::Providers(key, providers, duration) => {
				self.events.push_back(BehaviourOut::Dht(DhtEvent::Providers(key, providers), duration));
			}
			DiscoveryOut::GetProvidersFailed(key, duration) => {
				self.events.push_back(BehaviourOut::Dht(DhtEvent::GetProvidersFailed(key), duration));
			}
			DiscoveryOut::Providing(key, duration) => {
				self.events.push_back(BehaviourOut::Dht(DhtEvent::Providing(key), duration));
			}
			DiscoveryOut::StartProvidingFailed(key, duration) => {
				self.events.push_back(BehaviourOut::Dht(DhtEvent::StartProvidingFailed(key), duration));
			}
			...
			DiscoveryOut::BootstrapComplete(duration) => {
				self.events.push_back(BehaviourOut::Dht(DhtEvent::BootstrapComplete, duration));
			}
		}
	}
}

impl<B: BlockT, H: ExHashT, M: MultihashDigest> NetworkBehaviourEventProcess<libp2p_bitswap::BitswapEvent>
	for Behaviour<B, H, M> {
