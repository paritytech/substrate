...
use crate::request_responses::{OutboundFailure, RequestFailure};
use crate::{config::ProtocolId, protocol::message::BlockAttributes, schema, PeerId};
    ...
    fn poll_next(mut self: Pin<&mut Self>, cx: &mut Context) -> Poll<Option<Self::Item>> {
            ...
            self.sent_requests.poll_next_unpin(cx)
        {
            if let Some(info) = self.peers.get_mut(&sent_request.peer) {
                if info.status != PeerStatus::Busy {
                    ...
                    // and must be fixed!
                    panic!(
                        "unexpected peer status {:?} for {}",
                        info.status, sent_request.peer
                    );
                }
            ...
            let request_result = match request_result {
                ...
                Err(oneshot::Canceled) => {
                    log::debug!(
                        "Oneshot for request to peer {} was canceled.",
                        sent_request.peer
                    );
        ...
        while let Some(mut pending_request) = self.pending_requests.pop_front() {
            if pending_request.attempts_left == 0 {
                pending_request
                    .request
                    .return_reply(Err(ClientError::RemoteFetchFailed));
                continue;
            }
    ...
    fn serialize_request(&self) -> Result<Vec<u8>, prost::EncodeError> {
        let request = match self {
            ...
            Request::Header { request, .. } => {
                let r = schema::v1::light::RemoteHeaderRequest {
        ...
        let rq = schema::v1::light::Request {
    ...
    fn return_reply(self, result: Result<Reply<B>, ClientError>) {
        ...
        match self {
            ...
            Request::Header { request, sender } => match result {
                ...
                reply => log::error!(
                    "invalid reply for header request: {:?}, {:?}",
                    reply,
                    request
                ),
            },
            ...
            Request::ReadChild { request, sender } => match result {
                ...
                Ok(Reply::MapVecU8OptVecU8(x)) => send(Ok(x), sender),
                reply => log::error!(
                    "invalid reply for read child request: {:?}, {:?}",
                    reply,
                    request
                ),
            },
            ...
            Request::Changes { request, sender } => match result {
                ...
                reply => log::error!(
                    "invalid reply for changes request: {:?}, {:?}",
                    reply,
                    request
                ),
            },
...
mod tests {
    use super::*;
    use crate::light_client_requests::tests::{
        dummy_header, peerset, protocol_id, DummyFetchChecker,
    };
    ...
    fn disconnects_from_peer_if_request_times_out() {
        ...
        assert!(
            sender.pending_requests.is_empty(),
        ...
        assert_eq!(
            1,
            sender.pending_requests.len(),
            "Expect one pending request."
        );

        let OutEvent::SendRequest {
            target,
            pending_response,
            ..
        ...
        assert_eq!(
            1,
            sender.sent_requests.len(),
            "Expect one request to be sent."
        );
        ...
        assert_eq!(
            0,
            sender.pending_requests.len(),
            "Expect no request to be pending."
        );
        assert_eq!(
            1,
            sender.sent_requests.len(),
            "Expect new request to be issued."
        );
        ...
        assert_eq!(
            0,
            sender.sent_requests.len(),
            "Expect no other request to be in progress."
        );
    }
    ...
    fn disconnects_from_peer_on_incorrect_response() {
        ...
        assert_eq!(
            1,
            sender.pending_requests.len(),
            "Expect request to be pending again."
        );
        assert_eq!(
            0,
            sender.sent_requests.len(),
            "Expect no request to be sent."
        );
    }
