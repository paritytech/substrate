...
impl<B: BlockT> BlockRequestHandler<B> {
	...
	pub fn new(
		...
		(Self { client, request_receiver, seen_requests, verified_blocks }, protocol_config)
	}
	...
	fn get_block_response(
		...
		while let Some(header) = self.verified_blocks.header(&block_id).unwrap_or_default() {
