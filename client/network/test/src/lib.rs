...
use sc_network::config::{BoxFinalityProofRequestBuilder, NetworkConfiguration, TransportConfig};
use sc_network::{config::ProtocolId, NetworkService, NetworkWorker};
use sp_blockchain::{
    well_known_cache_keys::{self, Id as CacheKeyId},
    Info as BlockchainInfo, Result as ClientResult,
};
...
use sp_consensus::{
    BlockCheckParams, BlockImportParams, BlockOrigin, ForkChoiceStrategy, JustificationImport,
};
use sp_core::H256;
...
pub type PeersFullClient = sc_client::Client<
    substrate_test_runtime_client::Backend,
    substrate_test_runtime_client::Executor,
    Block,
    substrate_test_runtime_client::runtime::RuntimeApi,
>;
pub type PeersLightClient = sc_client::Client<
...
pub enum PeersClient {
    Full(
        Arc<PeersFullClient>,
        Arc<substrate_test_runtime_client::Backend>,
    ),
    Light(
        Arc<PeersLightClient>,
...
impl<D> Peer<D> {
    ...
    pub fn set_sync_fork_request(
        ...
        self.network
            .service()
            .set_sync_fork_request(peers, hash, number);
    }
        ...
        ) -> Block,
    {
        ...
        self.network
            .service()
            .announce_block(at.clone(), Vec::new());
        at
    }
    ...
    pub fn push_authorities_change_block(&mut self, new_authorities: Vec<AuthorityId>) -> H256 {
        self.generate_blocks(1, BlockOrigin::File, |mut builder| {
            builder
                .push(Extrinsic::AuthoritiesChange(new_authorities.clone()))
                .unwrap();
            builder.build().unwrap().block
        })
    }
...
impl<B: BlockT> Verifier<B> for VerifierAdapter<B> {
    fn verify(
        ...
        self.verifier
            .lock()
            .verify(origin, header, justification, body)
            .map_err(|e| {
...
pub trait TestNetFactory: Sized {
    ...
    fn make_block_import<Transaction>(
        &self,
        client: PeersClient,
    ) -> (
    ...
    fn add_full_peer_with_states(&mut self, keep_blocks: Option<u32>) {
        ...
        let network = NetworkWorker::new(sc_network::config::Params {
            ...
            network_config,
            chain: client.clone(),
            finality_proof_provider: self
                .make_finality_proof_provider(PeersClient::Full(client.clone(), backend.clone())),
            finality_proof_request_builder,
            on_demand: None,
    ...
    fn add_light_peer(&mut self) {
        ...
        let network = NetworkWorker::new(sc_network::config::Params {
            ...
            chain: client.clone(),
            finality_proof_provider: self
                .make_finality_proof_provider(PeersClient::Light(client.clone(), backend.clone())),
            finality_proof_request_builder,
            on_demand: None,
            transaction_pool: Arc::new(EmptyTransactionPool),
            protocol_id: ProtocolId::from(&b"test-protocol-name"[..]),
    ...
    fn poll(&mut self, cx: &mut FutureContext) {
        self.mut_peers(|peers| {
            for peer in peers {
                ...
                {
                    peer.network.on_block_imported(notification.header, true);
                    peer.network
                        .service()
