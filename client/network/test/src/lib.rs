...
use consensus::{
	BlockOrigin, ForkChoiceStrategy, BlockImportParams, BlockCheckParams, JustificationImport,
};
use futures::prelude::*;
...
impl<D, S: NetworkSpecialization<Block>> Peer<D, S> {
	...
	pub fn generate_blocks<F>(&mut self, count: usize, origin: BlockOrigin, edit_block: F) -> H256
		where F: FnMut(BlockBuilder<Block, PeersFullClient, test_client::Backend>) -> Block
	{
	...
	fn generate_blocks_at<F>(
		...
		origin: BlockOrigin,
		mut edit_block: F
	) -> H256 where F: FnMut(BlockBuilder<Block, PeersFullClient, test_client::Backend>) -> Block {
