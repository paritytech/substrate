...
async fn test_once() {
	...
	let mut peer_store_future = peer_store.run().boxed();
	let mut protocol_controller_future = protocol_controller.run().boxed();
	...
	for _ in 0..2500 {
		...
		// We make sure to poll `PeerStore` and `ProtocolController` every time we try to grab
		// a message from `from_controller`. This speeds the test up 10x compared to spawning
		// `PeerStore` and `ProtocolController` runners as separate tasks.
		let next_message = futures::future::poll_fn(|cx| {
			let _ = peer_store_future.poll_unpin(cx);
			let _ = protocol_controller_future.poll_unpin(cx);

			if let Poll::Ready(Some(msg)) = from_controller.poll_next_unpin(cx) {
				Poll::Ready(Some(msg))
			} else {
				Poll::Pending
			}
		...
		match WeightedIndex::new(&action_weights).unwrap().sample(&mut rng) {
			...
			0 => match futures::poll!(next_message) {
