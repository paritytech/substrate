...
use test_sc_client::{self, prelude::*};
use test_sc_client::runtime::{Block, Hash};
...
fn import_single_good_block_works() {
	...
	match import_single_block(&mut test_sc_client::new(), BlockOrigin::File, block, &mut PassThroughVerifier(true)) {
...
fn async_import_queue_drops() {
	...
	for _ in 0..100 {
		...
		let queue = BasicQueue::new(verifier, Box::new(test_sc_client::new()), None, None);
		drop(queue);
	}
}
