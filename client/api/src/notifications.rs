...
impl<Block: BlockT> StorageNotifications<Block> {
    ...
    pub fn trigger(
        ...
        for subscriber in subscribers {
            let should_remove = {
                let &(ref sink, ref filter, ref child_filters) =
                    self.sinks.get(&subscriber).expect(
                        "subscribers returned from self.listeners are always in self.sinks; qed",
                    );
                sink.unbounded_send((
                    hash.clone(),
                    StorageChangeSet {
    ...
    fn should_only_notify_interested_listeners() {
        ...
        assert_eq!(
            ...
            (
                Hash::from_low_u64_be(1),
                (
                    vec![(StorageKey(vec![2]), Some(StorageData(vec![3]))),],
                    vec![]
                )
                    .into()
            )
