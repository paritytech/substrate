...
use sp_runtime::traits::{
	...
	RuntimeArithmetic, CheckedConversion,
};
...
struct RootsStorage<'a, Number: RuntimeArithmetic, Hash: 'a> {
	roots: (Number, &'a [Hash]),
...
impl<'a, H, Number, Hash> ChangesTrieRootsStorage<H, Number> for RootsStorage<'a, Number, Hash>
	where
		H: Hasher,
		Number: ::std::fmt::Display + ::std::hash::Hash + Clone + RuntimeArithmetic + Encode + Decode + Send + Sync + 'static,
