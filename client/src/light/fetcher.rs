    ...
    S: BlockchainStorage<Block>,
{
    ...
    fn check_body_proof(
        ...
        if *request.header.extrinsics_root() == extrinsics_root {
            ...
            Err(format!(
                "RemoteBodyRequest: invalid extrinsics root expected: {} but got {}",
...
pub mod tests {
    ...
    use substrate_test_runtime_client::{
        ...
        tasks_executor, AccountKeyring, ClientBlockImportExt,
    };
    ...
    use sc_client_api::{ProofProvider, StorageProvider};
    use sp_core::storage::{well_known_keys, ChildInfo, StorageKey};
    ...
    fn check_body_proof_faulty() {
        ...
        assert!(
            local_checker
                .check_body_proof(&body_request, block.extrinsics)
                .is_err(),
            "vec![1, 2, 3, 4] != vec![]"
        );
    }
