...
use sp_consensus::{
    ...
    ForkChoiceStrategy, ImportResult, RecordProof, SelectChain,
};
use sp_core::{
    convert_hash,
    storage::{well_known_keys, ChildInfo, StorageData, StorageKey},
    traits::CodeExecutor,
...
pub use sc_client_api::{
    backend::{
        self, changes_tries_state_at_block, BlockImportOperation, ClientImportOperation, Finalizer,
        ImportSummary, LockImportRun, NewBlockState, PrunableStateChangesTrieStorage,
        StorageProvider,
    },
    client::{
    ...
    CallExecutor, CloneableSpawn, ExecutorProvider, ProofProvider,
};
            ...
            fn root(
                ...
                if block < self.min {
                    if let Some(ref root) = root {
                        self.required_roots_proofs
                            .lock()
                            .insert(block, root.clone());
                    }
                }
                Ok(root)
            }
            ...
            CoreApi<Block, Error = Error> + ApiExt<Block, StateBackend = B::State>,
    {
        ...
        match (
            ...
            (true, ref mut storage_changes @ None, Some(ref body)) => {
                ...
                runtime_api
                    .execute_block(&at, Block::new(import_block.header.clone(), body.clone()))?;
    ...
    Block: BlockT,
{
    ...
    fn read_child_proof(
        ...
        self.state_at(id).and_then(|state| {
            prove_child_read(state, storage_key, child_info, keys).map_err(Into::into)
        })
    }
    ...
    fn execution_proof(
        ...
        let code_proof = self.read_proof(
            id,
            &mut [well_known_keys::CODE, well_known_keys::HEAP_PAGES]
                .iter()
                .map(|v| *v),
        )?;
...
pub(crate) mod tests {
    ...
    use substrate_test_runtime_client::{
        ...
        prelude::*,
        runtime::{self, Block, RuntimeApi, TestAPI, Transfer},
    ...
    pub fn prepare_client_with_key_changes() -> (
        substrate_test_runtime_client::sc_client::Client<
        ...
        for (i, block_transfers) in blocks_transfers.into_iter().enumerate() {
            ...
            let header = remote_client
                .header(&BlockId::Number(i as u64 + 1))
                .unwrap()
                .unwrap();
        ...
        // prepare test cases
        let alice = blake2_256(&runtime::system::balance_of_key(
            AccountKeyring::Alice.into(),
        ))
        .to_vec();
        let bob = blake2_256(&runtime::system::balance_of_key(AccountKeyring::Bob.into())).to_vec();
        let charlie = blake2_256(&runtime::system::balance_of_key(
            AccountKeyring::Charlie.into(),
        ))
        .to_vec();
        let dave = blake2_256(&runtime::system::balance_of_key(
            AccountKeyring::Dave.into(),
        ))
        .to_vec();
        let eve = blake2_256(&runtime::system::balance_of_key(AccountKeyring::Eve.into())).to_vec();
        let ferdie = blake2_256(&runtime::system::balance_of_key(
    ...
    fn best_containing_on_longest_chain_with_single_chain_3_blocks() {
        ...
        let (mut client, longest_chain_select) =
    ...
    fn best_containing_on_longest_chain_with_multiple_forks() {
        ...
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a5.hash(), Some(4))
                .unwrap()
        );
        ...
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a4.hash(), Some(3))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a5.hash(), Some(3))
                .unwrap()
        );
        ...
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(b4.hash(), Some(3))
                .unwrap()
        );
        ...
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a3.hash(), Some(2))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a4.hash(), Some(2))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a5.hash(), Some(2))
                .unwrap()
        );
        ...
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(b3.hash(), Some(2))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(b4.hash(), Some(2))
                .unwrap()
        );
        ...
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(c3.hash(), Some(2))
                .unwrap()
        );
        ...
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a2.hash(), Some(1))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a3.hash(), Some(1))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a4.hash(), Some(1))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a5.hash(), Some(1))
                .unwrap()
        );
        ...
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(b2.hash(), Some(1))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(b3.hash(), Some(1))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(b4.hash(), Some(1))
                .unwrap()
        );
        ...
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(c3.hash(), Some(1))
                .unwrap()
        );
        ...
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(d2.hash(), Some(1))
                .unwrap()
        );
        ...
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a1.hash(), Some(0))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a2.hash(), Some(0))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a3.hash(), Some(0))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a4.hash(), Some(0))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(a5.hash(), Some(0))
                .unwrap()
        );
        ...
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(b2.hash(), Some(0))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(b3.hash(), Some(0))
                .unwrap()
        );
        assert_eq!(
            None,
            longest_chain_select
                .finality_target(b4.hash(), Some(0))
                .unwrap()
        );
    ...
    fn finalizing_diverged_block_should_trigger_reorg() {
        ...
        assert_eq!(select_chain.best_chain().unwrap().hash(), b2.hash(),);
    ...
    fn get_header_by_block_number_doesnt_panic() {
        ...
        client
            .header(&id)
            .expect_err("invalid block number overflows u32");
    }
    ...
    fn respects_block_rules() {
        fn run_test(
            ...
            assert_eq!(
                client.check_block(params).unwrap(),
            ...
            let mut block_not_ok = client
                ...
                .unwrap();
            block_not_ok
                .push_storage_change(vec![0], Some(vec![1]))
                .unwrap();
            ...
            let mut block_ok = client
                ...
                .unwrap();
            block_ok
                .push_storage_change(vec![0], Some(vec![2]))
                .unwrap();
            ...
            let mut block_not_ok = client
                ...
                .unwrap();
            block_not_ok
                .push_storage_change(vec![0], Some(vec![3]))
                .unwrap();
            let block_not_ok = block_not_ok.build().unwrap().block;
    ...
    fn returns_status_for_pruned_blocks() {
        ...
        assert_eq!(
            client
                .block_status(&BlockId::hash(check_block_a1.hash))
                .unwrap(),
            BlockStatus::Unknown
        );

        client
            .import_as_final(BlockOrigin::Own, a1.clone())
            .unwrap();
        ...
        let a2 = client
            ...
            .block;
        client
            .import_as_final(BlockOrigin::Own, a2.clone())
            .unwrap();
        ...
        assert_eq!(
            client
                .block_status(&BlockId::hash(check_block_a2.hash))
                .unwrap(),
            BlockStatus::InChainWithState
        );
        ...
        client
            .import_as_final(BlockOrigin::Own, a3.clone())
            .unwrap();
        ...
        assert_eq!(
            client
                .block_status(&BlockId::hash(check_block_a3.hash))
                .unwrap(),
            BlockStatus::InChainWithState
        );
    ...
    fn storage_keys_iter_works() {
        ...
        let res: Vec<_> = client
            .storage_keys_iter(
                ...
                Some(&prefix),
                Some(&StorageKey(
                    hex!("79c07e2b1d2e2abfd4855b936617eeff5e0621c4869aa60c02be9adcc98a0d1d")
                        .to_vec(),
                )),
            )
            .unwrap()
