...
use sc_network::{config::MultiaddrWithPeerId, DhtEvent, ExHashT, Multiaddr, NetworkStateInfo};
    ...
    Self: Future<Output = ()>,
{
    ...
    fn publish_ext_addresses(&mut self) -> Result<()> {
        ...
        let keys =
            AuthorityDiscovery::get_own_public_keys_within_authority_set(&key_store, &self.client)?
                .into_iter()
                .map(Into::into)
                .collect::<Vec<_>>();
        ...
        for (sign_result, key) in signatures.iter().zip(keys) {
            ...
            let signature = sign_result
                .as_ref()
                .map_err(|_| Error::MissingSignature(key.clone()))?;
    ...
    fn handle_dht_value_found_event(
        ...
        if !remote_addresses.is_empty() {
            self.addr_cache
                .insert(authority_id.clone(), remote_addresses);
