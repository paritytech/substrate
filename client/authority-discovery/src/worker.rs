    ...
    DhtEventStream: Stream<Item = DhtEvent> + Unpin,
{
    ...
    fn process_message_from_service(&self, msg: ServicetoWorkerMsg) {
        match msg {
            ...
            ServicetoWorkerMsg::GetAuthorityIdByPeerId(peer_id, sender) => {
                let _ = sender.send(
                    self.addr_cache
                        .get_authority_id_by_peer_id(&peer_id)
                        .map(Clone::clone),
                );
            }
        }
    ...
    fn addresses_to_publish(&self) -> impl ExactSizeIterator<Item = Multiaddr> {
        ...
        self.network.external_addresses().into_iter().map(move |a| {
        ...
        match event {
            ...
            DhtEvent::ValueNotFound(hash) => {
                if let Some(metrics) = &self.metrics {
                    metrics
                        .dht_event_received
                        .with_label_values(&["value_not_found"])
                        .inc();
                }
