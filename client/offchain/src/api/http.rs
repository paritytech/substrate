...
impl HttpApi {
        ...
        let mut poll_sender = move |sender: &mut hyper::body::Sender| -> Result<(), HttpError> {
            ...
            match when_ready {
                ...
                future::MaybeDone::Future(_) | future::MaybeDone::Gone => {
                    debug_assert!(if let future::MaybeDone::Done(_) = deadline {
                        true
                    } else {
                        false
                    });
            ...
            request = match request {
                ...
                HttpApiRequest::Dispatched(Some(mut sender)) => {
                    if !chunk.is_empty() {
                        ...
                        // the sender.
                        self.requests
                            .insert(request_id, HttpApiRequest::Dispatched(None));
                        return Ok(());
                    }
                }
    ...
    pub fn response_wait(
        ...
        for id in ids {
            ...
            let _ = self
                .to_worker
                .unbounded_send(ApiToWorker::Dispatch { id: *id, request });
            ...
            match request {
                HttpWorkerRequest::Dispatched(mut future) => {
                    ...
                    let mut response = match Future::poll(Pin::new(&mut future), cx) {
                        Poll::Pending => {
                            me.requests
                                .push((id, HttpWorkerRequest::Dispatched(future)));
                            continue;
                        }
                        ...
                        Poll::Ready(Err(err)) => {
                            let _ = me
                                .to_api
                                .unbounded_send(WorkerToApi::Fail { id, error: err });
                    ...
                    me.requests
                        .push((id, HttpWorkerRequest::ReadBody { body, tx: body_tx }));
                ...
                HttpWorkerRequest::ReadBody { mut body, mut tx } => {
                    ...
                    match tx.poll_ready(cx) {
                        ...
                        Poll::Pending => {
                            me.requests
                                .push((id, HttpWorkerRequest::ReadBody { body, tx }));
                            continue;
                        }
                    }
...
mod tests {
    ...
    fn basic_localhost() {
        ...
        let id = api
            .request_start("POST", &format!("http://{}", addr))
            .unwrap();
        ...
        let n = api
            .response_read_body(id, &mut buf, Some(deadline))
            .unwrap();
    ...
    fn request_add_header_invalid_call() {
        ...
        let id = api
            .request_start("GET", &format!("http://{}", addr))
            .unwrap();
