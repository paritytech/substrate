...
pub trait CustomMiddleware<M: Metadata>: Clone + Default + Send + Sync + 'static {
	...
	fn on_call<F, X>(&self, call: Call, meta: M, next: F) -> Either<FutureOutput, X>
	where
...
impl<M> CustomMiddleware<M> for () where M: Metadata {}
...
pub fn method_names<F, M, CM, E>(gen_handler: F) -> Result<HashSet<String>, E>
where
	F: FnOnce(RpcMiddleware<M, CM>) -> Result<RpcHandler<M, CM>, E>,
	M: PubSubMetadata + Sync,
...
pub struct RpcMiddleware<M: Metadata, CM: CustomMiddleware<M> = ()> {
...
impl<M: Metadata + Sync, CM: CustomMiddleware<M>> RequestMiddleware<M> for RpcMiddleware<M, CM> {
		...
		X: Future<Output = Option<jsonrpc_core::Response>> + Send + 'static,
	{
		...
		let r = self.custom_middleware.on_request(request, meta, next);
		Either::Left(
			async move {
				let r = r.await;
				if let Some(ref metrics) = metrics {
		...
		X: Future<Output = Option<jsonrpc_core::Output>> + Send + 'static,
	{
		...
		let r = self.custom_middleware.on_call(call, meta, next);
		Either::Left(
			async move {
				let r = r.await;
				#[cfg(not(target_os = "unknown"))]
