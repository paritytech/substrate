...
impl NodeParams {
	...
	pub fn node_name(&self) -> Option<String> {
		match (self.name.as_ref(), self.get_keyring()) {
			(Some(name), _) => Some(name.to_string()),
			(_, Some(keyring)) => Some(keyring.to_string()),
			(None, None) => None,
		}
	}

	/// Get the development key seed from the current object
	pub fn dev_key_seed(&self, is_dev: bool) -> Option<String> {
	...
	/// Gets the role
	pub fn role(&self, is_dev: bool) -> Role {
	...
	pub fn force_authoring(&self, is_dev: bool) -> bool {
		// Imply forced authoring on --dev
		is_dev || self.force_authoring
	}
	...
	pub fn disable_grandpa(&self) -> bool {
		self.no_grandpa
	}

	/// Get the RPC websockets maximum connections (`None` if unlimited).
	pub fn rpc_ws_max_connections(&self) -> Option<usize> {
		self.ws_max_connections
	}

	/// Get the RPC cors (`None` if disabled)
	pub fn rpc_cors(&self, is_dev: bool) -> Option<Vec<String>> {
		self.rpc_cors
			.clone()
			.unwrap_or_else(|| {
				if is_dev {
	...
	pub fn rpc_http(&self) -> Result<Option<SocketAddr>> {
		...
		Ok(Some(SocketAddr::new(
			interface,
			self.rpc_port.unwrap_or(9933),
		)))
	}

	/// Get the RPC IPC path (`None` if disabled).
	pub fn rpc_ipc(&self) -> Option<String> {
		self.ipc_path.clone()
	}

	/// Get the RPC websocket address (`None` if disabled).
	pub fn rpc_ws(&self) -> Result<Option<SocketAddr>> {
		let interface = rpc_interface(
			self.ws_external,
		...
		Ok(Some(SocketAddr::new(
			interface,
			self.ws_port.unwrap_or(9944),
		)))
	}

	/// Returns the RPC method set to expose.
	pub fn rpc_methods(&self) -> sc_service::config::RpcMethods {
		self.rpc_methods.into()
	}

	/// Get maximum runtime instances
	pub fn max_runtime_instances(&self) -> Option<usize> {
		self.max_runtime_instances.map(|x| x.min(256))
	}
