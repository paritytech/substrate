...
use crate::{RuntimeAdapter, read_message_from_stdin, decode_hex, read_uri, error, format_seed, SharedParams};
...
fn generate_key<C: RuntimeAdapter>(desired: &str) -> Result<KeyPair<C>, &'static str> where {
	if desired.is_empty() {
...
struct KeyPair<R: RuntimeAdapter> {
	pub pair: R::Pair,
	pub seed: <R::Pair as Pair>::Seed,
	pub score: usize,
}
