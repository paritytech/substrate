...
fn maintained_pool() -> (
    ...
    let (pool, background_task, notifier) =
        BasicPool::new_test(std::sync::Arc::new(TestApi::with_alice_nonce(209)));
...
fn fork_aware_finalization() {
    ...
    // block C2
    {
        let header = pool
            .api
            .push_fork_block_with_parent(b1, vec![from_dave.clone()]);
        from_dave_watcher =
    ...
    // block D2
    {
        ...
        let header = pool
            .api
            .push_fork_block_with_parent(c2, vec![from_bob.clone()]);
...
fn should_not_accept_old_signatures() {
    ...
    let pool = Arc::new(BasicPool::new_test(Arc::new(FullChainApi::new(client))).0);
