...
fn pool() -> Pool<TestApi> {
    Pool::new(
        Default::default(),
        true.into(),
        TestApi::with_alice_nonce(209).into(),
    )
}
...
fn block_event_with_retracted(
    ...
    let tree_route = api
        .tree_route(retracted_start, header.parent_hash)
...
fn finalization() {
    ...
    let header = pool.api.chain().read().block_by_number.get(&2).unwrap()[0]
        .0
        .header()
        .clone();
    let event = ChainEvent::NewBestBlock {
    ...
    assert_eq!(
        stream.next(),
        Some(TransactionStatus::InBlock(header.hash()))
    );
    assert_eq!(
        stream.next(),
        Some(TransactionStatus::Finalized(header.hash()))
    );
...
fn fork_aware_finalization() {
    ...
    // block C2
    {
        let header = pool
            .api
            .push_block_with_parent(b1, vec![from_dave.clone()], true);
        from_dave_watcher =
    ...
    // block D2
    {
        ...
        let header = pool
            .api
            .push_block_with_parent(c2, vec![from_bob.clone()], true);
...

    {
        ...
        assert_eq!(
            stream.next(),
            Some(TransactionStatus::Finalized(e1.clone()))
        );
        assert_eq!(stream.next(), None);
    }
...
fn resubmit_from_retracted_fork() {
    ...
    let e1 = {
        ...
        let header = pool
            .api
            .push_block_with_parent(d1.clone(), vec![tx4.clone()], true);
        assert_eq!(pool.status().ready, 2);
        header.hash()
    };
    ...
    let f1_header = {
        ...
        let header = pool
            .api
            .push_block_with_parent(e1.clone(), vec![tx5.clone()], true);
    ...
    let ready = pool
        ...
        .map(|t| t.data.encode())
        .collect::<BTreeSet<_>>();
    let expected_ready = vec![tx3, tx4, tx5]
        .iter()
        .map(Encode::encode)
        .collect::<BTreeSet<_>>();
    ...
    let ready = pool
        ...
        .map(|t| t.data.encode())
        .collect::<BTreeSet<_>>();
    let expected_ready = vec![tx0, tx1, tx2]
        .iter()
        .map(Encode::encode)
        .collect::<BTreeSet<_>>();
    assert_eq!(expected_ready, ready);
...
fn should_not_accept_old_signatures() {
    ...
    let pool = Arc::new(BasicPool::new_test(Arc::new(FullChainApi::new(client, None))).0);
...
fn import_notification_to_pool_maintain_works() {
    ...
    let pool = Arc::new(BasicPool::new_test(Arc::new(FullChainApi::new(client.clone(), None))).0);
    ...
    let evt = import_stream
        .next()
        .expect("Importing a block leads to an event");
