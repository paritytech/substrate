...
use futures::{channel::oneshot, future::ready, prelude::*};
...
type ReadyIteratorFor<PoolApi> = BoxedReadyIterator<
    sc_transaction_graph::ExHash<PoolApi>,
    ...
    PoolApi: 'static + sc_transaction_graph::ChainApi<Block = Block, Hash = Block::Hash>,
{
    ...
    fn submit_at(
        ...
        self.metrics
            .report(|metrics| metrics.validations_scheduled.inc_by(xts.len() as u64));
    ...
    fn submit_one(
        ...
        self.metrics
            .report(|metrics| metrics.validations_scheduled.inc());
                ...
                async move {
                    ...
                    if next_action.resubmit {
                        ...
                        for retracted_hash in retracted {
                            ...
                            pool.validated_pool()
                                .on_block_retracted(retracted_hash.clone());

                            let block_transactions = api
                                .block_body(&BlockId::hash(retracted_hash.clone()))
                                .await
                                .unwrap_or_else(|e| {
