...
use sc_rpc_api::{state::ReadProof, Subscriptions};
use sp_blockchain::{
...
use sc_client_api::{CallExecutor, ExecutorProvider, ProofProvider, StorageProvider};
        ...
        Box::new(result(
            self.block_or_best(block)
                .and_then(|block| {
                    ...
                    self.client
                        .child_storage_hash(&BlockId::Hash(block), &child_info, &key)
                })
                .map_err(client_err),
        ))
    }
