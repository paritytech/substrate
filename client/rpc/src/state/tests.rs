...
fn should_return_child_storage() {
    ...
    assert_matches!(
        child
            .storage_size(child_key.clone(), key.clone(), None,)
            .wait(),
        Ok(Some(1))
    );
}
...
fn should_notify_about_storage_changes() {
    ...
    assert_eq!(
        executor::block_on(next.into_future().compat()).unwrap().0,
        None
    );
}
...
fn should_query_storage() {
    fn run_tests(mut client: Arc<TestClient>, has_changes_trie_config: bool) {
        ...
        if has_changes_trie_config {
            assert_eq!(
                client
                    .max_key_changes_range(1, BlockId::Hash(block1_hash))
                    .unwrap(),
