...
use sp_core::{
    traits::{
        CodeExecutor, Externalities, MissingHostFunctions, RuntimeCode, RuntimeSpawn,
        RuntimeSpawnExt,
    },
    NativeOrEncoded,
};
...
impl RuntimeInstanceSpawn {
    ...
    pub fn register_on_externalities(module: Arc<dyn WasmModule>) {
        sp_externalities::with_externalities(move |mut ext| {
            if let Some(runtime_spawn) = Self::with_externalities_and_module(module.clone(), ext) {
