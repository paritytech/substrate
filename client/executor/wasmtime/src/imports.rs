...
use std::any::Any;
use wasmtime::{
    Extern, ExternType, Func, FuncType, ImportType, Limits, Memory, MemoryType, Module, Trap, Val,
};
...
impl HostFuncHandler {
    ...
    fn into_extern(self, module: &Module) -> Extern {
        ...
        let func = Func::new(module.store(), func_ty, move |_, params, result| {
...
impl MissingHostFuncHandler {
    ...
    fn into_extern(self, wasmtime_module: &Module, func_ty: &FuncType) -> Extern {
        ...
        let func = Func::new(wasmtime_module.store(), func_ty.clone(), move |_, _, _| {
            Err(Trap::new(format!(
