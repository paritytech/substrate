...
impl EntryPoint {
    ...
    pub fn call(&self, data_ptr: Pointer<u8>, data_len: WordSize) -> Result<u64> {
        ...
        (match self.call_type {
            EntryPointType::Direct => self
                .func
                .call(&[wasmtime::Val::I32(data_ptr), wasmtime::Val::I32(data_len)]),
            EntryPointType::Wrapped(func) => self.func.call(&[
        ...
        .map_err(|err| Error::from(format!("Wasm execution trapped: {}", err)))
    }
    ...
    pub fn direct(func: wasmtime::Func) -> std::result::Result<Self, &'static str> {
        ...
        if func.ty() == entry_point {
            Ok(Self {
                func,
                call_type: EntryPointType::Direct,
            })
        } else {
    ...
    pub fn wrapped(
        ...
        if dispatcher.ty() == entry_point {
            Ok(Self {
                func: dispatcher,
                call_type: EntryPointType::Wrapped(func),
            })
        } else {
