...
mod tests {
	...
	#[test]
	fn compact_proof_processing() {
		let mut pairs = vec![
			(hex!("dd8502").to_vec(), vec![1u8; 32]),
			(hex!("ff0203").to_vec(), vec![2u8; 32]),
		];
		let pairs_child = vec![
			(hex!("e502").to_vec(), vec![1u8; 32]),
			(hex!("0203").to_vec(), vec![2u8; 32]),
		];

		let mut memdb = MemoryDB::default();
		let mut populate_child_trie = |key: &[u8], content: Vec<(Vec<u8>, Vec<u8>)>| -> (Vec<u8>, Vec<u8>) {
			let mut root = Default::default();
			populate_trie::<Layout>(&mut memdb, &mut root, &content);
			let mut child_key = sp_core::storage::well_known_keys::DEFAULT_CHILD_STORAGE_KEY_PREFIX.to_vec();
			child_key.extend_from_slice(key);
			pairs.push((child_key.clone(), root.as_ref().to_vec()));
			(child_key, root.as_ref().to_vec())
		};
		let (child, child_root) = populate_child_trie(b"child", pairs_child);

		let mut root = Default::default();
		populate_trie::<Layout>(&mut memdb, &mut root, &pairs);

		let read_key_set = &[
			(hex!("dd8502").to_vec(), Some(vec![1u8; 32])),
			(child, Some(child_root)),
			(b"dummy".to_vec(), None),
		];
		...
		let proof = generate_trie_proof::<Layout, _, _, _>(
			&memdb,
			root,
			read_key_set.iter().map(|kv| &kv.0),
		).unwrap();

		let compact_proof = crate::trie_codec::encode_compact::<Layout>(
			StorageProof::new(proof),
			root.clone(),
		).unwrap();
		let mut memdb = MemoryDB::default();
		let _ = crate::trie_codec::decode_compact::<Layout, _, _>(
			&mut memdb,
			compact_proof.encoded_nodes.iter().map(|n| n.as_slice()),
			Some(&root),
		).unwrap();
		let nodes: Vec<Vec<u8>> = memdb.drain().into_iter().filter_map(|(_, v)| {
			(v.1 > 0).then(|| v.0)
		}).collect();
		// Check that a K, V included into the proof are verified.
		assert!(verify_trie_proof::<Layout, _, _, _>(
