...
impl<'db, H: Hasher> MmrDBMut<'db, H> {
	...
	pub fn get(&self, index: u64) -> Option<DBValue> {
		MmrDB { db: &self.db, root: &*self.root, number_element: self.number_element }.get(index)
	}
...
fn simple_test() {
	...
	for i in 0u64..nb {
		...
		trie_db.push(&val[..]); // TODO test with multiple push (actually not needed in practice, let's
						// not implement it).
		trie_db.commit();
