...
fn calling_function_with_strat(strat: ExecutionStrategy) {
	...
	assert_eq!(runtime_api.benchmark_add_one(at_hash, &1).unwrap(), 2);
}
...
fn calling_native_runtime_signature_changed_function() {
	...
	assert_eq!(runtime_api.function_signature_changed(at_hash).unwrap(), 1);
}
...
fn calling_wasm_runtime_signature_changed_old_function() {
	...
	#[allow(deprecated)]
	let res = runtime_api.function_signature_changed_before_version_2(at_hash).unwrap();
	assert_eq!(&res, &[1, 2]);
}
...
fn calling_with_both_strategy_and_fail_on_wasm_should_return_error() {
	...
	let runtime_api = client.runtime_api();
	let at_hash = client.chain_info().best_hash;
	assert!(runtime_api.fail_on_wasm(at_hash).is_err());
}
...
fn calling_with_both_strategy_and_fail_on_native_should_work() {
	...
	let runtime_api = client.runtime_api();
	let at_hash = client.chain_info().best_hash;
	assert_eq!(runtime_api.fail_on_native(at_hash).unwrap(), 1);
}
...
fn calling_with_native_else_wasm_and_fail_on_wasm_should_work() {
	...
	let runtime_api = client.runtime_api();
	let at_hash = client.chain_info().best_hash;
	assert_eq!(runtime_api.fail_on_wasm(at_hash).unwrap(), 1);
}
...
fn use_trie_function() {
	...
	let runtime_api = client.runtime_api();
	let at_hash = client.chain_info().best_hash;
	assert_eq!(runtime_api.use_trie(at_hash).unwrap(), 2);
}
...
fn initialize_block_works() {
	...
	runtime_api
		...
		.unwrap();
	assert_eq!(runtime_api.get_block_number(at_hash).unwrap(), 1);
}
...
fn call_runtime_api_with_multiple_arguments() {
	...
	client
		.runtime_api()
		.test_multiple_arguments(at_hash, data.clone(), data.clone(), data.len() as u32)
		.unwrap();
}
...
fn disable_logging_works() {
	if std::env::var("RUN_TEST").is_ok() {
		...
		let runtime_api = client.runtime_api();
		let at_hash = client.chain_info().genesis_hash;
		runtime_api.do_trace_log(at_hash).expect("Logging should not fail");
