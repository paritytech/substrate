...
// methods. `generate_versioned_api_traits` in `delc_runtime_apis.rs` generates a dummy trait for
// each version containing all required methods that should be implemented. Here an implementation
// for this dummy trait is generated by adding all methods defined in the api (and stripping their
// implementations to save compiling time). If a method is missed the compiler will generate an
// error which luckily is descriptive enough for the implementor.
fn generate_dummy_trait_impl(initial: &ItemImpl, trait_api_ver: u64) -> ItemImpl {
	let mut dummy_impl_ = initial.clone();
	let mut dummy_trait = dummy_impl_
		.trait_
		.as_ref()
		.expect("Api implementation should always contain a trait bound; qed")
		.clone();
	let mut trait_path_segment = dummy_trait
		.1
		.segments
		.last()
		.expect("Trait path should always contain at least one item; qed")
	...
	// attach dummy implementations to all methods
	for i in &mut dummy_impl_.items {
		match i {
			ImplItem::Method(m) => attach_default_method_implementation(m),
			_ => continue,
		}
	}
...
fn generate_runtime_api_versions(impls: &[ItemImpl]) -> Result<TokenStream> {
	...
	for impl_ in impls {
		...
		if api_ver.is_none() {
			...
			let version = api_ver.unwrap() as u32;
			populate_runtime_api_versions(&mut result, &mut sections, attrs, id, version, &c);
		}
	}
