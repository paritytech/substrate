...
pub fn mul_projective(base: Vec<u8>, scalar: Vec<u8>) -> Vec<u8> {
	...
	let base = twisted_edwards::Projective::<EdwardsParameters>::deserialize_with_mode(
		cursor,
		Compress::Yes,
		Validate::No,
	)
	...
	let res = <EdwardsParameters as TECurveConfig>::mul_projective(&base, &scalar);
...
pub fn mul_affine(base: Vec<u8>, scalar: Vec<u8>) -> Vec<u8> {
	let cursor = Cursor::new(base);
	let base = twisted_edwards::Affine::<EdwardsParameters>::deserialize_with_mode(
		cursor,
		Compress::Yes,
		Validate::No,
	)
	...
	let cursor = Cursor::new(scalar);
	let scalar = Vec::<u64>::deserialize_with_mode(cursor, Compress::Yes, Validate::No).unwrap();
	let res = <EdwardsParameters as TECurveConfig>::mul_affine(&base, &scalar);
	let mut serialized = vec![0; res.serialized_size(Compress::Yes)];
	let mut cursor = Cursor::new(&mut serialized[..]);
	res.serialize_with_mode(&mut cursor, Compress::Yes).unwrap();
	serialized
...
pub fn msm(bases: Vec<Vec<u8>>, scalars: Vec<Vec<u8>>) -> Vec<u8> {
	let bases: Vec<_> = bases
		...
		.map(|a| {
			...
			twisted_edwards::Affine::<EdwardsParameters>::deserialize_with_mode(
				cursor,
				Compress::Yes,
				Validate::No,
			)
	...
	let scalars: Vec<_> = scalars
		...
		.map(|a| {
			let cursor = Cursor::new(a);
			<EdwardsParameters as CurveConfig>::ScalarField::deserialize_with_mode(
				cursor,
				Compress::Yes,
				Validate::No,
			)
