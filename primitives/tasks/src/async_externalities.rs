...
impl Externalities for AsyncExternalities {
	...
	fn storage_store_offchain(&mut self, _offset: u32, _size: u32, _duration: u32) -> Result<Vec<u8>, ()> {
		unimplemented!("storage_store_offchain is not supported in AsyncExternalities");
	}

	fn storage_renew_offchain(&mut self, _hash: &[u8], _size: u32) -> Result<(), ()> {
		unimplemented!("storage_renew_offchain is not supported in AsyncExternalities");
	}
}
