...
impl<'a> ChildInfo<'a> {
    ...
    pub const fn new_default(unique_id: &'a [u8]) -> Self {
        ChildInfo::Default(ChildTrie { data: unique_id })
    }

    /// Instantiates a owned version of this child info.
    pub fn to_owned(&self) -> OwnedChildInfo {
        match self {
            ChildInfo::Default(ChildTrie { data }) => OwnedChildInfo::Default(OwnedChildTrie {
                data: data.to_vec(),
            }),
        }
    }
    ...
    pub fn info(&self) -> (&[u8], u32) {
        match self {
            ChildInfo::Default(ChildTrie { data }) => (data, ChildType::CryptoUniqueId as u32),
        }
    }
    ...
    pub fn keyspace(&self) -> &[u8] {
        match self {
            ChildInfo::Default(ChildTrie { data }) => &data[..],
        }
    }
}
...
impl OwnedChildInfo {
    ...
    pub fn new_default(unique_id: Vec<u8>) -> Self {
        OwnedChildInfo::Default(OwnedChildTrie { data: unique_id })
    }
    ...
    pub fn as_ref(&self) -> ChildInfo {
        match self {
            OwnedChildInfo::Default(OwnedChildTrie { data }) => ChildInfo::Default(ChildTrie {
                data: data.as_slice(),
            }),
        }
    }
