...
use crate::DBValue;
use crate::{Backend, Error, ExecutionError};
    ...
    fn proof_recorded_and_checked_with_child() {
        ...
        let in_memory_root = in_memory
            .full_storage_root::<_, Vec<_>, _>(
                ...
                in_memory
                    .child_storage_keys()
                    .map(|k| (k.0.to_vec(), Vec::new(), k.1.to_owned())),
            )
            .0;
        (0..64).for_each(|i| assert_eq!(in_memory.storage(&[i]).unwrap().unwrap(), vec![i]));
        (28..65).for_each(|i| {
            assert_eq!(
                in_memory
                    .child_storage(&own1[..], CHILD_INFO_1, &[i])
                    .unwrap()
                    .unwrap(),
                vec![i]
            )
        ...
        (10..15).for_each(|i| {
            assert_eq!(
                in_memory
                    .child_storage(&own2[..], CHILD_INFO_2, &[i])
                    .unwrap()
                    .unwrap(),
                vec![i]
            )
        ...
        let proving = ProvingBackend::new(trie);
        assert_eq!(
            proving.child_storage(&own1[..], CHILD_INFO_1, &[64]),
            Ok(Some(vec![64]))
        );
        ...
        assert_eq!(
            proof_check
                .child_storage(&own1[..], CHILD_INFO_1, &[64])
                .unwrap()
                .unwrap(),
            vec![64]
        );
