...
use crate::{Backend, DBValue, Error, ExecutionError};
...
mod tests {
    ...
    fn proof_recorded_and_checked_with_child() {
        ...
        let proving = ProvingBackend::new(trie);
        assert_eq!(
            proving.child_storage(child_info_1, &[64]),
            Ok(Some(vec![64]))
        );
