    ...
    Number: BlockNumber,
{
    let (committed, prospective, child_info) = if let Some(sk) = storage_key.as_ref() {
        ...
        (
            Some(&changes.committed.top),
            Some(&changes.prospective.top),
            None,
        )
    };
    committed
                        ...
                        if let Some(sk) = storage_key.as_ref() {
                                ...
                                .unwrap_or_default()
                            {
                                if let Some(child_info) = child_info.as_ref() {
                                    if !backend
                                        .exists_child_storage(sk, child_info.as_ref(), k)
                                        .map_err(|e| format!("{}", e))?
                                    {
                                        return Ok(map);
                                    }
...
mod test {
    ...
    use crate::overlayed_changes::{OverlayedChangeSet, OverlayedValue};
