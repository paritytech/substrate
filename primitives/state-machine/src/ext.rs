    ...
    N: crate::changes_trie::BlockNumber,
{
    ...
    fn next_child_storage_key(&self, child_info: &ChildInfo, key: &[u8]) -> Option<StorageKey> {
        ...
        let next_overlay_key_change = self
            .overlay
            .next_child_storage_key_change(child_info.storage_key(), key);
    ...
    fn kill_child_storage(&mut self, child_info: &ChildInfo) {
        ...
        self.backend.for_keys_in_child_storage(child_info, |key| {
            self.overlay
                .set_child_storage(child_info, key.to_vec(), None);
        });
    }
    ...
    fn clear_child_prefix(&mut self, child_info: &ChildInfo, prefix: &[u8]) {
        ...
        self.backend
            .for_child_keys_with_prefix(child_info, prefix, |key| {
                self.overlay
                    .set_child_storage(child_info, key.to_vec(), None);
            });
    }
    ...
    fn child_storage_root(&mut self, child_info: &ChildInfo) -> Vec<u8> {
            ...
            .is_some()
        {
            ...
            if let Some(child_info) = self.overlay.default_child_info(storage_key) {
                ...
                if is_empty {
                    self.overlay
                        .set_storage(prefixed_storage_key.into_inner(), None);
                } else {
                    self.overlay
                        .set_storage(prefixed_storage_key.into_inner(), Some(root.clone()));
                }
