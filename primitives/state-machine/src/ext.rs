...
/// `StorageAccumulator::default()`, then any number of Vec<u8> can
/// be pushed, each represented by some serialized entity. At the end,
/// it can be merged with another raw vector of bytes generated by
/// different `StorageAccumulator` instance (can also be merged with
/// empty vector).
#[derive(Debug, Default)]
pub struct StorageAccumulator(Vec<Vec<u8>>);

impl StorageAccumulator {
	/// Append new entry to the accumulator.
	pub fn append(&mut self, value: Vec<u8>) {
		self.0.push(value)
	}

	/// Merge with existing raw storage value.
	///
