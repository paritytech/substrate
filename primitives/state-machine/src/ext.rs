...
use crate::{
    backend::Backend, changes_trie::State as ChangesTrieState, OverlayedChanges, StorageKey,
...
use sp_core::{
    ...
    storage::{well_known_keys::is_child_storage_key, ChildInfo, ChildStorageKey},
    traits::Externalities,
};
use sp_externalities::{Extension, Extensions};
use sp_trie::{default_child_trie_root, trie_types::Layout};
    ...
    N: crate::changes_trie::BlockNumber,
{
    ...
    fn next_child_storage_key(
        ...
        let next_backend_key = self
            ...
            .expect(EXT_NOT_ALLOWED_TO_FAIL);
        let next_overlay_key_change = self
            .overlay
            .next_child_storage_key_change(storage_key.as_ref(), key);
        ...
        match (next_backend_key, next_overlay_key_change) {
            ...
            (_, Some(overlay_key)) => {
                if overlay_key.1.value.is_some() {
                    ...
                    self.next_child_storage_key(storage_key, child_info, &overlay_key.0[..])
                }
            }
        }
    }
    ...
    fn place_child_storage(
        ...
        self.mark_dirty();
        self.overlay
            .set_child_storage(storage_key.into_owned(), child_info, key, value);
    }
    ...
    fn kill_child_storage(&mut self, storage_key: ChildStorageKey, child_info: ChildInfo) {
        ...
        self.mark_dirty();
        self.overlay
            .clear_child_storage(storage_key.as_ref(), child_info);
        self.backend
            .for_keys_in_child_storage(storage_key.as_ref(), child_info, |key| {
                self.overlay.set_child_storage(
                    storage_key.as_ref().to_vec(),
                    child_info,
                    key.to_vec(),
                    None,
                );
    ...
    fn clear_child_prefix(
        ...
        self.mark_dirty();
        self.overlay
            .clear_child_prefix(storage_key.as_ref(), child_info, prefix);
        self.backend
            .for_child_keys_with_prefix(storage_key.as_ref(), child_info, prefix, |key| {
                self.overlay.set_child_storage(
                    storage_key.as_ref().to_vec(),
                    child_info,
                    key.to_vec(),
    ...
    fn storage_root(&mut self) -> Vec<u8> {
        ...
        let root = self
            .overlay
            .storage_root(self.backend, self.storage_transaction_cache);
    ...
    fn child_storage_root(&mut self, storage_key: ChildStorageKey) -> Vec<u8> {
            ...
            .is_some()
        {
            let root = self
                ...
                .unwrap_or(default_child_trie_root::<Layout<H>>(storage_key.as_ref()));
            ...
            if let Some(child_info) = self.overlay.child_info(storage_key).cloned() {
                    ...
                    let delta = self
                        ...
                        .chain(
                            self.overlay
                                ...
                                .into_iter()
                                .flat_map(|(map, _)| {
                                    map.clone().into_iter().map(|(k, v)| (k, v.value))
                                }),
                        );

                    self.backend
                        .child_storage_root(storage_key, child_info.as_ref(), delta)
                };
                ...
                if is_empty {
                    ...
                    self.overlay
                        .set_storage(storage_key.into(), Some(root.clone()));
                }
    ...
    fn commit(&mut self) {
        ...
        let changes = self
            ...
            .expect(EXT_NOT_ALLOWED_TO_FAIL);
        self.backend
            .commit(changes.transaction_storage_root, changes.transaction)
            .expect(EXT_NOT_ALLOWED_TO_FAIL);
        self.storage_transaction_cache.reset();
    }
}
...
mod tests {
    ...
    use sp_core::{map, storage::well_known_keys::EXTRINSIC_INDEX, Blake2Hasher, H256};
    ...
    fn storage_changes_root_is_some_when_extrinsic_changes_are_non_empty() {
        ...
        let state = Some(ChangesTrieState::new(
            changes_trie_config(),
            Zero::zero(),
            &storage,
        ));
    ...
    fn next_child_storage_key_works() {
        ...
        // next_backend < next_overlay
        assert_eq!(
            ext.next_child_storage_key(child(), CHILD_INFO_1, &[5]),
            Some(vec![10])
        );

        // next_backend == next_overlay but next_overlay is a delete
        assert_eq!(
            ext.next_child_storage_key(child(), CHILD_INFO_1, &[10]),
            Some(vec![30])
        );

        // next_overlay < next_backend
        assert_eq!(
            ext.next_child_storage_key(child(), CHILD_INFO_1, &[20]),
            Some(vec![30])
        );

        // next_backend exist but next_overlay doesn't exist
        assert_eq!(
            ext.next_child_storage_key(child(), CHILD_INFO_1, &[30]),
            Some(vec![40])
        );
        ...
        // next_overlay exist but next_backend doesn't exist
        assert_eq!(
            ext.next_child_storage_key(child(), CHILD_INFO_1, &[40]),
            Some(vec![50])
        );
    }
    ...
    fn child_storage_works() {
        ...
        assert_eq!(
            ext.child_storage(child(), CHILD_INFO_1, &[10]),
            Some(vec![10])
        );
        assert_eq!(
        ...
        assert_eq!(ext.child_storage_hash(child(), CHILD_INFO_1, &[20]), None,);

        assert_eq!(
            ext.child_storage(child(), CHILD_INFO_1, &[30]),
            Some(vec![31])
        );
        assert_eq!(
