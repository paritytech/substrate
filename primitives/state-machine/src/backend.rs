...
use sp_core::{
    storage::{well_known_keys, ChildInfo, OwnedChildInfo},
    traits::RuntimeCode,
};
use sp_trie::{trie_types::TrieDBMut, MemoryDB, TrieMut};

use crate::{
    trie_backend::TrieBackend, trie_backend_essence::TrieBackendStorage, StorageCollection,
...
pub trait Backend<H: Hasher>: std::fmt::Debug {
        ...
        H::Out: Ord + Encode,
    {
        ...
        let (root, parent_txs) =
            self.storage_root(delta.into_iter().chain(child_roots.into_iter()));
...
impl Consolidate for Vec<(Option<(StorageKey, OwnedChildInfo)>, StorageCollection)> {
