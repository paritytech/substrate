...
impl OverlayedChanges {
	...
	pub fn drain_committed(&mut self) -> (
		...
		let top = std::mem::replace(&mut self.changes.top, Default::default());
		let children = std::mem::replace(&mut self.changes.children, Default::default());
		let committed = self.changes.states.committed();
		let states = self.changes.states.clone();
		let states2 = std::mem::replace(&mut self.changes.states, Default::default());
		(
			top.into_iter()
