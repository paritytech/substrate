...
impl OverlayedChanges {
	...
	>(
		...
		let transaction = self.storage_root(backend).1;
		let changes_trie_transaction = self.changes_trie_root(
			backend,
			changes_trie_storage,
			parent_hash,
			false,
		).map_err(|_| "Failed to generate changes trie transaction")?.map(|t| t.1);
