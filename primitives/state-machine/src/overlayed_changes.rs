...
use crate::{
    backend::Backend,
    changes_trie::{
        build_changes_trie, BlockNumber, State as ChangesTrieState, NO_EXTRINSIC_INDEX,
    },
...
use sp_core::storage::{well_known_keys::EXTRINSIC_INDEX, ChildInfo, OwnedChildInfo};
...
impl OverlayedChanges {
    ...
    pub(crate) fn clear_child_storage(&mut self, storage_key: &[u8], child_info: ChildInfo) {
        let extrinsic_index = self.extrinsic_index();
        let map_entry = self
            .prospective
            .children
                ...
                self.committed
                    ...
                    .chain(
                        self.prospective
                            ...
                            .into_iter()
                            .flat_map(|(map, _)| {
                                map.iter().map(|(k, v)| (k.clone(), v.value.clone()))
                            }),
                    ),
...
mod tests {
    ...
    use hex_literal::hex;
    use sp_core::{storage::well_known_keys::EXTRINSIC_INDEX, traits::Externalities, Blake2Hasher};

    fn strip_extrinsic_index(
        map: &BTreeMap<StorageKey, OverlayedValue>,
    ...
    fn extrinsic_changes_are_collected() {
        ...
        assert_eq!(
            strip_extrinsic_index(&overlay.committed.top),
            vec![
                (
                    vec![1],
                    OverlayedValue {
        ...
        assert_eq!(overlay.prospective, Default::default());
    }
    ...
    fn next_child_storage_key_change_works() {
        ...
        // next_committed < next_prospective
        let next_to_10 = overlay
            .next_child_storage_key_change(&child, &[10])
            .unwrap();
        assert_eq!(next_to_10.0.to_vec(), vec![20]);
        assert_eq!(next_to_10.1.value, Some(vec![20]));

        // next_committed == next_prospective
        let next_to_20 = overlay
            .next_child_storage_key_change(&child, &[20])
            .unwrap();
        assert_eq!(next_to_20.0.to_vec(), vec![30]);
        assert_eq!(next_to_20.1.value, None);

        // next_committed, no next_prospective
        let next_to_30 = overlay
            .next_child_storage_key_change(&child, &[30])
            .unwrap();
        assert_eq!(next_to_30.0.to_vec(), vec![40]);
        assert_eq!(next_to_30.1.value, Some(vec![40]));
        ...
        // next_prospective, no next_committed
        let next_to_40 = overlay
            .next_child_storage_key_change(&child, &[40])
            .unwrap();
        assert_eq!(next_to_40.0.to_vec(), vec![50]);
        assert_eq!(next_to_40.1.value, Some(vec![50]));
    }
