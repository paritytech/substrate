...
use log::warn;
use sp_core::{
    storage::{well_known_keys::is_child_storage_key, ChildInfo, Storage, StorageChild},
    traits::Externalities,
    Blake2Hasher,
};
use sp_externalities::Extensions;
use sp_trie::trie_types::Layout;
use sp_trie::{empty_child_trie_root, TrieConfiguration};
use std::{
    any::{Any, TypeId},
    ...
    fn eq(&self, other: &BasicExternalities) -> bool {
        self.inner.top.eq(&other.inner.top)
            && self
                .inner
                .children_default
                .eq(&other.inner.children_default)
    }
}
...
impl Externalities for BasicExternalities {
    ...
    fn clear_child_prefix(&mut self, child_info: &ChildInfo, prefix: &[u8]) {
        if let Some(child) = self
            .inner
            .children_default
            .get_mut(child_info.storage_key())
        {
            let to_remove = child
                .data
                .range::<[u8], _>((Bound::Included(prefix), Bound::Unbounded))
