...
use sp_core::{
    storage::{
        well_known_keys::is_child_storage_key, ChildInfo, ChildStorageKey, Storage, StorageChild,
    },
...
use sp_trie::{default_child_trie_root, TrieConfiguration};
use std::{
    any::{Any, TypeId},
...
impl PartialEq for BasicExternalities {
    fn eq(&self, other: &BasicExternalities) -> bool {
        self.inner.top.eq(&other.inner.top) && self.inner.children.eq(&other.inner.children)
    }
}
...
impl Externalities for BasicExternalities {
    ...
    fn kill_child_storage(&mut self, storage_key: ChildStorageKey, _child_info: ChildInfo) {
    ...
    fn child_storage_root(&mut self, storage_key: ChildStorageKey) -> Vec<u8> {
        if let Some(child) = self.inner.children.get(storage_key.as_ref()) {
            ...
            InMemoryBackend::<Blake2Hasher>::default()
                .child_storage_root(storage_key.as_ref(), child.child_info.as_ref(), delta)
                .0
        } else {
