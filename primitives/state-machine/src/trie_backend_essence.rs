...
impl<S: TrieBackendStorage<H>, H: Hasher> TrieBackendEssence<S, H> where H::Out: Encode {
	...
	pub fn apply_to_key_values_while(
		...
		self.keys_values_with_prefix_inner(&root, prefix, |k, v| f(k, v), child_info, start_at)?;
		Ok(())
	}

	/// Retrieve all entries keys of child storage and call `f` for each of those keys.
	/// Aborts as soon as `f` returns false.
	pub fn apply_to_child_keys_while<F: FnMut(Vec<u8>) -> bool>(
		&self,
		child_info: &ChildInfo,
		f: F,
	) {
	...
	/// Execute given closure for all keys starting with prefix.
	pub fn for_keys_with_prefix(&self, prefix: &[u8], mut f: impl FnMut(Vec<u8>)) {
		// error is ignored
	...
	fn keys_values_with_prefix_inner(
		...
		result.map_err(|e| {
			...
			format!("TrieDB iteration error: {}", e)
		})
	}

	/// Execute given closure for all key and values starting with prefix.
	pub fn for_key_values_with_prefix(&self, prefix: &[u8], mut f: impl FnMut(Vec<u8>, Vec<u8>)) {
		// error is ignored
