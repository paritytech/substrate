...
struct Layers<V>(pub(crate) smallvec::SmallVec<[LayerEntry<V>; ALLOCATED_HISTORY]>);
...
impl<V> Layers<V> {
	/// Push a value without checking without transactionallayer
	/// consistency checks.
	/// layer.
	fn push_unchecked(&mut self, item: LayerEntry<V>) {
		self.0.push(item);
	}

	/// Discard all prospective values, keeping previous committed value
	/// only.
	fn discard_prospective(&mut self) -> LayeredOpsResult {
		if self.0.is_empty() {
	...
	fn get_mut(&mut self) -> Option<LayerEntry<&mut V>> {
	...
	/// otherwhise).
	fn set(&mut self, number_transactions: usize, value: V) {
...
impl<V> Layers<V> {
	...
	fn get_committed(&self) -> Option<&V> {
