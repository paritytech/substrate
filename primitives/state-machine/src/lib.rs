...
mod tests {
	...
	fn prove_read_with_size_limit_works() {
		let hashed_value = false;
		let remote_backend = trie_backend::tests::test_trie(hashed_value);
		let remote_root = remote_backend.storage_root(::std::iter::empty()).0;
		let (proof, count) =
	...
	fn compact_multiple_child_trie_inner(inner_hashed: bool) -> usize {
		...
		let remote_backend = trie_backend::tests::test_trie(inner_hashed);
		...
		let state_version = remote_backend.state_version();
		let mut remote_storage = remote_backend.into_storage();
		remote_storage.consolidate(transaction);
		let remote_backend = TrieBackend::new(remote_storage, remote_root, state_version);
