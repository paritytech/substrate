...
use sp_core::{
    ...
    storage::ChildInfo,
    traits::{CallInWasmExt, CodeExecutor, RuntimeCode},
...
pub use stats::{StateMachineStats, UsageInfo, UsageUnit};
pub use testing::TestExternalities;
pub use trie_backend::TrieBackend;
pub use trie_backend_essence::{Storage, TrieBackendStorage};
        ...
        let result = {
            ...
            match manager {
                ExecutionManager::Both(on_consensus_failure) => self
                    .execute_call_with_both_strategy(
                        native_call.take(),
                        orig_prospective,
                        on_consensus_failure,
                    ),
                ExecutionManager::NativeElseWasm => self
                    .execute_call_with_native_else_wasm_strategy(
                        native_call.take(),
                        orig_prospective,
                    ),
    ...
    I::Item: AsRef<[u8]>,
{
    ...
    for key in keys.into_iter() {
        let value =
            read_child_proof_check_on_proving_backend(&proving_backend, storage_key, key.as_ref())?;
        result.insert(key.as_ref().to_vec(), value);
    }
    Ok(result)
}
    ...
    H::Out: Ord + Codec,
{
    ...
    proving_backend
        .child_storage(storage_key, ChildInfo::new_default(&[]), key)
        .map_err(|e| Box::new(e) as Box<dyn Error>)
}
