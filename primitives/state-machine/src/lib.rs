...
pub use crate::{
	...
	ext::Ext,
	overlayed_changes::{
		Changes, ChildStorageCollection, IndexOperation, OffchainChanges,
		OffchainChangesCollection, StorageChanges, StorageCollection, StorageKey,
...
mod tests {
	...
	use sp_core::{
		map,
		storage::{ChildInfo, DefaultChild, StateVersion},
		...
		fn call(
			...
			match (using_native, self.native_succeeds, self.fallback_succeeds) {
				(true, true, _) | (false, _, true) => (
					Ok(vec![
						ext.storage(b"value1", 0, None).unwrap()[0] +
							ext.storage(b"value2", 0, None).unwrap()[0],
					]),
					using_native,
				),
	...
	fn limited_child_kill_works() {
		let child_info = DefaultChild::new(b"sub1");
		...
		let mut overlay = Changes::default();
		overlay.set_child_storage(&child_info, b"1", Some(b"1312"));
		overlay.set_child_storage(&child_info, b"2", Some(b"1312"));
		overlay.set_child_storage(&child_info, b"3", Some(b"1312"));
		overlay.set_child_storage(&child_info, b"4", Some(b"1312"));
	...
	fn set_child_storage_works() {
		...
		ext.set_child_storage(child_info, b"abc", b"def");
		assert_eq!(ext.child_storage(child_info, b"abc", 0, None), Some(b"def"[..].into()));
		let _ = ext.kill_child_storage(child_info, None, None);
		assert_eq!(ext.child_storage(child_info, b"abc", 0, None), None);
	}
	...
	fn append_storage_works() {
		...
		let mut cache = StorageTransactionCache::default();
		{
			...
			assert_eq!(
				ext.storage(key.as_slice(), 0, None),
				Some(vec![reference_data[0].clone()].encode().into())
			);
		}
		overlay.start_transaction();
		{
			...
			assert_eq!(ext.storage(key.as_slice(), 0, None), Some(reference_data.encode().into()));
		}
	...
	fn remove_with_append_then_rollback_appended_then_append_again() {
		...
		// For example, first transaction resulted in panic during block building
		{
			...
			assert_eq!(
				ext.storage(key.as_slice(), 0, None),
				Some(vec![Item::InitializationItem].encode().into())
			);
	...
	fn child_read_compact_stress_test() {
		...
		let mut storage: HashMap<Option<DefaultChild>, BTreeMap<StorageKey, StorageValue>> =
			Default::default();
		let mut seed = [0; 32];
		for i in 0..50u32 {
			...
			for _ in 0..nb_child_trie {
				...
				let child_info = DefaultChild::new(key);
	...
	fn compact_multiple_child_trie_inner(state_version: StateVersion) -> usize {
		...
		let child_info1 = DefaultChild::new(b"sub1");
		// this root will not be include in proof
		let child_info2 = DefaultChild::new(b"sub2");
		// this root will be include in proof
		let child_info3 = DefaultChild::new(b"sub");
		let remote_backend = trie_backend::tests::test_trie(state_version, None, None);
		...
		let child_info1 = ChildInfo::Default(child_info1);
	...
	fn child_storage_uuid() {
		...
		let mut transaction = {
			...
			let mut cache = StorageTransactionCache::default();
			let mut ext = Ext::new(&mut overlay, &mut cache, &backend, None);
			ext.set_child_storage(&child_info_1, b"abc", b"def");
			ext.set_child_storage(&child_info_2, b"abc", b"def");
	...
	fn set_storage_empty_allowed() {
...

		{
			let mut cache = StorageTransactionCache::default();
			let mut ext = Ext::new(&mut overlay, &mut cache, backend, None);
			assert_eq!(ext.storage(b"bbb", 0, None), Some(vec![].into()));
			assert_eq!(ext.storage(b"ccc", 0, None), Some(vec![].into()));
			ext.clear_storage(b"ccc");
			assert_eq!(ext.storage(b"ccc", 0, None), None);
		}
		overlay.commit_transaction().unwrap();
