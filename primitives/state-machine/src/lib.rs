...
mod execution {
	...
	where
		...
		H: Hasher,
		H::Out: Ord + Codec,
		I: IntoIterator<Item = (IT, bool)>,
		IT: AsRef<[u8]>,
	{
		let trie_backend = backend.as_trie_backend();
		prove_read_on_trie_backend_v2(trie_backend, child_info, keys)
	}
	...
	/// Generate storage read proof on pre-created trie backend.
	pub fn prove_read_on_trie_backend_v2<S, H, I, IT>(
		trie_backend: &TrieBackend<S, H>,
		child_info: Option<&ChildInfo>,
		keys: I,
	) -> Result<StorageProof, Box<dyn Error>>
		...
		IT: AsRef<[u8]>,
	{
		...
		if let Some(child_info) = child_info {
			for (key, only_hash) in keys.into_iter() {
				if only_hash {
					proving_backend
						.child_storage_hash(child_info, key.as_ref())
						.map_err(|e| Box::new(e) as Box<dyn Error>)?;
				} else {
					proving_backend
						.child_storage(child_info, key.as_ref())
	...
	/// Proof v2 can be either on hash access or value access.
	#[derive(Clone, Debug, PartialEq, Eq)]
	pub enum HashOrValue<H> {
		/// No item in state.
		None,
		/// Hash accessed in state.
		Hash(H),
		/// Value accessed in state.
		Value(Vec<u8>),
	}

	/// Check storage read proof, generated by `prove_read_v2` call.
	pub fn read_proof_check_v2<H, I, IT>(
		root: H::Out,
		proof: StorageProof,
		child_info: Option<&ChildInfo>,
		keys: I,
	) -> Result<HashMap<Vec<u8>, HashOrValue<H::Out>>, Box<dyn Error>>
	where
		H: Hasher + 'static,
		H::Out: Ord + Codec,
		I: IntoIterator<Item = (IT, bool)>,
		IT: AsRef<[u8]>,
	{
		...
		let mut result = HashMap::new();
		for key in keys.into_iter() {
			let value = read_proof_check_on_proving_backend_v2(
				&proving_backend,
				child_info,
				key.0.as_ref(),
				key.1,
			)?;
			result.insert(key.0.as_ref().to_vec(), value);
		}
		Ok(result)
	}
	...
	/// Check storage read proof on pre-created proving backend.
	pub fn read_proof_check_on_proving_backend_v2<H>(
		proving_backend: &TrieBackend<MemoryDB<H>, H>,
		child_info: Option<&ChildInfo>,
		key: &[u8],
		only_hash: bool,
	) -> Result<HashOrValue<H::Out>, Box<dyn Error>>
	where
		H: Hasher,
		H::Out: Ord + Codec,
	{
		Ok(if let Some(child_info) = child_info {
			if only_hash {
				proving_backend
					.child_storage_hash(child_info, key)
					.map_err(|e| Box::new(e) as Box<dyn Error>)?
					.map(HashOrValue::Hash)
					.unwrap_or(HashOrValue::None)
			} else {
				proving_backend
					.child_storage(child_info, key)
					.map_err(|e| Box::new(e) as Box<dyn Error>)?
					.map(HashOrValue::Value)
					.unwrap_or(HashOrValue::None)
			}
		} else {
			if only_hash {
...
mod tests {
	...
	fn compact_multiple_child_trie_inner(state_version: StateVersion) -> usize {
		...
		let accesses_2 = [(&b"k"[..], true), (&b"rando"[..], true), (&b"key1"[..], false)];
		let remote_backend = TrieBackendBuilder::new(&remote_storage, remote_root).build();
		let remote_proof_2 = prove_read_v2(
			remote_backend,
			Some(&child_info1),
			accesses_2.iter().map(|(k, h)| (k, *h)),
		)
		.unwrap();
		...
		assert_eq!(local_result1.get(&b"key1"[..]), Some(&Some(vec![5u8; 33])));
		let remote_proof = test_compact(remote_proof_2, &remote_root);
		let local_result1 = read_proof_check_v2::<BlakeTwo256, _, _>(
			remote_root,
			remote_proof,
			Some(&child_info1),
			accesses_2.iter().map(|(k, h)| (k, *h)),
		...
		.unwrap();
		assert_eq!(local_result1.len(), 3);
		assert!(matches!(local_result1.get(&b"k"[..]), Some(&HashOrValue::Hash(_))));
		assert_eq!(local_result1.get(&b"key1"[..]), Some(&HashOrValue::Value(vec![5u8; 33])));
		assert_eq!(local_result1.get(&b"rando"[..]), Some(&HashOrValue::None));
