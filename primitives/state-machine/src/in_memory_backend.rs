...
use crate::{
    backend::{insert_into_memory_db, Backend},
    stats::UsageInfo,
...
use sp_trie::{
    child_trie_root, default_child_trie_root, trie_types::Layout, MemoryDB, TrieConfiguration,
};
use std::{
...
impl<H: Hasher>
    From<HashMap<Option<(StorageKey, OwnedChildInfo)>, BTreeMap<StorageKey, StorageValue>>>
    for InMemory<H>
{
    fn from(
        inner: HashMap<Option<(StorageKey, OwnedChildInfo)>, BTreeMap<StorageKey, StorageValue>>,
...
impl<H: Hasher> From<Storage> for InMemory<H> {
    fn from(inners: Storage) -> Self {
        let mut inner: HashMap<
            ...
            BTreeMap<StorageKey, StorageValue>,
        > = inners
            .children
            .into_iter()
            .map(|(k, c)| (Some((k, c.child_info)), c.data))
            .collect();
...
impl<H: Hasher> From<Vec<(Option<(StorageKey, OwnedChildInfo)>, StorageCollection)>>
    for InMemory<H>
{
    fn from(inner: Vec<(Option<(StorageKey, OwnedChildInfo)>, StorageCollection)>) -> Self {
...
impl<H: Hasher> InMemory<H> {
    ...
    pub fn child_storage_keys(&self) -> impl Iterator<Item = (&[u8], ChildInfo)> {
        self.inner
            .iter()
            .filter_map(|item| item.0.as_ref().map(|v| (&v.0[..], v.1.as_ref())))
    }
}
    ...
    H::Out: Codec,
{
    ...
    fn child_storage(
        ...
        Ok(self
            .inner
            .get(&Some((storage_key.to_vec(), child_info.to_owned())))
            .and_then(|map| map.get(key).map(Clone::clone)))
    }
    ...
    fn exists_storage(&self, key: &[u8]) -> Result<bool, Self::Error> {
        Ok(self
            .inner
            .get(&None)
            .map(|map| map.get(key).is_some())
            .unwrap_or(false))
    }

    fn for_keys_with_prefix<F: FnMut(&[u8])>(&self, prefix: &[u8], f: F) {
        self.inner.get(&None).map(|map| {
            map.keys()
    ...
    fn for_key_values_with_prefix<F: FnMut(&[u8], &[u8])>(&self, prefix: &[u8], mut f: F) {
        self.inner.get(&None).map(|map| {
            map.iter()
                .filter(|(key, _val)| key.starts_with(prefix))
                .for_each(|(k, v)| f(k, v))
        });
    }
    ...
    fn as_trie_backend(&mut self) -> Option<&TrieBackend<Self::TrieBackendStorage, H>> {
        ...
        let root = match root_map {
            ...
            None => insert_into_memory_db::<H, _>(&mut mdb, new_child_roots.into_iter())?,
        };
...
mod tests {
    ...
    fn in_memory_with_child_trie_only() {
        ...
        assert_eq!(
            trie_backend
                .child_storage(b"1", child_info.as_ref(), b"2")
                .unwrap(),
            Some(b"3".to_vec())
        );
