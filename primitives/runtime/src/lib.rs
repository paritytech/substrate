...
pub enum DispatchError {
	...
	/// Storage layers limit has been reached, and we cannot spawn another one.
	StorageLayersLimit,
}
...
impl From<DispatchError> for &'static str {
	fn from(err: DispatchError) -> &'static str {
		match err {
			...
			DispatchError::StorageLayersLimit => "Too many storage layers",
		}
	}
}
...
impl traits::Printable for DispatchError {
	fn print(&self) {
		...
		match self {
			...
			Self::StorageLayersLimit => "Too many storage layers".print(),
		}
	}
}
