...
macro_rules! selected_benchmarks {
	([$($bench:ident),*], $run:expr) => {
		...
		impl<T: Trait> $crate::traits::Benchmarking<T, Call<T>, RawOrigin<T::AccountId>> for Module<T> {
			...
			fn run_benchmark(extrinsic: Vec<u8>, steps: u32, repeat: u32) -> Result<Vec<(Vec<(BenchmarkParameter, u32)>, u128)>, &'static str> {
				...
				let components = <Self::Setup as BenchmarkingSetup<T, Call<T>, RawOrigin<T::AccountId>>>::components(&benchmark);
				...
				for (name, low, high) in components.iter() {
					...
					for s in 0..num_of_steps {
						...
						for _ in 0..repeat {
							...
							let (call, caller) = <Self::Setup as BenchmarkingSetup<T, Call<T>, RawOrigin<T::AccountId>>>::instance(&benchmark, &c)?;
							$run(call, caller, c.clone(), &mut results)?;
						}
					}
				}
