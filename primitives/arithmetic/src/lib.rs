    ...
    T: Clone + Copy + Ord + BaseArithmetic + Unsigned + Debug,
{
    ...
    let count_t: T = count
        .try_into()
        .map_err(|_| "length of `inputs` cannot fit in `T`")?;
    ...
    if needs_bump {
        ...
        if !per_round.is_zero() {
            for _ in 0..count {
                output_with_idx[max_index].1 = output_with_idx[max_index]
                    .1
...
mod normalize_tests {
    ...
    fn works_for_vec() {
        assert_eq!(
            vec![8u32, 9, 7, 10].normalize(40).unwrap(),
            vec![10u32, 10, 10, 10]
        );
    }
...
mod threshold_compare_tests {
    ...
    fn saturating_mul_works() {
        assert_eq!(
            Saturating::saturating_mul(2, i32::min_value()),
            i32::min_value()
        );
        assert_eq!(
            Saturating::saturating_mul(2, i32::max_value()),
            i32::max_value()
        );
    }
    ...
    fn saturating_pow_works() {
        ...
        assert_eq!(
            Saturating::saturating_pow(i32::min_value(), 3),
            i32::min_value()
        );
        assert_eq!(
            Saturating::saturating_pow(i32::min_value(), 2),
            i32::max_value()
        );
        assert_eq!(
            Saturating::saturating_pow(i32::max_value(), 2),
            i32::max_value()
        );
    }
}
