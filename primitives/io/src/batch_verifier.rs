...
use futures::{channel::oneshot, future::FutureExt, task::FutureObj};
use sp_core::{crypto::Pair, ed25519, sr25519, traits::CloneableSpawn};
use std::sync::{
...
impl BatchVerifier {
    ...
    pub fn push_ed25519(
        ...
        if self
            .spawn_verification_task(move || ed25519::Pair::verify(&signature, &message, &pub_key))
            .is_err()
        {
            log::debug!(
                target: "runtime",
    ...
    pub fn verify_and_clear(&mut self) -> bool {
        ...
        if pending.len() > 0 {
            ...
            let mtx = mtx
                .lock()
                .expect("Locking can only fail when the mutex is poisoned; qed");
            let _ = cond_var
                .wait(mtx)
                .expect("Waiting can only fail when the mutex waited on is poisoned; qed");
        }
