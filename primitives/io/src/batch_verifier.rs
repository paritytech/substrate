...
use sp_core::{crypto::Pair, ecdsa, ed25519, sr25519, traits::CloneableSpawn};
use std::sync::{
...
impl BatchVerifier {
    ...
    pub fn push_sr25519(
        ...
        if self.sr25519_items.len() >= 128 {
            ...
            if self
                .spawn_verification_task(move || Self::verify_sr25519_batch(items))
                .is_err()
            {
                log::debug!(
                    target: "runtime",
    ...
    pub fn push_ecdsa(
        ...
        if self
            .spawn_verification_task(move || ecdsa::Pair::verify(&signature, &message, &pub_key))
            .is_err()
        {
            log::debug!(
                target: "runtime",
