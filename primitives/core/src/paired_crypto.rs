...
impl<LeftPublic: PublicKeyBound, RightPublic: PublicKeyBound, const LEFT_PLUS_RIGHT_LEN: usize>  TryFrom<&[u8]> for Public<LeftPublic, RightPublic, LEFT_PLUS_RIGHT_LEN> {
     type Error = ();
...
impl<LeftSignature: SignatureBound, RightSignature: SignatureBound, const LEFT_PLUS_RIGHT_LEN: usize>  TryFrom<&[u8]> for Signature<LeftSignature, RightSignature, LEFT_PLUS_RIGHT_LEN> where Signature<LeftSignature, RightSignature, LEFT_PLUS_RIGHT_LEN>: SignaturePair {
     type Error = ();
...
#[cfg(feature = "serde")]
impl<'de,LeftSignature: SignatureBound, RightSignature: SignatureBound, const LEFT_PLUS_RIGHT_LEN: usize,> Deserialize<'de> for Signature<LeftSignature, RightSignature, LEFT_PLUS_RIGHT_LEN> where Signature<LeftSignature, RightSignature, LEFT_PLUS_RIGHT_LEN>: SignaturePair
{
	fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
	where
		D: Deserializer<'de>,
	{
		let signature_hex = array_bytes::hex2bytes(&String::deserialize(deserializer)?)
			.map_err(|e| de::Error::custom(format!("{:?}", e)))?;
		Signature::<LeftSignature, RightSignature, LEFT_PLUS_RIGHT_LEN>::try_from(signature_hex.as_ref())
			.map_err(|e| de::Error::custom(format!("Error in converting deserialized data into signature: {:?}", e)))
	}
}
    ...
    RightPair::Seed: From<[u8; SECURE_SEED_LEN]> + Into<[u8; SECURE_SEED_LEN]>,
{
	...
	fn derive<Iter: Iterator<Item = DeriveJunction>>(
        ...
        let (left_seed_option, right_seed_option) = match seed {
            Some(seed) => {
                let (left_seed, right_seed): (LeftPair::Seed, RightPair::Seed) = (seed.clone().into(), seed.into());
...
mod test {
    ...
    let seed_for_right_and_left = hex!("9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60");
    	...
    	assert_eq!(
    		derived.to_raw_vec(),
    		[hex!("b8eefc4937200a8382d00050e050ced2d4ab72cc2ef1b061477afb51564fdd61"), hex!("3a0626d095148813cd1642d38254f1cfff7eb8cc1a2fc83b2a135377c3554c12")].concat()
    	);
    }
    ...
    let seed_left_and_right = hex!("9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60");
    ...
    fn test_vector_by_string_should_work() {
        let pair = Pair::from_string("0x9d61b19deffd5a60ba844af492ec2cc44449c5697b326919703bac031cae7f60",
    		None,
    	)
    	.unwrap();
		let public = pair.public();
    ...
    fn seeded_pair_should_work() {
    	let pair = Pair::from_seed(&(b"12345678901234567890123456789012".as_slice().try_into().unwrap()));
    	let public = pair.public();
    	assert_eq!(
    		public,
			Public::unchecked_from(
