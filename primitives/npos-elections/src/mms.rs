...
use crate::{
	...
	IdentifierT, PerThing128, Rc, Vec, VoteWeight, Voter, Zero,
};
...
/// - `config` contains `iterations` and `tolerance`, that represent the number of iterations to
///   perform while balancing, and the tolerance of the balancing, respectively.
///
...
pub fn mms<AccountId: IdentifierT, P: PerThing128>(
	...
	for _round in 0..to_elect {
		...
		for (index, candidate) in candidates.iter().enumerate() {
			...
			let support =
				calculate_support(winners.clone(), (*candidate).clone(), voters.clone(), config);
	...
	let (used_candidates, used_voters) = setup_inputs_with_balance(winners, voters, config);
...
fn setup_inputs_with_balance<AccountId: IdentifierT>(
	...
	balance(&mut used_voters, config);
...
fn calculate_support<AccountId: IdentifierT>(
	...
	let (used_candidates, _) = setup_inputs_with_balance(candidates, voters, config);
...
mod tests {
	...
	fn basic_election_works() {
		...
		let ElectionResult::<_, Perbill> { winners, assignments } =
			mms(2, candidates, voters, &config).unwrap();
	...
	fn linear_voting_example_works() {
		...
		let ElectionResult::<_, Perbill> { winners, assignments: _ } =
			mms(4, candidates, voters, &config).unwrap();
	...
	fn paper_example_works() {
		...
		let config = BalancingConfig { iterations: 2, tolerance: 0 };
		let ElectionResult::<_, Perbill> { winners, assignments: _ } =
			mms(3, candidates, voters, &config).unwrap();
	...
	fn mms_solution_house_monotonic() {
		...
		let config = BalancingConfig { iterations: 2, tolerance: 0 };
		let ElectionResult::<_, Perbill> { winners, assignments: _ } =
			mms(2, candidates.clone(), voters.clone(), &config).unwrap();
	...
	fn mms_not_strong_monotonicity() {
		...
		let config = BalancingConfig { iterations: 2, tolerance: 0 };
		let ElectionResult::<_, Perbill> { winners, assignments: _ } =
			mms(6, candidates.clone(), voters.clone(), &config).unwrap();
		...
		let ElectionResult::<_, Perbill> { winners, assignments: _ } =
			mms(6, candidates, voters, &config).unwrap();
	...
	fn mms_not_pjr() {
		...
		let config = BalancingConfig { iterations: 20, tolerance: 0 };
		let ElectionResult::<_, Perbill> { winners, assignments: _ } =
			mms(4, candidates, voters, &config).unwrap();
