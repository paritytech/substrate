...
fn voter_normalize_ops_works() {
    ...
    // normalize
    {
        ...
        assert_eq!(
            v.edges.iter().map(|e| e.weight).collect::<Vec<_>>(),
            vec![34, 33, 33]
        );
    }
    // // normalize_elected
...
fn phragmen_large_scale_test_2() {
    ...
    assert_eq_uvec!(
        winners,
        vec![(2, 500000000005000000u128), (4, 500000000003000000)]
    );
...
mod solution_type {
    ...
    fn remove_voter_works() {
        let mut compact = TestSolutionCompact {
            ...
            votes3: vec![(
                4,
                [
                    (3, TestAccuracy::from_percent(50)),
                    (4, TestAccuracy::from_percent(25)),
                ],
                5,
            )],
            ..Default::default()
    ...
    fn basic_from_and_into_compact_works_assignments() {
        ...
        let voter_at = |a: u32| -> Option<AccountId> {
            voters
                .get(<u32 as TryInto<usize>>::try_into(a).unwrap())
                .cloned()
        };
        let target_at = |a: u8| -> Option<AccountId> {
            targets
                .get(<u8 as TryInto<usize>>::try_into(a).unwrap())
                .cloned()
        };
    ...
    fn compact_into_assignment_must_report_overflow() {
        ...
        let compact = TestSolutionCompact {
            ...
            votes3: vec![(
                0,
                [
                    (1, TestAccuracy::from_percent(70)),
                    (2, TestAccuracy::from_percent(80)),
                ],
                3,
            )],
            ..Default::default()
