...
use sp_npos_elections::{
    assignment_ratio_to_staked, assignment_ratio_to_staked, balance_solution, seq_phragmen,
    to_support_map, to_without_backing, Assignment, ExtendedBalance, IdentifierT, StakedAssignment,
    VoteWeight,
};
...
fn do_phragmen(
    ...
    b.iter(|| {
        let ElectionResult {
            winners,
            assignments,
        } = seq_phragmen::<AccountId, Perbill>(
            to_elect,
            Zero::zero(),
            candidates.clone(),
            voters.clone(),
