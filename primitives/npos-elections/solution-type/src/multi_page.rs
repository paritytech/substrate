...
fn voter_count_impl(voter_pages: u8) -> TokenStream2 {
...
fn edge_count_impl(voter_pages: u8) -> TokenStream2 {
	let per_page = (0..voter_pages).map(|i| {
		let filed = page_field(i);
...
fn unique_targets_impl(voter_pages: u8) -> TokenStream2 {
	let per_page = (0..voter_pages).map(|i| {
		let filed = page_field(i);
...
fn remove_voter_impl_per_page(voter_pages: u8) -> TokenStream2 {
	(0..voter_pages).map(|i| {
		let filed = page_field(i);
