...
pub trait CryptoStore: Send + Sync {
    ...
    /// Returns all redjubjub public keys for the given key type.
    async fn redjubjub_public_keys(&self, id: KeyTypeId) -> Vec<redjubjub::Public>;
    /// Generate a new redjubjub key pair for the given key type and an optional seed.
    ///
    /// If the given seed is `Some(_)`, the key pair will only be stored in memory.
    ///
    /// Returns the public key of the generated key pair.
    ...
    async fn sign_with_any(
        ...
        if keys.len() == 1 {
            return self
                .sign_with(id, &keys[0], msg)
                .await
                .map(|s| (keys[0].clone(), s));
        } else {
...
pub trait SyncCryptoStore: CryptoStore + Send + Sync {
    ...
    fn sign_with_any(
        ...
        if keys.len() == 1 {
            return SyncCryptoStore::sign_with(self, id, &keys[0], msg)
                .map(|s| (keys[0].clone(), s));
        } else {
    ...
    fn sign_with_all(
        ...
        Ok(keys
            .iter()
            .map(|k| SyncCryptoStore::sign_with(self, id, k, msg))
            .collect())
    }
