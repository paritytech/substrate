...
use sp_phragmen::{
    assignment_ratio_to_staked, build_support_map, elect, equalize, evaluate_support,
    is_score_better, to_without_backing, PhragmenResult, VoteWeight,
};
...
fn generate_random_phragmen_result(
    ...
    PhragmenResult<AccountId, Perbill>,
    ...
    (
        elect::<AccountId, sp_runtime::Perbill>(to_elect, 0, candidates, voters).unwrap(),
        stake_of_tree,
    )
}
...
fn main() {
    loop {
        fuzz!(|data: (usize, usize, usize, usize, usize, u64)| {
            ...
            let i = equalize(&mut staked, &mut support, 10, iterations);
            ...
            println!(
                "iter = {} // {:?} -> {:?} [{}]",
                i, initial_score, final_score, enhance,
            );
