...
pub fn generate_compact_solution_type(item: TokenStream) -> TokenStream {
    let CompactSolutionDef { vis, ident, count } =
    ...
    let staked_impls = staked::staked(ident, voter_type, target_type, count);
