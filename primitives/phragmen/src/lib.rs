...
use sp_runtime::traits::{Bounded, Member, Saturating, Zero};
use sp_runtime::{
    helpers_128bit::multiply_by_rational, PerThing, Rational128, RuntimeDebug, SaturatedConversion,
};
use sp_std::{
    cmp::Ordering, collections::btree_map::BTreeMap, convert::TryFrom, fmt::Debug, prelude::*,
};
                ...
                let per_bill_parts: R::Inner = {
                    if n.load == e.load {
                        ...
                        if e.load.d() == n.load.d() {
                            ...
                            let parts = multiply_by_rational(desired_scale, e.load.n(), n.load.n())
    ...
    AccountId: Ord + Clone,
{
    ...
    loop {
        ...
        for assignment in assignments.iter_mut() {
            ...
            let diff = do_equalize(who, voter_budget, distribution, supports, tolerance);
            if diff > max_diff {
                max_diff = diff;
            }
        }
