...
use crate::mock::*;
use crate::{
    build_support_map, elect, equalize, helpers::*, is_score_better, Assignment, ExtendedBalance,
    PhragmenResult, StakedAssignment, Support,
};
use sp_runtime::{PerU16, Perbill, Percent, Permill};
...
fn phragmen_poc_works() {
    ...
    assert_eq!(
        *support_map.get(&3).unwrap(),
        Support::<AccountId> {
            total: 35,
            voters: vec![(20, 20), (30, 15)]
        },
    );

    equalize(&mut staked, &mut support_map, 0, 2);
...
fn self_votes_should_be_kept() {
    ...
    assert_eq!(
        ...
        vec![
            Assignment {
                who: 10,
                distribution: vec![(10, Perbill::from_percent(100))]
            },
            Assignment {
                who: 20,
    ...
    let (mut supports, _) = build_support_map::<AccountId>(&winners, &staked_assignments);
    ...
    assert_eq!(
        ...
        &Support {
            total: 14u128,
            voters: vec![(10u64, 10u128), (1u64, 4u128)]
        },
    );
    assert_eq!(
        ...
        &Support {
            total: 24u128,
            voters: vec![(20u64, 20u128), (1u64, 4u128)]
        },
    );

    equalize(&mut staked_assignments, &mut supports, 0, 2usize);

    assert_eq!(
        supports.get(&10u64).unwrap(),
        &Support {
            total: 18u128,
            voters: vec![(10u64, 10u128), (1u64, 8u128)]
        },
    );
    assert_eq!(
        supports.get(&20u64).unwrap(),
...
fn assignment_convert_works() {
    ...
    assert_eq!(assignment.into_staked(125, true), staked,);
}

#[test]
fn score_comparison_is_lexicographical() {
    // only better in the fist parameter, worse in the other two ✅
    assert_eq!(is_score_better([10, 20, 30], [12, 10, 35]), true,);

    // worse in the first, better in the other two ❌
    assert_eq!(is_score_better([10, 20, 30], [9, 30, 10]), false,);

    // equal in the first, the second one dictates.
    assert_eq!(is_score_better([10, 20, 30], [10, 25, 40]), true,);

    // equal in the first two, the last one dictates.
    assert_eq!(is_score_better([10, 20, 30], [10, 20, 40]), false,);
}
...
mod compact {
    ...
    use sp_phragmen::{Assignment, Error as PhragmenError, ExtendedBalance, StakedAssignment};
        ...
        <T as TryInto<usize>>::Error: std::fmt::Debug,
    {
        ...
        assert_eq!(
            ...
            TestCompact {
                ...
                votes2: vec![
                    (
                        V::from(2u8),
                        (T::from(0u8), Accuracy::from_percent(80)),
                        T::from(1u8)
                    ),
                    (
                        V::from(3u8),
                        (T::from(7u8), Accuracy::from_percent(85)),
                        T::from(8u8)
                    ),
                ],
                votes3: vec![(
                    V::from(4),
                    [
                        (T::from(3u8), Accuracy::from_percent(50)),
                        (T::from(4u8), Accuracy::from_percent(25))
                    ],
                    T::from(5u8),
                ),],
                ..Default::default()
        ...
        let voter_at = |a: V| -> Option<AccountId> {
            voters
                .get(<V as TryInto<usize>>::try_into(a).unwrap())
                .cloned()
        };
        let target_at = |a: T| -> Option<AccountId> {
            targets
                .get(<T as TryInto<usize>>::try_into(a).unwrap())
                .cloned()
        };
    ...
    fn basic_from_and_into_compact_works_staked_assignments() {
        let voters = vec![2 as AccountId, 4, 1, 5, 3];
        let targets = vec![10 as AccountId, 11, 20, 30, 31, 32, 40, 50, 51];
        ...
        let assignments = vec![
            ...
            StakedAssignment {
                who: 1,
                distribution: vec![(10, 80), (11, 20)],
            },
            StakedAssignment {
                who: 5,
                distribution: vec![(50, 85), (51, 15)],
            },
            StakedAssignment {
                who: 3,
                distribution: vec![(30, 50), (31, 25), (32, 25)],
            },
        ];
        ...
        assert_eq!(
            ...
            TestCompact {
                votes1: vec![(0, 2), (1, 6)],
                votes2: vec![(2, (0, 80), 1), (3, (7, 85), 8),],
                votes3: vec![(4, [(3, 50), (4, 25)], 5),],
                ..Default::default()
            }
        );
        ...
        assert_eq!(
            compacted
                .into_staked(max_of_fn, voter_at, target_at,)
                .unwrap(),
            assignments,
        );
    }
    ...
    fn compact_into_stake_must_report_overflow() {
        ...
        assert_eq!(
            compact
                .into_staked(&max_of, &entity_at, &entity_at)
                .unwrap_err(),
    ...
    fn compact_into_assignment_must_report_overflow() {
        ...
        let compact = TestCompact::<u16, u16, Accuracy> {
            ...
            votes3: vec![(
                0,
                [
                    (1, Accuracy::from_percent(70)),
                    (2, Accuracy::from_percent(80)),
                ],
                3,
            )],
            ..Default::default()
    ...
    fn target_count_overflow_is_detected() {
        let assignments = vec![StakedAssignment {
            who: 1 as AccountId,
            distribution: (10..26)
                .map(|i| (i as AccountId, i as ExtendedBalance))
                .collect::<Vec<_>>(),
        }];
