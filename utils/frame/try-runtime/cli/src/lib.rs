...
//! and after the migration. A `Vec<u8>` is generated by `pre_upgrade` as a encoded state digest
//! which will be passed to `post_upgrade` after upgrading for post checking. NOTE: `post_upgrade`
//! should decode it into the same type as `pre_upgrade` encoded before using it.
//!
//! #### Logging
//!
