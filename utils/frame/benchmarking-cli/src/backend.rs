...
use sc_client_db::storage_cache::{CachingState, SharedCache, new_shared_cache};
...
pub struct BenchmarkingState<B: BlockT> {
	...
	read_write_tracker: Cell<ReadWriteTracker>,
}
...
impl<B: BlockT> BenchmarkingState<B> {
	...
	fn add_read_key(&self, key: &[u8]) {
		let mut key_tracker = self.key_tracker.get_mut();
		let mut read_write_tracker = self.read_write_tracker.get_mut();
