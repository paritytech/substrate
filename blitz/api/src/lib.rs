...
//! Strongly typed API for Blitz based around the locally-compiled native
//! runtime.
...
error_chain! {
	errors {
		...
		/// Some other error.
		// TODO: allow to be specified as associated type of BlitzApi
...
/// Trait encapsulating the Blitz API.
///
...
/// Mark for all Blitz API implementations, that are making use of state data, stored locally.
pub trait LocalBlitzApi: BlitzApi {}

/// Mark for all Blitz API implementations, that are fetching required state data from remote nodes.
pub trait RemoteBlitzApi: BlitzApi {}
