...
extern crate blitz_transaction_pool as transaction_pool;
...
use blitz_api::PolkadotApi;
use blitz_primitives::{Hash, Block, BlockId, BlockNumber, Header, Timestamp, SessionKey};
...
impl<N, A> bft::Environment<Block> for ProposerFactory<N, P>
	where
		N: Network,
		A: BlitzApi + Send + Sync + 'static,
		N::TableRouter: Send + 'static,
{
	type Proposer = Proposer<A>;
	type Input = N::Input;
...
impl<C> bft::Proposer<Block> for Proposer<C> where
