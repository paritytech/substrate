...
use blitz_api::{BlitzApi, LocalBlitzApi};
use blitz_consensus::Network;
use blitz_primitives::{AccountId, Block, Hash, SessionKey};
use codec::Decode;
...
impl<P: LocalPolkadotApi + Send + Sync + 'static> Network for ConsensusNetwork<P> {
	...
	fn communication_for(&self, validators: &[SessionKey], task_executor: TaskExecutor) -> (Self::Input, Self::Output) {
