...
extern crate blitz_service as service;
...
pub trait Worker: IntoExit {
	...
	/// Return configuration for the blitz node.
	...
	W: Worker,
{
	...
	match cli::prepare_execution::<service::Factory, _, _, _, _>(args, worker, version, load_spec, "parity-blitz")? {
		cli::Action::ExecutedInternally => (),
		cli::Action::RunService((mut config, worker)) => {
			info!("Parity ·:· Blitz Demo");
			info!("  version {}", config.full_version());
			info!("  by Parity Technologies, 2018");
