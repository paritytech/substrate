...
pub fn init_telemetry(config: TelemetryConfig) -> slog_scope::GlobalLoggerGuard {
    ...
    config.endpoints.0.iter().for_each(|(url, verbosity)| {
        ...
        let filter = slog::Filter(slog_json::Json::default(writer).fuse(), move |rec| {
            let tag = rec
                .tag()
                .parse::<u8>()
    ...
    config.endpoints.0.into_iter().for_each(|(url, verbosity)| {
        ...
        thread::spawn(move || {
            loop {
                ...
                let _ = ws::connect(url.to_owned(), |out| {
                    Connection::new(
                        out,
                        Arc::clone(&out_sync),
                ...
                thread::sleep(
                    time::Duration::from_secs(5) + time::Duration::from_secs(random_sleep),
                );
            }
        });
    });
...
impl io::Write for TelemetryWriter {
    fn write(&mut self, msg: &[u8]) -> io::Result<usize> {
        ...
        let first = iter
            .next()
            .expect("Split iterator always has at least one element; qed");
