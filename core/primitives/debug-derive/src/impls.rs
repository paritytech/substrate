...
use syn::{Ident, Index, Data, DeriveInput, parse_quote};

pub fn debug_derive(ast: DeriveInput) -> proc_macro::TokenStream {
	let name_str = ast.ident.to_string();
	let implementation = derive(&name_str, &ast.data);
	let name = &ast.ident;
...
///
/// In non-std environment we do nothing to prevent bloating the size of runtime.
/// Implement `Printable` if you need to print the details.
pub fn derive(name_str: &str, data: &Data) -> TokenStream {
	if std::env::var("CARGO_CFG_STD").is_err() {
		return quote! {
			let _ = fmt;
			Ok(())
		}
