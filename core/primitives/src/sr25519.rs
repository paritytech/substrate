...
#[cfg(feature = "std")]
use crate::crypto::{
    Derive, DeriveJunction, Infallible, Pair as TraitPair, SecretStringError, Ss58Codec,
};
use crate::{
...
mod test {
    ...
    fn derive_soft_should_work() {
        ...
        let derive_1 = pair
            .derive(Some(DeriveJunction::soft(1)).into_iter())
            .unwrap();
        let derive_1b = pair
            .derive(Some(DeriveJunction::soft(1)).into_iter())
            .unwrap();
        let derive_2 = pair
            .derive(Some(DeriveJunction::soft(2)).into_iter())
            .unwrap();
    ...
    fn derive_hard_should_work() {
        ...
        let derive_1 = pair
            .derive(Some(DeriveJunction::hard(1)).into_iter())
            .unwrap();
        let derive_1b = pair
            .derive(Some(DeriveJunction::hard(1)).into_iter())
            .unwrap();
        let derive_2 = pair
            .derive(Some(DeriveJunction::hard(2)).into_iter())
            .unwrap();
        assert_eq!(derive_1.public(), derive_1b.public());
        assert_ne!(derive_1.public(), derive_2.public());
    }
