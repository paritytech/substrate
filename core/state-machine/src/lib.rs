...
use std::{fmt, marker::PhantomData, panic::UnwindSafe, result};
...
impl ExecutionStrategy {
    ...
    pub fn get_manager<E: std::fmt::Debug, R: Decode + Encode>(
        self,
...
impl<'a, H, B, T, O, Exec> StateMachine<'a, H, B, T, O, Exec>
where
    H: Hasher,
    Exec: CodeExecutor<H>,
    B: Backend<H>,
        ...
        ) -> CallResult<R, Exec::Error>,
    {
        ...
        let result = {
            ...
            let (result, storage_delta, changes_delta) = match manager {
                ...
                ExecutionManager::NativeWhenPossible => {
                    let (result, _was_native, storage_delta, changes_delta) =
...
pub(crate) fn set_changes_trie_config(
    ...
    let config = match config {
        Some(v) => Some(Decode::decode(&mut &v[..]).ok_or_else(|| {
