...
use primitives::{
	...
	subtrie::{SubTrie, SubTrieReadRef},
};
	...
	H::Out: Ord
{
	let trie_backend = backend.try_into_trie_backend()
		.ok_or_else(|| Box::new(ExecutionError::UnableToGenerateProof) as Box<Error>)?;
	prove_child_read_on_trie_backend(&trie_backend, subtrie, key)
}
...
{
	let proving_backend = proving_backend::ProvingBackend::<_, H>::new(trie_backend);
	let result = proving_backend.child_storage(subtrie, key)
...
where
	...
	H::Out: Ord
{
	proving_backend.child_storage(subtrie, key).map_err(|e| Box::new(e) as Box<Error>)
}
...
mod tests {
	...
	fn set_child_storage_works() {
		...
		let subtrie = SubTrie::new(&mut TestKeySpaceGenerator::new(), b"testchild");
	...
	fn child_storage_keyspace() {
		...
		let subtrie1 = SubTrie::new(&mut ks_gen, &[0x01]);
