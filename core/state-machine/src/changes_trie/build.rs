...
use crate::changes_trie::input::{DigestIndex, ExtrinsicIndex, InputKey, InputPair};
    ...
    H::Out: HeapSizeOf,
{
    ...
    input.extend(prepare_digest_input::<_, H>(parent, config, storage)?);
    ...
    H: Hasher,
{
    ...
    Ok(extrinsic_map.into_iter().map(move |(key, extrinsics)| {
        InputPair::ExtrinsicIndex(
            ExtrinsicIndex { block, key },
    ...
    H::Out: 'a + HeapSizeOf,
{
    ...
    for digest_build_block in digest_build_iterator(config, parent.number + 1) {
        ...
        let trie_root = trie_root
            .ok_or_else(|| format!("No changes trie root for block {}", digest_build_block))?;
    ...
    Ok(digest_map.into_iter().map(move |(key, set)| {
...
mod test {
    ...
    fn prepare_for_build() -> (
        ...
        let changes = OverlayedChanges {
            ...
            .into_iter()
            .collect(),
            changes_trie_config: Some(Configuration {
                digest_interval: 4,
                digest_levels: 2,
            }),
        };
    ...
    fn build_changes_trie_nodes_ignores_temporary_storage_values() {
        ...
        changes.prospective.top.insert(
            vec![110],
            OverlayedValue {
                value: None,
                extrinsics: Some(vec![1].into_iter().collect()),
