...
fn pruning_range(
    ...
    let (prune_interval, blocks_to_keep) = if config.is_digest_build_enabled() {
        ...
        let max_digest_intervals_to_keep =
            max_digest_intervals_to_keep(min_blocks_to_keep, max_digest_interval);
    ...
    let first_block_to_prune = last_block_to_prune
        .clone()
        .and_then(|b| b.checked_sub(prune_interval));

    last_block_to_prune.and_then(|last| first_block_to_prune.map(|first| (first + 1, last)))
}
...
mod tests {
    ...
    fn prune_works() {
        fn prepare_storage() -> InMemoryStorage<Blake2Hasher> {
            ...
            let root1 =
                insert_into_memory_db::<Blake2Hasher, _>(&mut mdb1, vec![(vec![10], vec![20])])
                    .unwrap();
            ...
            let root4 =
                insert_into_memory_db::<Blake2Hasher, _>(&mut mdb4, vec![(vec![15], vec![25])])
                    .unwrap();
    ...
    fn pruning_range_works() {
        ...
        assert_eq!(
            pruning_range(&config(::std::u64::MAX, 2), ::std::u64::MAX, 1024),
            None
        );
        ...
        assert_eq!(
            pruning_range(&config(32, 2), 4096, 6144),
            Some((1025, 2048))
        );
    }
