...
use std::sync::Arc;
use trie::{
    default_child_trie_root, for_keys_in_child_trie, read_child_trie_value, read_trie_value,
    DBValue, MemoryDB, PrefixedMemoryDB, Trie, TrieDB, TrieError,
};
    ...
    H::Out: HeapSizeOf,
{
    ...
    pub fn new(storage: S, root: H::Out) -> Self {
        TrieBackendEssence { storage, root }
    }
    ...
    pub fn child_storage(&self, storage_key: &[u8], key: &[u8]) -> Result<Option<Vec<u8>>, String> {
        let root = self
            .storage(storage_key)?
            .unwrap_or(default_child_trie_root::<H>(storage_key));
...
where
    H::Out: HeapSizeOf,
{
    fn as_plain_db<'b>(&'b self) -> &'b (hash_db::PlainDB<H::Out, DBValue> + 'b) {
        self
    }
    fn as_plain_db_mut<'b>(&'b mut self) -> &'b mut (hash_db::PlainDB<H::Out, DBValue> + 'b) {
        self
    }
}
...
where
    H::Out: HeapSizeOf,
{
    fn as_hash_db<'b>(&'b self) -> &'b (hash_db::HashDB<H, DBValue> + 'b) {
        self
    }
    fn as_hash_db_mut<'b>(&'b mut self) -> &'b mut (hash_db::HashDB<H, DBValue> + 'b) {
        self
    }
}
...
{
    fn get(&self, key: &H::Out) -> Option<DBValue> {
        hash_db::PlainDB::get(self, key)
    }
    fn contains(&self, key: &H::Out) -> bool {
        hash_db::PlainDB::contains(self, key)
    }
}
...
impl<'a, S, H: Hasher> TrieBackendStorage<H> for &'a S
where
    S: ChangesTrieStorage<H>,
{
    type Overlay = MemoryDB<H>;
