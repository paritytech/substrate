...
use crate::utils::{
    block_id_to_lookup_key, db_err, meta_keys, open_database, read_db, read_meta, Meta,
};
...
use primitives::{convert_hash, Blake2Hasher, ChangesTrieConfiguration, H256};
use runtime_primitives::traits::{
    As, Block as BlockT, Digest, DigestItem, Header as HeaderT, NumberFor, Zero,
};
use runtime_primitives::BuildStorage;
use runtime_primitives::{generic::BlockId, ChildrenStorageOverlay, Justification, StorageOverlay};
pub use state_db::PruningMode;
use state_db::StateDb;
use state_machine::backend::Backend as StateBackend;
use state_machine::{CodeExecutor, DBValue};
use trie::{prefixed_key, MemoryDB, PrefixedMemoryDB};
...
pub type DbState =
    state_machine::TrieBackend<Arc<state_machine::Storage<Blake2Hasher>>, Blake2Hasher>;
...
pub fn new_client<E, S, Block, RA>(
    ...
    client::Client<Backend<Block>, client::LocalCallExecutor<Backend<Block>, E>, Block, RA>,
    client::error::Error,
>
where
...
impl<Block: BlockT> client::blockchain::Backend<Block> for BlockchainDb<Block> {
    ...
    fn justification(
        &self,
        id: BlockId<Block>,
    ) -> Result<Option<Justification>, client::error::Error> {
    ...
    Block: BlockT<Hash = H256>,
{
    ...
    fn update_db_storage(
        &mut self,
        update: PrefixedMemoryDB<Blake2Hasher>,
    ) -> Result<(), client::error::Error> {
    ...
    fn reset_storage(
        ...
        for (child_key, child_map) in children {
            ...
            let (root, is_default, update) = self
                .old_state
                .child_storage_root(&child_key, child_map.into_iter().map(|(k, v)| (k, Some(v))));
        ...
        let (root, update) = self
            .old_state
            .storage_root(top.into_iter().map(|(k, v)| (k, Some(v))));
        transaction.consolidate(update);
...
impl<Block: BlockT> client::backend::PrunableStateChangesTrieStorage<Blake2Hasher>
...
impl<Block: BlockT> state_machine::ChangesTrieRootsStorage<Blake2Hasher>
    for DbChangesTrieStorage<Block>
{
    fn root(
        &self,
        anchor: &state_machine::ChangesTrieAnchorBlockId<H256>,
        block: u64,
        ...
        let block_id = if block <= self.meta.read().finalized_number.as_() {
            ...
            let maybe_anchor_header: Block::Header = utils::require_header::<Block>(
                ...
                columns::KEY_LOOKUP,
                columns::HEADER,
                BlockId::Number(As::sa(current_num)),
            )
...
impl<Block: BlockT> state_machine::ChangesTrieStorage<Blake2Hasher>
    for DbChangesTrieStorage<Block>
{
    fn get(&self, key: &H256, _prefix: &[u8]) -> Result<Option<DBValue>, String> {
        self.db
            .get(columns::CHANGES_TRIE, &key[..])
...
impl<Block: BlockT<Hash = H256>> Backend<Block> {
    ...
    fn from_kvdb(
        ...
        let map_e = |e: state_db::Error<io::Error>| {
    ...
    pub fn as_in_memory(&self) -> InMemoryBackend<Block, Blake2Hasher> {
        ...
        for (number, hash, header) in headers {
            ...
            op.set_block_data(header, body, justification, new_block_state)
                .unwrap();
            op.update_db_storage(state.into_iter().map(|(k, v)| (None, k, Some(v))).collect())
                .unwrap();
        ...
        inmem
            .finalize_block(BlockId::Hash(info.finalized_hash), None)
            .unwrap();
    ...
    fn set_head_with_transaction(
    ...
    ) -> Result<(Vec<Block::Hash>, Vec<Block::Hash>), client::error::Error> {
        ...
        utils::insert_number_to_key_mapping(transaction, columns::KEY_LOOKUP, best_to.0, best_to.1);
    ...
    fn ensure_sequential_finalization(
        ...
        if *header.parent_hash() != last_finalized {
            return Err(
                ::client::error::ErrorKind::NonSequentialFinalization(format!(
    ...
    ) -> Result<(Block::Hash, <Block::Header as HeaderT>::Number, bool, bool), client::error::Error>
    {
        ...
        self.note_finalized(transaction, header, *hash, finalization_displaced)?;
    ...
    fn try_commit_operation(
        &self,
        mut operation: BlockImportOperation<Block, Blake2Hasher>,
    ) -> Result<(), client::error::Error> {
        ...
        let imported = if let Some(pending_block) = operation.pending_block {
            ...
            self.changes_tries_storage
                .commit(&mut transaction, changes_trie_updates);
        ...
        if let Some(set_head) = operation.set_head {
            if let Some(header) =
                ::client::blockchain::HeaderBackend::header(&self.blockchain, set_head)?
            {
                let number = header.number();
                let hash = header.hash();
                ...
                return Err(client::error::ErrorKind::UnknownBlock(format!(
                    "Cannot set head {:?}",
                    set_head
                ))
                .into());
        ...
        Block: BlockT<Hash = H256>,
    {
            ...
            .unwrap_or(true)
        {
            ...
            let changes_trie_config: Option<ChangesTrieConfiguration> = self
                .state_at(BlockId::Hash(parent_hash))?
                .storage(well_known_keys::CHANGES_TRIE_CONFIG)?
                .and_then(|v| Decode::decode(&mut &*v));
            self.changes_tries_storage
                .prune(changes_trie_config, transaction, f_hash, f_num);
        }
...
impl<Block> client::backend::Backend<Block, Blake2Hasher> for Backend<Block>
where
    Block: BlockT<Hash = H256>,
{
        ...
        for c in 0..n.as_() {
            ...
            match self.storage.state_db.revert_one() {
                Some(commit) => {
                    ...
                    let removed =
                        self.blockchain
                            .header(BlockId::Number(best))?
                            .ok_or_else(|| {
                                client::error::ErrorKind::UnknownBlock(format!(
        ...
        match self.blockchain.header(block) {
            Ok(Some(ref hdr)) => {
                ...
                if !self.storage.state_db.is_pruned(&hash, hdr.number().as_()) {
                    ...
                    Err(client::error::ErrorKind::UnknownBlock(format!(
            ...
            Ok(None) => Err(client::error::ErrorKind::UnknownBlock(format!(
                "Unknown state for block {:?}",
                block
            ))
            .into()),
            Err(e) => Err(e),
...
impl<Block> client::backend::LocalBackend<Block, Blake2Hasher> for Backend<Block> where
    Block: BlockT<Hash = H256>
{
}
...
mod tests {
    ...
    fn prepare_changes(changes: Vec<(Vec<u8>, Vec<u8>)>) -> (H256, MemoryDB<Blake2Hasher>) {
        ...
        {
            let mut trie =
                TrieDBMut::<Blake2Hasher>::new(&mut changes_trie_update, &mut changes_root);
    ...
    fn insert_header(
        ...
        let digest = Digest {
            logs: vec![DigestItem::ChangesTrieRoot(changes_root)],
        };
        let header = Header {
            number,
            parent_hash,
    ...
    fn set_state_data() {
        ...
        let hash = {
            ...
            op.reset_storage(storage.iter().cloned().collect(), Default::default())
                .unwrap();
        ...
        let hash = {
            ...
            assert_eq!(
                backend
                    .storage
                    .db
                    .get(columns::STATE, key.as_bytes())
                    .unwrap()
                    .unwrap(),
                &b"hello"[..]
            );
    ...
    fn changes_trie_storage_works() {
        ...
        let check_changes =
            |backend: &Backend<Block>, block: u64, changes: Vec<(Vec<u8>, Vec<u8>)>| {
                let (changes_root, mut changes_trie_update) = prepare_changes(changes);
                let anchor = state_machine::ChangesTrieAnchorBlockId {
                    hash: backend
                        .blockchain()
                        .header(BlockId::Number(block))
                        .unwrap()
                        .unwrap()
                        .hash(),
                    number: block,
                ...
                assert_eq!(
                    backend.changes_tries_storage.root(&anchor, block),
                    Ok(Some(changes_root))
                );

                for (key, (val, _)) in changes_trie_update.drain() {
                    assert_eq!(
                        backend.changes_trie_storage().unwrap().get(&key, &[]),
                        Ok(Some(val))
                    );
                }
            };
    ...
    fn changes_trie_storage_works_with_forks() {
        ...
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 1),
            Ok(Some(changes1_root))
        );
        ...
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 3),
            Ok(Some(changes2_1_0_root))
        );
    ...
    fn changes_tries_with_digest_are_pruned_on_finalization() {
        ...
        let block0 = insert_header(
            ...
            0,
            Default::default(),
            vec![(b"key_at_0".to_vec(), b"val_at_0".to_vec())],
            Default::default(),
        );
        let block1 = insert_header(
            &backend,
            1,
            block0,
            vec![(b"key_at_1".to_vec(), b"val_at_1".to_vec())],
            Default::default(),
        );
        let block2 = insert_header(
            &backend,
            2,
            block1,
            vec![(b"key_at_2".to_vec(), b"val_at_2".to_vec())],
            Default::default(),
        );
        let block3 = insert_header(
            &backend,
            3,
            block2,
            vec![(b"key_at_3".to_vec(), b"val_at_3".to_vec())],
            Default::default(),
        );
        let block4 = insert_header(
            &backend,
            4,
            block3,
            vec![(b"key_at_4".to_vec(), b"val_at_4".to_vec())],
            Default::default(),
        );
        let block5 = insert_header(
            &backend,
            5,
            block4,
            vec![(b"key_at_5".to_vec(), b"val_at_5".to_vec())],
            Default::default(),
        );
        let block6 = insert_header(
            &backend,
            6,
            block5,
            vec![(b"key_at_6".to_vec(), b"val_at_6".to_vec())],
            Default::default(),
        );
        let block7 = insert_header(
            &backend,
            7,
            block6,
            vec![(b"key_at_7".to_vec(), b"val_at_7".to_vec())],
            Default::default(),
        );
        let block8 = insert_header(
            &backend,
            8,
            block7,
            vec![(b"key_at_8".to_vec(), b"val_at_8".to_vec())],
            Default::default(),
        );
        let block9 = insert_header(
            &backend,
            9,
            block8,
            vec![(b"key_at_9".to_vec(), b"val_at_9".to_vec())],
            Default::default(),
        );
        let block10 = insert_header(
            &backend,
            10,
            block9,
            vec![(b"key_at_10".to_vec(), b"val_at_10".to_vec())],
            Default::default(),
        );
        let block11 = insert_header(
            &backend,
            11,
            block10,
            vec![(b"key_at_11".to_vec(), b"val_at_11".to_vec())],
            Default::default(),
        );
        let block12 = insert_header(
            &backend,
            12,
            block11,
            vec![(b"key_at_12".to_vec(), b"val_at_12".to_vec())],
            Default::default(),
        );
        let block13 = insert_header(
            &backend,
            13,
            block12,
            vec![(b"key_at_13".to_vec(), b"val_at_13".to_vec())],
            Default::default(),
        );
        ...
        fn read_changes_trie_root(backend: &Backend<Block>, num: u64) -> H256 {
            backend
                ...
                .logs()
                .iter()
                .find(|i| i.as_changes_trie_root().is_some())
                .unwrap()
        ...
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 1).unwrap(),
            Some(root1)
        );
        let root2 = read_changes_trie_root(&backend, 2);
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 2).unwrap(),
            Some(root2)
        );
        let root3 = read_changes_trie_root(&backend, 3);
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 3).unwrap(),
            Some(root3)
        );
        let root4 = read_changes_trie_root(&backend, 4);
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 4).unwrap(),
            Some(root4)
        );
        let root5 = read_changes_trie_root(&backend, 5);
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 5).unwrap(),
            Some(root5)
        );
        let root6 = read_changes_trie_root(&backend, 6);
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 6).unwrap(),
            Some(root6)
        );
        let root7 = read_changes_trie_root(&backend, 7);
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 7).unwrap(),
            Some(root7)
        );
        let root8 = read_changes_trie_root(&backend, 8);
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 8).unwrap(),
            Some(root8)
        );
        let root9 = read_changes_trie_root(&backend, 9);
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 9).unwrap(),
            Some(root9)
        );
        let root10 = read_changes_trie_root(&backend, 10);
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 10).unwrap(),
            Some(root10)
        );
        let root11 = read_changes_trie_root(&backend, 11);
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 11).unwrap(),
            Some(root11)
        );
        let root12 = read_changes_trie_root(&backend, 12);
        assert_eq!(
            backend.changes_tries_storage.root(&anchor, 12).unwrap(),
            Some(root12)
        );
        ...
        backend
            .changes_tries_storage
            .prune(Some(config.clone()), &mut tx, Default::default(), 12);
        ...
        backend
            .changes_tries_storage
            .prune(Some(config.clone()), &mut tx, Default::default(), 16);
        backend.storage.db.write(tx).unwrap();
        assert!(backend
            .changes_tries_storage
            .get(&root5, &[])
        ...
        backend
            .changes_tries_storage
            .prune(Some(config), &mut tx, Default::default(), 20);
        backend.storage.db.write(tx).unwrap();
        assert!(backend
            .changes_tries_storage
            .get(&root9, &[])
    ...
    fn changes_tries_without_digest_are_pruned_on_finalization() {
        ...
        backend
            .changes_tries_storage
            .prune(Some(config.clone()), &mut tx, block5, 5);
        backend.storage.db.write(tx).unwrap();
        assert!(backend
            .changes_tries_storage
            .get(&root1, &[])
        ...
        backend
            .changes_tries_storage
            .prune(Some(config.clone()), &mut tx, block6, 6);
        backend.storage.db.write(tx).unwrap();
        assert!(backend
            .changes_tries_storage
            .get(&root2, &[])
    ...
    fn test_leaves_with_complex_block_tree() {
        let backend: Arc<Backend<test_client::runtime::Block>> =
    ...
    fn test_finalize_block_with_justification() {
        ...
        assert_eq!(
            backend
                .blockchain()
                .justification(BlockId::Number(1))
                .unwrap(),
            justification,
        );
    }
