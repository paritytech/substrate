...
pub fn number_index_key<N>(n: N) -> NumberIndexKey
where
    N: As<u64>,
{
    let n: u64 = n.as_();
...
pub fn number_and_hash_to_lookup_key<N, H>(number: N, hash: H) -> Vec<u8>
where
    N: As<u64>,
    H: AsRef<[u8]>,
{
    ...
    N: As<u64>,
{
    ...
    Ok((key[0] as u64) << 24 | (key[1] as u64) << 16 | (key[2] as u64) << 8 | (key[3] as u64))
        .map(As::sa)
}
...
pub fn read_header<Block: BlockT>(
    ...
    match read_db(db, col_index, col, id)? {
        Some(header) => match Block::Header::decode(&mut &header[..]) {
            ...
            None => {
                return Err(client::error::ErrorKind::Backend("Error decoding header".into()).into());
            }
        },
        None => Ok(None),
    }
...
pub fn require_header<Block: BlockT>(
    ...
    read_header(db, col_index, col, id).and_then(|header| {
        header.ok_or_else(|| client::error::ErrorKind::UnknownBlock(format!("{}", id)).into())
    })
}
    ...
    Block: BlockT,
{
    let genesis_hash: Block::Hash =
        match db.get(col_meta, meta_keys::GENESIS_HASH).map_err(db_err)? {
            Some(h) => match Decode::decode(&mut &h[..]) {
                Some(h) => h,
                None => {
                    return Err(client::error::ErrorKind::Backend(
                        "Error decoding genesis hash".into(),
                    )
                    .into());
                }
            },
