...
pub(crate) mod tests {
	...
	fn authorites_are_cached() {
		...
		let (hash2, hash6) = {
			...
			let hash2 = insert_final_block(&db, Some(vec![auth1()]), || default_header(&hash1, 2));
			run_checks(&db, 2, &checks);
			let hash3 = insert_final_block(&db, Some(vec![auth1()]), || default_header(&hash2, 3));
			run_checks(&db, 3, &checks);
			let hash4 = insert_final_block(&db, Some(vec![auth1(), auth2()]), || default_header(&hash3, 4));
			run_checks(&db, 4, &checks);
			let hash5 = insert_final_block(&db, Some(vec![auth1(), auth2()]), || default_header(&hash4, 5));
...

		{
			...
			let hash2_1 = insert_non_best_block(&db, Some(vec![auth1()]), || default_header(&hash2, 3));
			assert_eq!(None, db.cache().authorities_at(BlockId::Hash(hash2_1)));
			let hash2_2 = insert_non_best_block(&db, Some(vec![auth1(), auth2()]), || default_header(&hash2_1, 4));
		...
		let (hash7, hash8, hash6_1, hash6_2, hash6_1_1, hash6_1_2) = {
			...
			let hash7 = insert_block(&db, Some(vec![auth3()]), || default_header(&hash6, 7));
			assert_eq!(
			...
			let hash8 = insert_block(&db, Some(vec![auth3()]), || default_header(&hash7, 8));
			assert_eq!(
				db.cache().authorities_at(BlockId::Hash(hash6)),
				Some(vec![auth1(), auth2()]),
			);
			...
			let hash6_1 = insert_block(&db, Some(vec![auth4()]), || default_header(&hash6, 7));
			assert_eq!(
				db.cache().authorities_at(BlockId::Hash(hash6)),
				Some(vec![auth1(), auth2()]),
			);
			...
			let hash6_1_1 = insert_non_best_block(&db, Some(vec![auth5()]), || default_header(&hash6_1, 8));
			assert_eq!(
				db.cache().authorities_at(BlockId::Hash(hash6)),
				Some(vec![auth1(), auth2()]),
			);
			...
			let hash6_1_2 = insert_non_best_block(&db, Some(vec![auth6()]), || default_header(&hash6_1, 8));
			assert_eq!(
				db.cache().authorities_at(BlockId::Hash(hash6)),
				Some(vec![auth1(), auth2()]),
			);
			...
			let hash6_2 = insert_block(&db, Some(vec![auth4()]), || default_header(&hash6_1, 8));
			assert_eq!(
				db.cache().authorities_at(BlockId::Hash(hash6)),
				Some(vec![auth1(), auth2()]),
			);
			...
			assert_eq!(db.cache().authorities_at(BlockId::Hash(hash6_1_1)), Some(vec![auth5()]));
			assert_eq!(db.cache().authorities_at(BlockId::Hash(hash6_1_2)), Some(vec![auth6()]));
			assert_eq!(db.cache().authorities_at(BlockId::Hash(hash6_2)), Some(vec![auth4()]));
