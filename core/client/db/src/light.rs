...
use crate::utils::{
    self, block_id_to_lookup_key, db_err, meta_keys, open_database, read_db, read_meta, Meta,
};
use crate::DatabaseSettings;
use client::backend::{AuxStore, NewBlockState};
use client::blockchain::{
    BlockStatus, Cache as BlockchainCache, HeaderBackend as BlockchainHeaderBackend,
...
use runtime_primitives::traits::{
    As, Block as BlockT, Digest, DigestItem, Header as HeaderT, NumberFor, One, Zero,
};
...
impl<Block: BlockT> LightStorage<Block> {
    ...
    fn note_finalized(
        ...
        if let Some(new_cht_number) = cht::is_build_required(cht::SIZE, *header.number()) {
            ...
            let new_header_cht_root = cht::compute_root::<Block::Header, Blake2Hasher, _>(
                cht::SIZE,
                new_cht_number,
                (new_cht_start.as_()..).map(|num| self.hash(As::sa(num))),
            )?;
            transaction.put(
                columns::CHT,
            ...
            while prune_block <= new_cht_end {
                if let Some(hash) = self.hash(prune_block)? {
                    ...
                    utils::remove_key_mappings(transaction, columns::KEY_LOOKUP, prune_block, hash);
    ...
    fn read_cht_root(
        ...
        let no_cht_for_block =
            || ClientErrorKind::Backend(format!("CHT for block {} not exists", block)).into();
...
pub(crate) mod tests {
    ...
    use runtime_primitives::testing::{Block as RawBlock, ExtrinsicWrapper, Header, H256 as Hash};
    ...
    fn header_with_changes_trie(parent: &Hash, number: u64) -> Header {
        ...
        header.digest.logs.push(DigestItem::ChangesTrieRoot(
            [(number % 256) as u8; 32].into(),
        ));
        header
    }
    ...
    pub fn insert_block<F: Fn() -> Header>(
        ...
        let header = header();
        let hash = header.hash();
        db.import_header(header, cache, NewBlockState::Best, Vec::new())
            .unwrap();
        hash
    }
    ...
    fn insert_final_block<F: Fn() -> Header>(
        ...
        let header = header();
        let hash = header.hash();
        db.import_header(header, cache, NewBlockState::Final, Vec::new())
            .unwrap();
        hash
    }
    ...
    fn insert_non_best_block<F: Fn() -> Header>(
        ...
        let header = header();
        let hash = header.hash();
        db.import_header(header, cache, NewBlockState::Normal, Vec::new())
            .unwrap();
        hash
    }
    ...
    fn returns_known_header() {
        ...
        let known_hash = insert_block(&db, HashMap::new(), || {
    ...
    fn does_not_return_unknown_header() {
        ...
        assert!(db
            .header(BlockId::Hash(Hash::from_low_u64_be(1)))
            .unwrap()
            .is_none());
    ...
    fn returns_info() {
        let db = LightStorage::new_test();
        let genesis_hash = insert_block(&db, HashMap::new(), || {
            default_header(&Default::default(), 0)
        });
    ...
    fn finalized_ancient_headers_are_replaced_with_cht() {
        fn insert_headers<F: Fn(&Hash, u64) -> Header>(header_producer: F) -> LightStorage<Block> {
            ...
            for number in 0..cht::SIZE {
                prev_hash = insert_block(&db, HashMap::new(), || {
                    header_producer(&prev_hash, 1 + cht::SIZE + number)
                });
            }
            assert_eq!(
                db.db.iter(columns::HEADER).count(),
                (1 + cht::SIZE + cht::SIZE) as usize
            );
            assert_eq!(db.db.iter(columns::CHT).count(), 0);
            ...
            prev_hash = insert_block(&db, HashMap::new(), || {
                header_producer(&prev_hash, 1 + cht::SIZE + cht::SIZE)
            });
            assert_eq!(
                db.db.iter(columns::HEADER).count(),
                (2 + cht::SIZE + cht::SIZE) as usize
            );
            assert_eq!(db.db.iter(columns::CHT).count(), 0);
        ...
        assert_eq!(
            db.db.iter(columns::HEADER).count(),
            (1 + cht::SIZE + 1) as usize
        );
        assert_eq!(
            db.db.iter(columns::KEY_LOOKUP).count(),
            (2 * (1 + cht::SIZE + 1)) as usize
        );
        ...
        assert!(db
            .header_cht_root(cht::SIZE, cht::SIZE + cht::SIZE / 2)
            .is_err());
        assert!(db.changes_trie_cht_root(cht::SIZE, cht::SIZE / 2).is_err());
        assert!(db
            .changes_trie_cht_root(cht::SIZE, cht::SIZE + cht::SIZE / 2)
            .is_err());
    ...
    fn get_cht_fails_for_non_existant_cht() {
        assert!(LightStorage::<Block>::new_test()
            .header_cht_root(cht::SIZE, (cht::SIZE / 2) as u64)
            .is_err());
    }
    ...
    fn get_cht_works() {
        ...
        let cht_root_1 = db
            .header_cht_root(cht::SIZE, cht::start_number(cht::SIZE, 0))
            .unwrap();
        let cht_root_2 = db
            .header_cht_root(
                cht::SIZE,
                (cht::start_number(cht::SIZE, 0) + cht::SIZE / 2) as u64,
            )
            .unwrap();
        let cht_root_3 = db
            .header_cht_root(cht::SIZE, cht::end_number(cht::SIZE, 0))
            .unwrap();
        assert_eq!(cht_root_1, cht_root_2);
        assert_eq!(cht_root_2, cht_root_3);

        let cht_root_1 = db
            .changes_trie_cht_root(cht::SIZE, cht::start_number(cht::SIZE, 0))
            .unwrap();
        let cht_root_2 = db
            .changes_trie_cht_root(
                cht::SIZE,
                (cht::start_number(cht::SIZE, 0) + cht::SIZE / 2) as u64,
            )
            .unwrap();
        let cht_root_3 = db
            .changes_trie_cht_root(cht::SIZE, cht::end_number(cht::SIZE, 0))
            .unwrap();
        assert_eq!(cht_root_1, cht_root_2);
        assert_eq!(cht_root_2, cht_root_3);
    }
    ...
    fn tree_route_works() {
        let db = LightStorage::new_test();
        let block0 = insert_block(&db, HashMap::new(), || {
            default_header(&Default::default(), 0)
        });
        ...
        {
            let tree_route =
                ::client::blockchain::tree_route(&db, BlockId::Hash(a3), BlockId::Hash(b2))
                    .unwrap();
        ...
        {
            let tree_route =
                ::client::blockchain::tree_route(&db, BlockId::Hash(a1), BlockId::Hash(a3))
                    .unwrap();
        ...
        {
            let tree_route =
                ::client::blockchain::tree_route(&db, BlockId::Hash(a3), BlockId::Hash(a1))
                    .unwrap();
        ...
        {
            let tree_route =
                ::client::blockchain::tree_route(&db, BlockId::Hash(a2), BlockId::Hash(a2))
                    .unwrap();
    ...
    fn authorities_are_cached() {
        ...
        fn run_checks(
            db: &LightStorage<Block>,
            max: u64,
            checks: &[(u64, Option<Vec<AuthorityIdFor<Block>>>)],
        ) {
        ...
        fn get_authorities(
            cache: &BlockchainCache<Block>,
            at: BlockId<Block>,
        ) -> Option<Vec<AuthorityId>> {
            cache
                .get_at(&well_known_cache_keys::AUTHORITIES, &at)
                .and_then(|val| Decode::decode(&mut &val[..]))
        }
        ...
        let (hash7, hash8, hash6_1, hash6_2, hash6_1_1, hash6_1_2) = {
            ...
            let hash7 = insert_block(&db, make_authorities(vec![auth3()]), || {
                default_header(&hash6, 7)
            });
            assert_eq!(
            ...
            assert_eq!(
                get_authorities(db.cache(), BlockId::Hash(hash7)),
                Some(vec![auth3()])
            );
            let hash8 = insert_block(&db, make_authorities(vec![auth3()]), || {
                default_header(&hash7, 8)
            });
            assert_eq!(
                get_authorities(db.cache(), BlockId::Hash(hash6)),
                Some(vec![auth1(), auth2()]),
            ...
            assert_eq!(
                get_authorities(db.cache(), BlockId::Hash(hash8)),
                Some(vec![auth3()])
            );
            let hash6_1 = insert_block(&db, make_authorities(vec![auth4()]), || {
                default_header(&hash6, 7)
            });
            assert_eq!(
                get_authorities(db.cache(), BlockId::Hash(hash6)),
            ...
            assert_eq!(
                get_authorities(db.cache(), BlockId::Hash(hash6_1)),
                Some(vec![auth4()])
            );
            ...
            assert_eq!(
                get_authorities(db.cache(), BlockId::Hash(hash6_1_1)),
                Some(vec![auth5()])
            );
            ...
            assert_eq!(
                get_authorities(db.cache(), BlockId::Hash(hash6_1_2)),
                Some(vec![auth6()])
            );
            let hash6_2 = insert_block(&db, make_authorities(vec![auth4()]), || {
                default_header(&hash6_1, 8)
            });
            assert_eq!(
                get_authorities(db.cache(), BlockId::Hash(hash6)),
            ...
            assert_eq!(
                get_authorities(db.cache(), BlockId::Hash(hash6_2)),
                Some(vec![auth4()])
            );
    ...
    fn database_is_reopened() {
        let db = LightStorage::new_test();
        let hash0 = insert_final_block(&db, HashMap::new(), || {
            default_header(&Default::default(), 0)
        });
        assert_eq!(db.info().unwrap().best_hash, hash0);
        assert_eq!(
            db.header(BlockId::Hash(hash0)).unwrap().unwrap().hash(),
            hash0
        );
        ...
        assert_eq!(db.info().unwrap().best_hash, hash0);
        assert_eq!(
            db.header(BlockId::Hash::<Block>(hash0))
                .unwrap()
                .unwrap()
                .hash(),
            hash0
    ...
    fn test_leaves_pruned_on_finality() {
        ...
        let block1_b = insert_block(&db, HashMap::new(), || {
            header_with_extrinsics_root(&block0, 1, [1; 32].into())
        });
        let block1_c = insert_block(&db, HashMap::new(), || {
            header_with_extrinsics_root(&block0, 1, [2; 32].into())
        });

        assert_eq!(
            db.leaves.read().hashes(),
            vec![block1_a, block1_b, block1_c]
        );

        let block2_a = insert_block(&db, HashMap::new(), || default_header(&block1_a, 2));
        let block2_b = insert_block(&db, HashMap::new(), || {
            header_with_extrinsics_root(&block1_b, 2, [1; 32].into())
        });
        let block2_c = insert_block(&db, HashMap::new(), || {
            header_with_extrinsics_root(&block1_b, 2, [2; 32].into())
        });
