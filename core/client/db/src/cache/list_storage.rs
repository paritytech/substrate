...
pub trait Storage<Block: BlockT, T: CacheItemT> {
    ...
    fn require_entry(&self, at: &ComplexBlockId<Block>) -> ClientResult<StorageEntry<Block, T>> {
        self.read_entry(at).and_then(|entry| {
            entry.ok_or_else(|| {
                ClientError::from(ClientErrorKind::Backend(format!(
        ...
        self.db
            ...
            .and_then(|entry| match entry {
                Some(entry) => StorageEntry::<Block, T>::decode(&mut &entry[..])
                    .ok_or_else(|| {
                        ClientErrorKind::Backend("Failed to decode cache entry".into()).into()
                    })
                    .map(Some),
                None => Ok(None),
            })
...
mod meta {
    ...
    pub fn decode<Block: BlockT>(encoded: &[u8]) -> ClientResult<Metadata<Block>> {
        ...
        let finalized: Option<ComplexBlockId<Block>> = Decode::decode(input).ok_or_else(|| {
            ClientError::from(ClientErrorKind::Backend("Error decoding cache meta".into()))
        })?;
        let unfinalized: Vec<ComplexBlockId<Block>> = Decode::decode(input).ok_or_else(|| {
            ClientError::from(ClientErrorKind::Backend("Error decoding cache meta".into()))
        })?;
