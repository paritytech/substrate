...
use consensus::{
    well_known_cache_keys::Id as CacheKeyId, BlockOrigin, Error as ConsensusError,
    ErrorKind as ConsensusErrorKind, ForkChoiceStrategy, ImportBlock, ImportResult,
};
...
use primitives::{
    convert_hash, Blake2Hasher, ChangesTrieConfiguration, ExecutionContext, NeverNativeValue, H256,
};
...
use state_machine::{
    ...
    ExecutionStrategy, NeverOffchainExt, OverlayedChanges,
};
use std::{
...
use crate::blockchain::{
    self, Backend as ChainBackend, Cache, HeaderBackend as ChainHeaderBackend, Info as ChainInfo,
    ProvideCache,
};
    ...
    Block: BlockT<Hash = H256>,
{
    ...
    pub fn max_key_changes_range(
        ...
        let last_num = self
            ...
            .as_();
        if first > last_num {
            return Err(error::ErrorKind::ChangesTrieAccessFailed(
    ...
    pub fn key_changes(
        ...
        .and_then(|r| {
            r.map(|r| r.map(|(block, tx)| (As::sa(block), tx)))
                .collect::<Result<_, _>>()
        })
    ...
    fn require_changes_trie(
        &self,
    ) -> error::Result<(ChangesTrieConfiguration, &B::ChangesTrieStorage)> {
    ...
    pub fn new_block(&self) -> error::Result<block_builder::BlockBuilder<Block, Self>>
    where
        E: Clone + Send + Sync,
        RA: Send + Sync,
    ...
    pub fn set_head(&self, id: BlockId<Block>) -> error::Result<()> {
        self.lock_import_and_run(|operation| self.apply_head(operation, id))
    }
        ...
        E: CallExecutor<Block, Blake2Hasher> + Send + Sync + Clone,
    {
        ...
        if make_notifications {
            ...
            operation.notify_imported = Some((
                hash,
                origin,
                import_headers.into_post(),
                is_new_best,
                storage_changes,
            ));
        }
        ...
        match transaction.state()? {
            Some(transaction_state) => {
                ...
                let (_, storage_update, changes_update) = self
                    .executor
                    .call_at_state::<_, _, _, NeverNativeValue, fn() -> _>(
                        transaction_state,
                        &mut overlay,
                        "Core_execute_block",
                        &<Block as BlockT>::new(
    ...
    Block: BlockT<Hash = H256>,
{
    ...
    >(
        ...
        self.lock_import_and_run(|operation| self.apply_aux(operation, insert, delete))
    }
    ...
    fn key_changes_works() {
        ...
        for (index, (begin, end, key, expected_result)) in test_cases.into_iter().enumerate() {
            ...
            let actual_result = client
                .key_changes(begin, BlockId::Hash(end), &StorageKey(key))
                .unwrap();
