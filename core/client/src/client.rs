...
pub(crate) mod tests {
	...
	pub fn prepare_client_with_key_changes() -> (
		...
		// prepare test cases
		let alice = twox_128(&runtime::system::balance_of_key(AccountKeyring::Alice.to_raw_public().into())).to_vec();
		let bob = twox_128(&runtime::system::balance_of_key(AccountKeyring::Bob.to_raw_public().into())).to_vec();
		let charlie = twox_128(&runtime::system::balance_of_key(AccountKeyring::Charlie.to_raw_public().into())).to_vec();
		let dave = twox_128(&runtime::system::balance_of_key(AccountKeyring::Dave.to_raw_public().into())).to_vec();
		let eve = twox_128(&runtime::system::balance_of_key(AccountKeyring::Eve.to_raw_public().into())).to_vec();
		let ferdie = twox_128(&runtime::system::balance_of_key(AccountKeyring::Ferdie.to_raw_public().into())).to_vec();
		let test_cases = vec![
	...
	fn client_initialises_from_genesis_ok() {
		...
		assert_eq!(
			client.runtime_api().balance_of(
				&BlockId::Number(client.info().unwrap().chain.best_number),
				AccountKeyring::Ferdie.to_raw_public().into()
			).unwrap(),
			0
		);
	}
	...
	fn block_builder_does_not_include_invalid() {
		...
		assert!(builder.push_transfer(Transfer {
			from: AccountKeyring::Eve.to_raw_public().into(),
