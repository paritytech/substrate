...
use runtime_primitives::traits::{
	...
	ApiRef, ProvideRuntimeApi, Digest, DigestItem, BlockAuthorityId
};
	...
	Block: BlockT<Hash=H256>,
{
	...
	pub fn authorities_at(&self, id: &BlockId<Block>) -> error::Result<Vec<BlockAuthorityId<Block>>> {
		match self.backend.blockchain().cache().and_then(|cache| cache.authorities_at(*id)) {
			...
			None => self.executor.call(id, "Core_authorities",&[])
				.and_then(|r| Vec::<BlockAuthorityId<Block>>::decode(&mut &r.return_data[..])
	...
	Block: BlockT<Hash=H256>,
{
	...
	fn import_block(
		...
		new_authorities: Option<Vec<BlockAuthorityId<Block>>>,
...
{
	type Error = Error;
	fn authorities(&self, at: &BlockId<Block>) -> Result<Vec<BlockAuthorityId<Block>>, Self::Error> {
