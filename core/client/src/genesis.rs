...
use runtime_primitives::traits::{Block as BlockT, Hash as HashT, Header as HeaderT, Zero};
...
mod tests {
    use super::*;
    use executor::{native_executor_instance, NativeExecutionDispatch};
    use hex::*;
    use parity_codec::{Decode, Encode, Joiner};
    use primitives::Blake2Hasher;
    use runtime_primitives::traits::BlakeTwo256;
    use state_machine::backend::InMemory;
    use state_machine::{self, ExecutionStrategy, InMemoryChangesTrieStorage, OverlayedChanges};
    use test_client::{
        runtime::genesismap::{additional_storage_with_genesis, GenesisConfig},
        runtime::{Block, BlockNumber, Digest, Extrinsic, Hash, Header, Transfer},
    ...
    fn construct_block(
        ...
        let transactions = txs
            .into_iter()
            .map(|tx| {
                let signature = AccountKeyring::from_public(&tx.from)
                    .unwrap()
                    .sign(&tx.encode())
                    .into();
        ...
        let extrinsics_root =
            ordered_trie_root::<Blake2Hasher, _, _>(transactions.iter().map(Encode::encode)).into();
