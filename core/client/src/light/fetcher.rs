...
use primitives::{convert_hash, ChangesTrieConfiguration};
use runtime_primitives::traits::{As, Block as BlockT, Header as HeaderT, NumberFor};
use state_machine::{
    create_proof_check_backend_storage, key_changes_proof_check, read_proof_check,
    ChangesTrieAnchorBlockId, ChangesTrieRootsStorage, CodeExecutor, TrieBackend,
};
    ...
    F: Send + Sync,
{
    fn check_header_proof(
        ...
        let remote_header =
            remote_header.ok_or_else(|| ClientError::from(ClientErrorKind::InvalidCHTProof))?;
    ...
    Hash: 'a + Send + Sync + Clone + AsRef<[u8]>,
{
    fn root(
        &self,
        _anchor: &ChangesTrieAnchorBlockId<H::Out>,
        block: u64,
...
pub mod tests {
    ...
    use test_client::{
        ...
        runtime::{self, Block, Hash, Header},
        AccountKeyring, TestClient,
    };

    use super::*;
    use crate::in_mem::Blockchain as InMemoryBlockchain;
    use crate::light::blockchain::tests::{DummyBlockchain, DummyStorage};
    use crate::light::fetcher::{
        FetchChecker, Fetcher, LightDataChecker, RemoteCallRequest, RemoteHeaderRequest,
    };
    ...
    fn prepare_for_read_proof_check() -> (TestChecker, Header, Vec<Vec<u8>>, u32) {
        ...
        let authorities_len = remote_client
            .storage(
                &remote_block_id,
                &StorageKey(well_known_keys::AUTHORITY_COUNT.to_vec()),
            )
            .unwrap()
            .and_then(|v| Decode::decode(&mut &v.0[..]))
            .unwrap();
        let remote_read_proof = remote_client
            .read_proof(&remote_block_id, well_known_keys::AUTHORITY_COUNT)
            .unwrap();
    ...
    fn prepare_for_header_proof_check(
        insert_cht: bool,
    ) -> (TestChecker, Hash, Header, Vec<Vec<u8>>) {
        // prepare remote client
        let remote_client = test_client::new();
        ...
        let (remote_block_header, remote_header_proof) = remote_client
            .header_proof_with_cht_size(&remote_block_id, 4)
            .unwrap();

        // check remote read proof locally
        let local_storage = InMemoryBlockchain::<Block>::new();
        let local_cht_root =
            cht::compute_root::<Header, Blake2Hasher, _>(4, 0, local_headers_hashes).unwrap();
        if insert_cht {
    ...
    fn storage_read_proof_is_generated_and_checked() {
        let (local_checker, remote_block_header, remote_read_proof, authorities_len) =
            prepare_for_read_proof_check();
        assert_eq!(
            (&local_checker as &FetchChecker<Block>)
                .check_read_proof(
    ...
    fn check_header_proof_fails_if_cht_root_is_invalid() {
        ...
        assert!((&local_checker as &FetchChecker<Block>)
            .check_header_proof(
                &RemoteHeaderRequest::<Header> {
    ...
    fn changes_proof_is_generated_and_checked_when_headers_are_not_pruned() {
        ...
        for (index, (begin, end, key, expected_result)) in test_cases.into_iter().enumerate() {
            ...
            let remote_proof = remote_client
                .key_changes_proof(begin_hash, end_hash, begin_hash, max_hash, &key)
                .unwrap();
    ...
    fn changes_proof_is_generated_and_checked_when_headers_are_pruned() {
        ...
        let dave = twox_128(&runtime::system::balance_of_key(
        ...
        let b1 = remote_client
            .block_hash_from_id(&BlockId::Number(1))
            .unwrap()
            .unwrap();
        let b3 = remote_client
            .block_hash_from_id(&BlockId::Number(3))
            .unwrap()
            .unwrap();
        let b4 = remote_client
            .block_hash_from_id(&BlockId::Number(4))
            .unwrap()
            .unwrap();
        let remote_proof = remote_client
            .key_changes_proof_with_cht_size(b1, b4, b3, b4, &dave, 4)
            .unwrap();
        ...
        local_storage
            .changes_tries_cht_roots
            .insert(0, local_cht_root);
        let local_checker = TestChecker::new(
        ...
        let request = RemoteChangesRequest::<Header> {
            ...
            tries_roots: (
                3,
                b3,
                vec![remote_roots[2].clone(), remote_roots[3].clone()],
            ),
            key: dave.0,
            retry_count: None,
        };
    ...
    fn check_changes_tries_proof_fails_if_proof_is_wrong() {
        ...
        assert!(local_checker
            .check_changes_tries_proof(4, &remote_proof.roots, vec![])
            .is_err());
    }
}
