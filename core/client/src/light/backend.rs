...
use crate::backend::{
    AuxStore, Backend as ClientBackend, BlockImportOperation, NewBlockState, RemoteBackend,
};
...
use state_machine::{backend::InMemory as InMemoryState, Backend as StateBackend, TrieBackend};
use trie::MemoryDB;
    ...
    H::Out: HeapSizeOf + Ord,
{
    ...
    fn update_db_storage(
        &mut self,
        _update: <Self::State as StateBackend<H>>::Transaction,
    ) -> ClientResult<()> {
    ...
    H: Hasher<Out = Block::Hash>,
{
    ...
    fn storage(&self, key: &[u8]) -> ClientResult<Option<Vec<u8>>> {
        ...
        self.fetcher
            .upgrade()
            .ok_or(ClientErrorKind::NotAvailableOnLightClient)?
    ...
    H::Out: HeapSizeOf + Ord,
{
    ...
    fn storage(&self, key: &[u8]) -> ClientResult<Option<Vec<u8>>> {
        match *self {
            OnDemandOrGenesisState::OnDemand(ref state) => StateBackend::<H>::storage(state, key),
    ...
    fn child_storage(&self, storage_key: &[u8], key: &[u8]) -> ClientResult<Option<Vec<u8>>> {
        match *self {
            ...
            OnDemandOrGenesisState::Genesis(ref state) => Ok(state
    ...
    fn pairs(&self) -> Vec<(Vec<u8>, Vec<u8>)> {
        match *self {
            OnDemandOrGenesisState::OnDemand(ref state) => StateBackend::<H>::pairs(state),
    ...
    fn keys(&self, prefix: &Vec<u8>) -> Vec<Vec<u8>> {
        match *self {
            OnDemandOrGenesisState::OnDemand(ref state) => StateBackend::<H>::keys(state, prefix),
...
mod tests {
    ...
    fn local_state_is_created_when_genesis_state_is_available() {
        ...
        let backend: Backend<_, _, Blake2Hasher> =
            Backend::new(Arc::new(DummyBlockchain::new(DummyStorage::new())));
        let mut op = backend.begin_operation().unwrap();
        op.set_block_data(header0, None, None, NewBlockState::Final)
            .unwrap();
        op.reset_storage(Default::default(), Default::default())
            .unwrap();
    ...
    fn light_aux_store_is_updated_via_non_importing_op() {
        ...
        BlockImportOperation::<Block, Blake2Hasher>::insert_aux(
            &mut op,
            vec![(vec![1], Some(vec![2]))],
        )
        .unwrap();
