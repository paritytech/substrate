...
use primitives::{ExecutionContext, H256};
use runtime_primitives::generic::BlockId;
use runtime_primitives::traits::{
    ApiRef, Block as BlockT, Hash, HashFor, Header as HeaderT, One, ProvideRuntimeApi,
};
use runtime_primitives::ApplyOutcome;
use std::vec::Vec;

/// Utility for building new (valid) blocks from a stream of extrinsics.
pub struct BlockBuilder<'a, Block, A: ProvideRuntimeApi>
where
    Block: BlockT,
{
    ...
    A::Api: BlockBuilderApi<Block>,
{
    ...
    pub fn new(api: &'a A) -> error::Result<Self> {
        api.info()
            .and_then(|i| Self::at_block(&BlockId::Hash(i.best_hash), api))
    }
    ...
    pub fn push(&mut self, xt: <Block as BlockT>::Extrinsic) -> error::Result<()> {
        ...
        self.api.map_api_result(|api| {
            match api.apply_extrinsic_with_context(
                ...
                Err(e) => Err(error::ErrorKind::ApplyExtrinsicFailed(e).into()),
            }
        })
    }
