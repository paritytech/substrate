...
use primitives::{convert_hash, H256};
use runtime_primitives::traits::{As, Header as HeaderT, One, SimpleArithmetic};
use state_machine::backend::InMemory as InMemoryState;
use state_machine::{
    prove_read_on_trie_backend, read_proof_check, read_proof_check_on_proving_backend,
    Backend as StateBackend, MemoryDB, TrieBackend,
};
    ...
    HashesI: IntoIterator<Item = ClientResult<Option<Header::Hash>>>,
{
    ...
    for block in blocks.into_iter() {
        ...
        assert!(
            value.is_some(),
            "we have just built trie that includes the value for block"
        );
    ...
    Hasher::Out: Ord + HeapSizeOf,
{
    do_check_proof::<Header, Hasher, _>(
        ...
        move |local_root, local_cht_key| {
            read_proof_check::<Hasher>(local_root, remote_proof, local_cht_key)
    ...
    F: FnOnce(Hasher::Out, &[u8]) -> ClientResult<Option<Vec<u8>>>,
{
    ...
    let local_hash =
        decode_cht_value(&local_cht_value).ok_or_else(|| ClientErrorKind::InvalidCHTProof)?;
    ...
    I: IntoIterator<Item = ClientResult<Option<Header::Hash>>>,
{
    ...
    for hash in hashes.into_iter().take(cht_size as usize) {
        let hash = hash?.ok_or_else(|| {
            ClientError::from(ClientErrorKind::MissingHashRequiredForCHT(
                cht_num.as_(),
                hash_number.as_(),
            ))
        })?;
        pairs.push((encode_cht_key(hash_number).to_vec(), encode_cht_value(hash)));
