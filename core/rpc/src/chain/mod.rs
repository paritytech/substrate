...
pub trait ChainApi<Number, Hash, Header, SignedBlock>
{
	...
	fn block_hash(&self, Option<Number>) -> Result<Option<Hash>>;
...
#[rpc(pubsub, name = "chain_newHead")]
pub trait ChainApiNewHead<Header> {
	type Metadata;

	/// New head subscription
	#[rpc(subscribe, name = "chain_subscribeNewHead", alias("subscribe_newHead"))]
	fn subscribe_new_head(&self, Self::Metadata, typed::Subscriber<Header>);

	/// Unsubscribe from new head subscription.
	#[rpc(unsubscribe, name = "chain_unsubscribeNewHead", alias("unsubscribe_newHead"))]
	fn unsubscribe_new_head(&self, Option<Self::Metadata>, SubscriptionId) -> RpcResult<bool>;
}

#[rpc(pubsub, name = "chain_finalisedHead")]
pub trait ChainApiFinalisedHead<Header> {
	type Metadata;

	/// New head subscription
	#[rpc(subscribe, name = "chain_subscribeFinalisedHeads")]
	fn subscribe_finalised_heads(&self, Self::Metadata, typed::Subscriber<Header>);

	/// Unsubscribe from new head subscription.
	#[rpc(unsubscribe, name = "chain_unsubscribeFinalisedHeads")]
	...
	RA: Send + Sync + 'static
{
	...
	fn block_hash(&self, number: Option<NumberFor<Block>>) -> Result<Option<Block::Hash>> {
...
impl<B, E, Block, RA> ChainApiNewHead<Block::Header> for Chain<B, E, Block, RA> where
	Block: BlockT<Hash=H256> + 'static,
	B: client::backend::Backend<Block, Blake2Hasher> + Send + Sync + 'static,
	E: client::CallExecutor<Block, Blake2Hasher> + Send + Sync + 'static,
	RA: Send + Sync + 'static
...
impl<B, E, Block, RA> ChainApiFinalisedHead<Block::Header> for Chain<B, E, Block, RA> where
	Block: BlockT<Hash=H256> + 'static,
	B: client::backend::Backend<Block, Blake2Hasher> + Send + Sync + 'static,
	E: client::CallExecutor<Block, Blake2Hasher> + Send + Sync + 'static,
	RA: Send + Sync + 'static
