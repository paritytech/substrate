...
use crate::{error, maybe_start_server, Service};
...
use std::{marker::PhantomData, net::SocketAddr, ops::Deref, ops::DerefMut, sync::Arc};
...
pub type NetworkService<F> =
    network::Service<<F as ServiceFactory>::Block, <F as ServiceFactory>::NetworkProtocol>;
...
pub trait ServiceFactory: 'static + Sized {
    ...
    type FullTransactionPoolApi: txpool::ChainApi<
            Hash = <Self::Block as BlockT>::Hash,
            Block = Self::Block,
        > + Send
        + 'static;
    /// Extrinsic pool backend type for the light client.
    type LightTransactionPoolApi: txpool::ChainApi<
            Hash = <Self::Block as BlockT>::Hash,
            Block = Self::Block,
        > + 'static;
    ...
    fn build_full_import_queue(
        ...
        if let Some(name) = config.chain_spec.consensus_engine() {
            match name {
                _ => Err(format!(
                    "Chain Specification defines unknown consensus engine '{}'",
                    name
                )
                .into()),
            }
...
pub trait Components: Sized + 'static {
    ...
    /// Extrinsic pool type.
    type TransactionPoolApi: 'static
        + txpool::ChainApi<
...
impl<Factory: ServiceFactory> Components for LightComponents<Factory> {
    ...
    > {
        ...
        let fetch_checker = Arc::new(client::light::new_fetch_checker(
...
mod tests {
    ...
    fn should_remove_transactions_from_the_pool() {
        ...
        pool.submit_one(
            &BlockId::hash(client.best_block_header().unwrap().hash()),
            transaction.clone(),
        )
        .unwrap();
