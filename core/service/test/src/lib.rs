...
use consensus::{BlockImport, ImportBlock};
use futures::{Future, Stream};
use log::info;
use network::config::{NetworkConfiguration, NodeKeyConfig, NonReservedPeerMode, Secret};
use network::{multiaddr, ManageNetwork, SyncProvider};
use service::{
    Configuration, FactoryChainSpec, FactoryExtrinsic, FactoryFullConfiguration, Roles,
    ServiceFactory,
};
...
impl<F: ServiceFactory> TestNet<F> {
    pub fn run_until_all_full<P: Send + Sync + Fn(u32, &F::FullService) -> bool + 'static>(
        &mut self,
        predicate: P,
    ) {
        ...
        let interval = Interval::new_interval(Duration::from_millis(100))
            ...
            .for_each(move |_| {
                if full_nodes
                    .iter()
                    .all(|&(ref id, ref service)| predicate(*id, service))
                {
                    Err(())
                } else {
                    Ok(())
...
impl<F: ServiceFactory> TestNet<F> {
    ...
    fn insert_nodes(&mut self, temp: &TempDir, full: u32, light: u32, authorities: Vec<String>) {
        ...
        self.authority_nodes
            .extend(authorities.iter().enumerate().map(|(index, key)| {
                (
                    (index + nodes) as u32,
                    Arc::new(
                        F::new_full(
        ...
        self.full_nodes
            .extend((nodes..nodes + full as usize).map(|index| {
                (
                    index as u32,
                    Arc::new(
                        F::new_full(
        ...
        self._light_nodes
            .extend((nodes..nodes + light as usize).map(|index| {
                (
                    index as u32,
                    Arc::new(
                        F::new_light(
...
pub fn connectivity<F: ServiceFactory>(spec: FactoryChainSpec<F>) {
    const NUM_NODES: u32 = 10;
    {
        ...
        let runtime = {
            ...
            let first_address = network.full_nodes[0]
                .1
                .network()
                .node_id()
                .expect("No node address");
        ...
        runtime
            .shutdown_on_idle()
            .wait()
            .expect("Error shutting down runtime");
    ...
    }
    {
        let temp = TempDir::new("substrate-connectivity-test").expect("Error creating test dir");
        {
            ...
            let mut address = network.full_nodes[0]
                .1
                .network()
                .node_id()
                .expect("No node address");
            for (_, service) in network.full_nodes.iter().skip(1) {
                service
                    .network()
                    .add_reserved_peer(address.clone())
                    .expect("Error adding reserved peer");
    ...
    E: Fn(&F::FullService) -> FactoryExtrinsic<F>,
{
    ...
    let first_address = {
        ...
        for i in 0..NUM_BLOCKS {
            ...
            first_service
                .client()
                .import_block(import_data, HashMap::new())
    ...
    first_service
        .transaction_pool()
        .submit_one(&best_block, extrinsic_factory(&first_service))
        .unwrap();
