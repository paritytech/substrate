...
struct TestNet<F: ServiceFactory> {
	runtime: Runtime,
	authority_nodes: Vec<(u32, SyncService<F::FullService>, Multiaddr)>,
	full_nodes: Vec<(u32, SyncService<F::FullService>, Multiaddr)>,
	light_nodes: Vec<(u32, SyncService<F::LightService>, Multiaddr)>,
...
impl<F: ServiceFactory> TestNet<F> {
	pub fn run_until_all_full<FP, LP>(
		...
		where
			FP: Send + Fn(u32, &SyncService<F::FullService>) -> bool + 'static,
			LP: Send + Fn(u32, &SyncService<F::LightService>) -> bool + 'static,
	{
	...
	const NUM_LIGHT_NODES: u32 = 5;
	{
		...
		let runtime = {
			...
			network.run_until_all_full(
				|_index, service| service.get().network().peers_debug_info().len() == NUM_FULL_NODES as usize - 1
					+ NUM_LIGHT_NODES as usize,
				|_index, service| service.get().network().peers_debug_info().len() == NUM_FULL_NODES as usize,
			);
			network.runtime
		};
	...
	E: FnMut(&SyncService<F::FullService>) -> FactoryExtrinsic<F>,
{
	...
	let first_address = {
		...
		for i in 0 .. NUM_BLOCKS {
			...
			first_service.get().client().import_block(import_data, HashMap::new()).expect("Error importing test block");
		}
	...
	network.run_until_all_full(
		|_index, service|
			service.get().client().info().chain.best_number == NUM_BLOCKS.into(),
		|_index, service|
			service.get().client().info().chain.best_number == NUM_BLOCKS.into(),
	);
	...
	first_service.get().transaction_pool().submit_one(&best_block, extrinsic_factory(&first_service)).unwrap();
	network.run_until_all_full(
	...
	F::LightService: Future<Item=(), Error=()>,
{
	...
	network.run_until_all_full(
		|_index, service|
			service.get().client().info().chain.finalized_number >= (NUM_BLOCKS / 2).into(),
		|_index, service|
			service.get().client().info().chain.best_number >= (NUM_BLOCKS / 2).into(),
	);
	...
	network.run_until_all_full(
		|_index, service|
			service.get().client().info().chain.finalized_number >= NUM_BLOCKS.into(),
		|_index, service|
			service.get().client().info().chain.best_number >= NUM_BLOCKS.into(),
	);
}
