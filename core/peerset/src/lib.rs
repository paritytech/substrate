...
impl Peerset {
    ...
    pub fn add_reserved_peer(&self, peer_id: PeerId) {
        ...
        if inner
            .out_slots
            .iter()
            .chain(inner.in_slots.iter())
            .any(|s| s.as_ref() == Some(&peer_id))
        {
            return;
        }

        // Assign a slot for this reserved peer.
        if let Some(pos) = inner.out_slots.iter().position(|s| {
            s.as_ref()
                .map(|n| !inner.reserved.contains(n))
                .unwrap_or(true)
        }) {
    ...
    pub fn dropped(&self, peer_id: &PeerId) {
        ...
        if inner.reserved.contains(peer_id) {
            let _ = self
                .parent
                .tx
                .unbounded_send(Message::Connect(peer_id.clone()));
            return;
        }
