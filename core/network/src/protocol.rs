...
use crate::message::generic::{ConsensusMessage, Message as GenericMessage};
use crate::message::{self, Message};
use crate::on_demand::OnDemandService;
use crate::service::{ExHashT, NetworkChan, NetworkMsg, TransactionPool};
...
impl<B: BlockT, S: NetworkSpecialization<B>, F: FnOnce(&mut S, &mut Context<B>)> SpecTask<B, S>
    for F
{
...
impl<B: BlockT, S: NetworkSpecialization<B>, H: ExHashT> Protocol<B, S, H> {
    ...
    fn handle_network_msg(&mut self, msg: FromNetworkMsg<B>) -> bool {
        match msg {
            FromNetworkMsg::PeerDisconnected(who, debug_info) => {
                self.on_peer_disconnected(who, debug_info)
            }
            FromNetworkMsg::PeerConnected(who, debug_info) => {
                self.on_peer_connected(who, debug_info)
            }
            FromNetworkMsg::PeerClogged(who, message) => self.on_clogged_peer(who, message),
            FromNetworkMsg::CustomMessage(who, message) => self.on_custom_message(who, message),
        }
        true
    }
    ...
    fn handle_response(
        ...
        if let Some(ref mut peer) = self.context_data.peers.get_mut(&who) {
            ...
            self.network_chan
                .send(NetworkMsg::ReportPeer(who, severity))
        }
        None
    }
    ...
    fn on_status(&mut self) {
        ...
        self.status_sinks
            .lock()
            .retain(|sink| sink.unbounded_send(status.clone()).is_ok());
    }
    ...
    fn tick(&mut self) {
        ...
        self.maintain_peers();
        self.sync.tick(&mut ProtocolContext::new(
            &mut self.context_data,
            &self.network_chan,
        ));
        self.on_demand.as_ref().map(|s| s.maintain_peers());
    }
    ...
    fn on_status_message(&mut self, who: PeerId, status: message::Status<B>) {
        trace!(target: "sync", "New peer {} {:?}", who, status);
        {
            ...
            if status.genesis_hash != self.genesis_hash {
                ...
                self.network_chan
                    .send(NetworkMsg::ReportPeer(who, Severity::Bad(reason)));
                return;
            }
    ...
    fn propagate_extrinsics(&mut self) {
        ...
        for (who, peer) in self.context_data.peers.iter_mut() {
            ...
            if !to_send.is_empty() {
                ...
                self.network_chan.send(NetworkMsg::Outgoing(
                    who.clone(),
    ...
    pub fn announce_block(&mut self, hash: B::Hash) {
        ...
        for (who, ref mut peer) in self.context_data.peers.iter_mut() {
            ...
            self.network_chan
                .send(NetworkMsg::Outgoing(who.clone(), message.clone()))
        }
    }
