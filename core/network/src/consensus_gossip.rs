...
pub use crate::message::generic::{ConsensusMessage, Message};
...
impl<B: BlockT> ConsensusGossip<B> {
    ...
    pub fn register_validator(
        ...
        validator: Arc<Validator<B::Hash>>,
    ) {
    ...
    pub fn new_peer(&mut self, protocol: &mut Context<B>, who: PeerId, roles: Roles) {
        if roles.intersects(Roles::AUTHORITY) {
            ...
            for entry in self.messages.iter() {
                if let Status::Future = entry.status {
                    continue;
                }
    ...
    fn propagate<F>(
        ...
        F: Fn() -> ConsensusMessage,
    {
        let mut non_authorities: Vec<_> = self
            .peers
            .iter()
        ...
        let non_authorities: HashSet<_> = if non_authorities.is_empty() {
            ...
            non_authorities[0..non_authorities
                .len()
                .min(((non_authorities.len() as f64).sqrt() as usize).max(3))]
                .iter()
                .collect()
        };
...
mod tests {
    use futures::Stream;
    use runtime_primitives::testing::{Block as RawBlock, ExtrinsicWrapper, H256};
    ...
    fn can_keep_multiple_messages_per_topic() {
        ...
        consensus.register_message(
            HashFor::<Block>::hash(&msg_a.data),
            topic,
            Status::Live,
            || msg_a.clone(),
        );
        consensus.register_message(
            HashFor::<Block>::hash(&msg_b.data),
            topic,
            Status::Live,
            || msg_b.clone(),
        );
