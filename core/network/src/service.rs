...
use crate::protocol::{
    self, ConnectedPeer, Context, FromNetworkMsg, PeerInfo, Protocol, ProtocolMsg, ProtocolStatus,
};
use crate::specialization::NetworkSpecialization;
use consensus::import_queue::{ImportQueue, Link};
use crossbeam_channel::{self as channel, Receiver, Sender, TryRecvError};
use futures::{stream, sync::mpsc, sync::oneshot, Async, Future, Stream};
use log::{debug, error, info, trace, warn};
use network_libp2p::{multiaddr, NetworkState, RegisteredProtocol};
use network_libp2p::{
    parse_str_addr, start_service, Service as NetworkService, ServiceEvent as NetworkServiceEvent,
};
use network_libp2p::{NetworkConfiguration, ProtocolId, Severity};
...
impl<B: BlockT, S: NetworkSpecialization<B>> Link<B> for NetworkLink<B, S> {
    fn block_imported(&self, hash: &B::Hash, number: NumberFor<B>) {
        let _ = self
            .protocol_sender
            .send(ProtocolMsg::BlockImportedSync(hash.clone(), number));
    }

    fn blocks_processed(&self, processed_blocks: Vec<B::Hash>, has_error: bool) {
        let _ = self
            .protocol_sender
            .send(ProtocolMsg::BlocksProcessed(processed_blocks, has_error));
    }
    ...
    fn justification_imported(
        ...
        let _ = self
            .protocol_sender
            .send(ProtocolMsg::JustificationImportResult(
                hash.clone(),
                number,
                success,
            ));
        if !success {
            let reason =
                Severity::Bad(format!("Invalid justification provided for #{}", hash).to_string());
            let _ = self
                .network_sender
    ...
    fn clear_justification_requests(&self) {
        let _ = self
            .protocol_sender
            .send(ProtocolMsg::ClearJustificationRequests);
    }
    ...
    fn useless_peer(&self, who: PeerId, reason: &str) {
        ...
        self.network_sender.send(NetworkMsg::ReportPeer(
            who,
            Severity::Useless(reason.to_string()),
        ));
    }
...
impl<B: BlockT + 'static, S: NetworkSpecialization<B>> Service<B, S> {
    ...
    where
        F: FnOnce(&mut S, &mut Context<B>) + Send + 'static,
    {
        let _ = self
            .protocol_sender
    ...
    where
        F: FnOnce(&mut ConsensusGossip<B>, &mut Context<B>) + Send + 'static,
    {
        let _ = self
            .protocol_sender
...
impl<B: BlockT + 'static, S: NetworkSpecialization<B>> SyncProvider<B> for Service<B, S> {
    ...
    fn peers(&self) -> Vec<(PeerId, PeerInfo<B>)> {
        ...
        peers
            .into_iter()
            .map(|(idx, connected)| (idx, connected.peer_info))
            .collect()
    }
}
...
impl<B: BlockT + 'static, S: NetworkSpecialization<B>> ManageNetwork for Service<B, S> {
    ...
    fn node_id(&self) -> Option<String> {
        ...
        let ret = network.listeners().next().map(|addr| {
...
fn start_thread<B: BlockT + 'static>(
    ...
    (
        ...
        Arc<Mutex<NetworkService<Message<B>>>>,
        Arc<Peerset>,
    ),
    Error,
> {
...
fn run_thread<B: BlockT + 'static>(
    ...
    let protocol = stream::poll_fn(move || match network_port.take_one_message() {
    ...
    .for_each(move |msg| {
        ...
        match msg {
            ...
            NetworkMsg::ReportPeer(who, severity) => match severity {
    ...
    let network = stream::poll_fn(move || network_service.lock().poll()).for_each(move |event| {
        match event {
            NetworkServiceEvent::OpenedCustomProtocol {
                peer_id,
                version,
                debug_info,
                ..
            ...
            NetworkServiceEvent::ClosedCustomProtocol {
                peer_id,
                debug_info,
                ..
            } => {
            ...
            NetworkServiceEvent::Clogged {
                ...
                for msg in messages.into_iter().take(5) {
                    ...
                    let _ = protocol_sender
                        .send(FromNetworkMsg::PeerClogged(peer_id.clone(), Some(msg)));
                }
            }
        };
        Ok(())
    ...
    let futures: Vec<Box<Future<Item = (), Error = io::Error> + Send>> =
        vec![Box::new(protocol) as Box<_>, Box::new(network) as Box<_>];
