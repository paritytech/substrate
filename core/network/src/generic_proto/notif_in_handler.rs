...
/// it is turned into a [`NotifsInProtoHandler`].
pub struct NotifsInProtoHandlerProto<TSubstream> {
...
impl<TSubstream> NotifsInProtoHandlerProto<TSubstream>
where
	TSubstream: AsyncRead + AsyncWrite,
{
	/// Builds a new `NotifsInProtoHandlerProto`.
	pub fn new(proto_name: impl Into<Cow<'static, [u8]>>, handshake_msg: impl Into<Vec<u8>>) -> Self {
		NotifsInProtoHandlerProto {
...
impl<TSubstream> IntoProtocolsHandler for NotifsInProtoHandlerProto<TSubstream>
where
	TSubstream: AsyncRead + AsyncWrite + 'static,
{
	type Handler = NotifsInProtoHandler<TSubstream>;
...
pub struct NotifsInProtoHandler<TSubstream> {
	...
	/// element.
	events_queue: SmallVec<[ProtocolsHandlerEvent<DeniedUpgrade, (), NotifsInProtoHandlerOut>; 16]>,

	/// Marker to pin the generic type.
	marker: PhantomData<TSubstream>,
}

/// Event that can be received by a `NotifsInProtoHandler`.
#[derive(Debug)]
pub enum NotifsInProtoHandlerIn {
...
/// Event that can be emitted by a `NotifsInProtoHandler`.
#[derive(Debug)]
pub enum NotifsInProtoHandlerOut {
...
impl<TSubstream> ProtocolsHandler for NotifsInProtoHandler<TSubstream>
where TSubstream: AsyncRead + AsyncWrite + 'static {
	type InEvent = NotifsInProtoHandlerIn;
	type OutEvent = NotifsInProtoHandlerOut;
	...
	fn inject_event(&mut self, message: NotifsInProtoHandlerIn) {
		match message {
			NotifsInProtoHandlerIn::UpdateNodeInfos { infos } =>
	...
	fn inject_dial_upgrade_error(&mut self, _: (), err: ProtocolsHandlerUpgrErr<void::Void>) {
		...
		self.events_queue.push(ProtocolsHandlerEvent::Custom(NotifsInProtoHandlerOut::ProtocolError {
			is_severe,
			error: Box::new(err),
		}));*/
	}
...
impl<TSubstream> fmt::Debug for NotifsInProtoHandler<TSubstream>
where
	TSubstream: AsyncRead + AsyncWrite,
{
	fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
		f.debug_struct("NotifsInProtoHandler")
			.finish()
	}
}
