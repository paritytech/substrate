...
use runtime_primitives::traits::{
    As, Block as BlockT, CheckedSub, Header as HeaderT, NumberFor, Zero,
};
...
impl<B: BlockT> PendingJustifications<B> {
    ...
    fn dispatch(&mut self, peers: &mut HashMap<PeerId, PeerSync<B>>, protocol: &mut Context<B>) {
        ...
        let mut available_peers = peers
            .iter()
            .filter_map(|(peer, sync)| {
    ...
    fn queue_request<F>(&mut self, justification: &PendingJustification<B>, is_descendent_of: F)
    where
        F: Fn(&B::Hash, &B::Hash) -> Result<bool, ClientError>,
    {
        match self.justifications.import(
            justification.0.clone(),
            justification.1.clone(),
            (),
            &is_descendent_of,
        ) {
            Ok(true) => {
                ...
                self.pending_requests
                    .push_back((justification.0, justification.1));
            }
            Err(err) => {
        ...
        F: Fn(&B::Hash, &B::Hash) -> Result<bool, ClientError>,
    {
        if self
            .importing_requests
            .contains(&(*best_finalized_hash, best_finalized_number))
        {
        ...
        self.pending_requests
            .retain(|(h, n)| roots.contains(&(h, n, &())));
        self.peer_requests
            .retain(|_, (h, n)| roots.contains(&(h, n, &())));
        self.previous_requests
            .retain(|(h, n), _| roots.contains(&(h, n, &())));
...
impl<B: BlockT> ChainSync<B> {
    ...
    fn state(&self, best_seen: &Option<NumberFor<B>>) -> SyncState {
        match best_seen {
            &Some(n) if n > self.best_queued_number && n - self.best_queued_number > As::sa(5) => {
    ...
    pub(crate) fn peer_info(&self, who: &PeerId) -> Option<PeerInfo<B>> {
        self.peers.get(who).map(|peer| PeerInfo {
    ...
    pub(crate) fn new_peer(&mut self, protocol: &mut Context<B>, who: PeerId) {
        ...
        if let Some(info) = protocol.peer_info(&who) {
            ...
            match (status, info.best_number) {
                ...
                (Ok(BlockStatus::KnownBad), _) => {
                    let reason = format!(
                        "New peer with known bad best block {} ({}).",
                        ...
                        self.peers.insert(
                            ...
                            PeerSync {
                                ...
                                state: PeerSyncState::AncestorSearch(
                                    common_best,
                                    AncestorSearchState::ExponentialBackoff(As::sa(1)),
                                ),
                                recently_announced: Default::default(),
                            },
                        );
        ...
        if previous_len < current_len {
            ...
            match (previous_state, current_state) {
                (SyncState::Idle, SyncState::Downloading) => {
                    self.is_major_syncing.store(true, Ordering::Relaxed)
                }
                (SyncState::Downloading, SyncState::Idle) => {
                    self.is_major_syncing.store(false, Ordering::Relaxed)
                }
                _ => {}
            }
        }
        ...
        match state {
            AncestorSearchState::ExponentialBackoff(next_distance_to_tip) => {
                ...
                if block_hash_match {
                    ...
                    let next_block_num = curr_block_num
                        .checked_sub(&next_distance_to_tip)
                        .unwrap_or(As::sa(0));
    ...
    pub(crate) fn on_block_data(
            ...
            match peer_state {
                ...
                PeerSyncState::AncestorSearch(num, state) => {
                    let block_hash_match = match (blocks.get(0), protocol.client().block_hash(num))
                    {
                        ...
                        (None, _) => {
                            ...
                            protocol.report_peer(
                                who,
                                Severity::Bad(
                                    "Invalid response when searching for ancestor".to_string(),
                                ),
                            );
                            return;
                        }
                        (_, Err(e)) => {
                            let reason =
                                format!("Error answering legitimate blockchain query: {:?}", e);
                            protocol.report_peer(who, Severity::Useless(reason));
                            return;
                        }
                    };
                    ...
                    if !block_hash_match && num == As::sa(0) {
                        ...
                        protocol.report_peer(
                            who,
                            Severity::Bad("Ancestry search: genesis mismatch for peer".to_string()),
                        );
                        return;
                    }
        ...
        let is_recent = new_blocks
            ...
            .map(|block| {
                self.peers
                    .iter()
                    .any(|(_, peer)| peer.recently_announced.contains(&block.hash))
            })
            .unwrap_or(false);
        let origin = if is_recent {
            BlockOrigin::NetworkBroadcast
        } else {
            BlockOrigin::NetworkInitialSync
        };

        if let Some((hash, number)) = new_blocks
            .last()
    ...
    pub fn request_justification(
        ...
        self.justifications
            .queue_request(&(*hash, number), |base, block| {
    ...
    pub fn justification_import_result(
        ...
        self.justifications
            .justification_import_result(hash, number, success);
    }
    ...
    pub fn on_block_finalized(
        ...
        if let Err(err) = self
            .justifications
            .on_block_finalized(hash, number, |base, block| {
                protocol.client().is_descendent_of(base, block)
            })
        {
    ...
    pub(crate) fn on_block_announce(
        ...
        let parent_status = block_status(
            &*protocol.client(),
            &self.queue_blocks,
        ...
        if !(known || self.is_already_downloading(&hash)) {
            ...
            if stale {
                if !(known_parent || self.is_already_downloading(header.parent_hash())) {
                    if protocol
                        .client()
                        .block_status(&BlockId::Number(*header.number()))
                        .unwrap_or(BlockStatus::Unknown)
    ...
    fn is_known(&self, protocol: &mut Context<B>, hash: &B::Hash) -> bool {
        block_status(&*protocol.client(), &self.queue_blocks, *hash)
            .ok()
            .map_or(false, |s| s != BlockStatus::Unknown)
    }
        ...
        if let Some(ref mut peer) = self.peers.get_mut(&who) {
            ...
            match peer.state {
                PeerSyncState::Available => {
                    ...
                    if let Some(range) = self.blocks.needed_blocks(
                        who.clone(),
...
fn block_status<B: BlockT>(
    ...
    queue_blocks: &HashSet<B::Hash>,
    hash: B::Hash,
) -> Result<BlockStatus, ClientError> {
