...
impl<B> OnDemandService<B> for OnDemand<B>
where
    B: BlockT,
    B::Header: HeaderT,
{
    ...
    fn on_remote_read_response(&self, peer: PeerId, response: message::RemoteReadResponse) {
        self.accept_response("read", peer, response.id, |request| match request.data {
            RequestData::RemoteRead(request, sender) => match self
                .checker
                .check_read_proof(&request, response.proof)
            {
                Ok(response) => {
                    // we do not bother if receiver has been dropped already
                    let _ = sender.send(Ok(response));
    ...
    fn on_remote_call_response(&self, peer: PeerId, response: message::RemoteCallResponse) {
        self.accept_response("call", peer, response.id, |request| match request.data {
            RequestData::RemoteCall(request, sender) => match self
                .checker
                .check_execution_proof(&request, response.proof)
            {
                Ok(response) => {
                    // we do not bother if receiver has been dropped already
                    let _ = sender.send(Ok(response));
...
pub mod tests {
    ...
    use crate::config::Roles;
    use crate::message;
    use crate::service::{network_channel, NetworkMsg, NetworkPort};
    ...
    fn disconnects_from_timeouted_peer() {
        ...
        on_demand.core.lock().active_peers[&peer0].timestamp =
    ...
    fn disconnects_from_peer_on_response_with_wrong_id() {
        ...
        assert_disconnected_peer(
            network_port,
            Severity::Bad("Invalid remote call response from peer".to_string()),
        );
    ...
    fn disconnects_from_peer_on_incorrect_response() {
        ...
        assert_disconnected_peer(
            network_port,
            Severity::Bad(
                "Failed to check remote call response from peer: Backend error: Test error"
                    .to_string(),
            ),
        );
        assert_eq!(on_demand.core.lock().pending_requests.len(), 1);
    ...
    fn disconnects_from_peer_on_wrong_response_type() {
        ...
        assert_disconnected_peer(
            network_port,
            Severity::Bad("Unexpected response to remote read from peer".to_string()),
        );
        assert_eq!(on_demand.core.lock().pending_requests.len(), 1);
    }
