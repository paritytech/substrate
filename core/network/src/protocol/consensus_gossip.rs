...
//! under a single topic to all peers who don't have them yet, and `send_topic` to send all messages
//! under a single topic to a specific peer.
//!
//! Each consensus engine ID can have an associated, registered `Validator` for all gossip messages.
//! The primary role of this `Validator` is to process incoming messages from peers, and decide whether
//! to discard them or process them. It also decides whether to re-broadcast the message.
//!
//! The secondary role of the `Validator` is to check if a message is allowed to be sent to a given
//! peer. All messages, before being sent, will be checked against this filter. This enables the validator
//! to use information it's aware of about connected peers to decide whether to send messages to them at
//! any given moment in time - In particular, to wait until peers can accept and process the message before
//! sending it.
//!
//! Lastly, the fact that gossip validators can decide not to rebroadcast messages opens the door for neighbor
//! status packets to be baked into the gossip protocol. These status packets will typically contain light pieces of information
