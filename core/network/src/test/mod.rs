...
impl<S: NetworkSpecialization<Block>> ProtocolChannel<S> {
	...
	pub fn sync_channels(&self) {
		// there's no currently API for nosync send to the network_to_protocol_sender
		// => we only need to sync client -> protocol channel
	...
	/// Send message from client to protocol without synching the channel.
	pub fn send_from_client_nosync(&self, message: ProtocolMsg<Block, S>) {
		let _ = self.client_to_protocol_sender.unbounded_send(message);
	}
...
impl<D, S: NetworkSpecialization<Block>> Peer<D, S> {
	...
	/// Synchronize peer message channels (if required).
	fn channels_sync(&self) {
		self.net_proto_channel.sync_channels();
	}
	...
	{
		self.net_proto_channel.send_from_client_nosync(ProtocolMsg::ExecuteWithGossip(Box::new(f)));
	}
...
pub trait TestNetFactory: Sized {
	...
	fn route_single(
		...
		// we might still have messages in channels that are not yet processed (sent using send_unsync)
		peers.iter().for_each(|peer| peer.channels_sync());
