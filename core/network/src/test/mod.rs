...
use crate::service::{
    network_channel, NetworkChan, NetworkLink, NetworkMsg, NetworkPort, TransactionPool,
};
...
use crossbeam_channel::{self as channel, select, Sender};
...
use primitives::{ed25519::Public as AuthorityId, H256};
use runtime_primitives::generic::BlockId;
use runtime_primitives::traits::{
    AuthorityIdFor, Block as BlockT, Digest, DigestItem, Header, NumberFor,
};
use runtime_primitives::{ConsensusEngineId, Justification};
...
impl<D, S: NetworkSpecialization<Block> + Clone> Peer<D, S> {
    fn new(
        ...
        import_queue
            .start(Box::new(network_link.clone()))
    ...
    fn on_connect(&self, other: &Self) {
        let _ = self
            .network_to_protocol_sender
            .send(FromNetworkMsg::PeerConnected(
                other.peer_id.clone(),
                String::new(),
            ));
    }
    ...
    fn on_disconnect(&self, other: &Self) {
        let _ = self
            .network_to_protocol_sender
            .send(FromNetworkMsg::PeerDisconnected(
                other.peer_id.clone(),
                String::new(),
            ));
    }
    ...
    pub fn send_finality_notifications(&self) {
        ...
        let header = self
            .client
            .header(&BlockId::Hash(info.chain.finalized_hash))
            .unwrap()
            .unwrap();
        let _ = self.protocol_sender.send(ProtocolMsg::BlockFinalized(
    ...
    pub fn consensus_gossip_messages_for(
        ...
        rx.wait()
            .ok()
            .expect("1. Network is running, 2. it should handle the above closure successfully")
    }

    /// Execute a closure with the consensus gossip.
    pub fn with_gossip<F>(&self, f: F)
    where
        F: FnOnce(&mut ConsensusGossip<Block>, &mut Context<Block>) + Send + 'static,
    {
        let _ = self
            .protocol_sender
        ...
        if with_tx {
            self.generate_blocks_at(at, count, BlockOrigin::File, |mut builder| {
                ...
                let signature = AccountKeyring::from_public(&transfer.from)
                    ...
                    .into();
                builder
                    .push(Extrinsic::Transfer(transfer, signature))
                    .unwrap();
                nonce = nonce + 1;
                builder.bake().unwrap()
            })
...
pub trait TestNetFactory: Sized {
    ...
    fn mut_peers<F: FnOnce(&mut Vec<Arc<Peer<Self::PeerData, Self::Specialization>>>)>(
        &mut self,
        closure: F,
    );
    ...
    fn add_peer(&mut self, config: &ProtocolConfig) {
        ...
        let peers: Arc<RwLock<HashMap<PeerId, ConnectedPeer<Block>>>> =
        ...
        self.mut_peers(|peers| peers.push(peer));
    }
    ...
    fn route_fast(&mut self) {
        self.mut_peers(move |peers| {
            for peer in 0..peers.len() {
                while let Some(NetworkMsg::Outgoing(recipient, packet)) =
                    peers[peer].pending_message_fast()
                {
...
impl TestNetFactory for JustificationTestNet {
    ...
    fn make_block_import(
        ...
        (
            client.clone(),
            Some(Arc::new(ForceFinalized(client))),
            Default::default(),
        )
    }
}
