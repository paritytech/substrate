...
impl NetTopology {
	/// Initializes a new empty `NetTopology`.
	#[inline]
	pub fn empty(local_peer_id: PeerId) -> NetTopology {
		NetTopology {
	...
	pub fn try_load(mut file: impl Read, local_peer_id: PeerId) -> Result<Self, IoError> {
		...
		let num_read = file.read(&mut first_byte)?;

		if num_read == 0 {
			// File is empty.
			debug!(target: "sub-libp2p", "Peer storage file is empty");
			Ok(Self::empty(local_peer_id))

		} else {
			let data = Cursor::new(first_byte).chain(file);
			match serde_json::from_reader::<_, serde_json::Value>(data) {
				Ok(serde_json::Value::Null) => Ok(Self::empty(local_peer_id)),
				Ok(serde_json::Value::Object(map)) => {
					let store = deserialize_tolerant(map.into_iter(), &local_peer_id);
					Ok(NetTopology {
						store,
						local_peer_id,
					})
				...
				Ok(_) =>
					...
					Err(IoError::new(IoErrorKind::InvalidData, "peer store didn't contain a Map")),
				Err(err) =>
					Err(IoError::new(IoErrorKind::InvalidData, err.to_string())),
			}
		}
	}

	/// Writes the topology to the writer given.
	//
	// Note: skips entries that are invalid or expired.
	pub fn serialize(&mut self, out: impl Write) -> Result<(), IoError> {
		let now = SystemTime::now();
		let array: FnvHashMap<_, _> = self.store.iter_mut().filter_map(|(peer, info)| {
