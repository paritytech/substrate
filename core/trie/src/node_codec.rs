...
use super::{
    branch_node, node_header::NodeHeader, partial_to_key, take, EMPTY_TRIE, EXTENSION_NODE_BIG,
    EXTENSION_NODE_OFFSET, LEAF_NODE_BIG, LEAF_NODE_OFFSET,
};
use crate::error::Error;
use codec::{Compact, Decode, Encode};
use hash_db::Hasher;
use std::marker::PhantomData;
use trie_db::{self, node::Node, ChildReference, DBValue, NibbleSlice};
...
impl<H: Hasher> trie_db::NodeCodec<H> for NodeCodec<H> {
    ...
    fn decode(data: &[u8]) -> ::std::result::Result<Node, Self::Error> {
        ...
        match NodeHeader::decode(input).ok_or(BadFormat)? {
            ...
            NodeHeader::Extension(nibble_count) => {
                ...
                Ok(Node::Extension(
                    nibble_slice,
                    take(input, count).ok_or(BadFormat)?,
                ))
            }
            ...
            + Iterator<Item = Option<ChildReference<H::Out>>>,
    {
        ...
        let prefix = branch_node(
            have_value,
            children.map(|maybe_child| match maybe_child {
