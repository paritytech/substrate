...
pub use memory_db::{prefixed_key, KeyFunction};
/// The Substrate format implementation of `NodeCodec`.
pub use node_codec::NodeCodec;
/// Various re-exports from the `trie-db` crate.
pub use trie_db::{DBValue, Query, Recorder, Trie, TrieMut};
...
pub fn trie_root<H: Hasher, I, A, B>(input: I) -> H::Out
where
...
pub fn delta_trie_root<H: Hasher, I, A, B, DB>(
    ...
    mut root: H::Out,
    delta: I,
) -> Result<H::Out, Box<TrieError<H::Out>>>
where
...
pub fn unhashed_trie<H: Hasher, I, A, B>(input: I) -> Vec<u8>
where
    I: IntoIterator<Item = (A, B)>,
    A: AsRef<[u8]> + Ord,
    B: AsRef<[u8]>,
...
pub fn child_trie_root<H: Hasher, I, A, B>(_storage_key: &[u8], input: I) -> Vec<u8>
where
    I: IntoIterator<Item = (A, B)>,
    A: AsRef<[u8]> + Ord,
    B: AsRef<[u8]>,
...
pub fn child_delta_trie_root<H: Hasher, I, A, B, DB>(
    ...
    root_vec: Vec<u8>,
    delta: I,
) -> Result<Vec<u8>, Box<TrieError<H::Out>>>
where
    I: IntoIterator<Item = (A, Option<B>)>,
    A: AsRef<[u8]> + Ord,
    B: AsRef<[u8]>,
...
fn partial_to_key(partial: &[u8], offset: u8, big: u8) -> Vec<u8> {
    ...
    if nibble_count >= big_threshold {
        output.push((nibble_count - big_threshold) as u8)
    }
    if nibble_count % 2 == 1 {
...
mod tests {
    ...
    fn random_should_work() {
        ...
        for test_i in 0..10000 {
            ...
            memtrie.commit();
            if *memtrie.root()
                != <NodeCodec<Blake2Hasher> as trie_db::NodeCodec<Blake2Hasher>>::hashed_null_node()
            {
                println!("- TRIE MISMATCH");
                println!("");
                println!(
    ...
    fn iterator_works() {
        let pairs = vec![
            (
                ...
                hex!("0400000000").to_vec(),
            ),
            (
                hex!("0103000000000000000469").to_vec(),
                hex!("0401000000").to_vec(),
            ),
        ];
