    ...
    fn poll(&mut self) -> Option<CustomProtoHandlerOut<TMessage>> {
        for n in (0..self.pending_response.len()).rev() {
            ...
            match substream.poll() {
                Ok(Async::Ready(Some(RegisteredProtocolEvent::Message(message)))) => {
                    if message.request_id() == CustomMessageId::Response(request_id) {
                        ...
                        let event = CustomProtoHandlerOut::ProtocolError {
                            is_severe: true,
                            error: format!(
                                "Request ID doesn't match substream: expected {:?}, \
                                 got {:?}",
                                request_id,
                                message.request_id()
                            )
                ...
                Ok(Async::NotReady) => self.pending_response.push((request_id, substream)),
                Ok(Async::Ready(None)) => {
                    self.shutdown.push(substream);
                    let event = CustomProtoHandlerOut::ProtocolError {
                        is_severe: false,
                        error: format!("Request ID {:?} didn't receive an answer", request_id)
                            .into(),
                    };
                    return Some(event);
                }
                Err(err) => {
                    ...
                    let event = CustomProtoHandlerOut::ProtocolError {
                        is_severe: false,
                        error: format!(
                            "Error while waiting for an answer for {:?}: {}",
                            request_id, err
                        )
                        .into(),
                    };
        ...
        for n in (0..self.incoming_substreams.len()).rev() {
            ...
            match substream.poll() {
                Ok(Async::Ready(Some(RegisteredProtocolEvent::Message(message)))) => {
                    return match message.request_id() {
                        CustomMessageId::Request(id) => {
                            ...
                            Some(CustomProtoHandlerOut::CustomMessage { message })
                        }
                ...
                Ok(Async::NotReady) => self.incoming_substreams.push(substream),
                Ok(Async::Ready(None)) => {}
                Err(err) => {
                    self.shutdown.push(substream);
    ...
    fn disable(&mut self) {
        self.state = match mem::replace(&mut self.state, ProtocolState::Poisoned) {
            ...
            ProtocolState::BackCompat {
                ...
                let event = CustomProtoHandlerOut::CustomProtocolClosed { result: Ok(()) };
                self.events_queue.push(ProtocolsHandlerEvent::Custom(event));
                ProtocolState::Disabled {
    ...
    > {
        ...
        self.state = match mem::replace(&mut self.state, ProtocolState::Poisoned) {
            ...
            ProtocolState::BackCompat {
                ...
                Err(err) => {
                    let event = CustomProtoHandlerOut::CustomProtocolClosed { result: Err(err) };
                    return_value = Some(ProtocolsHandlerEvent::Custom(event));
                    ProtocolState::Disabled {
                        shutdown: shutdown.into_iter().collect(),
                        reenable: false,
        ...
        self.state = match mem::replace(&mut self.state, ProtocolState::Poisoned) {
            ...
            ProtocolState::Opening { .. } => {
                ...
                match (substream.endpoint(), substream.is_multiplex()) {
                    (Endpoint::Dialer, true) => ProtocolState::Normal(PerProtocolNormalState {
                    ...
                    (Endpoint::Listener, true) => ProtocolState::Normal(PerProtocolNormalState {
                        outgoing_substream: None,
                    ...
                    (_, false) => ProtocolState::BackCompat {
                        substream,
                        shutdown: SmallVec::new(),
                    },
                }
            ...
            ProtocolState::Normal(ref mut state) => {
                if let CustomMessageId::Request(request_id) = message.request_id() {
                    if let Some(mut outgoing_substream) = state.outgoing_substream.take() {
                        ...
                        state
                            .pending_response
                            .push((request_id, outgoing_substream));
                    } else {
    ...
    TMessage: CustomMessage,
{
    ...
    fn connection_keep_alive(&self) -> KeepAlive {
        ...
        match self.state {
            ...
            ProtocolState::BackCompat { .. } | ProtocolState::Normal { .. } => keep_forever = true,
