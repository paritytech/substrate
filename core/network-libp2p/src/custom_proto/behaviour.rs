...
use std::{
    cmp, collections::hash_map::Entry, error, io, marker::PhantomData, mem, time::Duration,
    time::Instant,
};
...
pub struct CustomProto<TMessage, TSubstream> {
    ...
    events: SmallVec<
        [NetworkBehaviourAction<CustomProtoHandlerIn<TMessage>, CustomProtoOut<TMessage>>; 4],
    >,
    ...
    TMessage: CustomMessage,
{
    ...
    fn inject_connected(&mut self, peer_id: PeerId, connected_point: ConnectedPoint) {
        match (self.peers.entry(peer_id), connected_point) {
            ...
            (Entry::Vacant(entry), connected_point) => {
                ...
                entry.insert(PeerState::Disabled {
                    open: false,
                    connected_point,
                    banned_until: None,
                });
    ...
    fn inject_node_event(&mut self, source: PeerId, event: CustomProtoHandlerOut<TMessage>) {
        match event {
            CustomProtoHandlerOut::CustomProtocolClosed { result } => {
                ...
                match self.peers.get_mut(&source) {
                    Some(PeerState::Enabled { ref mut open, .. }) if *open => *open = false,
                    Some(PeerState::Disabled { ref mut open, .. }) if *open => *open = false,
    ...
    ) -> Async<NetworkBehaviourAction<CustomProtoHandlerIn<TMessage>, Self::OutEvent>> {
