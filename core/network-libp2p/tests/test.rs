...
use futures::{future, prelude::*, stream, try_ready};
use rand::seq::SliceRandom;
use std::io;
use substrate_network_libp2p::{build_multiaddr, multiaddr::Protocol, CustomMessage, ServiceEvent};
    ...
    TMsg: CustomMessage + Send + 'static,
{
    ...
    for _ in 0..num {
        ...
        result.push(
            substrate_network_libp2p::start_service(config, proto)
                .unwrap()
                .0,
        );
    }
...
fn basic_two_nodes_requests_in_parallel() {
    ...
    let mut to_send = {
        ...
        for _ in 0..200 {
            ...
            let msg = (
                id,
                (0..10).map(|_| rand::random::<u8>()).collect::<Vec<_>>(),
            );
            to_send.push(msg);
        }
        to_send
