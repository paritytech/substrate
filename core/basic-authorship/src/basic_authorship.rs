...
//
use std::{self, sync::Arc, time};

use log::{debug, info, trace, warn};

use client::{
    self, block_builder::api::BlockBuilder as BlockBuilderApi, error, runtime_api::Core,
    CallExecutor, Client as SubstrateClient,
};
use codec::Decode;
use consensus_common::{self, evaluation};
use inherents::{pool::InherentsPool, InherentData};
use primitives::{Blake2Hasher, ExecutionContext, H256};
use runtime_primitives::generic::BlockId;
use runtime_primitives::traits::{
    AuthorityIdFor, Block as BlockT, Hash as HashT, Header as HeaderT, ProvideRuntimeApi,
};
...
impl<C, A> consensus_common::Environment<<C as AuthoringApi>::Block> for ProposerFactory<C, A>
where
    C: AuthoringApi,
    ...
    Proposer<<C as AuthoringApi>::Block, C, A>:
        consensus_common::Proposer<<C as AuthoringApi>::Block>,
{
...
impl<Block, C, A> consensus_common::Proposer<<C as AuthoringApi>::Block> for Proposer<Block, C, A>
where
    Block: BlockT,
...
mod tests {
    ...
    fn should_cease_building_block_when_deadline_is_reached() {
        ...
        txpool
            .submit_at(&BlockId::number(0), vec![extrinsic(0), extrinsic(1)])
            .unwrap();
