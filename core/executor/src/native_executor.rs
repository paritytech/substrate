...
use std::{result, panic::UnwindSafe};
...
impl<D: NativeExecutionDispatch> RuntimeInfo for NativeExecutor<D> {
	...
	fn runtime_version<E: Externalities<Blake2Hasher>>(
		...
		RUNTIMES_POOL.with(|pool| {
			pool.fetch_runtime_from_pool(&self.fallback, ext, self.default_heap_pages, None)
				.ok()?.1.clone()
		})
	}
}
...
impl<D: NativeExecutionDispatch> CodeExecutor<Blake2Hasher> for NativeExecutor<D> {
	...
	>(
		...
		RUNTIMES_POOL.with(|pool| {
			let (module, onchain_version) = match pool.fetch_runtime_from_pool(
