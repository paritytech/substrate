...
use std::{collections::HashSet, pin::Pin, sync::Arc, task::Context, task::Poll, time::Duration};
...
impl super::Network<Block> for TestNetwork {
	type In = Pin<Box<dyn Stream<Item = Result<network_gossip::TopicNotification, ()>> + Send>>;
