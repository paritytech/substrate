...
use consensus_common::{BlockOrigin, ForkChoiceStrategy, ImportBlock, ImportResult, ImportedAux};
...
use std::result;
use substrate_primitives::{ExecutionContext, NativeOrEncoded};
...
impl TestNetFactory for GrandpaTestNet {
    ...
    fn make_block_import(
        ...
        let (import, link) = block_import(client, Arc::new(self.test_config.clone()))
...
impl Network<Block> for MessageRouting {
    ...
    fn messages_for(&self, round: u64, set_id: u64) -> Self::In {
        ...
        let peer = inner.peer(self.peer_id);
        let messages =
            peer.consensus_gossip_messages_for(GRANDPA_ENGINE_ID, make_topic(round, set_id));

        let messages =
            messages.map_err(move |_| panic!("Messages for round {} dropped too early", round));
    ...
    fn commit_messages(&self, set_id: u64) -> Self::In {
        ...
        let peer = inner.peer(self.peer_id);
        let messages =
            peer.consensus_gossip_messages_for(GRANDPA_ENGINE_ID, make_commit_topic(set_id));

        let messages = messages
            .map_err(move |_| panic!("Commit messages for set {} dropped too early", set_id));
...
fn finalize_3_voters_no_observers() {
    ...
    for i in 0..3 {
        assert_eq!(
            net.peer(i).client().info().unwrap().chain.best_number,
            20,
            "Peer #{} failed to sync",
            i
        );
    ...
    assert!(
        net.lock()
            ...
            .backend()
            .blockchain()
            .justification(BlockId::Number(20))
            .unwrap()
            .is_none(),
...
fn transition_3_voters_twice_1_observer() {
    ...
    for (i, peer) in net.lock().peers().iter().enumerate() {
        assert_eq!(
            peer.client().info().unwrap().chain.best_number,
            1,
            "Peer #{} failed to sync",
            i
        );
        ...
        let set: AuthoritySet<Hash, BlockNumber> =
            crate::aux_schema::load_authorities(&**peer.client().backend()).unwrap();
    ...
    for (peer_id, local_key) in all_peers {
        ...
        finality_notifications.push(
            client
                ...
                .map(move |()| {
                    let set: AuthoritySet<Hash, BlockNumber> =
                        crate::aux_schema::load_authorities(&**client.backend()).unwrap();
...
fn justification_is_generated_periodically() {
    ...
    for i in 0..3 {
        assert!(net
            ...
            .backend()
            .blockchain()
            .justification(BlockId::Number(32))
            .unwrap()
            .is_some());
    }
}
...
fn consensus_changes_works() {
    ...
    assert_eq!(
        changes
            .finalize((5, H256::from_low_u64_be(5)), |_| Ok(None))
            .unwrap(),
        (false, false)
    );
    ...
    assert_eq!(
        changes
            .finalize((10, H256::from_low_u64_be(10)), |_| Ok(Some(
                H256::from_low_u64_be(1001)
            )))
            .unwrap(),
        (true, false)
    );
...
fn sync_justifications_on_change_blocks() {
    ...
    for i in 0..3 {
        assert!(net
            ...
            .peer(i)
            .client()
            .justification(&BlockId::Number(21))
            .unwrap()
            .is_some());
    }
...
fn force_change_to_new_set() {
    ...
    let add_blocks = move || {
...

        {
            ...
            forced_transitions.lock().insert(
                parent_hash,
                (
                    0,
                    ScheduledChange {
            ...
            normal_transitions.lock().insert(
                parent_hash,
                ScheduledChange {
...
fn allows_reimporting_change_blocks() {
    ...
    let block = builder.bake().unwrap();
    api.scheduled_changes.lock().insert(
        *block.header.parent_hash(),
        ScheduledChange {
            next_authorities: make_ids(peers_b),
            delay: 0,
