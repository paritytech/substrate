...
use runtime_primitives::traits::{Block as BlockT, Header as HeaderT, NumberFor, One};
    ...
    G: Fn(&BlockId<Block>, &str, &[u8]) -> ClientResult<Vec<Vec<u8>>>,
{
    ...
    let authorities_proof =
        generate_execution_proof(&parent_block_id, "GrandpaApi_grandpa_authorities", &[])?;
    ...
    Err(ClientErrorKind::Backend("cannot find justification for finalized block".into()).into())
}
    ...
    J: ProvableJustification<Block::Header>,
{
    ...
    let proof =
        FinalityProof::<Block::Header, J>::decode(&mut &remote_proof[..]).ok_or_else(|| {
            ClientErrorKind::BadJustification("failed to decode finality proof".into())
        })?;

    // check that the first header in finalization path is the block itself
    {
        let finalized_header = proof.finalization_path.first().ok_or_else(|| {
            ClientError::from(ClientErrorKind::BadJustification(
    ...
    let grandpa_authorities: Vec<(AuthorityId, u64)> =
        Decode::decode(&mut &grandpa_authorities[..]).ok_or_else(|| {
            ClientErrorKind::BadJustification(
    ...
    proof
        .justification
        .verify(set_id, &grandpa_authorities.into_iter().collect())?;
...
mod tests {
    ...
    fn finality_proof_is_not_generated_for_non_final_block() {
        ...
        let proof_of_4 =
            prove_finality(&blockchain, |_, _, _| Ok(vec![vec![42]]), header(4).hash()).unwrap();
    ...
    fn finality_proof_fails_for_non_canonical_block() {
        ...
        let proof_of_side_4_fails = prove_finality(
            &blockchain,
            |_, _, _| Ok(vec![vec![42]]),
    ...
    fn finality_proof_fails_if_no_justification_known() {
        ...
        let proof_of_4_fails = prove_finality(
            &blockchain,
            |_, _, _| Ok(vec![vec![42]]),
            H256::from_low_u64_be(42),
        )
    ...
    fn prove_finality_is_generated() {
        ...
        let proof_of_2: FinalityProof =
            prove_finality(&blockchain, |_, _, _| Ok(vec![vec![42]]), header(2).hash())
                .unwrap()
                .and_then(|p| Decode::decode(&mut &p[..]))
                .unwrap();
        assert_eq!(
            proof_of_2,
            FinalityProof {
        ...
        let proof_of_3: FinalityProof =
            prove_finality(&blockchain, |_, _, _| Ok(vec![vec![42]]), header(3).hash())
                .unwrap()
                .and_then(|p| Decode::decode(&mut &p[..]))
                .unwrap();
        assert_eq!(
    ...
    fn finality_proof_check_fails_when_block_is_not_included() {
        ...
        assert_eq!(
            do_check_finality_proof::<Block, _, ValidFinalityProof>(
    ...
    fn finality_proof_check_fails_when_justification_verification_fails() {
        ...
        impl ProvableJustification<Header> for InvalidFinalityProof {
            ...
            fn verify(
                &self,
                _set_id: u64,
                _authorities: &VoterSet<AuthorityId>,
            ) -> ClientResult<()> {
        ...
        assert_eq!(
            do_check_finality_proof::<Block, _, InvalidFinalityProof>(
                |_| Ok(Vec::<u8>::new().encode()),
                header(1),
                (2, header(2).hash()),
                1,
