...
use crate::{
    Commit, CompactCommit, Error, FullCommitMessage, GossipMessage, Message, Network,
    SignedMessage, VoteOrPrecommitMessage,
};
use ed25519::{Public as AuthorityId, Signature as AuthoritySignature};
use futures::prelude::*;
use futures::sync::mpsc;
use grandpa::Message::{Precommit, Prevote};
...
pub(crate) fn rebroadcasting_network<B: BlockT, N: Network<B>>(
    network: N,
) -> (BroadcastWorker<B, N>, BroadcastHandle<B, N>) {
    ...
    (
        ...
        BroadcastHandle { relay: tx, network },
    )
}
    ...
    fn poll(&mut self) -> Poll<(), Error> {
        {
            ...
            if rebroadcast {
                ...
                if let Some((Round(c_round), ref c_commit)) = self.last_commit {
                    self.network
                        .send_commit(c_round, set_id, c_commit.clone(), true);
                }
        ...
        loop {
            match self
                .incoming_broadcast
                .poll()
                .expect("UnboundedReceiver does not yield errors; qed")
            {
                    ...
                    match item {
                        Broadcast::Commit(round, set_id, commit) => {
                            if self.set_id == set_id {
                                if round >= self.last_commit.as_ref().map_or(Round(0), |&(r, _)| r)
                                {
                            ...
                            self.network
                                .send_commit(round.0, self.set_id.0, commit, false);
                        }
...
impl<B: BlockT, N: Network<B>> Network<B> for BroadcastHandle<B, N> {
    ...
    fn send_message(&self, round: u64, set_id: u64, message: Vec<u8>, _force: bool) {
        let _ = self
            .relay
            .unbounded_send(Broadcast::Message(Round(round), SetId(set_id), message));
    }

    fn drop_round_messages(&self, round: u64, set_id: u64) {
        let _ = self
            .relay
            .unbounded_send(Broadcast::DropRound(Round(round), SetId(set_id)));
    }
    ...
    fn send_commit(&self, round: u64, set_id: u64, message: Vec<u8>, _force: bool) {
        let _ = self
            .relay
            .unbounded_send(Broadcast::Commit(Round(round), SetId(set_id), message));
    }

    fn announce(&self, round: u64, set_id: u64, block: B::Hash) {
        let _ =
            self.relay
                .unbounded_send(Broadcast::Announcement(Round(round), SetId(set_id), block));
    }
}
...
impl<Block: BlockT, N: Network<Block>> Sink for OutgoingMessages<Block, N> {
    ...
    fn start_send(&mut self, msg: Message<Block>) -> StartSend<Message<Block>, Error> {
        ...
        if let Some((ref pair, ref local_id)) = self.locals {
            ...
            self.network
                .send_message(self.round, self.set_id, message.encode(), false);
...
impl<Block: BlockT, N: Network<Block>> Sink for CommitsOut<Block, N> {
    ...
    fn start_send(&mut self, input: (u64, Commit<Block>)) -> StartSend<Self::SinkItem, Error> {
        ...
        self.network
            .send_commit(round, self.set_id, Encode::encode(&message), false);
