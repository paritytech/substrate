...
use runtime_primitives::traits::{
    Block as BlockT, DigestFor, DigestItem, DigestItemFor, Hash as HashT, Header as HeaderT,
    NumberFor, ProvideRuntimeApi,
};
...
use grandpa::{round::State as RoundState, voter, BlockNumberOps, VoterSet};
...
pub use finality_proof::{check_finality_proof, prove_finality};
...
impl<Block: BlockT> GossipValidator<Block> {
    ...
    fn validate_commit_message(
        ...
        for (precommit, &(ref sig, ref id)) in
            full.message.precommits.iter().zip(&full.message.auth_data)
        {
            if let Err(()) = communication::check_message_sig::<Block>(
        ...
        let precommits_signed_by: Vec<String> = full
            .message
            .auth_data
            .iter()
...
impl<Block: BlockT> network_gossip::Validator<Block::Hash> for GossipValidator<Block> {
    ...
    fn message_expired<'a>(&'a self) -> Box<FnMut(Block::Hash, &[u8]) -> bool + 'a> {
        ...
        Box::new(
            move |_topic, mut data| match GossipMessage::<Block>::decode(&mut data) {
                None => true,
...
impl<B: BlockT, S: network::specialization::NetworkSpecialization<B>> Clone
    for NetworkBridge<B, S>
{
    fn clone(&self) -> Self {
        NetworkBridge {
...
impl<B: BlockT, S: network::specialization::NetworkSpecialization<B>> Network<B>
    for NetworkBridge<B, S>
{
    ...
    fn messages_for(&self, round: u64, set_id: u64) -> Self::In {
        ...
        self.service.with_gossip(move |gossip, _| {
            let inner_rx =
                gossip.messages_for(GRANDPA_ENGINE_ID, message_topic::<B>(round, set_id));
    ...
    fn send_message(&self, round: u64, set_id: u64, message: Vec<u8>, force: bool) {
        ...
        self.service
            .gossip_consensus_message(topic, GRANDPA_ENGINE_ID, message, force);
    }

    fn drop_round_messages(&self, round: u64, set_id: u64) {
        self.validator.drop_round(round, set_id);
        self.service
            .with_gossip(move |gossip, _| gossip.collect_garbage());
    }
...
fn committer_communication<Block: BlockT<Hash = H256>, B, E, N, RA>(
    ...
    impl Stream<
        Item = (
            u64,
            ::grandpa::CompactCommit<H256, NumberFor<Block>, AuthoritySignature, AuthorityId>,
        ),
    ...
    impl Sink<
        SinkItem = (
            u64,
            ::grandpa::Commit<H256, NumberFor<Block>, AuthoritySignature, AuthorityId>,
        ),
    ...
    DigestItemFor<Block>: DigestItem<AuthorityId = AuthorityId>,
{
    ...
    let commit_out =
        crate::communication::CommitsOut::<Block, _>::new(network.clone(), set_id, is_voter);
    ...
    if !inherent_data_providers.has_provider(&srml_finality_tracker::INHERENT_IDENTIFIER) {
        inherent_data_providers
            .register_provider(srml_finality_tracker::InherentDataProvider::new(
                move || match client.backend().blockchain().info() {
        ...
        let handle_voter_command = move |command: VoterCommand<_, _>, voter_commands_rx| {
            match command {
                ...
                VoterCommand::Pause(reason) => {
                    ...
                    let set_state = VoterSetState::Paused(last_round_number, last_round_state);
