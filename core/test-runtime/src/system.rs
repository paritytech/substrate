...
use rstd::prelude::*;
use runtime_io::{storage_root, enumerated_trie_root, storage_changes_root, twox_128, blake2_128};
...
pub fn balance_of(who: AccountId) -> u64 {
	storage::hashed::get_or(&blake2_128, &balance_of_key(who), 0)
}

pub fn nonce_of(who: AccountId) -> u64 {
	storage::hashed::get_or(&blake2_128, &who.to_keyed_vec(NONCE_OF), 0)
}
...
pub fn validate_transaction(utx: Extrinsic) -> TransactionValidity {
	...
	let expected_nonce: u64 = storage::hashed::get_or(&blake2_128, &nonce_key, 0);
...
fn execute_transfer_backend(tx: &Transfer) -> ApplyResult {
	...
	storage::hashed::put(&blake2_128, &nonce_key, &(expected_nonce + 1));

	// check sender balance
	let from_balance_key = tx.from.to_keyed_vec(BALANCE_OF);
	let from_balance: u64 = storage::hashed::get_or(&blake2_128, &from_balance_key, 0);
	...
	let to_balance: u64 = storage::hashed::get_or(&blake2_128, &to_balance_key, 0);
	storage::hashed::put(&blake2_128, &from_balance_key, &(from_balance - tx.amount));
	storage::hashed::put(&blake2_128, &to_balance_key, &(to_balance + tx.amount));
...
mod tests {
	...
	use runtime_io::{with_externalities, twox_128, blake2_128, TestExternalities};
	...
	fn new_test_ext() -> TestExternalities<Blake2Hasher> {
		TestExternalities::new(map![
			...
			blake2_128(&AccountKeyring::Alice.to_raw_public().to_keyed_vec(b"balance:")).to_vec() => vec![111u8, 0, 0, 0, 0, 0, 0, 0]
		])
	}
