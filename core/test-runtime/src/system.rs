...
use super::{AccountId, Block, BlockNumber, Digest, Extrinsic, Header, Transfer, H256 as Hash};
...
use runtime_primitives::traits::{
    BlakeTwo256, Block as BlockT, Digest as DigestT, Hash as HashT, NumberFor,
};
...
pub fn validate_transaction(utx: Extrinsic) -> TransactionValidity {
    ...
    let hash =
        |from: &AccountId, nonce: u64| twox_128(&nonce.to_keyed_vec(&from.encode())).to_vec();
...
mod tests {
    ...
    use substrate_test_client::{AccountKeyring, AuthorityKeyring};

    const WASM_CODE: &'static [u8] = include_bytes!(
    ...
    where
        F: Fn(Block, &mut TestExternalities<Blake2Hasher>),
    {
        let h = Header {
    ...
    fn block_import_works_wasm() {
        block_import_works(|b, ext| {
            WasmExecutor::new()
                .call(ext, 8, &WASM_CODE, "Core_execute_block", &b.encode())
                .unwrap();
        })
    }
