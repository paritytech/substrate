...
impl Encode for BabeSealSignature {
	fn encode(&self) -> Vec<u8> {
		parity_codec::Encode::encode(&(self.public_key.to_bytes(), self.proof.to_bytes(), self.signature.to_bytes()))
	}
}

impl Decode for BabeSealSignature {
	fn decode<R: Input>(i: &mut R) -> Option<Self> {
		let (public_key, proof, sig): ([u8; PUBLIC_KEY_LENGTH], [u8; VRF_PROOF_LENGTH], [u8; SIGNATURE_LENGTH]) = Decode::decode(i)?;
