...
use primitives::{blake2_256, Blake2Hasher, H256, Pair, Public, U256, ExecutionContext};
	...
	T: SubmitExtrinsic<B>,
{
	...
	let equivocation_proof = check_equivocation(
		client,
		slot_now,
		slot_number,
		&header,
...
impl<B, E, Block, RA, PRA, T> Verifier<Block> for BabeVerifier<B, E, Block, RA, PRA, T> where
	...
	RA: Send + Sync,
	PRA: ProvideRuntimeApi + HeaderBackend<Block> + ProvideCache<Block> + AuxStore + Send + Sync + 'static,
	...
	fn verify(
		...
		match checked_header {
			CheckedHeader::Checked(pre_header, (pre_digest, seal)) => {
				let babe_pre_digest: BabePreDigest = pre_digest.as_babe_pre_digest()
