...
use rhd::messages::{Message as PrimitiveMessage, Action as PrimitiveAction, Justification};
...
#[serde(bound = "B: DeserializeOwned")]
pub struct Consensus<B: Block>(::std::marker::PhantomData<B>);


impl<B> ConsensusT for Consensus<B>
where B: Block {
	type Block = B;
	type Message = Message<B>;
	type Signature = Justification<B::Hash>;

	fn confirm(block: Self::Block, signature: Self::Signature, authorities: &[AuthorityId])
		-> Result<(), &'static str>
	{
		let just = UncheckedJustification::from(signature.clone());
...
impl<H> Into<Justification<H>> for RhdJustification<H> {
	fn into(self) -> Justification<H> {
		let p : Justification<H> = UncheckedJustification(self.0.uncheck()).into();
		p
	}
}
...
impl<H> From<Justification<H>> for UncheckedJustification<H> {
	fn from(just: Justification<H>) -> Self {
		UncheckedJustification(rhododendron::UncheckedJustification {
...
impl<H> Into<Justification<H>> for UncheckedJustification<H> {
	fn into(self) -> Justification<H> {
		Justification {
	...
	OutSink: Sink<SinkItem=Communication<B>, SinkError=P::Error>,
{
	...
	fn poll(&mut self) -> ::futures::Poll<(), ()> {
		...
		if let Some(justified_block) = committed.candidate {
			...
			let just : Justification<B::Hash> = RhdJustification(committed.justification).into();
