...
use runtime_primitives::traits::{Block, ProvideRuntimeApi};
    ...
    OnExit: Future<Item = (), Error = ()> + Send + 'static,
{
    ...
    result_recv
        .recv()
        .expect("Aura start thread result sender dropped")
}
...
impl SlotDuration {
    ...
    /// state.
    pub fn get_or_compute<B: Block, C>(client: &C) -> ::client::error::Result<Self>
    where
        ...
        C::Api: AuraApi<B>,
    {
        ...
        match client.get_aux(SLOT_KEY)? {
            Some(v) => u64::decode(&mut &v[..]).map(SlotDuration).ok_or_else(|| {
                ::client::error::ErrorKind::Backend(format!(
                    "Aura slot duration kept in invalid format"
                ))
                .into()
            }),
            None => {
                use runtime_primitives::traits::Zero;
                let genesis_slot_duration = client
                    .runtime_api()
                ...
                genesis_slot_duration
                    .using_encoded(|s| client.insert_aux(&[(SLOT_KEY, &s[..])], &[]))?;
