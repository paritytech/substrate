...
use aura_slots::{CheckedHeader, SlotWorker, SlotInfo, SlotCompatible, check_equivocation};
		...
		if P::verify(&sig, &to_sign[..], public) {
			match check_equivocation(client, slot_num, header.clone()) {
				Ok(Some(equivocation_proof)) => {
					// sign_and_dispatch_encoded_call(construct_report_call(equivocation_proof));
