...
#![deny(deprecated)]
use std::{fmt::Debug, hash::Hash, marker::PhantomData, sync::Arc, thread, time::Duration};
...
use consensus_common::import_queue::{
    BasicQueue, SharedBlockImport, SharedJustificationImport, Verifier,
};
use consensus_common::well_known_cache_keys;
use consensus_common::{
    self, Authorities, BlockImport, BlockOrigin, Environment, Error as ConsensusError,
    ForkChoiceStrategy, ImportBlock, Proposer,
};
use inherents::{InherentData, InherentDataProviders, RuntimeString};
use parity_codec::{Decode, Encode};
use primitives::Pair;
use runtime_primitives::traits::{
    AuthorityIdFor, Block, Digest, DigestItem, DigestItemFor, Header, ProvideRuntimeApi,
};
...
use substrate_telemetry::{
    telemetry, CONSENSUS_DEBUG, CONSENSUS_INFO, CONSENSUS_TRACE, CONSENSUS_WARN,
};
...
impl<B: Block, C, E, I, P, Error, SO> SlotWorker<B> for AuraWorker<C, E, I, P, SO>
where
        ...
        let proposal_work = match maybe_author {
            ...
            Some(author) => {
                if author == public_key {
                    ...
                    Timeout::new(
                        proposer
                            .propose(slot_info.inherent_data, remaining_duration)
                            .into_future(),
                        remaining_duration,
                    )
                } else {
...
#[forbid(deprecated)]
impl<B: Block, C, E, P> Verifier<B> for AuraVerifier<C, E, P>
where
    ...
    P: Pair + Send + Sync + 'static,
    P::Public:
        Send + Sync + Hash + Eq + Clone + Decode + Encode + Debug + AsRef<P::Public> + 'static,
    ...
    Self: Authorities<B>,
{
    fn verify(
        ...
        let (timestamp_now, slot_now) =
            AuraSlotCompatible::extract_timestamp_and_slot(&inherent_data)
            ...
            CheckedHeader::Checked(pre_header, slot_num, sig) => {
                ...
                if let Some(inner_body) = body.take() {
                    ...
                    if self
                        .client
                        .runtime_api()
                        .has_api_with::<BlockBuilderApi<B>, _>(&BlockId::Hash(parent_hash), |v| {
                            v >= 2
                        })
                        .map_err(|e| format!("{:?}", e))?
                    {
...
impl<B, C, E, P> Authorities<B> for AuraVerifier<C, E, P>
where
    B: Block,
    C: ProvideRuntimeApi + ProvideCache<B>,
    C::Api: AuthoritiesApi<B>,
...
fn authorities<B, C>(client: &C, at: &BlockId<B>) -> Result<Vec<AuthorityIdFor<B>>, ConsensusError>
where
    B: Block,
    C: ProvideRuntimeApi + ProvideCache<B>,
    C::Api: AuthoritiesApi<B>,
...
pub fn import_queue<B, C, E, P>(
    ...
    inherent_data_providers: InherentDataProviders,
    allow_old_seals: bool,
) -> Result<AuraImportQueue<B>, consensus_common::Error>
where
    B: Block,
    ...
    P::Signature: Encode + Decode,
{
    ...
    let verifier = Arc::new(AuraVerifier {
        client: client.clone(),
        extra,
        inherent_data_providers,
        phantom: PhantomData,
...
mod tests {
    ...
    impl Environment<TestBlock> for DummyFactory {
        ...
        fn init(
            ...
            _authorities: &[AuthorityId<ed25519::Pair>],
    ...
    impl TestNetFactory for AuraTestNet {
        ...
        fn make_verifier(
            ...
            let slot_duration =
                SlotDuration::get_or_compute(&*client).expect("slot duration available");
