...
use crate::block_import::{
    BlockImport, BlockOrigin, ImportBlock, ImportResult, ImportedAux, JustificationImport,
};
...
impl<B: BlockT> BlockImporter<B> {
        ...
        for (result, hash) in results {
            ...
            match result {
                ...
                Err(BlockImportError::IncompleteHeader(who)) => {
                    if let Some(peer) = who {
                        link.note_useless_and_restart_sync(
                            peer,
                            "Sent block with incomplete header to import",
                        );
                    }
                }
    ...
    fn handle_import_justification(
        &self,
        who: Origin,
        hash: B::Hash,
        number: NumberFor<B>,
            ...
            .spawn(move || {
                ...
                for msg in port.iter() {
                    ...
                    match msg {
                        ...
                        _ => {
                            unreachable!("Import Worker does not receive the Imported message; qed")
                        }
                    }
                }
            })
...
pub fn import_single_block<B: BlockT, V: Verifier<B>>(
    ...
    if let Some(authorities) = new_authorities {
        cache.insert(
            crate::well_known_cache_keys::AUTHORITIES,
            authorities.encode(),
        );
    }
...
mod tests {
    ...
    fn process_import_result_works() {
        ...
        // Send a known
        let results = vec![(
            Ok(BlockImportResult::ImportedKnown(Default::default())),
            Default::default(),
        )];
        let _ = result_sender
            .send(BlockImportWorkerMsg::Imported(results))
        ...
        let results = vec![(
            Err(BlockImportError::IncompleteHeader(Some(peer_id.clone()))),
            Default::default(),
        )];
        let _ = result_sender
            .send(BlockImportWorkerMsg::Imported(results))
