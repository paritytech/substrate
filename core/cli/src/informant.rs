    ...
    C: Components,
{
    ...
    let mut last = match client.info() {
        ...
        Err(e) => {
            warn!("Error getting best block information: {:?}", e);
            None
        }
    };
...
fn speed(best_number: u64, last_number: Option<u64>, last_update: time::Instant) -> String {
    ...
    let speed = match last_number {
        Some(num) => {
            (best_number.saturating_sub(num) * 10_000
                / (since_last_millis + since_last_subsec_millis)) as f64
        }
        None => 0.0,
    };
