...
use network::{
    self, build_multiaddr,
    config::{NetworkConfiguration, NodeKeyConfig, NonReservedPeerMode},
    multiaddr::Protocol,
};
use primitives::H256;
use runtime_primitives::traits::As;
use service::{
    ChainSpec, FactoryFullConfiguration, FactoryGenesis, PruningMode, RuntimeGenesis,
    ServiceFactory,
};
...
use params::{
    ...
    NodeKeyParams, NodeKeyType, PurgeChainCmd, RevertCmd, RunCmd, SharedParams,
    TransactionPoolParams,
};
pub use params::{CoreParams, NoCustom};
use regex::Regex;
#[doc(hidden)]
...
where
    G: RuntimeGenesis,
    F: FnOnce(&str) -> Result<Option<ChainSpec<G>>, String>,
{
    ...
    T: Into<std::ffi::OsString> + Clone,
{
    ...
    match cli_args {
        params::CoreParams::Run(params) => {
            run_node::<F, _, _, _, _>(params, spec_factory, exit, run_service, impl_name, version)
                .map(|_| None)
        }
...
fn node_key_config<P>(
    params: NodeKeyParams,
    net_config_dir: &Option<P>,
    ...
    S: FnOnce(&str) -> Result<Option<ChainSpec<FactoryGenesis<F>>>, String>,
{
    ...
    config.pruning = match cli.pruning {
        ...
        Some(s) => PruningMode::keep_blocks(
            s.parse()
                .map_err(|_| input_err("Invalid pruning mode specified"))?,
        ),
    };
    ...
    let rpc_interface: &str = if cli.rpc_external {
        "0.0.0.0"
    } else {
        "127.0.0.1"
    };
    let ws_interface: &str = if cli.ws_external {
        "0.0.0.0"
    } else {
        "127.0.0.1"
    };

    config.rpc_http = Some(parse_address(
        &format!("{}:{}", rpc_interface, 9933),
        cli.rpc_port,
    )?);
    config.rpc_ws = Some(parse_address(
...
fn build_spec<F, S>(cli: BuildSpecCmd, spec_factory: S, version: &VersionInfo) -> error::Result<()>
where
    F: ServiceFactory,
    S: FnOnce(&str) -> Result<Option<ChainSpec<FactoryGenesis<F>>>, String>,
{
...
fn revert_chain<F, S>(cli: RevertCmd, spec_factory: S, version: &VersionInfo) -> error::Result<()>
where
    F: ServiceFactory,
    S: FnOnce(&str) -> Result<Option<ChainSpec<FactoryGenesis<F>>>, String>,
{
    let config = create_config_with_db_path::<F, _>(spec_factory, &cli.shared_params, version)?;
    let blocks = cli.num;
    Ok(service::chain_ops::revert_chain::<F>(
        config,
        As::sa(blocks),
    )?)
}
...
mod tests {
    use super::*;
    use network::identity::{ed25519, secp256k1};
    use tempdir::TempDir;
            ...
            NodeKeyType::variants().into_iter().try_for_each(|t| {
                ...
                node_key_config(params, &net_config_dir).and_then(|c| match c {
                    NodeKeyConfig::Secp256k1(network::Secret::Input(ref ski))
                        if node_key_type == NodeKeyType::Secp256k1 && &sk[..] == ski.as_ref() =>
                    {
                        Ok(())
                    }
            ...
            NodeKeyType::variants().into_iter().try_for_each(|t| {
                ...
                node_key_config(params, &net_config_dir).and_then(|c| match c {
                    NodeKeyConfig::Secp256k1(network::Secret::File(ref f))
                        if node_key_type == NodeKeyType::Secp256k1 && f == &file =>
                    {
                        Ok(())
                    }
    ...
    fn test_node_key_config_default() {
        ...
        fn no_config_dir() -> error::Result<()> {
            with_def_params(|params| {
                ...
                node_key_config::<String>(params, &None).and_then(|c| match c {
                    NodeKeyConfig::Secp256k1(network::Secret::New)
                        if typ == NodeKeyType::Secp256k1 =>
                    {
                        Ok(())
                    }
                    NodeKeyConfig::Ed25519(network::Secret::New) if typ == NodeKeyType::Ed25519 => {
                        Ok(())
                    }
                    _ => Err(input_err("Unexpected node key config")),
                })
        ...
        fn some_config_dir(net_config_dir: String) -> error::Result<()> {
            with_def_params(|params| {
                ...
                let typ = params.node_key_type;
                node_key_config(params, &Some(net_config_dir.clone())).and_then(move |c| match c {
                    NodeKeyConfig::Secp256k1(network::Secret::File(ref f))
                        if typ == NodeKeyType::Secp256k1
                            && f == &dir.join(NODE_KEY_SECP256K1_FILE) =>
                    {
                        Ok(())
                    }
                    NodeKeyConfig::Ed25519(network::Secret::File(ref f))
