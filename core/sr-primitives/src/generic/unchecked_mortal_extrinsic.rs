...
mod tests {
    ...
    fn large_signed_codec_should_work() {
        let ux = Ex::new_signed(
            ...
            TestSig(
                DUMMY_ACCOUNTID,
                (DUMMY_ACCOUNTID, vec![0u8; 257], Era::immortal(), 0u64).using_encoded(blake2_256)
                    [..]
                    .to_owned(),
            ),
            Era::immortal(),
    ...
    fn mortal_signed_check_should_work() {
        let ux = Ex::new_signed(
            ...
            TestSig(
                DUMMY_ACCOUNTID,
                (DUMMY_ACCOUNTID, vec![0u8; 0], Era::mortal(32, 42), 42u64).encode(),
            ),
            Era::mortal(32, 42),
        );
        assert!(ux.is_signed().unwrap_or(false));
    ...
    fn later_mortal_signed_check_should_work() {
        let ux = Ex::new_signed(
            ...
            TestSig(
                DUMMY_ACCOUNTID,
                (DUMMY_ACCOUNTID, vec![0u8; 0], Era::mortal(32, 11), 11u64).encode(),
            ),
            Era::mortal(32, 11),
        );
        assert!(ux.is_signed().unwrap_or(false));
