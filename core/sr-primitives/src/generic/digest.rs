...
//!
//! There are three k
//! _Inherent digests_ are digests that are provided to the runtime by the
//! consensus algorithm. They are inherent because the runtime trusts them
//! implicitly, although the consensus algorithm does verify them.
//!
//! This PR passes the digests in the header to the runtime via
//! `initialize_block`. It does _not_, however, provide any means for runtimes
//! to actually use the information. This will be provided later.
//!
//! # Changes to digest items
//!
//! The `DigestItem::Seal` variant had been
//! deprecated in an earlier PR. This PR removes the deprecation, and changes
//! it to contain `ConsensusEngineId` and a `SealSignature`. It also adds the
//! variant `DigestItem::Inherent` for pre-runtime digests.
//!
//! A consequence of these changes is that the following invariants must be
//! upheld:
//!
//! * Runtimes **MUST NOT** produce a `DigestItem::Seal` or
//! `DigestItem::Inherent`.  Only consensus engines can produce these
//! varients.
//!
//! * Consensus engines **MUST NOT** produce any variants other
//!   than `DigestItem::Seal` or `DigestItem::Inherent`.  Furthermore, there
//!   **MUST** be exactly one `DigestItem::Seal` per combination of block and
//!   consensus engine, and they must come after all other digests.  * The
//!   SRML reserves `DigestItem::ChangesTreeRoot` for itself.  Runtimes that
//!   use the SRML **MUST NOT** produce any `DigestItem` variant **except**
//! `DigestItem::AuthoritiesChange` and `DigestItem::Consensus`.
//!
//! These invarients are currently not checked. However, I plan on improving
//! the API to catch violations of them at compile-time, by making illegal
//! states unrepresentable.
//!
//! # Backwards compatibility There isn’t any. Versions of Substrate
//! that include this PR are 100% incompatible with versions that came
//! before. Attempting to combine incompatible versions will result in blocks
//! failing to import.
//!
//! If necessary, I can include read-only backwards-compatibility support. This
//! will allow nodes including this PR to import blocks generated by older
//! nodes, but not create blocks that can be imported by older nodes.
//!
//! # `Cargo.lock` changes Much of the changes are to `Cargo.lock` files,
//! as some of the code depends on newer versions of dependencies. Reverting
//! these dependencies is trivial, but I believe that upgrading dependencies
//! is a good thing, so that Substrate can take advantage of upstream
//! bug-fixes. This is especially true of libraries like _ring_, which do not
//! do any backports at all.
//!
//! Fixes #2232
