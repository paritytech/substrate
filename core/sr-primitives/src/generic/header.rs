...
use crate::codec::{Codec, Decode, Encode, EncodeAsRef, HasCompact, Input, Output};
use crate::generic::Digest;
use crate::traits::{
    self, DigestItem as DigestItemT, Hash as HashT, MaybeDisplay, MaybeSerializeDebug,
    MaybeSerializeDebugButNotDeserialize, Member, SimpleArithmetic, SimpleBitOps,
};
#[cfg(feature = "std")]
...
#[cfg(feature = "std")]
pub fn serialize_number<S, T: Copy + Into<u128>>(val: &T, s: S) -> Result<S::Ok, S::Error>
where
    S: ::serde::Serializer,
{
...
impl<Number, Hash, DigestItem> Decode for Header<Number, Hash, DigestItem>
where
...
impl<Number, Hash, DigestItem> Encode for Header<Number, Hash, DigestItem>
where
    Number: HasCompact + Copy + Into<u128>,
    Hash: HashT,
    Hash::Output: Encode,
...
impl<Number, Hash, DigestItem> traits::Header for Header<Number, Hash, DigestItem>
where
    Number: Member
        + MaybeSerializeDebug
        + ::rstd::hash::Hash
...
impl<Number, Hash, DigestItem> Header<Number, Hash, DigestItem>
where
    Number:
        Member + ::rstd::hash::Hash + Copy + MaybeDisplay + SimpleArithmetic + Codec + Into<u128>,
    Hash: HashT,
    DigestItem: DigestItemT + Codec,
    Hash::Output:
        Default + ::rstd::hash::Hash + Copy + Member + MaybeDisplay + SimpleBitOps + Codec,
{
