...
use crate::utils::{
    ...
    generate_runtime_mod_name_for_trait, return_type_extract_type, unwrap_or_error,
};
...
use syn::{
    ...
    spanned::Spanned,
    GenericArgument, Ident, ImplItem, ItemImpl, MethodSig, Path, PathArguments, Type, TypePath,
};
    ...
    fn fold_impl_item_method(&mut self, mut input: syn::ImplItemMethod) -> syn::ImplItemMethod {
        let block = {
            ...
            let (param_types, error) =
                match extract_parameter_names_types_and_borrows(&input.sig.decl) {
                    Ok(res) => (
                        res.into_iter()
                            .map(|v| {
                                let ty = v.1;
