...
use crate::utils::{
    ...
    return_type_extract_type, unwrap_or_error,
};
...
use syn::{
    ...
    spanned::Spanned,
    visit::{self, Visit},
    Attribute, FnArg, GenericParam, Generics, Ident, ItemTrait, Lit, Meta, NestedMeta, Pat,
    ReturnType, TraitBound, TraitItem, TraitItemMethod, Type,
};
...
fn extend_generics_with_block(generics: &mut Generics) {
    ...
    generics
        .params
        .insert(0, parse_quote!( Block: #c::runtime_api::BlockT ));
...
fn remove_supported_attributes(attrs: &mut Vec<Attribute>) -> HashMap<&'static str, Attribute> {
    ...
    attrs.retain(|v| {
        match SUPPORTED_ATTRIBUTE_NAMES
            .iter()
            .filter(|a| v.path.is_ident(a))
            .next()
        {
            Some(attribute) => {
...
fn generate_native_call_generators(decl: &ItemTrait) -> Result<TokenStream> {
    ...
    for fn_ in fns {
        ...
        let conversions = params
            .iter()
            .filter(|v| type_is_using_block(&v.1))
            .map(|(n, t, _)| {
                let name_str = format!(
                    "Could not convert parameter `{}` between node and runtime!",
                    quote!(#n)
                );
                quote!(
        ...
        let output_conversion = if return_type_is_using_block(&fn_.decl.output) {
            quote!(convert_between_block_types(
                &res,
        ...
        let input_borrows = params.iter().map(|v| {
            if type_is_using_block(&v.1) {
                v.2.clone()
            } else {
                quote!()
            }
...
fn generate_runtime_decls(decls: &[ItemTrait]) -> TokenStream {
    ...
    for decl in decls {
        ...
        let api_version = unwrap_or_error(
...
impl<'a> ToClientSideDecl<'a> {
    ...
    fn create_method_decl_with_context(&mut self, method: TraitItemMethod) -> TraitItemMethod {
        let crate_ = self.crate_;
        let context_arg: syn::FnArg =
            parse_quote!( context: #crate_::runtime_api::ExecutionContext );
        let mut fn_decl_ctx = self.create_method_decl(method, quote!(context));
        fn_decl_ctx.sig.ident = Ident::new(
            &format!("{}_with_context", &fn_decl_ctx.sig.ident),
            Span::call_site(),
        );
    ...
    fn create_method_decl(
        ...
        method.sig.decl =
            fold_fn_decl_for_client_side(method.sig.decl.clone(), &self.block_id, &self.crate_);
        ...
        let (native_handling, param_tuple) = match get_changed_in(&found_attributes) {
            Ok(Some(version)) => {
                ...
                let panic = format!(
                    "Calling `{}` should not return a native value!",
                    method.sig.ident
                );
    ...
    fn fold_item_trait(&mut self, mut input: ItemTrait) -> ItemTrait {
        ...
        *self.found_attributes = remove_supported_attributes(&mut input.attrs);
        ...
        if is_core_trait {
            ...
            input
                .supertraits
                .push(parse_quote!( #crate_::runtime_api::Core<#block_ident> ));
        }
        ...
        Meta::List(list) => {
            if list.nested.len() > 1 && list.nested.is_empty() {
                ...
                match list.nested.first().as_ref().map(|v| v.value()) {
                    Some(NestedMeta::Literal(Lit::Int(i))) => Ok(i.value()),
                    _ => err,
                }
            }
        }
...
fn generate_client_side_decls(decls: &[ItemTrait]) -> TokenStream {
    ...
    for decl in decls {
        ...
        let runtime_info =
            unwrap_or_error(api_version.map(|v| generate_runtime_info_impl(&decl, v)));
    ...
    fn visit_fn_arg(&mut self, input: &'ast FnArg) {
        match input {
            FnArg::Captured(ref arg) => match arg.pat {
                Pat::Ident(ref pat) if pat.ident == "at" => self.errors.push(Error::new(
                    pat.span(),
            ...
            FnArg::SelfRef(_) | FnArg::SelfValue(_) => self
                .errors
                .push(Error::new(input.span(), "Self values are not supported.")),
...
fn check_trait_decls(decls: &[ItemTrait]) -> Option<TokenStream> {
    ...
    decls
        .iter()
        .for_each(|decl| visit::visit_item_trait(&mut checker, &decl));
