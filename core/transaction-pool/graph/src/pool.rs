...
mod tests {
	...
	impl ChainApi for TestApi {
		...
		fn hash_and_length(&self, uxt: &ExtrinsicFor<Self>) -> (Self::Hash, usize) {
			...
			(
				(uxt.transfer().from.0.to_low_u64_be() << 5) + uxt.transfer().nonce,
				len
			)
		}
	}
	...
	fn should_validate_and_import_transaction() {
		...
		let hash = pool.submit_one(&BlockId::Number(0), uxt(Transfer {
			from: H256::from_low_u64_be(1).into(),
			to: H256::from_low_u64_be(2).into(),
			amount: 5,
			nonce: 0,
		})).unwrap();
	...
	fn should_limit_futures() {
		...
		// when
		let hash2 = pool.submit_one(&BlockId::Number(0), uxt(Transfer {
			from: H256::from_low_u64_be(2).into(),
			to: H256::from_low_u64_be(2).into(),
			amount: 5,
			nonce: 10,
		})).unwrap();
	...
	mod listener {
		...
		fn should_trigger_dropped() {
			...
			let xt = uxt(Transfer {
				from: H256::from_low_u64_be(2).into(),
				to: H256::from_low_u64_be(1).into(),
				amount: 4,
				nonce: 1,
			});
