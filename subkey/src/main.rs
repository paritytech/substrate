...
use substrate_primitives::{crypto::Ss58Codec, ed25519, hexdisplay::HexDisplay, sr25519, Pair};
...
trait Crypto {
    ...
    fn print_from_uri(uri: &str, password: Option<&str>)
    where
...
impl Crypto for Ed25519 {
    ...
    fn pair_from_seed(seed: &Self::Seed) -> Self::Pair {
        ed25519::Pair::from_seed(seed.clone())
    }
    fn ss58_from_pair(pair: &Self::Pair) -> String {
        pair.public().to_ss58check()
    }
    fn public_from_pair(pair: &Self::Pair) -> Vec<u8> {
        (&pair.public().0[..]).to_owned()
    }
    fn seed_from_pair(pair: &Self::Pair) -> Option<&Self::Seed> {
        Some(pair.seed())
    }
}
...
where
    ...
    <<C as Crypto>::Pair as Pair>::Public:
        Sized + AsRef<[u8]> + Ss58Codec + AsRef<<<C as Crypto>::Pair as Pair>::Public>,
{
    ...
    match matches.subcommand() {
        ...
        ("verify", Some(matches)) => {
            ...
            let pubkey = <<C as Crypto>::Pair as Pair>::Public::from_string(uri)
                .ok()
                .or_else(|| {
